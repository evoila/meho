...
async def create_connector(
    ...
    try:
        async with session_maker() as session:
            ...
            if (
                ...
                try:
                    ...
                    # httpx.HTTPError covers RequestError / TimeoutException / ConnectError;
                    # none inherit OSError, so a DNS or timeout would otherwise 500 even
                    # though the connector row was already committed above.
                    logger.warning(
            ...
            try:
                ...
                await session.commit()
            except Exception as audit_err:  # noqa: BLE001 -- audit side-effect must not fail the connector create response
        ...
        raise InternalError(message="Failed to create connector") from e
...
async def list_connectors(user: Annotated[UserContext, Depends(get_current_user)]) -> list[dict]:
    ...
    try:
        ...
        raise InternalError(message="Failed to list connectors") from e
...
async def get_connector(connector_id: str, user: Annotated[UserContext, Depends(get_current_user)]):
    ...
    try:
        ...
        raise InternalError(message="Failed to get connector") from e
    ...
    try:
        async with session_maker() as session:
            ...
            try:
                ...
                await session.commit()
            except Exception as audit_err:  # noqa: BLE001 -- audit side-effect must not fail the connector update response
        ...
        raise InternalError(message=f"Failed to update connector {connector_id}") from e
...
async def delete_connector(
    ...
    async with openapi_session_maker() as session:
        ...
        try:
            ...
            await session.commit()
        except Exception as audit_err:  # noqa: BLE001 -- audit side-effect must not fail the connector delete response
