...
"""
Email Operations Auto-Sync.

Automatically syncs Email operations to the database when the operations
...
This enables semantic search (BM25 + embeddings) across all Email operations.

Usage:
    On API startup, call sync_all_email_connectors() to update any
...
from meho_app.modules.connectors.email_connector.operations import (
    EMAIL_OPERATIONS,
...
def _format_email_operation_as_text(op: OperationDefinition, connector_name: str = "Email") -> str:
    """
    Format Email operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.

    Args:
        op: Email operation definition
    ...
    keywords = _generate_email_search_keywords(op)
    if keywords:
...
def _generate_email_search_keywords(op: OperationDefinition) -> str:
    """
    Generate search keywords for better BM25 matching.

    Adds abbreviations, synonyms, and variations that users might search for.
    Email-specific vocabulary: email/mail/message/notification, send/deliver,
    status/delivery/receipt, template/HTML/branded, etc.
    ...
    # Email-specific abbreviations and synonyms
    email_terms = {
        "email": [
            "email",
            "emails",
    ...
    for key, terms in email_terms.items():
        if key in combined_text:
            keywords.update(terms)

    # Common action words
    if "send" in combined_text:
        keywords.update(["send", "email", "notify", "deliver"])
    if "check" in combined_text or "status" in combined_text:
        keywords.update(["check", "status", "delivery", "track"])

    return " ".join(sorted(keywords))


async def sync_email_operations_if_needed(
    ...
    """
    Sync Email operations if the version is outdated.
    ...
    if current_version == EMAIL_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing Email operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {EMAIL_OPERATIONS_VERSION}"
    )
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(EMAIL_OPERATIONS)} total, "
...
async def _sync_email_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for Email operations.

    Enables hybrid search (BM25 + semantic) for Email operations.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "email",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing Email knowledge chunks for connector {connector_id}")
    ...
    for op in EMAIL_OPERATIONS:
        try:
            ...
            text = _format_email_operation_as_text(op, connector_name)
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=["api", "operation", "email", op.category],
    ...
    logger.info(f"Created {chunks_created} knowledge chunks for Email connector {connector_id}")
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = EMAIL_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await session.flush()
        logger.debug(f"Updated connector {connector_id} to version {EMAIL_OPERATIONS_VERSION}")


async def sync_all_email_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all Email connectors to the latest operations version.
    ...
    # Find all Email connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "email")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No Email connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} Email connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        # Sync operations
        added, updated, chunks = await sync_email_operations_if_needed(
            session=session,
            connector_id=str(connector.id),
            tenant_id=str(connector.tenant_id),
            current_version=current_version,
            knowledge_store=knowledge_store,
            connector_name=str(connector.name) if connector.name else "Email",
        )
    ...
    summary = {
        ...
        "current_version": EMAIL_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"Email operation sync complete: "
        ...
        logger.info("All Email connectors already at latest version")
