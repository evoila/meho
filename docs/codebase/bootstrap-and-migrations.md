...
### Invocation paths (current state)

Today, there are **three distinct paths** that drive stages 2-8, and they do not agree:

- **`docker compose up` (plain, against `docker-compose.yml`)** — builds `Dockerfile.meho`, whose `ENTRYPOINT` runs `docker-entrypoint.sh`, which runs `run-migrations-monolith.sh`. Migrations run automatically.
- **`docker compose -f docker-compose.debug.yml up`** — builds `Dockerfile.debug.meho`, which has **no entrypoint**. Migrations are **not** run. The application crashes on first query.
- **`./scripts/dev-env.sh up`** — delegates to `docker compose up -d`, then separately calls `run_migrations()` inside the container via `docker compose exec`. Works, but depends on the developer knowing to use the wrapper.
- **`./scripts/dev-env.sh local`** — starts infrastructure in docker, runs the backend and frontend on the host machine, and calls `run_migrations_local()`, which is a **reimplementation** of the migration loop in bash that runs `alembic` against `localhost:5432`.

The two `dev-env.sh` paths and the docker entrypoint path each call migrations differently. The `docker-compose.debug.yml` path does not call them at all. This divergence is the primary cause of team-wide "works on my machine" reports: which path the developer happens to use determines whether the schema is up to date, and no command fails loudly when it isn't.
...
Two Dockerfiles exist for the backend:

- `docker/Dockerfile.meho` — the production image. Multi-stage build with CPU/GPU/slim variants controlled by `CUDA_ENABLED` and `INCLUDE_DOCLING` build args. Defines `ENTRYPOINT ["/docker-entrypoint.sh"]` so migrations always run before the app starts. Default `CMD` is `uvicorn meho_app.main:app --host 0.0.0.0 --port 8000`.
- `docker/Dockerfile.debug.meho` — the hot-reload development image. Single-stage `python:3.13-slim`, mounts the source directory as a volume for live reload, wraps uvicorn in `debugpy` for remote debugging on port 5678. **Has no entrypoint.** Migrations are not run.
...
- `docker-compose.yml` — the full production-ish stack. Builds `Dockerfile.meho`, uses `redis-stack-server` (which provides the `redisearch` and `rejson` modules the application requires), mounts `init-db.sql`, includes Keycloak, Seq, and optional TEI sidecars. `CREDENTIAL_ENCRYPTION_KEY` has an insecure fallback default that allows the app to start with a known-bad key.
- `docker-compose.debug.yml` — the hot-reload stack. Builds `Dockerfile.debug.meho`, uses plain `redis:7-alpine` (which does **not** provide redisearch/rejson — the app will fail at runtime), does **not** mount `init-db.sql`, does **not** include Keycloak, does **not** include Seq. Makes `CREDENTIAL_ENCRYPTION_KEY` required (no fallback). The top of the file has a comment warning "DO NOT USE docker compose DIRECTLY!" because migrations will not run — a documented bug rather than a fix.
...
| Prod image | `docker/Dockerfile.meho` | Multi-stage; has ENTRYPOINT |
| Debug image | `docker/Dockerfile.debug.meho` | Single-stage; **no ENTRYPOINT** |
| Prod compose | `docker-compose.yml` | Full stack; redis-stack; insecure encryption-key fallback |
| Debug compose | `docker-compose.debug.yml` | Missing keycloak + init-db.sql + redis modules |
...
- `docker/Dockerfile.meho`, `docker/Dockerfile.debug.meho`, `docker/docker-entrypoint.sh`
