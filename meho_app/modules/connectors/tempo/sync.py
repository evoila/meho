...
"""
Tempo Operations Auto-Sync

Automatically syncs Tempo operations to the database when the operations
...
This enables semantic search (BM25 + embeddings) across all Tempo operations.

Usage:
    On API startup, call sync_all_tempo_connectors() to update any
...
from meho_app.modules.connectors.tempo.operations import (
    TEMPO_OPERATIONS,
...
def _format_tempo_operation_as_text(op: OperationDefinition, connector_name: str = "Tempo") -> str:
    """
    Format Tempo operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.

    Args:
        op: Tempo operation definition
    ...
    keywords = _generate_tempo_search_keywords(op)
    if keywords:
...
def _generate_tempo_search_keywords(op: OperationDefinition) -> str:
    """
    ...
    # Tempo-specific abbreviations and synonyms
    tempo_terms = {
        "trace": ["trace", "traces", "tracing", "distributed", "request", "flow"],
        "span": ["span", "spans", "operation", "segment"],
        "error": ["error", "errors", "err", "exception", "failure", "fault", "status"],
        "search": ["search", "find", "filter", "query", "match"],
        "tag": ["tag", "tags", "label", "labels", "attribute", "attributes", "metadata"],
        "service": ["service", "services", "microservice", "application", "app"],
        "graph": ["graph", "topology", "dependency", "dependencies", "map"],
        "latency": ["latency", "duration", "slow", "performance", "bottleneck", "p95", "p99"],
        "traceql": ["traceql", "query", "expression", "custom"],
        "metric": ["metric", "metrics", "aggregate", "statistics", "stats", "count", "rate"],
    }

    # Add relevant terms based on operation name/description
    combined_text = f"{op.operation_id} {op.name} {op.description}".lower()
    for key, terms in tempo_terms.items():
    ...
    if "get" in combined_text and "list" not in combined_text:
        keywords.update(["get", "show", "check", "inspect"])
    if "search" in combined_text:
        keywords.update(["search", "find", "filter", "query", "match"])
...
async def sync_tempo_operations_if_needed(
    session: AsyncSession,
    connector_id: str,
    tenant_id: str,
    current_version: str | None,
    knowledge_store: Optional["KnowledgeStore"] = None,
    connector_name: str = "Tempo",
) -> tuple[int, int, int]:
    """
    Sync Tempo operations if the version is outdated.
    ...
    if current_version == TEMPO_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing Tempo operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {TEMPO_OPERATIONS_VERSION}"
    )
    ...
    if knowledge_store is not None:
        chunks_created = await _sync_tempo_knowledge_chunks(
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(TEMPO_OPERATIONS)} total, "
...
async def _sync_tempo_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for Tempo operations.

    Enables hybrid search (BM25 + semantic) for Tempo operations.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "tempo",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing Tempo knowledge chunks for connector {connector_id}")
    ...
    for op in TEMPO_OPERATIONS:
        try:
            ...
            text = _format_tempo_operation_as_text(op, connector_name)
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=["api", "operation", "tempo", op.category],
    ...
    logger.info(f"Created {chunks_created} knowledge chunks for Tempo connector {connector_id}")
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = TEMPO_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await session.flush()
        logger.debug(f"Updated connector {connector_id} to version {TEMPO_OPERATIONS_VERSION}")


async def sync_all_tempo_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all Tempo connectors to the latest operations version.
    ...
    # Find all Tempo connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "tempo")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No Tempo connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} Tempo connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        # Sync operations
        added, updated, chunks = await sync_tempo_operations_if_needed(
            session=session,
            connector_id=str(connector.id),
            tenant_id=str(connector.tenant_id),
            current_version=current_version,
            knowledge_store=knowledge_store,
            connector_name=str(connector.name) if connector.name else "Tempo",
        )
    ...
    summary = {
        ...
        "current_version": TEMPO_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"Tempo operation sync complete: "
        ...
        logger.info("All Tempo connectors already at latest version")
