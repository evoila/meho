...
class NsxHandlerMixin:
    ...
    async def _list_nsx_segments(self, params: dict[str, Any]) -> list[dict[str, Any]]:
        """List NSX logical segments to trace VM-to-segment connectivity."""
        if not self._nsx_available():
            return [{"error": _NSX_NOT_CONFIGURED}]

        try:
            raw_segments = await self._nsx_client.paginated_get(
            ...
            logger.error("Failed to list NSX segments: %s", e, exc_info=True)
    ...
    async def _get_nsx_segment(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            segment = await self._nsx_client.get(
            ...
            logger.error("Failed to get NSX segment %s: %s", segment_id, e, exc_info=True)
    ...
    async def _list_nsx_firewall_policies(self, params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to list NSX firewall policies: %s", e, exc_info=True)
    ...
    async def _get_nsx_firewall_rule(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "Failed to get NSX firewall rule %s/%s: %s", policy_id, rule_id, e, exc_info=True
            )
            raise RuntimeError(
    ...
    async def _list_nsx_security_groups(self, params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to list NSX security groups: %s", e, exc_info=True)
    ...
    async def _list_nsx_tier0_gateways(self, params: dict[str, Any]) -> list[dict[str, Any]]:
        """List NSX Tier-0 gateways for north-south routing analysis."""
        if not self._nsx_available():
            return [{"error": _NSX_NOT_CONFIGURED}]

        try:
            raw_gateways = await self._nsx_client.paginated_get(
            ...
            logger.error("Failed to list NSX Tier-0 gateways: %s", e, exc_info=True)
            raise RuntimeError(f"NSX list_tier0_gateways failed: {e}") from e

    async def _list_nsx_tier1_gateways(self, params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to list NSX Tier-1 gateways: %s", e, exc_info=True)
    ...
    async def _list_nsx_load_balancers(self, params: dict[str, Any]) -> list[dict[str, Any]]:
        """List NSX load balancer services for traffic distribution analysis."""
        if not self._nsx_available():
            return [{"error": _NSX_NOT_CONFIGURED}]

        try:
            raw_lbs = await self._nsx_client.paginated_get(
            ...
            logger.error("Failed to list NSX load balancers: %s", e, exc_info=True)
    ...
    async def _list_nsx_transport_zones(self, params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to list NSX transport zones: %s", e, exc_info=True)
    ...
    async def _list_nsx_transport_nodes(self, params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raw_nodes = await self._nsx_client.paginated_get(
            ...
            logger.error("Failed to list NSX transport nodes: %s", e, exc_info=True)
    ...
    async def _get_nsx_transport_node(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            node = await self._nsx_client.get(
            ...
            logger.error("Failed to get NSX transport node %s: %s", node_id, e, exc_info=True)
    ...
    async def _search_nsx(self, params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if resource_type:
            search_query = f"resource_type:{resource_type} AND {query}"
        else:
            search_query = query
        ...
        try:
            ...
            logger.error("NSX search failed for query '%s': %s", query, e, exc_info=True)
