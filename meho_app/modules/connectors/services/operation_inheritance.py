...
async def resolve_operations(  # NOSONAR (cognitive complexity)
    ...
    for custom_op in custom_ops:
        if custom_op.type_operation_id is not None:
            override_by_type_op_id[custom_op.type_operation_id] = custom_op  # type: ignore[index]  # SQLAlchemy ORM attribute access
        else:
    ...
    for type_op in type_ops:
        override = override_by_type_op_id.get(type_op.id)  # type: ignore[call-overload]  # SQLAlchemy ORM attribute access
...
async def sync_type_operations(
    ...
    for op_def in type_operations:
        ...
        if op_def.operation_id in existing_ops:
            ...
            db_op = existing_ops[op_def.operation_id]  # type: ignore[index]  # SQLAlchemy ORM attribute access
            db_op.name = op_def.name  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            db_op.description = op_def.description  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            db_op.category = op_def.category  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            db_op.parameters = list(op_def.parameters)  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            db_op.example = op_def.example  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            db_op.search_content = search_content  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            db_op.response_entity_type = op_def.response_entity_type  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            db_op.response_identifier_field = op_def.response_identifier_field  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            db_op.response_display_name_field = op_def.response_display_name_field  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            db_op.source = "type"  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            await session.flush()
        else:
    ...
    stale_op_ids = set(existing_ops.keys()) - incoming_op_ids  # type: ignore[operator]  # SQLAlchemy Column[str] keys; str at runtime
    if stale_op_ids:
