...
class TopologyRepository:
    ...
    async def delete_stale_entities(
        ...
        result = await self.session.execute(delete(TopologyEntityModel).where(and_(*conditions)))
        return result.rowcount or 0  # type: ignore[attr-defined]  # SQLAlchemy Result.rowcount exists at runtime
    ...
    async def traverse_topology(  # NOSONAR (cognitive complexity)
        self,
        ...
        async def _traverse(  # NOSONAR (cognitive complexity)
            entity_id: UUID, depth: int, prev_relationship: str | None = None
        ) -> None:
    ...
    async def delete_same_as(
        ...
        deleted: bool = result.rowcount > 0  # type: ignore[attr-defined]  # SQLAlchemy Result.rowcount exists at runtime
        if deleted:
        ...
        result = await self.session.execute(
            select(TopologySameAsModel).where(
                and_(
                    ...
                    TopologySameAsModel.verified_via.any(literal("deterministic_resolution")),
                )
            )
        )
        return list(result.scalars().all())
