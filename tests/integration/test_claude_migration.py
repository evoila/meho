...
async def test_claude_streaming_event_mapping():  # NOSONAR (cognitive complexity)
...
def test_claude_cost_estimation():
    ...
    assert MODEL_COSTS["claude-opus-4"]["input"] == pytest.approx(0.015)
    assert MODEL_COSTS["claude-opus-4"]["output"] == pytest.approx(0.075)

    # Sonnet: $3/1M input = $0.003/1K, $15/1M output = $0.015/1K
    assert MODEL_COSTS["claude-sonnet-4"]["input"] == pytest.approx(0.003)
    assert MODEL_COSTS["claude-sonnet-4"]["output"] == pytest.approx(0.015)
