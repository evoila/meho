...
from meho_app.modules.agents.models import RecipeExecutionModel, RecipeModel
from meho_app.modules.agents.models import RecipeExecutionStatus as DBStatus
...
class RecipeRepository:
    ...
    def _model_to_recipe(self, model: RecipeModel) -> Recipe:
        ...
        parameters = [RecipeParameter.model_validate(p) for p in (model.parameters or [])]
    ...
    async def get_recipe(self, recipe_id: UUID) -> Recipe | None:
        ...
        """
        result = await self._session.execute(select(RecipeModel).where(RecipeModel.id == recipe_id))
    ...
    async def get_recipe_by_name(
        ...
        result = await self._session.execute(
            select(RecipeModel).where(
                and_(RecipeModel.name == name, RecipeModel.tenant_id == tenant_id)
            )
        )
        model = result.scalar_one_or_none()
        return self._model_to_recipe(model) if model else None
    ...
    async def list_recipes(
        ...
        if tags:
            ...
            tag_conditions = [RecipeModel.tags.contains([tag]) for tag in tags]
            query = query.where(or_(*tag_conditions))

        # Sort by last executed (nulls last), then by name
        query = (
            query.order_by(RecipeModel.last_executed_at.desc().nullslast(), RecipeModel.name)
            .limit(limit)
            .offset(offset)
        )
    ...
    async def delete_recipe(self, recipe_id: UUID) -> bool:
        ...
        """
        result = await self._session.execute(delete(RecipeModel).where(RecipeModel.id == recipe_id))
    ...
    async def list_executions(
        ...
        Args:
            recipe_id: Recipe ID
            limit: Maximum number to return
            tenant_id: Scope to this tenant only (security)

        Returns:
            List of executions, most recent first
        """
        stmt = select(RecipeExecutionModel).where(RecipeExecutionModel.recipe_id == recipe_id)
        if tenant_id:
            stmt = stmt.where(RecipeExecutionModel.tenant_id == tenant_id)
        stmt = stmt.order_by(RecipeExecutionModel.started_at.desc().nullslast()).limit(limit)
    ...
    async def get_recent_recipes(
        ...
        result = await self._session.execute(
            select(RecipeModel)
            .where(
                and_(RecipeModel.tenant_id == tenant_id, RecipeModel.last_executed_at.isnot(None))
            )
