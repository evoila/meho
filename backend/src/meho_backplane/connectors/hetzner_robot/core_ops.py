    ...
    RobotCoreOp(
        ...
        llm_instructions=_instructions(
            when_to_call=(
                ...
                "may return many servers; the response is a JSON array. A large "
                "list is reduced to a JSONFlux handle carrying a bounded inline "
                "sample plus a ``fetch_more`` envelope; re-call with a narrower "
                ...
                "NEVER retries. If you receive an auth_failed error, do NOT "
                "retry immediately — fix the Webservice-user credentials at the "
                "target's Vault path first. The Webservice user is DISTINCT from "
                "the Robot portal login and must be created separately."
            ),
            output_shape=(
