...
| **v0.35.0** | The vSphere 9.0 ingested `vmware-rest` catalog is now guarded against unqualified targets (#3565): an enabled 9.0 ingested descriptor dispatched against a non-`vmware` target or one whose version falls outside `>=9,<10` (missing, invalid, or **8.0.x**) is refused **before policy/approval** with the structured error `unqualified_target_version` and one synchronous audit row. The same release ships the `vmware-rest-8.0` catalog (#3569) as the remedy for 8.0.x targets. No schema migrations in this release (alembic head stays `0101`). | To keep governed generic `vmware-rest` operations working on fingerprinted **8.0.x** targets, ingest the 8.0 catalog — `meho connector ingest --catalog vmware/8.0` — then enable `vmware-rest-8.0`. 9.x targets are unaffected, and typed/composite ops are unaffected either way. No config or schema action is required for the guard itself. |
