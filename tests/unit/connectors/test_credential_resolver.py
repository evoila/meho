...
def mock_keycloak_checker():
    ...
    checker = AsyncMock(spec=KeycloakUserChecker)
    checker.is_user_active = AsyncMock(return_value=True)
    return checker
...
class TestAutomatedResolution:
    ...
    async def test_automated_falls_back_to_delegated_credential(
        ...
        mock_keycloak_checker.is_user_active.assert_called_once_with("alice@example.com", "test-tenant")
...
class TestKeycloakUserChecker:
    ...
    @pytest.mark.asyncio
    async def test_returns_true_for_active_user(self):
        ...
        with patch("meho_app.modules.connectors.keycloak_user_checker.KeycloakAdmin") as MockAdmin:
            ...
            result = await checker.is_user_active("alice@example.com", "test-tenant")

        assert result is True

    @pytest.mark.asyncio
    async def test_returns_false_for_disabled_user(self):
    ...
    async def test_caches_result_within_ttl(self):
        ...
        with patch("meho_app.modules.connectors.keycloak_user_checker.KeycloakAdmin") as MockAdmin:
            ...
            # First call
            result1 = await checker.is_user_active("alice@example.com", "test-tenant")
            # Second call -- should use cache
            result2 = await checker.is_user_active("alice@example.com", "test-tenant")
    ...
    @pytest.mark.asyncio
    async def test_fails_open_when_keycloak_unreachable(self):
