...
class RecipeStep(BaseModel):
    ...
    connector_id: UUID | None = Field(default=None)
    ...
    iterate_over: str | None = Field(
        default=None, description="JSONPath to array to iterate over, e.g., '{{get_vms.data.vms}}'"
    )
    ...
    extraction_path: str | None = Field(
        default=None, description="JSONPath to extract, e.g., 'data.items[*].id'"
    )

    # For reduce action
    reduce_query: dict[str, Any] | None = Field(
        default=None, description="DataQuery specification for reduction"
    )

    # For aggregate action
    source_steps: list[str] = Field(default_factory=list)
    aggregation_type: str | None = Field(
        default="merge", description="How to combine: merge, zip, flatten"
    )
...
class MultiStepRecipe(BaseModel):
    ...
    # The steps
    steps: list[RecipeStep] = Field(description="Ordered list of steps to execute")
