...
def _compute_chapter_batches(  # NOSONAR (cognitive complexity)
    toc: list[TocEntry],
    total_pages: int,
    max_batch_size: int,
) -> list[tuple[int, int]]:
...
class DoclingDocumentConverter:
    ...
    EXCLUDED_LABELS: ClassVar[set[DocItemLabel]] = {
    ...
    def __init__(
        ...
        self._chunker = HybridChunker(  # type: ignore[call-arg]  # max_tokens exists at runtime, stubs lag
            max_tokens=max_tokens,
            merge_peers=True,
        )

    def convert_file(self, file_bytes: bytes, filename: str, mime_type: str) -> DoclingDocument:
    ...
    def chunk_document(  # NOSONAR (cognitive complexity)
