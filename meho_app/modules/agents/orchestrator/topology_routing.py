...
async def route_via_topology(  # NOSONAR (cognitive complexity)
    ...
    available_by_id: dict[str, dict[str, Any]] = {c["id"]: c for c in available_connectors}
    ...
    for entity in discovered_entities:
        ...
        try:
            ...
            logger.warning(f"Topology lookup failed for entity '{entity_name}': {e}")

        # 3. Query connector relationships for the entity's source connector
        source_connector_id = entity_connector_id
        if source_connector_id:
            try:
                rels = await get_connector_relationships(source_connector_id, tenant_id, session)
                for rel in rels:
                    ...
                    if related_id in available_by_id:
                        ...
                        if direction == "from":
                            reason = (
                                f"{source_name} {rel_type} {related_name} (related via topology)"
                            )
                        else:
                            reason = (
                                f"{related_name} {rel_type} {source_name} (related via topology)"
                            )
...
def build_topology_investigation_context(  # NOSONAR (cognitive complexity)
    ...
    entity_names = [f"{e.get('type')} '{e.get('name')}'" for e in entities[:3]]
    summary_parts = []
