...
"""
Kubernetes Operations Auto-Sync

Automatically syncs Kubernetes operations to the database when the operations
...
This enables semantic search (BM25 + embeddings) across all Kubernetes operations.

Usage:
    On API startup, call sync_all_kubernetes_connectors() to update any
...
def _format_kubernetes_operation_as_text(
    op: OperationDefinition, connector_name: str = "Kubernetes"
) -> str:
    """
    Format Kubernetes operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.

    Args:
        op: Kubernetes operation definition
    ...
    keywords = _generate_kubernetes_search_keywords(op)
    if keywords:
...
def _generate_kubernetes_search_keywords(op: OperationDefinition) -> str:
    """
    ...
    # Kubernetes-specific abbreviations
    k8s_abbrevs = {
        ...
        "configmap": ["cm", "configmap", "config"],
        "secret": ["secret", "secrets", "credentials"],
        "ingress": ["ingress", "ing", "route"],
        "statefulset": ["sts", "statefulset", "stateful"],
        "daemonset": ["ds", "daemonset", "daemon"],
        "replicaset": ["rs", "replicaset", "replica"],
        "job": ["job", "jobs", "batch"],
        "cronjob": ["cj", "cronjob", "cron", "schedule"],
        "pvc": ["pvc", "persistentvolumeclaim", "claim", "storage"],
        "pv": ["pv", "persistentvolume", "volume"],
        "event": ["event", "events", "log"],
    }
    ...
    for key, abbrevs in k8s_abbrevs.items():
    ...
    if "describe" in combined_text:
        keywords.update(["describe", "info", "details", "events"])
    if "scale" in combined_text:
        keywords.update(["scale", "replicas", "resize"])
    if "restart" in combined_text:
        keywords.update(["restart", "rollout", "redeploy"])
    if "logs" in combined_text:
        keywords.update(["logs", "log", "output", "stdout"])
...
async def sync_kubernetes_operations_if_needed(
    session: AsyncSession,
    connector_id: str,
    tenant_id: str,
    current_version: str | None,
    knowledge_store: Optional["KnowledgeStore"] = None,
    connector_name: str = "Kubernetes",
) -> tuple[int, int, int]:
    """
    Sync Kubernetes operations if the version is outdated.
    ...
    if current_version == KUBERNETES_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing Kubernetes operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {KUBERNETES_OPERATIONS_VERSION}"
    )
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(KUBERNETES_OPERATIONS)} total, "
...
async def _sync_kubernetes_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for Kubernetes operations.

    Enables hybrid search (BM25 + semantic) for Kubernetes operations.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "kubernetes",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing Kubernetes knowledge chunks for connector {connector_id}")
    ...
    for op in KUBERNETES_OPERATIONS:
        try:
            ...
            text = _format_kubernetes_operation_as_text(op, connector_name)
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=["api", "operation", "kubernetes", op.category],
    ...
    logger.info(
        f"Created {chunks_created} knowledge chunks for Kubernetes connector {connector_id}"
    )
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = KUBERNETES_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]
        await session.flush()
        logger.debug(f"Updated connector {connector_id} to version {KUBERNETES_OPERATIONS_VERSION}")


async def sync_all_kubernetes_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all Kubernetes connectors to the latest operations version.
    ...
    # Find all Kubernetes connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "kubernetes")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No Kubernetes connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} Kubernetes connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        # Sync operations
        added, updated, chunks = await sync_kubernetes_operations_if_needed(
            session=session,
            connector_id=str(connector.id),
            tenant_id=str(connector.tenant_id),
            current_version=current_version,
            knowledge_store=knowledge_store,
            connector_name=str(connector.name) if connector.name else "Kubernetes",
        )
    ...
    summary = {
        ...
        "current_version": KUBERNETES_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"Kubernetes operation sync complete: "
        ...
        logger.info("All Kubernetes connectors already at latest version")
