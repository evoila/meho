...
class RedisStateStore:
    ...
    async def save_state(
        self, session_id: str, state: AgentSessionState, ttl: timedelta | None = None
    ) -> bool:
        """
    ...
    async def load_state(self, session_id: str) -> AgentSessionState | None:
        """
        ...
        try:
            ...
            logger.info(
                f"📬 Loaded state for session {session_id[:8]}... (size: {len(data)} bytes)"
            )
            return state
...
from meho_app.core.redis import get_redis_client  # noqa: E402, F401
