...
class MockZeepClient:
    ...
    def _mock_add(self, int_a: int, int_b: int) -> int:
        return int_a + int_b

    def _mock_subtract(self, int_a: int, int_b: int) -> int:
        return int_a - int_b

    def _mock_get_user(self, user_id: str) -> dict:
        return {"id": user_id, "name": f"User-{user_id}", "email": f"user{user_id}@test.com"}

    def _mock_retrieve_properties(self, spec_set: list) -> list:
...
class TestVMwareSOAPClientIntegration:
    ...
    @patch("meho_openapi.soap.client.Client")
    def test_vmware_client_initialization(self, mock_client_class, vmware_config):
