...
class PerformanceHandlerMixin:
    ...
    def _parse_perf_results(
        ...
        if hasattr(entity_data, 'sampleInfo') and entity_data.sampleInfo:
            result["timestamps"] = [
                info.timestamp.isoformat() if hasattr(info.timestamp, 'isoformat') 
    ...
    def _summarize_metrics(self, parsed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for metric_name, data in metrics.items():
            ...
            simple_name = base_name.split(".")[-2] if "." in base_name else base_name
    ...
    async def _get_detailed_vm_performance(self, params: Dict[str, Any]) -> Dict:
        ...
        try:
            ...
            return {
                "vm_name": vm_name,
                "interval": interval,
                "error": f"PerformanceManager query failed: {str(e)}",
                "quick_stats": self._get_quick_stats_fallback(vm),
            }
    ...
    async def _get_detailed_datastore_performance(self, params: Dict[str, Any]) -> Dict:
        """
        Get detailed datastore performance metrics.
        
        Provides IOPS, latency, and throughput metrics that the basic
        datastore info doesn't include.
        """
        from pyVmomi import vim
        ...
        # Datastore-specific metrics
        metric_names = METRIC_GROUPS.get("datastore", [])
        
        counter_ids = self._get_counter_ids(metric_names)
        if not counter_ids:
            # Fallback to basic capacity info
            return {
                "datastore_name": ds_name,
                "interval": interval,
                "error": "No performance counters available for datastore",
                "capacity": {
                    "total_gb": ds.summary.capacity // (1024**3),
                    "free_gb": ds.summary.freeSpace // (1024**3),
                    "used_gb": (ds.summary.capacity - ds.summary.freeSpace) // (1024**3),
                    "percent_used": round((1 - ds.summary.freeSpace / ds.summary.capacity) * 100, 1),
                },
            }
    ...
    def _generate_diagnostic_highlights(
        self,
        ...
        for metric_name, data in disk_metrics.items():
            if "Latency" in metric_name and data.get("avg"):
                ...
                if latency > 50:
                    ...
                    highlights["warnings"].append(
                        f"Elevated disk latency ({latency:.1f}ms)"
                    )
        ...
        for metric_name, data in net_metrics.items():
            if "dropped" in metric_name and data.get("avg") and data["avg"] > 0:
                highlights["warnings"].append(
                    f"Network packet drops detected - possible network congestion"
                )
    ...
    def _build_key_metrics_summary(self, summary: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if "mem.usage.average" in mem:
            key_metrics["memory"]["usage_percent"] = f"{mem['mem.usage.average'].get('avg', 0):.1f}%"
        ...
        if "mem.swapped.average" in mem:
            ...
            key_metrics["memory"]["swapped"] = f"{swapped_kb:.0f} KB" if swapped_kb > 0 else "0 (none)"
        if "mem.vmmemctl.average" in mem:
            balloon_kb = mem["mem.vmmemctl.average"].get("avg", 0)
            key_metrics["memory"]["balloon"] = f"{balloon_kb:.0f} KB" if balloon_kb > 0 else "0 (none)"
        ...
        if "disk.write.average" in disk:
            key_metrics["disk"]["write_rate"] = f"{disk['disk.write.average'].get('avg', 0):.0f} KB/s"
        if "disk.numberRead.summation" in disk:
            key_metrics["disk"]["read_iops"] = f"{disk['disk.numberRead.summation'].get('avg', 0):.0f}"
        if "disk.numberWrite.summation" in disk:
            key_metrics["disk"]["write_iops"] = f"{disk['disk.numberWrite.summation'].get('avg', 0):.0f}"
        if "disk.maxTotalLatency.latest" in disk:
            key_metrics["disk"]["latency"] = f"{disk['disk.maxTotalLatency.latest'].get('avg', 0):.1f}ms"
        ...
        if "net.packetsRx.summation" in net:
            key_metrics["network"]["packets_received"] = f"{net['net.packetsRx.summation'].get('avg', 0):.0f}/interval"
        if "net.packetsTx.summation" in net:
            key_metrics["network"]["packets_sent"] = f"{net['net.packetsTx.summation'].get('avg', 0):.0f}/interval"
    ...
    def _generate_host_diagnostic_highlights(
        self,
        summary: Dict[str, Any],
        host: Any,
    ) -> Dict[str, List[str]]:
        ...
        for metric_name, data in cpu_metrics.items():
            if "usage" in metric_name and data.get("avg"):
                ...
                if data["avg"] > 90:
                    ...
                    highlights["warnings"].append(
                        f"Elevated CPU utilization ({data['avg']:.1f}%)"
                    )
        ...
        for metric_name, data in mem_metrics.items():
            if "usage" in metric_name and data.get("avg"):
                if data["avg"] > 95:
                    ...
                    highlights["warnings"].append(
                        f"High memory usage ({data['avg']:.1f}%)"
                    )
    ...
    async def _list_available_metrics(self, params: Dict[str, Any]) -> Dict:
        """
