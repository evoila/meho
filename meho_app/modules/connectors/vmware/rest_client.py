...
class VMwareRESTClient:
    ...
    Usage::
        ...
        await client.connect({"Authorization": "Basic ..."})
    ...
    async def connect(self, headers: dict[str, str]) -> None:
        ...
        logger.info("REST client connected to %s", self.base_url)
    ...
    async def disconnect(self) -> None:
        ...
        if self._client:
            ...
            self._client = None
            logger.info("REST client disconnected from %s", self.base_url)
    ...
    async def get(self, path: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if response.status_code == 429:
            ...
            logger.warning("429 rate-limited on %s, retrying after %ds", path, retry_after)
    ...
    async def post(self, path: str, json: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if response.status_code == 429:
            retry_after = int(response.headers.get("Retry-After", "5"))
            logger.warning("429 rate-limited on POST %s, retrying after %ds", path, retry_after)
            await asyncio.sleep(retry_after)
            response = await self._client.post(path, json=json)
    ...
    async def update_headers(self, headers: dict[str, str]) -> None:
        ...
        logger.info("REST client headers updated for %s", self.base_url)
