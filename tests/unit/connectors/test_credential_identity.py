...
Verifies:
- EventRegistrationModel has identity columns (created_by_user_id, allowed_connector_ids,
  delegate_credentials, delegation_active)
...
class TestEventModelIdentityColumns:
    """EventRegistrationModel has all four identity columns."""
    ...
    def test_event_model_has_delegate_credentials(self):
        assert hasattr(EventRegistrationModel, "delegate_credentials"), (
            "EventRegistrationModel missing delegate_credentials column"
        )
        col = EventRegistrationModel.__table__.columns["delegate_credentials"]
...
class TestServiceCredentialCRUD:
    ...
    async def test_service_credential_store(self, mock_credential_repo):
        ...
        await mock_credential_repo.store_credentials("__service__", mock_credential_repo._test_credential)
    ...
    async def test_service_credential_retrieve(self, mock_credential_repo):
        ...
        mock_credential_repo.get_credentials.return_value = {"username": "svc", "password": "secret"}
...
class TestEventCapturesCreator:
    ...
    def test_event_captures_creator(self):
        ...
        assert req.delegate_credentials is True

    def test_event_response_includes_identity_fields(self):
        """EventResponse schema includes all identity fields."""
        ...
        assert "allowed_connector_ids" in fields
        assert "delegate_credentials" in fields
    ...
    def test_event_service_accepts_identity_params(self):
        ...
        assert "delegate_credentials" in sig.parameters
...
class TestCreationRejectsDelegationWithoutCredentials:
    ...
    def test_event_endpoint_has_credential_validation(self):
        """Event create endpoint contains credential pre-validation logic."""
        import inspect
        from meho_app.api.connectors.operations.events import create_event_registration

        source = inspect.getsource(create_event_registration)
