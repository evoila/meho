...
class QueryEngine:
    ...
    def register(
            ...
            if unwrap == "auto":
                ...
                for tname, tdata, tdesc, tier_hint in tables:
                    ...
                    self._register_table(
                        tname, tdata, tdesc, append=should_append,
                        tier_hint=tier_hint,
                        source_desc=source_desc, path=path,
                    )
                return self
            elif unwrap is True:
        ...
        self._register_table(
            name, data, description, append=append, tier_hint=None,
            source_desc=source_desc, path=path,
        )
        return self
    ...
    def _detect_and_split(
        self, name: str, data: dict, description: str | None,
        ...
        if len(list_of_dicts_keys) == 0:
            ...
            for k, v in scalar_list_keys.items():
                tables.append((
                    f"{name}_{k}",
                    [{k: item} for item in v],
                    f"Scalar list from {name}.{k}",
                    None,
                ))

            # Build metadata companion from scalars + nested objects
            meta: dict[str, Any] = dict(scalar_keys)
            for k, v in nested_obj_keys.items():
                meta.update(self._flatten_metadata(v, prefix=k))
            if meta:
            ...
            for key in list_of_dicts_keys:
                tables.append((
                    f"{name}_{key}", data[key], f"{key} from {name}", None,
                ))
    ...
    def _flatten_metadata(self, data: dict, prefix: str = "") -> dict[str, Any]:
    ...
    def _register_table(
        ...
        *,
        ...
        if append and name in self.tables:
            ...
            # nosemgrep: sqlalchemy-execute-raw-query -- DuckDB in-memory query, table name from server-generated operation ID (not user input)
    ...
    def explain(self, sql: str) -> str:
        ...
        # nosemgrep: sqlalchemy-execute-raw-query -- DuckDB in-memory EXPLAIN, sql from LLM-generated query (not direct user input)
    ...
    def schema(self, table: str) -> str:
        ...
        # nosemgrep: sqlalchemy-execute-raw-query -- DuckDB in-memory DESCRIBE, table name from registered tables (not user input)
    ...
    def describe_tables(
        ...
        for table_name, info in self.tables.items():
            ...
            if summary:
                ...
                # nosemgrep: sqlalchemy-execute-raw-query -- DuckDB in-memory DESCRIBE, table_name from registered tables dict
