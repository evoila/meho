...
async def get_topology_graph(  # NOSONAR (cognitive complexity)
    user_context: UserContext = Depends(get_current_user),
    session: AsyncSession = Depends(get_db_session),
    include_stale: bool = Query(False, description="Include stale entities"),
...
async def reject_suggestion(
    ...
    user_context: UserContext = Depends(get_current_user),
    session: AsyncSession = Depends(get_db_session),
    body: SuggestionRejectRequest | None = None,
) -> Any:
    """
...
@router.post(
    "/connector-relationships", response_model=ConnectorRelationshipResponse, status_code=201
)
...
@router.put(
    "/connector-relationships/{relationship_id}", response_model=ConnectorRelationshipResponse
)
async def update_connector_relationship(
    ...
    rel.last_verified_at = dt.now(tz=UTC)
    await session.commit()
    await session.refresh(rel)
