...
class ParameterHandlingRule(BaseModel):
    ...
    examples: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description="Example parameter instances showing the pattern"
    )
...
class PrerequisiteFlow(BaseModel):
    ...
    steps: Optional[List[str]] = Field(
        default=None, 
        description="Suggested step sequence"
    )
...
class ConversationTurn(BaseModel):
    ...
    context: Optional[str] = Field(
        default=None, 
        description="Why agent said this (for LLM understanding)"
    )
...
class UserGuidance(BaseModel):
    ...
    complexity_warning: Optional[str] = Field(
        default=None,
        description="Warning about operation complexity"
    )
    ...
    time_estimate: Optional[str] = Field(
        default=None,
        description="Estimated time (e.g., '5-10 minutes')"
    )
...
class LLMInstructions(BaseModel):
    ...
    parameter_handling: List[ParameterHandlingRule] = Field(
        default_factory=list,
        description="Rules for handling different parameter patterns"
    )
    
    prerequisite_flows: List[PrerequisiteFlow] = Field(
        default_factory=list,
        description="Common prerequisite sequences for this endpoint"
    )
    ...
    user_guidance: Optional[UserGuidance] = Field(
        default=None,
        description="Tips and warnings to show users"
    )
    
    reasoning_hints: Optional[str] = Field(
        default=None,
        description="Additional context to help LLM reason about this endpoint"
    )
...
""",
        hints={
            "max_items_recommendation": "4-5 (ask if user really needs more)"
        }
    ),
