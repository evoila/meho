...
import type { IngestionJobStatus } from '@/api/types/knowledge';
...
export function GlobalJobMonitor() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      inFlight = true;
      try {
        const jobs = await knowledgeClient.getActiveJobs();
    ...
    // Initial fetch
    knowledgeClient.getActiveJobs().then(jobs => {
      const visibleJobs = jobs.filter(job => !dismissedJobs.has(job.id));
      setActiveJobs(visibleJobs);
  ...
  }, [knowledgeClient, dismissedJobs, queryClient, isAuthenticated]);
