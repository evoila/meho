...
def _classify_fingerprint_error(exc: Exception) -> FingerprintFailureReason:
    ...
    Mirrors the :meth:`MongoDbConnector.probe` taxonomy so a target degrades
    the same way whether the failure surfaces on ``fingerprint`` or ``probe``:
    an unresolvable credential or a server-side auth rejection
    (:exc:`~pymongo.errors.OperationFailure` with an auth error code) is
    ``auth_failed`` (never the raw driver message), a TCP / server-selection
    failure is ``tcp_unreachable``, and any other pymongo failure is
    ``connect_failed``. Only ever called for an exception the fingerprint arm
    catches, so :exc:`~pymongo.errors.PyMongoError` is the residual.
    """
    if isinstance(exc, CredentialsReadError | ValueError):
        return "auth_failed"
    if isinstance(exc, OSError):
        return "tcp_unreachable"
    if isinstance(exc, OperationFailure):
        return "auth_failed" if exc.code in _AUTH_ERROR_CODES else "connect_failed"
