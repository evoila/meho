    ...
    try:
        ...
        async with session_maker() as session:
            ...
            if row is None:
                ...
                sys.exit("ERROR: Run migrations first -- ./scripts/migrate-all.sh")
            dimension = row[0]
            if dimension == 1536:
                print(f"    FAIL: Embedding dimension is 1536 (old OpenAI). Expected 1024 (Voyage AI).")
                sys.exit("ERROR: Run migrations first -- ./scripts/migrate-all.sh")
            elif dimension == 1024:
                print(f"    Migrations: Applied (1024D)")
            else:
