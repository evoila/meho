...
"""
Confluence Operations Auto-Sync.

Automatically syncs Confluence operations to the database when the operations
...
This enables semantic search (BM25 + embeddings) across all Confluence operations.

Usage:
    On API startup, call sync_all_confluence_connectors() to update any
...
def _format_confluence_operation_as_text(
    op: OperationDefinition, connector_name: str = "Confluence"
) -> str:
    """
    Format Confluence operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.

    Args:
        op: Confluence operation definition
    ...
    keywords = _generate_confluence_search_keywords(op)
    if keywords:
...
def _generate_confluence_search_keywords(op: OperationDefinition) -> str:
    """
    Generate search keywords for better BM25 matching.

    Adds abbreviations, synonyms, and variations that users might search for.
    Confluence-specific vocabulary: page/wiki/document/documentation,
    cql/query/search/find, comment/note, space/knowledge-base, etc.
    ...
    # Confluence-specific abbreviations and synonyms
    confluence_terms = {
        "page": [
            "page",
            "pages",
    ...
    for key, terms in confluence_terms.items():
    ...
    if "get" in combined_text and "list" not in combined_text:
        keywords.update(["get", "show", "check", "inspect", "detail", "view", "read"])
    if "create" in combined_text:
        keywords.update(["create", "add", "new", "write"])
    if "recent" in combined_text:
        keywords.update(["recent", "latest", "new", "changed", "updated", "modified"])

    return " ".join(sorted(keywords))


async def sync_confluence_operations_if_needed(
    ...
    """
    Sync Confluence operations if the version is outdated.
    ...
    if current_version == CONFLUENCE_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing Confluence operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {CONFLUENCE_OPERATIONS_VERSION}"
    )
    ...
    for op in CONFLUENCE_OPERATIONS:
        ...
        # Determine safety level -- WRITE operations require trust approval
        safety_level: Literal["read", "write"] = (
            "write" if op.operation_id in WRITE_OPERATIONS else "read"
        )
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(CONFLUENCE_OPERATIONS)} total, "
...
async def _sync_confluence_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for Confluence operations.

    Enables hybrid search (BM25 + semantic) for Confluence operations.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "confluence",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing Confluence knowledge chunks for connector {connector_id}")
    ...
    for op in CONFLUENCE_OPERATIONS:
        try:
            ...
            text = _format_confluence_operation_as_text(op, connector_name)
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=["api", "operation", "confluence", op.category],
    ...
    logger.info(
        f"Created {chunks_created} knowledge chunks for Confluence connector {connector_id}"
    )
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = CONFLUENCE_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await session.flush()
        logger.debug(f"Updated connector {connector_id} to version {CONFLUENCE_OPERATIONS_VERSION}")


async def sync_all_confluence_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all Confluence connectors to the latest operations version.
    ...
    # Find all Confluence connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "confluence")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No Confluence connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} Confluence connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        # Sync operations
        added, updated, chunks = await sync_confluence_operations_if_needed(
            session=session,
            connector_id=str(connector.id),
            tenant_id=str(connector.tenant_id),
            current_version=current_version,
            knowledge_store=knowledge_store,
            connector_name=str(connector.name) if connector.name else "Confluence",
        )
    ...
    summary = {
        ...
        "current_version": CONFLUENCE_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"Confluence operation sync complete: "
        ...
        logger.info("All Confluence connectors already at latest version")
