...
class RESTConnectorService:
    ...
    def __init__(
        ...
        *,
        ...
        if session is not None:
            ...
            self.session_manager = session_manager or SessionManager(self.credential_repo)  # type: ignore[arg-type]
            ...
            self.endpoint_repo = endpoint_repo  # type: ignore[assignment]
            self.operation_repo = operation_repo  # type: ignore[assignment]
            self.credential_repo = None  # type: ignore[assignment]
            self.http_client = http_client or GenericHTTPClient()
            self.session_manager = session_manager  # type: ignore[assignment]
            self.spec_parser = OpenAPIParser()
        else:
            raise ValueError(
    ...
    async def create_connector(
        ...
        return await self.connector_repo.create(data, tenant_id)  # type: ignore[attr-defined, no-any-return]

    async def get_connector(self, connector_id: str) -> Connector | None:
        """Get a connector by ID."""
        return await self.connector_repo.get_by_id(connector_id)  # type: ignore[attr-defined, no-any-return]
    ...
    async def update_connector(
        ...
        """Update a connector."""
        return await self.connector_repo.update(connector_id, data)  # type: ignore[attr-defined, no-any-return]
    ...
    async def list_connectors(
        ...
        return await self.connector_repo.list_by_tenant(  # type: ignore[attr-defined, no-any-return]
            tenant_id=tenant_id,
            is_active=is_active,
            limit=limit,
        )
    ...
    async def delete_connector(self, connector_id: str) -> bool:
        """Delete a connector."""
        return await self.connector_repo.delete(connector_id)  # type: ignore[attr-defined, no-any-return]
    ...
    async def search_endpoints(
        ...
        return await self.endpoint_repo.search(  # type: ignore[union-attr]
            query=query,
    ...
    async def get_endpoint(self, endpoint_id: str) -> EndpointDescriptor | None:
        ...
        return await self.endpoint_repo.get_by_id(endpoint_id)  # type: ignore[union-attr]
    ...
    async def list_endpoints(
        ...
        return await self.endpoint_repo.list_by_connector(connector_id, limit)  # type: ignore[union-attr]
    ...
    async def ingest_openapi_spec(
        ...
        # Parse spec
        spec_text = (
            spec_content.decode("utf-8") if isinstance(spec_content, bytes) else spec_content
        )
        spec_dict = self.spec_parser.parse(spec_text)

        # Create endpoints
        endpoints = await self.endpoint_repo.create_from_spec(connector_id, spec_dict)  # type: ignore[union-attr]
    ...
    async def call_endpoint(
        ...
        connector = await self.connector_repo.get_by_id(connector_id)  # type: ignore[attr-defined]
        endpoint = await self.endpoint_repo.get_by_id(endpoint_id)  # type: ignore[union-attr]
        ...
        if self.session_manager:
            session = await self.session_manager.ensure_session(connector, credentials)  # type: ignore[union-attr]
        else:
            session = None

        # Make the call
        result = await self.http_client.call(  # type: ignore[union-attr]
            connector=connector,
            endpoint=endpoint,
            session=session,
