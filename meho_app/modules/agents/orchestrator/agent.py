...
class OrchestratorAgent(BaseAgent):
    ...
    async def run_streaming(  # NOSONAR (cognitive complexity)
        self,
        user_message: str,
    ...
    async def _dispatch_parallel(  # NOSONAR (cognitive complexity)
        self,
        state: OrchestratorState,
    ...
    async def _run_single_agent(  # NOSONAR (cognitive complexity)
        self,
        state: OrchestratorState,
        ...
        try:
            ...
            if connector.connector_type and connector.connector_type != "generic":
                try:
                    ...
                    logger.debug(
                        f"DB connector skill resolution failed for {connector.connector_type}: {e}"
                    )
    ...
    async def _synthesize_streaming(  # NOSONAR (cognitive complexity)
    ...
    def _build_session_context(  # NOSONAR (cognitive complexity)
        self,
    ...
    async def _call_llm_streaming(  # NOSONAR (cognitive complexity)
        self,
        prompt: str,
        session_id: str | None = None,
        ...
        try:
            ...
            for _attempt in range(4):  # 1 initial + 3 retries
                try:
                    ...
                    # Success -- yield the complete response as final item
                    ...
                    if e.status_code in (429, 500, 502, 503, 529) and _attempt < 3:
                        ...
                        logger.warning(
                            f"Routing LLM stream error (HTTP {e.status_code}), "
                            f"retrying in {_delay:.1f}s"
                        )
                        await asyncio.sleep(_delay)
                        accumulated_text = ""  # Reset for retry
                        continue
                    raise
                except _ModelAPIError:
                    if _attempt < 3:
                        _delay = 1.0 * (2**_attempt) * (0.8 + _random.random() * 0.4)  # noqa: S311
                        logger.warning(
                            f"Routing LLM stream connection error, retrying in {_delay:.1f}s"
                        )
                        await asyncio.sleep(_delay)
                        accumulated_text = ""  # Reset for retry
                        continue
            ...
            # Exhausted retries -- fall back to non-streaming
            ...
            # Fall back to non-streaming on any unexpected error
            logger.warning("Routing LLM streaming failed, falling back to _call_llm", exc_info=True)
            fallback_response = await self._call_llm(prompt)
            yield fallback_response

    async def _call_llm(self, prompt: str) -> str:  # NOSONAR (cognitive complexity)
