...
from meho_openapi.llm_instructions import (
    LLMInstructions,
...
class InstructionGenerator:
    ...
    def __init__(self, use_llm: bool = False) -> None:
        ...
        self._llm_agent: Optional[Agent[None, LLMInstructions]] = None
    ...
    async def generate_for_endpoint(
        ...
        try:
            return await self._generate_llm_enhanced(
                analysis, description, summary, body_schema
            )
...
def should_generate_instructions(
    ...
    return (
        len(properties) > 5 or
        any(p.endswith("_id") for p in properties) or
        any(v.get("type") == "array" for v in properties.values()) or
        any(v.get("type") == "object" for v in properties.values())
    )
