...
"""
Slack Operations Auto-Sync.

Automatically syncs Slack operations to the database when the operations
version changes. Also creates knowledge_chunk entries for hybrid search
(BM25 + semantic) so the agent can discover Slack operations.

Based on aws/sync.py pattern.
"""
...
from meho_app.modules.connectors.slack.operations import (
    SLACK_OPERATIONS,
...
def _format_slack_operation_as_text(op: OperationDefinition, connector_name: str = "Slack") -> str:
    """
    Format Slack operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.

    Args:
        op: Slack operation definition.
...
def _generate_slack_search_keywords(op: OperationDefinition) -> str:
    """
    Generate search keywords for better BM25 matching.

    Maps Slack-specific terms to synonyms and related concepts.
    ...
    # Slack-specific term mappings
    slack_terms: dict[str, list[str]] = {
        "channel": ["channels", "conversation", "room"],
        "message": ["messages", "chat", "text", "post"],
        "reaction": ["emoji", "react", "emote"],
        "search": ["find", "query", "lookup"],
        "user": ["member", "profile", "person"],
        "history": ["messages", "log", "timeline"],
        "list": ["show", "get", "all", "browse"],
        "post": ["send", "write", "publish"],
    }

    # Add relevant terms based on operation content
    combined_text = f"{op.operation_id} {op.name} {op.description}".lower()
    for key, synonyms in slack_terms.items():
    ...
    # Add Slack-specific keywords
    keywords.update(["slack", "workspace"])

    return " ".join(sorted(keywords))


async def sync_slack_operations_if_needed(
    ...
    """
    Sync Slack operations if the version is outdated.
    ...
    if current_version == SLACK_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing Slack operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {SLACK_OPERATIONS_VERSION}"
    )
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(SLACK_OPERATIONS)} total, "
...
async def _sync_slack_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for Slack operations.

    Deletes existing Slack operation chunks for this connector, then
    recreates them with the latest operation definitions.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "slack",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing Slack knowledge chunks for connector {connector_id}")
    ...
    for op in SLACK_OPERATIONS:
        try:
            ...
            text = _format_slack_operation_as_text(op, connector_name)

            from meho_app.modules.knowledge.schemas import ChunkMetadata

            metadata_dict = {
                "resource_type": op.category,
                "keywords": [op.operation_id, op.name, op.category, "slack", "messaging"],
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=["api", "operation", "slack", "messaging", op.category],
    ...
    logger.info(f"Created {chunks_created} knowledge chunks for Slack connector {connector_id}")
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = SLACK_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await session.flush()
        logger.debug(f"Updated connector {connector_id} to version {SLACK_OPERATIONS_VERSION}")


async def sync_all_slack_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all Slack connectors to the latest operations version.
    ...
    # Find all Slack connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "slack")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No Slack connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} Slack connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        added, updated, chunks = await sync_slack_operations_if_needed(
            session=session,
    ...
    summary = {
        ...
        "current_version": SLACK_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"Slack operation sync complete: "
        ...
        logger.info("All Slack connectors already at latest version")
