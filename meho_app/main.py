...
@asynccontextmanager
async def lifespan(app: FastAPI) -> Any:  # NOSONAR (cognitive complexity)
    ...
    if flags.topology and config.topology_auto_discovery_enabled:
        try:
            ...
            # Singleton factory so get_processor_instance() can return this
            # processor for immediate triggering when new items are queued.
            ...
            async with session_maker() as session:
                ...
                for ctype, module_path, func_name in sync_funcs:
                    try:
                        ...
                        # Safe (non-literal-import): module paths from hardcoded (ctype, module_path, func_name) tuples, not user input
                ...
                try:
                    ...
                    if reconciled:
                        logger.info(
                            f"Knowledge chunk reconciliation: repaired {reconciled} connector(s)"
                        )
                except Exception as e:
