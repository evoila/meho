...
async def sync_mcp_knowledge_chunks(
    ...
    try:
        ...
        logger.debug("Deleted existing MCP knowledge chunks for connector %s", connector_id)
    except Exception as e:
        logger.warning("Failed to delete existing MCP chunks (may not exist): %s", e)
    ...
    for op in operations:
        try:
            ...
            logger.error("Failed to create knowledge chunk for %s: %s", op.operation_id, e)
            continue

    logger.info(
        "Created %d knowledge chunks for MCP connector %s", chunks_created, connector_id
    )
...
async def sync_all_mcp_connectors(
    ...
    logger.info("Checking %d MCP connector(s) for operation consistency", len(connectors))
    ...
    for connector in connectors:
        ...
        if not tools_hash:
            logger.debug(
                "MCP connector %s has no operations_hash -- operations "
                "will be synced on next connect",
                connector.id,
            )

    logger.info("MCP startup sync: checked %d connector(s)", connectors_checked)
