...
* Operator authentication — the `verify_jwt` FastAPI dependency
  ...
  *before* tenant-claim extraction): each authlib failure mode now
  ...
  response body. G0.13-T1 (#1131) folds the most common residual case
  into the classifier as `malformed_jws`: any authlib `DecodeError`
  (non-JWT bearer prefix like `Bearer not-a-real-jwt`, wrong segment
  count, base64 garbage) surfaces the specific code with the authlib
  description in the structlog event — closing the v0.6.0 dogfood
  signal that flagged `/api/v1/health` still returning bare
  `invalid_token` for a non-JWT bearer. The residual `invalid_token`
  is now kept only for the small set of failures that aren't
  `DecodeError` (`alg: none` rejection via `UnsupportedAlgorithmError`,
  future `JoseError` subclasses, post-refresh kid miss).
...
* Broadcast override CRUD (G6.3-T4 #381) —
  ...
  (`meho_broadcast_overrides_set` / `.remove`) and
...
* Admin MCP override-CRUD tools (G6.3-T5 #382) — three
  `tenant_admin` admin tools `meho_broadcast_overrides_list|set|remove`
...
* Forward-compat regression test (Task #30) —
  `backend/tests/migrations/test_migration_rollback.py` is the unit-test-level
...
* MCP Streamable HTTP transport entrypoint (Task #246, G0.5-T1) —
  ...
  reference `meho_status` tool and `meho://tenant/<id>/info` resource;
...
* MCP reference tool + resource (Task #249, G0.5-T4) — the two
  ...
  registers `meho_status` — a no-arg tool whose handler calls
...
* Broadcast chart-CI chaos test (G6.1-T9 shape #2, #433) — a
  ...
  reframe note on #433). Runs as part of the `helm-test` required
  check in the chart-CI workflow (gating-posture hardened by #349).
...
* MCP tool + resource registries (Task #248, G0.5-T3) — the substrate
  ...
  of the three `TenantRole` values) and an optional
  `required_capability: str | None` (G4.5-T1 #1519); the list methods
  filter against the calling operator's role AND capability set, and
  `tools/call` / `resources/read` re-check both gates at call time.
  The role gate uses `role_at_least`; the capability gate uses
  `capability_satisfied` (a tool/template with `required_capability`
  set is *absent* from the list and 403s on the call for an operator
  whose `Operator.capabilities` frozenset lacks the key — true absence
  for an unprovisioned tenant, mirroring the connector enable model,
  not a packaging/entitlement system). MEHO-internal fields are
  stripped from the wire shape by `to_wire()`: `ToolDefinition` strips
  `required_role`, `op_class`, and `required_capability`;
  `ResourceTemplateDefinition` strips `required_role` and
  `required_capability` (the resource-template model has no `op_class`
  field — it's a tool-only audit-classification hint T5 will consume).
...
| `metrics` (route) | `src/meho_backplane/main.py` | `GET /metrics` returning the Prometheus exposition format from the default registry (process / GC collectors + the `http_requests_total` counter and the `http_request_duration_seconds` histogram). |
...
| `configure_logging` | `src/meho_backplane/logging.py` | Configures structlog: `merge_contextvars` → `add_log_level` → `TimeStamper(iso, utc)` → `ExceptionRenderer(ExceptionDictTransformer(show_locals=False))` → `JSONRenderer`, writing to stdout. Idempotent. The traceback transformer (the `dict_tracebacks` processor) runs with **`show_locals=False`** (#1488): structlog's default `show_locals=True` writes every frame's local variables into the log line, which is a credential-disclosure vector (CWE-532) — any secret held as a frame local on a captured traceback would be serialised to stdout verbatim. The motivating leak was a failed scheduled agent run (`scheduler_fire_failed`) rendering the agent's `client_credentials` secret as a frame local. Disabling locals keeps the structured frames (file / line / function / exception type + message) for triage while closing the vector for every frame across the backplane, including frames that must hold a secret as a plain `str` (the httpx form-post in `auth/agent_token.py`). The logger factory is constructed as `PrintLoggerFactory()` with **no `file=` argument** — structlog's `PrintLogger.msg` then resolves `sys.stdout` lazily at write time (calls `print()` with no `file=` keyword whenever `self._file is sys.stdout`). Tests under `pytest`'s `capfd` swap `sys.stdout` for a wrapped fd that gets closed on test teardown; the lazy shape avoids capturing that wrapped fd at lifespan-startup time and surviving into a later test as a closed-fd `ValueError` from `cache_logger_on_first_use=True`. Production behaviour unchanged because the real process `sys.stdout` does not get rebound at runtime. **Test convention — asserting on a module-level production logger:** because `configure_logging` sets `cache_logger_on_first_use=True`, the first emit through a module-level `_log = structlog.get_logger(__name__)` pins a `BoundLogger` to the *then-current* processor-list instance; any later `structlog.configure(...)` (a `TestClient` app boot, a per-test `configure` + `reset_defaults`, or another module's `configure_logging`) **replaces** that instance, orphaning the cached logger. `structlog.testing.capture_logs()` patches only the *current* instance, so under the `pytest -n 6 --dist loadscope` lane a logger warmed-and-orphaned by an earlier test on the same xdist worker is missed — the capture list comes back **empty** while the event still prints to stdout, and any negative assertion (e.g. "the JWT never appears") then passes vacuously. A test that asserts on events from such a logger therefore must **not** use bare `capture_logs`; instead bind a private capture — `capture = structlog.testing.LogCapture(); private_log = structlog.wrap_logger(structlog.PrintLogger(), processors=[capture])` — and `monkeypatch.setattr(<subject_module>, "_log", private_log)`, asserting on `capture.entries`. This is process-local, contextvar-free, immune to any concurrent `configure`, and auto-restored by `monkeypatch` on teardown. Established by #1254 (`tests/test_operations_register_ingested.py::test_check_version_covered_logs_orphan_when_no_class_registered`), then generalised to the remaining `test_operations_register_ingested.py` orphan-class assertions by #1712 and to `test_corpus_client.py::test_forwarded_jwt_never_logged` by #1622; the subject-side alternative is per-call logger resolution (`auth/jwt.py` / `retrieval/embedding.EmbeddingService`, #738). Do **not** "fix" this by disabling `cache_logger_on_first_use` — it is the intended production performance default. |
| `RequestContextMiddleware` | `src/meho_backplane/middleware.py` | Pure-ASGI middleware. Per request: extracts/mints a `request_id`, clears any leftover contextvars and binds the new `request_id`, mirrors it onto the `X-Request-Id` response header, records `http_requests_total{method,path,status}` (counter) **and** `http_request_duration_seconds{method,path,status}` (histogram, observed in seconds at the same seam), emits one `request_completed` JSON log line with method / path / status / duration_ms (which inherits any contextvars bound during the request, including `operator_sub` and `tenant_id` from `verify_jwt_and_bind`). The metric `path` label is the matched route template or the `__unmatched__` constant (`UNMATCHED_ROUTE_LABEL`) for 404s — bounded cardinality — while the *log* line keeps the literal request path for unmatched routes (forensics; a log is not a bounded label set). Metrics are recorded only on the normal-completion path; a handler that raises re-raises after the `request_failed` log without touching either metric (symmetric with the counter's pre-existing behaviour). |
| `verify_jwt_and_bind` | `src/meho_backplane/middleware.py` | FastAPI dependency wrapper around `verify_jwt`. On successful validation, binds `operator_sub` (the JWT's `sub` claim) and `tenant_id` (`str(operator.tenant_id)` — JSON renderer cannot serialise raw `uuid.UUID`) into structlog contextvars so every subsequent log line in the request scope carries both fields automatically. Authenticated routes use `Depends(verify_jwt_and_bind)` instead of `Depends(verify_jwt)` directly. Lives alongside the middleware because `RequestContextMiddleware`'s request-entry `clear_contextvars` call is what guarantees the bound keys do not leak across requests reusing the same asyncio task. `tenant_role` is intentionally *not* bound — it's enforced at the dependency layer (G0.1-T4 `require_role`) so handlers reach for the typed `Operator` instead of pulling roles out of contextvars. G0.8-T1 (#628): after binding contextvars the wrapper calls `tenancy.ensure_tenant(operator.tenant_id, …)` in its own short `get_sessionmaker()` session (the same own-session pattern `AuditMiddleware` uses), committed before the route runs so the row is visible to the route's transaction. This is the just-in-time tenant-seed seam — every authenticated route flows through this wrapper, and it is the first point `operator.tenant_id` is available as a verified value, so seeding here guarantees no tenant-scoped write hits `documents_tenant_id_fkey` on a fresh deploy. Initiative #2415 (#2502): the wrapper also hosts the runner **negative cage** — after binding contextvars (so the 403 is attributable) and before tenant seeding, a `principal_kind=runner` operator whose `request.url.path` is not under `RUNNER_ALLOWED_PATH_PREFIXES` (`/api/v1/gateway/`, `/api/v1/checks/`) is 403'd `runner_scope_violation`. The cage keys on the unforgeable `principal_kind` discriminator so a runner client missing any other mapper still cannot roam the read API. |
...
| `HTTP_REQUESTS_TOTAL` | `src/meho_backplane/metrics.py` | Module-level `prometheus_client.Counter` registered against the default registry. Labels: `method`, `path`, `status`. `path` is the matched FastAPI route template, or the `__unmatched__` constant (`middleware.UNMATCHED_ROUTE_LABEL`) for unmatched routes (404s) — bounding label cardinality so an unauthenticated 404 scan cannot fan out the `path` label on the unauthenticated `/metrics` endpoint. |
| `HTTP_REQUEST_DURATION_SECONDS` | `src/meho_backplane/metrics.py` | Module-level `prometheus_client.Histogram` on the default registry (#2886). Same `method` / `path` / `status` label set and same middleware seam as `HTTP_REQUESTS_TOTAL`; observed in **seconds** (full-precision `time.monotonic` delta, not the log's rounded `duration_ms`). No explicit `buckets` → inherits `prometheus_client`'s default latency buckets (5 ms … 10 s, plus `+Inf`). Makes p50/p90/p99 request latency answerable from Prometheus via `histogram_quantile` instead of SQL over `audit_log.duration_ms`. `path` cardinality is bounded identically to the counter. |
...
| `Settings` / `get_settings` | `src/meho_backplane/settings.py` | Pydantic v2 model + `lru_cache`-singleton accessor for the Keycloak knobs (`KEYCLOAK_ISSUER_URL`, `KEYCLOAK_AUDIENCE`, `KEYCLOAK_JWKS_CACHE_TTL_SECONDS`, `KEYCLOAK_JWT_LEEWAY_SECONDS`), the per-tenant JWT-claim-name knobs (`JWT_TENANT_CLAIM_NAME` default `tenant_id`, `JWT_TENANT_ROLE_CLAIM_NAME` default `tenant_role`, `JWT_PRINCIPAL_KIND_CLAIM_NAME` default `principal_kind` — G11.2-T1 #815, `JWT_CAPABILITIES_CLAIM_NAME` default `capabilities` — G4.5-T1 #1519, drives the MCP capability gate; absent/malformed claim → empty set, fail-closed), the Keycloak Admin API knobs (`KEYCLOAK_ADMIN_URL`, `KEYCLOAK_ADMIN_CLIENT_ID`, `KEYCLOAK_ADMIN_CLIENT_SECRET` — all default to empty string; required for the agent-principal register/revoke verbs; if unset the `AgentPrincipalService` raises 503 `keycloak_admin_not_configured`), the RBAC stub-route gate (`MEHO_ENABLE_RBAC_TEST_ROUTE` default `False`; only `1` / `true` / `yes` / `on` are truthy — anything else, including `disabled`, evaluates to `False` so a typo cannot silently mount the routes in production), the Vault knobs (`VAULT_ADDR`, `VAULT_OIDC_ROLE`, `VAULT_CHECK_RUNNER_ROLE`, `VAULT_OIDC_MOUNT_PATH`, `VAULT_NAMESPACE`, `VAULT_TIMEOUT_SECONDS` — `VAULT_ADDR` is optional since #2277: a GSM-backend (Vault-free) install leaves it unset/blank and `vault_addr` coerces to `None`, the readiness probe skips instead of failing, and the first actual Vault use raises `VaultNotConfiguredError`; `VAULT_CHECK_RUNNER_ROLE` is optional since #2757: unset keeps every JWT login on `VAULT_OIDC_ROLE` byte-for-byte, set routes only the in-process check-runner's background dispatch — the operator carrying the internal-only `Operator.check_runner_dispatch` marker — to a dedicated bounded role, fail-closed on denial), and the database knobs (`DATABASE_URL`, `DATABASE_POOL_SIZE`, `DATABASE_POOL_TIMEOUT`). `DATABASE_URL` is required and validated by a Pydantic `@field_validator` that rejects sync DSNs (`postgresql://`, `sqlite:///`, `postgresql+psycopg2://`) — only `postgresql+asyncpg://` and `sqlite+aiosqlite://` are accepted, matching ADR 0004's async-only mandate. Tests reset via `get_settings.cache_clear()`. |
...
| **Test convention: seed-then-read audit-window route tests** | `tests/conftest.py` autouse `_default_database_url` + `db/engine.py::reset_engine_for_testing` (engine isolation) · `tests/test_retrieval_usage.py` (the time-window convention) | Per-test DB isolation is provided by the autouse `_default_database_url` fixture: it copies the per-worker migrated SQLite template (`_schema_template_db`) to a fresh `tmp_path/default.db`, `monkeypatch.setenv("DATABASE_URL", …)`, then `get_settings.cache_clear()` + `reset_engine_for_testing()` so the process-wide memoized `_engine`/`_sessionmaker` rebind to *this* test's file before the body runs (and again on teardown). That wiring is sound — a route test's seed write through `get_sessionmaker()` and the route's read through the same factory bind to the same per-test file even though the seed runs on pytest-asyncio's per-test event loop and the route runs on the `TestClient` portal's separate loop (verified: the route's connection attaches to the correct `default.db` and sees the seeded rows at the raw-SQL level). **The convention that bites is time, not engine binding.** A *route* test (one that drives `GET /api/v1/retrieve/usage` via `TestClient`, as opposed to the service-level tests that call `compute_usage(...)` directly) lets the route resolve its `since` window relative to the real wall clock (`datetime.now(UTC)`; default `30d`). So any audit row such a test seeds **must anchor `occurred_at` to `datetime.now(UTC)`**, never to a fixed module constant like `_NOW = datetime(2026, 5, 14, …)`: a fixed seed date falls out of the rolling `[now-30d, now]` window once the calendar advances past it, and `compute_usage`'s `WHERE occurred_at >= since` filter silently drops the rows, turning `total_searches` to `0` (#1722 — `test_route_audit_row_count_matches_total_searches`; presented as an xdist flake because the boundary crossed mid-day, but reproduced serially). Service-level `compute_usage` tests are exempt because they pass an explicit `since`/`until` pinned to `_NOW`, so seed and window share the same fixed timeline. The same fixed-seed mistake has a second, quieter failure mode: a route test that asserts an *expected-zero* (e.g. `test_rest_only_dogfood_zero_is_not_context_free`, which seeds `/api/v1/retrieve` rows and asserts `total_searches == 0` to prove REST is **excluded** from the counted surfaces, #632/#1734) does not flake when its seed ages out — it keeps passing, but **vacuously**: the rows drop out of the window before the surface-exclusion path runs, so the `0` is satisfied by window expiry instead of by exclusion, and a regression that wrongly counted REST rows would no longer be caught. Anchoring the seed to `datetime.now(UTC)` keeps the rows in-window so the zero genuinely exercises exclusion. **Rule, both variants: any route-window test must seed `occurred_at` window-relative (`datetime.now(UTC) - timedelta(...)`), never a fixed `_NOW`** — whether the assertion is a positive count or an expected-zero. Same family as the canary's "deterministic past timestamp instead of a 1-second TTL + sleep" rule and #983's per-test reset discipline. |
...
| `actor_delegation` / `resolve_actor_sub` + `get_client_credentials_token` | `src/meho_backplane/auth/delegation.py` + `src/meho_backplane/auth/agent_token.py` | G11.2-T2 (#816) resource-server delegation context. Keycloak has no RFC 8693 delegation token exchange (impersonation-only; keycloak#38279 open), so MEHO synthesises the `sub` (human) + `act` (agent) binding itself. `actor_delegation(actor_sub)` is a context manager that binds the acting agent's principal (the `AgentDefinition.identity_ref`) into structlog's contextvars; `resolve_actor_sub()` reads it at each of the three audit write paths (chassis `audit._write_audit_row`, dispatcher `operations/_audit.write_audit_row`, MCP `mcp/audit.write_mcp_audit_row`). A user-initiated `AgentInvoker.run` binds it around `asyncio.create_task` so the background loop task snapshots the actor for its whole life — every audit row its in-process tool calls produce records `operator_sub`=human + `actor_sub`=agent (no second token over the wire, no IdP `act` claim; `Operator` deliberately does **not** carry the actor — it is run context, not a token claim). `actor_delegation` fails closed on an empty ref. `get_client_credentials_token(issuer_url, client_id, client_secret, audience?)` is the autonomous-run auth primitive: the agent authenticates as itself via the Keycloak-supported `client_credentials` grant (so it is the subject, `actor_sub` stays NULL), with typed `AgentTokenError` (`network_error` / `http_<status>` / `missing_access_token`) and the client secret never logged directly. `AgentInvoker.run_scheduled` consumes it for `SCHEDULED`-trigger runs; the scheduler that fires them is G11.3. The agent `client_credentials` secret is threaded as a `pydantic.SecretStr` from `scheduler/loop.py`'s `_PreparedInvocation.agent_client_secret` through `run_scheduled`'s `agent_client_secret` parameter — masking to `'**********'` even as a bare frame local on a captured traceback (CWE-532) — and unwrapped via `.get_secret_value()` only at the single `get_client_credentials_token` call site (#1488; the first `SecretStr` in the backplane). The raw value still lands as a plain `str` inside `get_client_credentials_token` (httpx needs it for the form-post); that frame is covered by `configure_logging`'s `show_locals=False`. Supersedes the unimplementable RFC 8693 token-exchange of PR #1051. |
| `0021_add_audit_log_actor_sub` | `backend/alembic/versions/0021_add_audit_log_actor_sub.py` | G11.2-T2 (#816) migration. Adds the nullable `audit_log.actor_sub text` column + `audit_log_actor_sub_idx` b-tree index — soft-column discipline (no FK, no NOT NULL) mirroring `agent_session_id` (`0014`). Reversible round-trip (`downgrade` drops index then column). |
...
| `0011_backfill_operation_group_when_to_use` | `backend/alembic/versions/0011_backfill_operation_group_when_to_use.py` | G0.9.1-T2 (Task #774, Signal #5 refined) data migration backfilling curated `when_to_use` strings onto pre-existing `operation_group` rows. PR #731 killed the auto-derive default at the Python boundary and PR #732 curated the per-group strings in source for the four typed connectors shipped at v0.3.1, but the first-write-wins contract on `_resolve_or_create_group` means existing rows in a live DB are never overwritten on connector re-registration — every v0.3.0-era deployment kept serving the kill-switched template (`Operations grouped under '<key>' for <product> <impl>.`) on `list_operation_groups`. This migration closes the gap: one UPDATE per natural-key tuple `(product, version, impl_id, group_key)` over `tenant_id IS NULL` rows whose `when_to_use` still matches the template prefix (`LIKE 'Operations grouped under%'`); operator-edited rows (via `meho_connector_edit_group`) and tenant-scoped rows are left untouched by the predicate. Covers the 22 built-in groups across bind9 (`identity` / `zone` / `record` / `config`), kubernetes (`cluster` / `inventory` / `workload` / `network` / `config` / `events` / `logs`), vault (`kv` / `auth` / `sys`), vmware-rest (`cluster` / `events` / `performance` / `storage` / `networking` / `vm` / `host`), plus the harbor `robot` group whose placeholder PR #732 missed. Curated strings are inlined verbatim in the migration (self-contained discipline — migrations cannot import connector modules without coupling history-replay to the modules' current API). `downgrade()` is a documented no-op: reconstructing the template per row carries no operator value and would require remembering each row's pre-upgrade value. Idempotent: re-running is a no-op because the `LIKE 'Operations grouped under%'` predicate no longer matches after the first pass. Test coverage: `backend/tests/migrations/test_migration_0011_backfill_when_to_use.py` exercises template-row → curated, operator-edit → preserved, tenant-scoped → preserved, unmapped natural key → preserved, and the second-run no-op. |
...
| `Operator` | `src/meho_backplane/auth/operator.py` | Frozen pydantic v2 model carrying validated claims (`sub`, `name`, `email`, `raw_jwt`, `tenant_id: UUID`, `tenant_role: TenantRole`, `principal_kind: PrincipalKind`, `capabilities: frozenset[str]`, `platform_admin: bool`, `runner_id: UUID | None`). Returned by `verify_jwt`; consumed by every authenticated route from G2.2-T3 onward. `raw_jwt` is preserved verbatim for G2.2-T2's Vault forward-auth. `tenant_id` / `tenant_role` (G0.1-T2) are required — the model can no longer be constructed without them, so any future regression that drops the claim extraction surfaces as a pydantic `ValidationError` rather than a silent anonymous request. `capabilities` (G4.5-T1 #1519) is the tenant's provisioned capability-key set, lifted from the `JWT_CAPABILITIES_CLAIM_NAME` claim (default `capabilities`); it drives the MCP capability gate (`ToolDefinition.required_capability`) and defaults to the empty set (fail-closed) so pre-G4.5 tokens see no capability-gated tools. Being a `frozenset` keeps the frozen model immutable and the gate membership test O(1). `platform_admin` (#1638) is the cross-tenant capability flag (default `False`, fail-closed); it derives from the boolean claim named by `JWT_PLATFORM_ADMIN_CLAIM_NAME` and, when the optional `JWT_PLATFORM_ADMIN_ROLE_NAME` setting is set, is OR-granted by an exact-match realm role under `realm_access.roles` (both sources fail-closed; see `jwt._extract_platform_admin`). `runner_id` (Initiative #2415 #2502) is the satellite-runner principal's row UUID, lifted from `JWT_RUNNER_ID_CLAIM_NAME` (default `runner_id`) and defaulting to `None` for every non-runner token; a `principal_kind=runner` token that carries no `runner_id` is rejected 401 `missing_runner_id_claim` (fail-closed kind/claim pairing in `_operator_from_claims`). |
...
| `PrincipalKind` | `src/meho_backplane/auth/operator.py` | G11.2-T1 (#815) discriminator `StrEnum` added to `Operator` distinguishing what authenticated a request. Four values: `user` (human via device-code flow — default when no `principal_kind` claim is present, so all pre-G11.2 tokens are unaffected), `service` (non-interactive service-account client-credentials flow that is not a MEHO-managed agent), `agent` (Keycloak client registered by `meho agent-principal register`; token carries `principal_kind=agent`), and `runner` (Initiative #2415 #2502 — a satellite runner's read-only service principal registered by `meho runner-principal register`; token carries `principal_kind=runner` + a `runner_id` claim). The claim name is settings-controlled via `JWT_PRINCIPAL_KIND_CLAIM_NAME` (default `principal_kind`). `_extract_principal_kind` in `auth/jwt.py` grace-falls-back only for an **absent** claim (→ `USER`, the pre-G11.2 legacy contract); a present-but-unrecognised value fails closed with a 401 (`unknown_principal_kind`, structured-log warning first), mirroring the unknown-`tenant_role` handling — the discriminator drives agent-vs-human authorization, so an out-of-enum value is rejected rather than coerced to the human default. G11.2-T2 / T3 branch on this discriminator to enforce per-principal permission models and the resource-server delegation context (#816). A `runner`-kind token is caged: `verify_jwt_and_bind` fail-closed 403s it on every authenticated REST route outside `RUNNER_ALLOWED_PATH_PREFIXES` (`/api/v1/gateway/`, `/api/v1/checks/`) and the MCP surface (`verify_mcp_jwt_and_bind`) rejects it outright — both with `runner_scope_violation`; the fine-grained per-runner binding lives in `auth/runner_guard.py` (`require_runner` + `assert_runner_scope`). |
| `verify_jwt` | `src/meho_backplane/auth/jwt.py` | FastAPI dependency: parses `Authorization: Bearer ...`, fetches/caches Keycloak's JWKS, validates signature + `iss` + `aud` + `exp` + `sub` (all four essential; `sub` REQUIRED per RFC 9068 §2.2.1), refreshes JWKS on a kid miss, extracts `tenant_id` (UUID) and `tenant_role` (`TenantRole`) under the settings-controlled claim names, and returns an `Operator`. Every failure mode surfaces a structured 401. **Decode-stage** failures dispatch through `_classify_decode_error` into specific codes — `invalid_audience` / `invalid_issuer` / `missing_sub` / `token_expired` / `signature_verification_failed` / `token_not_yet_valid` / `invalid_claim` / `missing_claim` / `malformed_jws` — each paired with a structlog event carrying the diagnostic value (expected audience, expected issuer, claim name, authlib `DecodeError` description); the 401 body carries only the code, never the expected value, mirroring the existing `malformed_tenant_claim` body-vs-log split (RFC 6750 §3.1 — public response is machine-readable error code only). The residual `invalid_token` covers only the small set of failures that aren't `DecodeError` (`alg: none` via `UnsupportedAlgorithmError`, future `JoseError` subclasses, post-refresh kid miss). **Post-decode** tenant-claim failures keep their existing four codes (`missing_tenant_claim` / `missing_tenant_role_claim` / `malformed_tenant_claim` / `unknown_tenant_role`) with the same body-vs-log split. The decode-stage promotion was G0.9.1-T12 / #797 (consumer Addendum II Ask #1 — walls #2 + #3); `malformed_jws` was G0.13-T1 / #1131 (v0.6.0 dogfood signal — the consumer's `Bearer not-a-real-jwt` probe still surfaced bare `invalid_token` because `DecodeError` was the unclassified residual); the tenant-claim codes are G0.1-T2 / #232. |
...
| JWKS cache | `src/meho_backplane/auth/jwt.py` (`_jwks_cache`, `_jwks_fetched_at`, `_jwks_lock`, `_last_forced_refresh_at`, `_negative_kids`) | Module-level dict + monotonic-fetched timestamp + asyncio lock. TTL-bounded (default 5 min) and kid-rotation refreshed on a kid miss. The forced re-fetch is **bounded across requests** (S22 / #310): authlib resolves `kid`→key *before* signature verification, so a well-formed but unsigned bearer token carrying a random `kid` reaches the refresh path — unbounded, a stream of them would force one Keycloak discovery+JWKS round-trip *per request*. Two guards gate the `force_refresh=True` call in `_decode_with_kid_rotation`: a cooldown (`_FORCE_REFRESH_COOLDOWN_SECONDS`, refuses a second live refresh within the window, timed from the last *permitted forced refresh* — not from an ordinary cache-fill, so the bounded single-retry and genuine-rotation contracts hold) reserved under `_jwks_lock` so concurrent misses reserve at most one slot, plus a size- and TTL-bounded negative-`kid` cache (`_negative_kids`, keyed on the JWS header `kid` peeked without verification via `_peek_kid`; a `kid` confirmed absent after a forced refresh is remembered so a slow replay is short-circuited past the cooldown, and entries expire after `_NEGATIVE_KID_TTL_SECONDS` so a `kid` Keycloak later publishes recovers within the JWKS staleness envelope). Net: a burst of unknown-`kid` tokens forces at most one round-trip per window, while a genuine rotation still refreshes once and authenticates. Both the chassis (`verify_jwt`) and MCP (`verify_mcp_jwt`) chains inherit the bound through the shared `_decode_with_kid_rotation`. Single-worker design; v0.2 may move to Redis when multi-worker uvicorn is needed. |
...
| `HttpConnector` | `src/meho_backplane/connectors/adapters/http.py` | Abstract HTTP-API connector base (G0.2-T3). Every HTTP-based vendor connector (vSphere, NSX, Harbor, Hetzner Robot, etc.) inherits this and overrides `auth_headers(target, operator) -> dict[str, str]` plus the three ABC methods. The auth surface threads the full frozen `Operator` (G3.9-T1, not just `operator.raw_jwt`) from `dispatch_ingested` through `_request_json` / `_post_json` / `mount_op_path` to `auth_headers`, so a connector's credential loader can read its per-target secret under the operator's identity (`vault_client_for_operator`). Operator-less probe paths (`fingerprint` / `probe`) synthesise a system operator via `connectors/_shared/system_operator.py`. Provides: (1) per-target `httpx.AsyncClient` pooling — one client per `_client_cache_key(target)` (`target_cache_key(target)` = `(tenant_id, id)` plus `extra_cache_dimensions(target)` = `(verify_tls, ca_pin_digest, base_url)`), not per `target.name`, so a client is never reused across tenants, across a TLS-trust change, or across a base-URL change — a PATCH to `extras.scheme` / `host` / `port` rotates the pool entry because `base_url` is baked into the client at construction, and the `_base_url` read is polymorphic so a subclass override (Prometheus/Loki plain-HTTP) rotates the key too (evoila/meho#2873); created lazily, reused across all ops, closed via `aclose()`; (2) retry policy — 3 retries on idempotent verbs (GET, HEAD, OPTIONS) with exponential backoff (0.5 s → 1 s → 2 s) via `tenacity`; 4xx responses are not retried, 5xx + connection errors are; (3) cert-bundle passthrough — `httpx` honours `SSL_CERT_FILE` natively so no custom cert logic is needed; (4) `_get_json()` / `_post_json()` helpers — `_get_json` routes through the retried `_request_json`; `_post_json` calls the client directly (no automatic retry for non-idempotent verbs); (5) **same-origin-only redirect following** — the pooled client is a `_SameOriginRedirectClient` (an `httpx.AsyncClient` built with `follow_redirects=False`) that re-implements a bounded redirect loop in `send()`: a benign **same-origin** `3xx` (the vendor trailing-slash `301` that vcsim's `/rest` mount and some appliances emit) is still followed so no vendor flow regresses, but a **cross-origin** `Location` is refused and the `3xx` is returned unfollowed — the request is never replayed off-origin, so neither NSX's `X-XSRF-TOKEN` header nor a login credential body can leak to an attacker-controlled redirect target (open-redirect SSRF / auth-token forwarding, evoila/meho-internal#101 row L11). The default `follow_redirects=True` would have forwarded them: `httpx` strips `Authorization` cross-origin but not vendor auth headers or a method-preserving `307`/`308` body. Auth-bearer plumbing per `target.auth_model` is vendor-specific — `auth_headers()` raises `NotImplementedError` on the base; per-vendor overrides land in G3.1+ connectors. |
...
| `api/v1/kb.router` + `KbEntryCreate` / `KbListResponse` / `KbEntryPreview` / `IngestKbRequest` | `src/meho_backplane/api/v1/kb.py` | G4.1-T2 (#416) HTTP surface for the knowledge-base layer. Five routes under `/api/v1/kb*` mapping to `KbService` (T1 #415): `GET /api/v1/kb` (list, operator role, returns `KbListResponse` envelope with 200-char preview per entry; query params `filter` + `limit` + `offset`), `GET /api/v1/kb/{slug}` (show, operator role, returns full `KbEntry` or 404 `slug_not_found`), `POST /api/v1/kb` (create, tenant_admin role, body `KbEntryCreate`, returns 201 + entry, invalid slug → 422 `invalid_slug`), `DELETE /api/v1/kb/{slug}` (delete, tenant_admin role, idempotent — returns 204 whether the row existed or not so cross-tenant probes can't enumerate via status-code differential), `POST /api/v1/kb/ingest` (bulk ingest, tenant_admin role, body `IngestKbRequest`). **Tenant scoping**: every route passes `operator.tenant_id` straight to the substrate; cross-tenant slug probes surface as 404 (not 403, not the other tenant's entry) — same conflation `connectors_ingest.py` uses. **Audit + broadcast op_ids**: every route binds `audit_op_id` ∈ {`kb.list`, `kb.show`, `kb.create`, `kb.delete`, `kb.ingest`} + `audit_op_class` ∈ {`read`, `write`} via `bind_contextvars` *before* the substrate call so a handler exception still produces an audit row classified under the canonical op id. `audit_op_class` is bound explicitly because `classify_op` would only match `kb.list` / `kb.create` / `kb.delete` against its suffix tables — `kb.show` (no `.get` / `.info` / `.list` suffix) and `kb.ingest` (no `.create` / `.update` suffix) would otherwise fall through to the `other` bucket and broadcast under the wrong sensitivity class. The `kb.create` route additionally binds `audit_slug` (slug only, NEVER the body — the body content is recoverable via `kb.show` and the audit row is for the operation, not the document content); `kb.delete` binds `audit_slug` + `audit_existed` (boolean — true for real deletion, false for no-op); `kb.ingest` binds `audit_inserted_count` / `audit_updated_count` / `audit_skipped_count` / `audit_error_count` (NOT the file contents per the task body's contract). **`IngestKbRequest` validator**: exactly-one-of `directory` / `tarball_url` is enforced via a model-level `@model_validator(mode="after")` returning 422 when both or neither set. `tarball_url` is accepted by the request schema for forward-compat with the task body's contract but the substrate's `KbService` only exposes `ingest_directory`; a request with `tarball_url` set returns **501 Not Implemented** (mirrors `retrieve_eval.py`'s posture toward its unimplemented `baseline` field — honest about the unimplemented branch rather than silently dropping). v0.2.next can wire up `KbService.ingest_tarball` and flip the 501 branch. Unit coverage in `tests/test_api_v1_kb.py` (33 tests across mount / auth / RBAC / per-route happy + error paths / audit-row assertions); PG-real coverage in `tests/integration/test_kb_routes_pg.py` exercises the full lifecycle through all 5 routes + tenant boundary + dry-run no-write. Sibling waves: T3 #417 MCP meta-tools (`search_knowledge` / `add_to_knowledge`) reach the same substrate; T4 #418 CLI verbs call the routes via the Go HTTP client; T5 #419 canary acceptance against the 44-entry consumer kb. G0.12-T9 (#1267, Initiative #1118) migrated the `meho kb {list, show, add, ingest, delete, search}` CLI verbs off the per-package hand-rolled `doAuthedRequest` + consumer-side `Entry` / `ListResponse` / `IngestionResult` / `RetrievalHit` / `RetrieveResponse` duplicates onto the generated typed client (`api.ClientWithResponses` via `api.AuthedClient`); `api.KbEntry`, `api.KbEntryPreview`, `api.KbListResponse`, `api.KbIngestionResult`, `api.RetrievalHit`, and `api.RetrieveResponse` are the single source of truth on the CLI side, kept in lock-step with the FastAPI Pydantic models by the `cli-api-snapshot-freshness` CI gate. |
| `api/v1/runbook_templates.router` + `RunbookTemplateListResponse` | `src/meho_backplane/api/v1/runbook_templates.py` | G12.2-T3 (#1297) HTTP surface for the runbook template lifecycle. Six routes under `/api/v1/runbooks/templates*` mapping to `RunbookTemplateService` (T2 #1296): `POST /api/v1/runbooks/templates` (draft, tenant_admin role, body `DraftTemplateRequest`, returns 201 + `DraftTemplateResponse`; `DuplicateDraftError` → 409, service-side `InvalidKbSlugError` → 422), `GET /api/v1/runbooks/templates` (list, **operator** role — the only non-admin route, returns `RunbookTemplateListResponse` envelope `{"templates": [...]}` of `TemplateSummary`; query params `status` / `target_kind` / `limit`, `status` typed as the closed `draft`/`published`/`deprecated` literal so a bogus value is a clean query-boundary 422 rather than a 500 from building `ListTemplatesFilter` in-handler), `GET /api/v1/runbooks/templates/{slug}` (show, **operator** role floor with a run-state-conditional carve-out (G12.3-T4 #1309) — `tenant_admin` is a pass-through; `operator` is granted the read only when `RunbookRunService.can_show_template_post_completion()` returns `True` for the resolved `(slug, version)` (i.e. the operator has a `completed` or `abandoned` run against that pinned version — post-mortem / learning carve-out), otherwise 403 `detail="opacity_floor"`; returns `ShowTemplateResponse`, optional `?version=`. Differential-timing posture on the operator path: when `?version` is omitted, the latest version is resolved **first** so the authorization check uses the same version the response would return; a slug the operator has no run against still surfaces as 403 `opacity_floor` rather than a 404 channel that would leak template presence to enumerators. Cross-tenant probes for `tenant_admin` callers still surface as 404 (the service's tenant-filtered query raises `TemplateNotFoundError` exactly as for an absent slug — anti-enumeration, same conflation `kb.py` uses)), `PATCH /api/v1/runbooks/templates/{slug}` (edit, tenant_admin role, body `RunbookTemplateBody`, returns `EditTemplateResponse` whose `forked_from` is populated only when the service forks a new draft off a published version; `TemplateNotFoundError` → 404), `POST /api/v1/runbooks/templates/{slug}/publish` and `POST /api/v1/runbooks/templates/{slug}/deprecate` (tenant_admin role, body `_VersionBody` `{"version": int}` with `extra="forbid"` so a body-smuggled slug 422s — the URL `{slug}` is the authoritative target; `TemplateNotDraftError` / `TemplateNotPublishedError` → 400, `TemplateNotFoundError` → 404). The publish/deprecate/edit routes rebuild the service's `PublishTemplateRequest` / `DeprecateTemplateRequest` / `EditTemplateRequest` from the path slug + body so the URL is the single source of truth. **Tenant scoping**: every route passes `operator.tenant_id` to the service; cross-tenant probes surface as 404 (the service's tenant-filtered query raises `TemplateNotFoundError` exactly as for an absent slug — anti-enumeration, same conflation `kb.py` uses). **Audit + broadcast op_ids**: every route binds `audit_op_id` ∈ {`runbook.draft_template`, `runbook.list_templates`, `runbook.show_template`, `runbook.edit_template`, `runbook.publish_template`, `runbook.deprecate_template`} + `audit_op_class` ∈ {`read`, `write`} via `bind_contextvars` *before* the service call so a handler exception still classifies the audit row under the canonical op id; `audit_op_class` is bound explicitly because `classify_op`'s suffix tables wouldn't match these op ids (no `.list`/`.get` tail on the reads, no `.create`/`.update`/`.delete` tail on the writes) and would otherwise bucket them as `other`. Per-slug routes bind `audit_slug`; publish/deprecate additionally bind `audit_version`; the template body / step contents are NEVER in the payload. Unit coverage in `tests/test_runbook_templates_api.py` (29 tests across mount / 401 / RBAC / per-route happy + error paths / cross-tenant 404 / audit-row op_id binding). The MCP tools (T4 #1298) reach the same service over the MCP transport; CLI verbs land in G12.5. |
| `api/v1/runbook_runs.router` + `RunbookListRunsResponse` | `src/meho_backplane/api/v1/runbook_runs.py` | G12.3-T5 (#1311) HTTP surface for the runbook *run* lifecycle, sibling to `runbook_templates.router` (T3 #1297, template lifecycle). Five routes under `/api/v1/runbooks/runs*` mapping to `RunbookRunService` (T3 #1308): `POST /api/v1/runbooks/runs` (start, **operator** role, body `StartRunRequest`, returns 201 + `NextStepResponse` — declared as the broader union rather than `CurrentStepResponse` so the OpenAPI schema documents the same response surface as `next` and callers can parse both endpoints through one discriminated-union decoder; the service's start path always returns the `current_step` variant since a fresh run is never terminal; the caller is auto-assigned as the run's assignee; `DeprecatedTemplateError` → 400, `TemplateNotFoundError` → 404, `MissingParamsError` → 422), `POST /api/v1/runbooks/runs/{run_id}/next` (advance, **operator** role, body `NextStepRequest`, returns `NextStepResponse` discriminated on `kind` — a `CurrentStepResponse` carrying the next step's body or a `RunCompletedResponse` marker; the substrate's verify oracle is authoritative — the caller's `last_verified` claim is informational only and never trusted as a gate; the single-assignee invariant is enforced at the *service* layer not the role gate — a `tenant_admin` who is not the run's assignee still gets 403 `NotRunAssigneeError`, because the right way for a senior to take over is `reassign_run` not a role-based bypass; `RunNotFoundError` → 404, `NotRunAssigneeError` → 403, `RunAlreadyTerminalError` / `PreviousStepFailedError` / `PreviousStepNotVerifiedError` / `DeprecatedTemplateError` → 400, `MissingParamsError` / `VerifyResponseRequiredError` / `VerifyResponseMismatchError` → 422), `POST /api/v1/runbooks/runs/{run_id}/abort` (abort, **operator** role at the gate, body `AbortRunRequest`, returns `AbortRunResponse`; the handler passes `caller_is_admin=(operator.tenant_role == TENANT_ADMIN)` so the service widens its allowance from "only the assignee" to "the assignee or any tenant_admin" — the load-bearing `caller_is_admin` widen-allowance pattern, so admins can mop up abandoned runs without reassigning to themselves first per #1198; `RunNotFoundError` → 404, `NotRunAssigneeError` → 403, `RunAlreadyTerminalError` → 400; the service also writes a *direct* audit row (not through the dispatcher) with `method="DISPATCH"` + `path="runbook.abort"` carrying the abort `reason` in `raw_payload` in addition to the route's envelope row), `POST /api/v1/runbooks/runs/{run_id}/reassign` (reassign, **tenant_admin** role — the load-bearing escalation knob per Initiative #1198, the *only* RBAC enforcement on this surface; the service does no role check by design — the senior taking over is the documented escalation path, an operator hitting this route gets 403 at the gate before the service is touched; body `ReassignRunRequest`, returns `ReassignRunResponse`; `RunNotFoundError` → 404, `RunAlreadyTerminalError` → 400), `GET /api/v1/runbooks/runs` (list, **operator** role at the gate, returns `RunbookListRunsResponse` envelope `{"runs": [...]}` of `RunSummary` — same forward-compat paging envelope pattern `runbook_templates.router` adopted; query params `assignee` / `status` / `template_slug` / `limit`, `status` typed as the closed `in_progress`/`completed`/`abandoned` literal so a bogus value trips a clean 422 at FastAPI's query-parameter validation boundary before the handler runs; the handler computes `caller_is_admin` from `operator.tenant_role` and passes it to the service so an `OPERATOR` only ever sees their own runs — the service forces `assignee=operator.sub` regardless of what the query string says, the `assignee` filter is a *narrow* surface for their own view never an *escape* into another operator's — while a `TENANT_ADMIN` can pass `?assignee=<other-sub>` to inspect a junior's runs). **Centralised typed-exception → HTTP-status mapping**: a module-level `_EXC_STATUS: Final[dict[type[Exception], int]]` table maps every exception type the engine + service vocabularies promise — `RunNotFoundError` / `TemplateNotFoundError` → 404, `NotRunAssigneeError` → 403, `DeprecatedTemplateError` / `RunAlreadyTerminalError` / `PreviousStepFailedError` / `PreviousStepNotVerifiedError` → 400, `MissingParamsError` / `VerifyResponseRequiredError` / `VerifyResponseMismatchError` → 422 — accessed via the `_http_for(exc)` helper which does a strict `_EXC_STATUS[type(exc)]` lookup (a `KeyError` is a contract bug, not a fall-through to 500: the strict lookup forces the contributor to either add the type to `_EXC_STATUS` or stop catching it at all). So each handler declares its mapping as a one-line `except (...) as exc: raise _http_for(exc) from exc` tuple rather than open-coding the same `raise HTTPException(...) from exc` body N times; ordering within a per-handler tuple matters only when one exception type is a subclass of another (none of these are — the engine and service vocabularies inherit only from stdlib `ValueError` / `LookupError` / `PermissionError`, never each other), so the tuples can be ordered for readability (404 / 403 / 400 / 422 by severity). **Tenant scoping**: every route derives `tenant_id` from the JWT-validated `Operator`; no surface accepts a tenant id from the body, query string, or path. Cross-tenant `next` / `abort` / `reassign` / `show` probes on someone else's `run_id` surface as 404 (the service's tenant filter makes the row invisible, so `RunNotFoundError` fires exactly as for a genuinely absent id) — the conflation prevents enumerating another tenant's runs via status-code differential, same posture as `runbook_templates.router`. **Audit + broadcast op_ids**: every route binds `audit_op_id` ∈ {`runbook.start_run`, `runbook.next_step`, `runbook.abort_run`, `runbook.reassign_run`, `runbook.list_runs`} (pinned as the module-level `_RUNBOOK_RUN_OP_IDS: Final[dict[str, str]]` so the contract is greppable from tests + G8 dashboards and a typo in a handler surfaces at first call rather than as a silent broadcast under the wrong op_id) + `audit_op_class` ∈ {`read`, `write`} via `bind_contextvars` *before* the service call so a handler exception still classifies the audit row under the canonical op id; `audit_op_class` is bound explicitly because `classify_op`'s suffix tables wouldn't reliably match these op ids (no `.list` / `.get` tail on the read, no `.create` / `.update` / `.delete` tail on the writes) and they would otherwise fall through to the `other` bucket and broadcast under the wrong sensitivity class — same gotcha `kb.py` and `runbook_templates.router` document. For the `next` route there is a *second* audit row when the verify predicate is an `operation_call`: the route binds `audit_op_id="runbook.next_step"` on the outer envelope row, and the service binds `run_id_var` + `step_id_var` (per G12.1-T2 #1294) around the dispatched verify call — that inner audit row carries its own `audit_op_id` (the verify call's op id) but is correlated to this run via `run_id` + `step_id` (two audit rows per `runbook_next` invocation on an `operation_call` verify; one row on a `confirm` verify). **`/next` opacity contract**: the response is the discriminated union `NextStepResponse` (`CurrentStepResponse` or `RunCompletedResponse`) carrying *only* the next step's body with `${run.target}` / `${run.params.X}` substitutions resolved — the opacity-by-construction shape from T1 #1300, so a caller can never enumerate future steps. Unit coverage in `tests/test_runbook_runs_api.py` (30 tests across mount / 401 unauthenticated / RBAC / per-route happy + error paths / `caller_is_admin` widen-allowance assertions on abort + list / cross-tenant 404 conflation / audit-row op_id binding). The MCP tools (T6 #1313) wrap the same service over the MCP transport; CLI verbs land in G12.5; the architecture doc lands in T7 #1314. |
...
| `AgentPrincipal` / `AgentPrincipalService` / `AgentPrincipalCreate` / `AgentPrincipalRead` / `AgentPrincipalListResponse` | `src/meho_backplane/auth/agent_principals.py` (service) + `src/meho_backplane/api/v1/agent_principals.py` (routes) + `src/meho_backplane/db/models.py` (ORM row) | G11.2-T1 (#815) agent-principal substrate. An agent principal is a Keycloak client tagged `kind=agent` that allows an autonomous agent to authenticate to MEHO via client-credentials flow. The `AgentPrincipal` SQLAlchemy model (migration `0019`) carries `tenant_id uuid NOT NULL REFERENCES tenant(id)`, a `name` unique-per-tenant natural key enforced by `agent_principal_tenant_name_idx`, the Keycloak-side identifiers (`keycloak_client_id`, `keycloak_internal_id`), `owner_sub` (the JWT `sub` of the owning operator), `revoked bool NOT NULL DEFAULT FALSE` (the kill-switch flag), and `created_by_sub`. The `AgentPrincipalService` exposes three async methods: `list(tenant_id, include_revoked=False)` returns all non-revoked (or all) principals sorted by name, `register(tenant_id, name, owner_sub, created_by_sub)` creates the Keycloak client (via a thin httpx-based `KeycloakAdminClient` wrapper, not `python-keycloak` — configured by `KEYCLOAK_ADMIN_URL` / `KEYCLOAK_ADMIN_CLIENT_ID` / `KEYCLOAK_ADMIN_CLIENT_SECRET`) with the `kind=agent` client attribute + inserts the DB row, and `revoke(tenant_id, name)` disables the Keycloak client (GET-then-PUT to preserve all client attributes including `kind=agent`) and sets `revoked=True`. A missing or empty `KEYCLOAK_ADMIN_URL` raises HTTP 503 `keycloak_admin_not_configured`; a name collision raises 409 `agent_principal_already_exists`; an unknown name raises 404 `agent_principal_not_found`. Four REST routes under `/api/v1/agent-principals`: `GET` (list, operator role), `POST` (register, tenant_admin role **+ `require_human_principal`** — a machine `principal_kind` gets 403 `human_principal_required` so an agent cannot mint another agent principal over REST, meho-internal#289), `GET /{name}` (show, operator role), `DELETE /{name}/revoke` (revoke, tenant_admin role). CLI surface: `meho agent-principal list/register/revoke` verbs in `cli/internal/cmd/agent-principal/`. G0.12-T4 (#1262, Initiative #1118) migrated those verbs off the per-package hand-rolled `doAuthedRequest` + consumer-side `Entry` / `ListResponse` duplicates onto the generated typed client (`api.ClientWithResponses` via `api.AuthedClient`); `api.AgentPrincipalRead` and `api.AgentPrincipalListResponse` are the single source of truth on the CLI side, kept in lock-step with the FastAPI Pydantic models by the `cli-api-snapshot-freshness` CI gate. Register is fail-closed on partial creation: the Phase-1 Keycloak create + secret capture lives in `_provision_keycloak_client`, whose `except BaseException` arm deletes the just-created client on **any** post-create failure (secret capture, Vault persist, or DB write) before re-raising — so a `get_client_secret` / Vault / DB error never leaves a live, un-revocable orphan client (#2523, mirroring the runner path #2502). The principal `name` is bounded by the shared `NAME_MAX_LENGTH` (128) reused from `auth.runner_principals` on both the `AgentPrincipalCreate` intake `Field` and the show/revoke `Path` params, so an over-long name is rejected 422 at the schema boundary rather than registering into a by-name kill switch that show/revoke can no longer reach. |
...
| `tests/integration/test_auth_keycloak_client_credentials.py` + `KeycloakBootstrap` / `keycloak_bootstrap` fixture | `backend/tests/integration/test_auth_keycloak_client_credentials.py` + `backend/tests/integration/conftest.py` + `backend/tests/integration/_fixtures/meho-integration-realm.json` | G11.2-T7 (#1098) live-Keycloak integration test that closes the deferred AC of G11.2-T2 (#816). The `keycloak_bootstrap` module-scoped fixture boots a Keycloak 26.x container (image pinned via `MEHO_TEST_KEYCLOAK_IMAGE` — **no docker.io default**, the fixture skips when unset because Keycloak images are ~600 MB and would exhaust the shared NAT egress pull bucket; CI sets the env var to a Harbor-proxied tag), bind-mounts the `meho-integration-realm.json` realm import at `/opt/keycloak/data/import/` (filename follows Keycloak's `<realm-name>-realm.json` convention), launches `start-dev --import-realm`, waits for the Quarkus "Listening on:" log line, and yields a frozen `KeycloakBootstrap` dataclass with the live `base_url` / `issuer_url` / `client_id` / `client_secret` / `audience` plus the expected `tenant_id` / `tenant_role` / `principal_kind` claim values the realm's hardcoded-claim mappers stamp onto every issued access token. The realm holds one confidential `serviceAccountsEnabled` client (`agent:test-bot`) with four protocol mappers: `oidc-audience-mapper` (stamps `meho-backplane-test` audience) plus three `oidc-hardcoded-claim-mapper` rows (`tenant_id` / `tenant_role=tenant_admin` / `principal_kind=agent`). The integration test exercises the **real** `client_credentials` grant end to end: `get_client_credentials_token(...)` against the container's token endpoint, then `verify_jwt_for_audience(f"Bearer {token}", expected_audience=...)` through the production JWT chain (not respx), asserting the resulting `Operator` carries `sub` (non-empty service-account UUID), `principal_kind=AGENT`, `tenant_role=TENANT_ADMIN`, `tenant_id` matching the realm's hardcoded value. The test temporarily overrides `KEYCLOAK_ISSUER_URL` to the container's realm issuer (the autouse `_integration_default_env` fixture pins a synthetic value at module load; `monkeypatch.setenv` + `get_settings.cache_clear()` + `clear_jwks_cache()` switch the chain to the live container). Bootstrap admin credentials use the Keycloak 26.x `KC_BOOTSTRAP_ADMIN_USERNAME` / `KC_BOOTSTRAP_ADMIN_PASSWORD` env vars (replacing the legacy `KEYCLOAK_ADMIN` / `KEYCLOAK_ADMIN_PASSWORD`). Skip path mirrors the other testcontainer suites: `MEHO_TEST_KEYCLOAK_IMAGE` unset → explicit-remediation skip; Docker socket missing → conftest's uniform skip; container boot failure (privileged denied, image pull rate-limit) → skip with the exception class name. Closes the integration AC of #816 deferred when no Keycloak testcontainer existed in the suite. |
| `tests/integration/test_tenant_isolation.py` | `backend/tests/integration/test_tenant_isolation.py` | G0.1-T6 (Task #236) broad-spectrum end-to-end test for the tenancy chain. Boots `pgvector/pgvector:pg16` via `testcontainers` (module-scoped fixture in `tests/integration/conftest.py`; image name env-overridable via `MEHO_TEST_PGVECTOR_IMAGE`), applies `alembic upgrade head` against the asyncpg URL, builds a fresh `FastAPI` with the production middleware stack plus the `/api/v1/rbac-test` stub routes mounted unconditionally, and exercises five integration cases: (1) two operators in two tenants generate correctly tenant-scoped audit rows (5+3 split, no cross-pollination); (2) a JWT signed with an unknown tenant_id still authenticates and lands its row under the bogus UUID — documents v0.2's "trust the issuer's claim" model; (3) the per-tenant query helper returns only the requesting tenant's rows (forward-compat for G8); (4) a `read_only` JWT on `/api/v1/rbac-test/operator` returns 403 — sanity for T4's RBAC primitive; (5) the highest-value test — eight interleaved requests under `asyncio.gather` from four operators across two tenants must produce audit rows attributed to the right tenant_id, catching structlog contextvar leaks across concurrent asyncio tasks. The `_skip_no_docker` class-level mark mirrors `tests/migrations/test_migration_rollback.py`'s pattern: agent sandboxes without Docker skip the PG-driven tests; CI runners with Docker provisioned run them. The cheap import-smoke test `test_module_imports_cleanly` runs unconditionally so a renamed fixture surfaces at collection time. |
...
5. Each HTTP request enters `RequestContextMiddleware.__call__`,
   ...
   - on the first `http.response.start` ASGI message, appends
     ...
     code,
   - records `http_requests_total{method,path,status}` and observes
     `http_request_duration_seconds{method,path,status}` (both bounding
     `path` to the matched route template or the `__unmatched__`
     constant), then emits a single `request_completed` log line with
     `duration_ms` (whose `path` keeps the literal URL for 404s).
...
| `prometheus-client` | ≥ 0.20 | Default process / GC collectors + the `http_requests_total` counter and `http_request_duration_seconds` histogram exposed on `/metrics`. |
...
| (dev) `testcontainers` | ≥ 4.0 | Spins up `pgvector/pgvector:pg16` for the testcontainer suites (`tests/test_db_engine.py::TestPostgresIntegration`, `tests/migrations/test_migration_rollback.py`, `tests/integration/test_tenant_isolation.py`); image name overridable via `MEHO_TEST_PGVECTOR_IMAGE`. Also drives the Vault dev-mode container (`MEHO_TEST_VAULT_IMAGE`), the vcsim container (`MEHO_TEST_VCSIM_IMAGE`), the k3s container (`MEHO_TEST_K3S_IMAGE`), and the Keycloak 26.x container (`MEHO_TEST_KEYCLOAK_IMAGE`, **no docker.io default** — G11.2-T7 #1098). Skipped gracefully when the Docker socket is absent — the SQLite-async coverage stays always-on. |
