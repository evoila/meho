...
| `FingerprintResult` / `ProbeResult` / `OperationResult` | `src/meho_backplane/connectors/schemas.py` | Frozen pydantic v2 result models for the three connector operations. `FingerprintResult` carries the full product identity shape (vendor / product / version / build / edition / reachable / probed_at / probe_method / extras). `ProbeResult` is the lightweight reachability + auth-challenge verdict (ok / reason / latency_ms / probed_at). `OperationResult` carries the typed op result as raw JSON dict or list (`result` field — no JSONFlux reduction at this layer) plus status / op_id / error / duration_ms / extras. All three are importable from `meho_backplane.connectors` (package root). Note: `connectors.ProbeResult` is distinct from `health.ProbeResult` (the readiness-probe dataclass). |
