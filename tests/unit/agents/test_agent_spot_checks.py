...
class TestSpotCheck3CrossSystemTrace:
    ...
    async def test_k8s_to_vmware_cross_system_trace(self) -> None:
        ...
        connector_ids_used = {
            tc[1].get("connector_id") for tc in tool_calls if "connector_id" in tc[1]
        }
    ...
    async def test_topology_exploration_sequence(self) -> None:
        ...
        tool_results = {
            ...
            "lookup_topology": make_tool_result(
                {
                    "found": True,
                    "entity": {"name": "payment-svc", "type": "Pod", "namespace": "production"},
                    "relationships": [
        ...
        llm_responses = make_step_sequence(
            [
                ...
                },
                {
                    ...
                    "action": "call_operation",
                    "action_input": {
                        "connector_id": "loki-main",
                        "operation_id": "query_logs",
                        "params": {"query": "{app='payment-svc'} |= 'error'", "limit": 10},
                    },
                },
                {
...
class TestScratchpadAccumulation:
    ...
    async def test_scratchpad_records_full_chain(self) -> None:
        ...
        llm_responses = make_step_sequence(
            [
                {"thought": "Need connectors", "action": "list_connectors", "action_input": {}},
                {"thought": "Done", "final_answer": "Found connectors."},
            ]
        )
