...
7. After all three image jobs succeed, `publish-to-public-repo` runs. It
   ...
   plus a copy-pastable `cosign verify` block for every image variant, and
   runs `gh release create <tag> --repo evoila/meho --notes-file <path>`.
   The PR-summary half generated by
   `POST /repos/evoila/meho/releases/generate-notes` is no longer used;
   the curated CHANGELOG entry replaces it. The private workflow repo
   no longer receives a Release. See
   ...
   and [Image signing with cosign](#image-signing-with-cosign).
