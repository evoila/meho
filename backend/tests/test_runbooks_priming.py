...
async def test_six_runs_collapses_to_summary() -> None:
    ...
    The summary references meho_runbook_list_runs and drops per-run blocks.
    """
    tenant_id = uuid.uuid4()
    await _seed_published_template(tenant_id, "drain", body=_two_step_template())
...
async def test_priming_text_includes_no_skip_no_force_advance_implication() -> None:
    ...
    ``meho_runbook_abort``, or ``meho_runbook_next`` from the rendered text regresses
    ...
    assert "meho_runbook_abort" in result.text
    assert "meho_runbook_next" in result.text
