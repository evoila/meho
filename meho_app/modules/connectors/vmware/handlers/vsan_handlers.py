...
class VsanHandlerMixin:
    ...
    def _check_vsan_enabled(self, cluster: Any, cluster_name: str) -> dict[str, Any] | None:
    ...
    async def _get_vsan_cluster_health(self, params: dict[str, Any]) -> dict:
        ...
        try:
            ...
            default_policy_uuid = (
                str(default_config.uuid)
                if default_config and hasattr(default_config, "uuid")
                else None
            )
    ...
    async def _get_vsan_disk_groups(
        self, params: dict[str, Any]
    ) -> list[dict] | dict:  # NOSONAR (cognitive complexity)
        ...
        for host in cluster.host or []:
            ...
            try:
                vsan_system = getattr(getattr(host, "configManager", None), "vsanSystem", None)
                if not vsan_system:
                    disk_groups.append({"host": host_name, "message": "vSAN system not available"})
                    continue

                config = getattr(vsan_system, "config", None)
                storage_info = getattr(config, "storageInfo", None) if config else None
                disk_mappings = getattr(storage_info, "diskMapping", None) if storage_info else None
    ...
    async def _get_vsan_capacity(self, params: dict[str, Any]) -> dict:
        ...
        try:
            ...
            uncommitted_gb = round(uncommitted_bytes / 1073741824, 2) if uncommitted_bytes else None
        ...
        try:
            ...
            if default_config:
                ...
                if hasattr(default_config, "hostFailuresToTolerate"):
                    policy_info["host_failures_to_tolerate"] = default_config.hostFailuresToTolerate
    ...
    async def _get_vsan_objects_health(
        self, params: dict[str, Any]
    ) -> dict | list:  # NOSONAR (cognitive complexity)
        ...
        for host in cluster.host or []:
            ...
            try:
                vsan_system = getattr(getattr(host, "configManager", None), "vsanSystem", None)
                if not vsan_system:
                    host_health.append({"host": host_name, "status": "vsan_system_unavailable"})
                    continue
