...
async def execute_workflow(  # NOSONAR (cognitive complexity)
    user_goal: str,
    connector_id: str,
    connector_name: str,
    connector_type: str,
    ...
    *,
    ...
    try:
        ...
        if isinstance(selection, no_relevant_operation_cls):
            reasoning = getattr(selection, "reasoning", "no reason provided")
            return WorkflowResult(
                success=True,
                findings=f"No relevant operation on {connector_name}: {reasoning}",
                steps_executed=state.steps_executed,
            )
        ...
        op_id: str = getattr(selection, "operation_id", "")
        op_params: dict = getattr(selection, "parameters", {})
        call_result = await call_op_node.run(state, emitter, op_id, op_params)
