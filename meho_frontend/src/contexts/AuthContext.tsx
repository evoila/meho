...
import {
  ...
  clearAuthToken,
  setRefreshTokenFn,
  onSessionExpired as setSessionExpiredCallback,
} from '@/api/clients/transport';
 ...
 *
 * Uses 'login-required' instead of 'check-sso' because modern browsers
 * (Chrome 146+) block third-party cookies, which breaks the silent SSO
 * iframe approach. With 'login-required', Keycloak redirects to the login
 * page only if there is no active session, and a valid session is picked
 * up via the redirect flow (no iframe needed).
 */
...
export function AuthProvider({ children }: Readonly<{ children: ReactNode }>) {
  ...
  /** Sync a new token to React state + transport layer (memory only, no storage) */
  ...
  const handleReAuth = useCallback(async () => {
    ...
    //
    // Note: INIT_OPTIONS no longer carries `silentCheckSsoRedirectUri` because
    // the default onLoad is 'login-required' (Chrome 146+ blocks the third-party
    // cookies that silent SSO depends on). That means this check-sso re-init
    // cannot perform a hidden-iframe check — `checkLoginIframe: false` is
    // inherited from INIT_OPTIONS. In practice the popup flow still works on
    // modern browsers because kc.init() reads the session cookie set on the
    // Keycloak origin during the popup's successful login and returns
    // authenticated=true via the standard redirect-less init path. If a browser
    // blocks all Keycloak cookies in the parent frame, the re-init will simply
    // return authenticated=false and the user falls through to the normal
    // 'login-required' redirect on next load — no worse than before this PR.
    try {
