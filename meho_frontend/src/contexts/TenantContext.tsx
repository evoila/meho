...
import {
  setTenantContext as setTransportTenantContext,
  clearTenantContext as clearTransportTenantContext,
...
function getInitialState(): TenantContextState | null {
  try {
    ...
    if (tenantId && displayName) {
      // Restore transport-level tenant context header
      setTransportTenantContext(tenantId);
...
export function TenantContextProvider({ children }: Readonly<TenantContextProviderProps>) {
  ...
  // Sync state changes to sessionStorage and transport layer
  useEffect(() => {
    saveState(state);

    if (state) {
      setTransportTenantContext(state.tenantId);
    } else {
      clearTransportTenantContext();
    }
  }, [state]);
