...
export function PlanPreview({ // NOSONAR (cognitive complexity)
  plan,
  workflowId,
            ...
            <div className="flex-1">
              <h3 className="text-lg font-semibold text-white flex items-center gap-2">
                {(() => {
                  ...
                  if (isFailed) return <XCircle className="h-5 w-5 text-red-400" />;
                  return <AlertTriangle className="h-5 w-5 text-yellow-400" />;
                })()}
                {(() => {
                  if (isCompleted) return 'Plan Completed';
                  if (isFailed) return 'Plan Failed';
