...
import { getOrchestratorSkillsClient } from '@/api/clients/orchestratorSkills';
import type {
...
export function OrchestratorSkillsPage() {
  const api = getOrchestratorSkillsClient();
  ...
  const editorSkill: OrchestratorSkill | null = (() => {
    if (isCreating && generatedContent) return {
      id: '',
      tenant_id: '',
      name: '',
      description: '',
            ...
            <div className="bg-zinc-800/50 border border-zinc-700/50 rounded-xl p-6 flex-1 overflow-y-auto">
              {(() => {
                if (isCreating || selectedId) {
                  if (isLoadingSkill && selectedId && !isCreating) return (
                    <div className="flex items-center justify-center h-full">
