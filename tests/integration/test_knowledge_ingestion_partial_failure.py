...
@pytest.mark.asyncio
@patch("meho_app.modules.knowledge.ingestion.generate_document_summary", new_callable=AsyncMock, return_value="Test summary")
@patch("meho_app.modules.knowledge.ingestion.build_chunk_prefix", return_value="")
async def test_partial_ingestion_cleans_up_chunks(
    _mock_prefix, _mock_summary, mock_knowledge_store_with_failure, mock_object_storage
):
    ...
    with pytest.raises(ValueError, match="Failed to create chunk|Failed to ingest"):
...
@patch("meho_app.modules.knowledge.ingestion.generate_document_summary", new_callable=AsyncMock, return_value="Test summary")
@patch("meho_app.modules.knowledge.ingestion.build_chunk_prefix", return_value="")
async def test_partial_ingestion_error_message(_mock_prefix, _mock_summary):
...
@patch("meho_app.modules.knowledge.ingestion.generate_document_summary", new_callable=AsyncMock, return_value="Test summary")
@patch("meho_app.modules.knowledge.ingestion.build_chunk_prefix", return_value="")
async def test_successful_ingestion_no_cleanup(_mock_prefix, _mock_summary):
