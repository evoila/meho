...
from sqlalchemy import delete, select, and_, func as sa_func
...
async def get_cleanup_statistics(session: AsyncSession) -> dict:
    ...
    total_result = await session.execute(
        select(sa_func.count()).select_from(KnowledgeChunkModel)
    )
    ...
    type_result = await session.execute(
        select(
            ...
            sa_func.count()
        ).group_by(KnowledgeChunkModel.knowledge_type)
    )
    chunks_by_type = dict(type_result.all())
    
    # Expired but not deleted (shouldn't happen if cleanup runs)
    expired_result = await session.execute(
        select(sa_func.count()).select_from(KnowledgeChunkModel).where(
            and_(
                KnowledgeChunkModel.expires_at.is_not(None),
                KnowledgeChunkModel.expires_at < now
            )
