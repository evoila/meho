...
class IngestTextRequest(BaseModel):
    ...
    scope: str = Field(
        default="tenant", description="ACL scope: global, tenant, system, team, or private"
    )
    ...
    try:
        ...
        global_doc_result = await session.execute(
            select(
                func.count(
                    func.distinct(
                        func.regexp_replace(KnowledgeChunkModel.source_uri, r"#chunk=\d+$", "", "g")
                    )
                ).label("doc_count"),
            ).where(
                KnowledgeChunkModel.tenant_id == user.tenant_id,
        ...
        type_counts = {
            row.connector_type_scope: {"chunk_count": row.chunk_count, "doc_count": row.doc_count}
        ...
        instance_counts = {
            str(row.connector_id): {"chunk_count": row.chunk_count, "doc_count": row.doc_count}
...
async def upload_document(
    ...
    connector_id: str | None = Form(
        default=None
    ),  # Required for instance scope, null for global/type
    knowledge_type: str = Form(default="documentation"),
    tags: str = Form(default="[]"),  # JSON array as string
    scope_type: str = Form(default="instance"),  # global, type, or instance
    connector_type_scope: str | None = Form(default=None),  # e.g. "kubernetes" for type-scoped docs
    user: UserContext = Depends(RequirePermission(Permission.KNOWLEDGE_INGEST)),
):
    """
    ...
    **PDF ONLY** - HTML and DOCX files should be converted to PDF first.
    ...
    if scope_type == "instance" and not connector_id:
        raise HTTPException(
            status_code=400, detail="connector_id is required for instance-scoped uploads"
        )
    if scope_type == "type" and not connector_type_scope:
        raise HTTPException(
            status_code=400, detail="connector_type_scope is required for type-scoped uploads"
        )
    ...
    **Temporary Notice (Event):**
    ...
    try:
        ...
        return IngestTextResponse(
            chunk_ids=result.get("chunk_ids", []), count=len(result.get("chunk_ids", []))
        )
    except Exception as e:
...
async def list_documents(
    user: CurrentUser,
    session: DbSession,
    status: str | None = Query(None, description="Filter by ingestion job status"),
...
@router.get("/connectors/{connector_id}/documents", response_model=ListDocumentsResponse)
...
@router.delete("/connectors/{connector_id}/documents/{document_id}")
async def delete_connector_document(
    connector_id: str,
    document_id: str,
    user: UserContext = Depends(RequirePermission(Permission.KNOWLEDGE_DELETE)),
    session: AsyncSession = Depends(get_db_session),
):
    """
...
async def _delete_document_background(
    document_id: str, deletion_job_id: str, chunk_ids: list[str], tenant_id: str, total_chunks: int
):
    """
