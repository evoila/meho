...
class TenantConfigRequest(BaseModel):
    ...
    model_override: str | None = Field(
        None, description="Override the default LLM model (e.g., 'anthropic:claude-opus-4-6')"
    )
...
async def get_dashboard_activity(
    ...
    session: AsyncSession = Depends(get_agent_session),
    repository: TenantConfigRepository = Depends(get_repository),
    limit: int = Query(default=20, ge=1, le=100, description="Maximum number of activity items"),
) -> list[ActivityItem]:
        ...
        if job.completed_at:
            activities.append(
                ActivityItem(
                    id=f"error-{job.id}",
                    type="error",
                    description=f"Ingestion job failed: {job.error_message or 'Unknown error'}",
