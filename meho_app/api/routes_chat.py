...
async def _handle_explain_last(
    session_id: str | None,
    user: "UserContext",
    db_session: AsyncSession,
) -> str:
...
async def get_conversation_history(
    session_id: str,
    agent_service: "AgentService",  # type: ignore  # noqa: F821
    limit: int = 10,
) -> list[dict[str, str]]:
    """
...
async def chat_stream(
    ...
    async def generate():
        ...
        async with session_maker() as session:
            ...
            agent_service = await get_agent_service_dep(session)
            ...
            try:
                ...
                dependencies = await create_agent_dependencies(user, session, request.message)
