...
import { parseSynthesis } from '@/features/chat/utils/parseSynthesis';
...
export function ChatPage() {
  ...
  const createSessionMutation = useMutation({
    mutationFn: () => chatClient.createSession(),
    onSuccess: (session) => {
  ...
  const hydrateApprovals = useCallback(async (sessionId: string) => {
    const store = useChatStore.getState();
    try {
      const dbApprovals = await chatClient.getPendingApprovals(sessionId);
      store.setApprovals(
        dbApprovals.map((pa) => ({
  ...
  const handleSelectSession = async (session: ChatSession | null) => {
    ...
    try {
      const store = useChatStore.getState();
      const sessionData = await chatClient.getSession(session.id);
      ...
      const loadedMessages: ChatMessage[] = sessionData.messages.map((msg) => {
        ...
        if (msg.role === 'assistant') {
          const structured = parseSynthesis(msg.content);
          if (structured) {
  ...
  useEffect(() => {
    ...
    if (sessionParam && sessionParam !== currentSessionId) {
      (async () => {
        try {
          const store = useChatStore.getState();
          const sessionData = await chatClient.getSession(sessionParam);
  ...
  }, [searchParams, chatClient, currentSessionId, setSearchParams, hydrateApprovals]);
          ...
          for (const iter of storeState.iterations) {
            for (const [connId, connector] of iter.connectors) {
              paneSnapshot.set(connId, {
                ...
                status: ({ running: 'running' as const, success: 'complete' as const })[connector.status as 'running' | 'success'] ?? 'error',
                isExpanded: false,
  ...
  const handleSessionModeChange = useCallback((mode: 'ask' | 'agent') => {
    ...
    if (sessionId) {
      chatClient.updateSessionMode(sessionId, mode).catch((err) => {
  ...
  const handleStartNewChat = useCallback(async () => {
    ...
    try {
      const result = await chatClient.summarizeSession(sessionId);
      ...
      const sessionData = await chatClient.getSession(result.new_session_id);
      const loadedMessages: ChatMessage[] = sessionData.messages.map((msg) => {
        const base: ChatMessage = {
          id: msg.id,
          role: msg.role,
  ...
  const handleApproveAction = async (approvalId: string) => {
    ...
    try {
      await chatClient.approveAction(currentSessionId, approvalId);
    } catch (error: unknown) {
  ...
  const handleRejectAction = async (approvalId: string) => {
    ...
    useChatStore.getState().setIsApprovingAction(true);
    try {
      await chatClient.rejectAction(currentSessionId, approvalId);
    } catch (error: unknown) {
                ...
                <button
                  onClick={() => {
                    ...
                    for (const [key, value] of Object.entries(pendingRecipe.params)) {
                      query = query.replaceAll(new RegExp(`\\{\\{${key}\\}\\}`, 'g'), value);
                    }
                    setPendingRecipe(null);
