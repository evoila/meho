...
export function SOAPWSDLPanel({ connectorId, connector, onSuccess }: Readonly<SOAPWSDLPanelProps>) {
  ...
  const handleIngestWSDL = async () => {
    ...
    setResult(null);
    try {
      const response = await connectorsClient.ingestWSDL(connectorId, wsdlUrl || undefined);
      setResult({
        success: true,
