...
class OrchestratorSkillService:
    ...
    async def update_skill(
        ...
        if content_changed:
            skill.is_customized = True  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment

        # Regenerate summary if content or name changed
        if content_changed or "name" in update_fields:
            skill.summary = await generate_skill_summary(str(skill.name), str(skill.content))  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment

        skill.updated_at = datetime.now(UTC)  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await self.session.flush()
        return skill
    ...
    async def delete_skill(self, tenant_id: str, skill_id: UUID) -> bool:
        ...
        return (result.rowcount or 0) > 0  # type: ignore[attr-defined]  # SQLAlchemy Result.rowcount exists at runtime
