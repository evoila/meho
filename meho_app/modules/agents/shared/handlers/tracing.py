        ...
        # For dicts, truncate string values
            ...
            for key, value in data.items():
                if isinstance(value, str) and len(value) > 500:
                    truncated[key] = value[:500] + f"... [truncated, total: {len(value)} chars]"
                elif isinstance(value, (list, dict)):
                    truncated[key], _ = truncate_data(value, max_size // 2)
                else:
                    truncated[key] = value
...
def _make_json_serializable(data: Any, max_depth: int = 10) -> Any:
    """
...
async def traced_tool_call(
    ...
    *,
    ...
    if connector_id:
        attrs["connector_id"] = str(connector_id) if connector_id else None
    if connector_name:
        attrs["connector_name"] = str(connector_name) if connector_name else None
    if connector_type:
        attrs["connector_type"] = str(connector_type) if connector_type else None
    if operation_id:
        attrs["operation_id"] = str(operation_id) if operation_id else None
    if user_id:
        attrs["user_id"] = str(user_id) if user_id else None
    if tenant_id:
        attrs["tenant_id"] = str(tenant_id) if tenant_id else None
    if session_id:
        attrs["session_id"] = str(session_id) if session_id else None
    if extra_attrs:
    ...
    class TracedSpan:
        """Wrapper for span to allow setting output after execution."""
