...
import { getRecipesClient } from '@/api/clients/recipes';
...
export function ChatHeader({
  ...
  const recipesClient = getRecipesClient();
  ...
  const visibilityMutation = useMutation({
    mutationFn: () => chatClient.updateSessionVisibility(sessionId ?? '', pendingVisibility ?? 'private'),
    onSuccess: () => {
  ...
  const createRecipeMutation = useMutation({
    mutationFn: () => recipesClient.createRecipeFromSession(sessionId ?? ''),
    onSuccess: () => {
          ...
          <div className="flex items-center gap-3">
            ...
            {sessionId && (
              ...
              >
                ...
                {(() => {
                  if (recipeCreated) return 'Created!';
                  if (createRecipeMutation.isPending) return 'Creating...';
