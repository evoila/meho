...
export function TeamSessionItem({ session, isActive, onClick }: Readonly<TeamSessionItemProps>) {
  // Show creator name or trigger source
  const attribution = (() => {
    if (session.trigger_source) return `via ${session.trigger_source}`;
    if (session.created_by_name) return `by ${session.created_by_name}`;
    return 'by Unknown';
  })();
