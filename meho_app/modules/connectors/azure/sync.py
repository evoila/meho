...
"""
Azure Operations Auto-Sync (Phase 92).

Automatically syncs Azure operations to the database when the operations
version changes. Also creates knowledge_chunk entries for hybrid search.

Based on gcp/sync.py pattern.
"""
...
from meho_app.modules.connectors.azure.operations import (
    AZURE_OPERATIONS,
...
def _format_azure_operation_as_text(op: OperationDefinition, connector_name: str = "Azure") -> str:
    """
    Format Azure operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.

    Args:
        op: Azure operation definition
    ...
    parts.append("Platform: Microsoft Azure")
    parts.append("")
    ...
    # Search keywords
    keywords = _generate_azure_search_keywords(op)
    if keywords:
...
def _generate_azure_search_keywords(op: OperationDefinition) -> str:
    """
    ...
    azure_abbrevs = {
        "vm": ["virtual machine", "vms", "compute", "instance"],
        "disk": ["managed disk", "storage", "volume"],
        "aks": ["kubernetes", "k8s", "cluster", "container service"],
        "node_pool": ["nodepool", "nodes", "worker", "agent pool"],
        "vnet": ["virtual network", "vpc", "networking"],
        "subnet": ["subnetwork", "subnets"],
        "nsg": ["network security group", "firewall", "security rules"],
        "load_balancer": ["lb", "load balancing", "frontend ip"],
        "storage_account": ["blob", "storage", "container"],
        "web_app": ["app service", "webapp", "web application"],
        "function": ["functions", "serverless", "function app"],
        "metric": ["monitoring", "metrics", "stats", "azure monitor"],
        "alert": ["alerting", "notification", "alarm"],
        "activity": ["activity log", "audit", "events"],
        "resource_group": ["rg", "resource groups"],
    }
    ...
    for key, abbrevs in azure_abbrevs.items():
    ...
    if "stop" in combined_text:
        keywords.update(["stop", "power off", "shutdown", "deallocate"])

    # Add Azure-specific keywords
    keywords.update(["azure", "microsoft", "cloud"])

    return " ".join(sorted(keywords))


async def sync_azure_operations_if_needed(
    session: AsyncSession,
    connector_id: str,
    tenant_id: str,
    current_version: str | None,
    knowledge_store: Optional["KnowledgeStore"] = None,
    connector_name: str = "Azure",
) -> tuple[int, int, int]:
    """
    Sync Azure operations if the version is outdated.
    ...
    if current_version == AZURE_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing Azure operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {AZURE_OPERATIONS_VERSION}"
    )
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(AZURE_OPERATIONS)} total, "
...
async def _sync_azure_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for Azure operations.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "azure",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing Azure knowledge chunks for connector {connector_id}")
    ...
    for op in AZURE_OPERATIONS:
        try:
            ...
            text = _format_azure_operation_as_text(op, connector_name)

            from meho_app.modules.knowledge.schemas import ChunkMetadata

            metadata_dict = {
                "resource_type": op.category,
                "keywords": [op.operation_id, op.name, op.category, "azure", "microsoft cloud"],
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=["api", "operation", "azure", "microsoft", op.category],
    ...
    logger.info(f"Created {chunks_created} knowledge chunks for Azure connector {connector_id}")
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = AZURE_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]
        await session.flush()
        logger.debug(f"Updated connector {connector_id} to version {AZURE_OPERATIONS_VERSION}")


async def sync_all_azure_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all Azure connectors to the latest operations version.
    ...
    # Find all Azure connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "azure")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No Azure connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} Azure connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        added, updated, chunks = await sync_azure_operations_if_needed(
            session=session,
            connector_id=str(connector.id),
            tenant_id=str(connector.tenant_id),
            current_version=current_version,
            knowledge_store=knowledge_store,
            connector_name=str(connector.name) if connector.name else "Azure",
        )
    ...
    summary = {
        ...
        "current_version": AZURE_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"Azure operation sync complete: "
        ...
        logger.info("All Azure connectors already at latest version")
