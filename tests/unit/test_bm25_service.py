...
@pytest.mark.asyncio
def test_tokenization_handles_mixed_case_and_special_chars(bm25_service, sample_tenant_id):
...
@pytest.mark.asyncio
def test_no_pickle_import_in_bm25_service():
    ...
    source_file = mod.__file__  # NOSONAR -- sync I/O in test is acceptable
    ...
    assert "pickle.loads" not in source_code, "bm25_service.py should not use pickle.loads"
    assert "pickle.dumps" not in source_code, "bm25_service.py should not use pickle.dumps"
