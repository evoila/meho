...
class TestToolRegistry:
    ...
    def test_tool_registry_total_count(self) -> None:
        """TOOL_REGISTRY has exactly 15 entries (10 existing + 4 diagnostics + search_types)."""
        assert len(TOOL_REGISTRY) == 15, (
            f"Expected 15 tools, got {len(TOOL_REGISTRY)}: {sorted(TOOL_REGISTRY.keys())}"
        )
    ...
    def test_tool_classes_have_required_attributes(self) -> None:
        ...
        for tool_cls in tools:
            ...
            assert hasattr(tool_cls, "TOOL_DESCRIPTION"), (
                f"{tool_cls.__name__} missing TOOL_DESCRIPTION"
            )
...
class TestToolActionModels:
    ...
    def test_react_step_parses_diagnostic_actions(self) -> None:
        ...
        for tool_name, action_data in [
            (
                "dns_resolve",
                {"tool": "dns_resolve", "hostname": "example.com", "record_types": ["A"]},
            ),
...
class TestDnsResolveExecution:
    ...
    async def test_dns_resolve_multiple_types(self) -> None:
        ...
        def mock_query(hostname, rtype):
            if rtype == "A":
                return mock_a_result
            if rtype == "MX":
                return mock_mx_result
            raise RuntimeError(f"Unexpected type: {rtype}")
...
class TestTcpProbeExecution:
    ...
    async def test_tcp_probe_connected(self) -> None:
        ...
        with patch(
            "meho_app.modules.agents.react_agent.tools.tcp_probe.asyncio.open_connection",
            new_callable=AsyncMock,
        ) as mock_conn:
...
class TestTlsCheckExecution:
    ...
    async def test_tls_check_invalid_chain(self) -> None:
        ...
        assert output.error is not None
        assert (
            "verification failed" in output.error.lower() or "verify failed" in output.error.lower()
        )
