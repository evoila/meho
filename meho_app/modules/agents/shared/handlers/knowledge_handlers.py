...
async def reduce_data_handler(deps: MEHOGraphDeps, args: dict[str, Any]) -> str:
    ...
    async with traced_tool_call(
        ...
        try:
            sql_result = await executor.execute_sql_async(session_id, sql)
            ...
            trace_sql_query(
                ...
                sql=sql,
                row_count=row_count,
                error=sql_result.get("error") if not success else None,
            )

            if success:
                logger.info(f"✅ SQL returned {row_count} rows")
                span.set_output(sql_result)
                span.add_attribute("row_count", row_count)
            else:
                logger.warning(f"⚠️ SQL error: {sql_result.get('error')}")
                span.set_error(sql_result.get("error", "SQL execution failed"))

            return json.dumps(sql_result, indent=2, default=str)
