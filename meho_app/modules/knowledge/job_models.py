...
class DeletionStage(StrEnum):
    ...
    UPDATING_INDEX = "updating_index"  # Finalizing search state
...
class IngestionJob(Base):
    ...
    scope_type = Column(String(20), nullable=True, index=True)  # global, type, instance
    connector_type_scope = Column(String(100), nullable=True)  # e.g. "vmware", "kubernetes"
    ...
    # Nullable at DB level (kept nullable for historical rows and to keep the
    # migration reversible), but enforced as required by the API for new uploads.
    family_id = Column(
        UUID(as_uuid=True),
    ...
    # Resume support
    file_hash = Column(String(64), nullable=True)  # SHA-256 hex digest for file identity
    storage_key = Column(String(1024), nullable=True)  # MinIO key for the uploaded document

    # Document summary (LLM-generated, 1-2 sentences for preview)
