...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX ix_endpoint_enabled ON endpoint_descriptor (connector_id, is_enabled)"
    )
    op.execute(
    ...
    """)
    op.execute(
        "CREATE INDEX ix_user_connector_credential_user_id "
        "ON user_connector_credential (user_id)"
    )
    op.execute(
        "CREATE UNIQUE INDEX ix_user_connector "
        "ON user_connector_credential (user_id, connector_id)"
    )
    ...
    """)
    op.execute(
        "CREATE INDEX ix_soap_op_connector ON soap_operation_descriptor (connector_id)"
    )
    op.execute(
    ...
    op.execute(
        "CREATE INDEX ix_soap_type_connector_name "
        "ON soap_type_descriptor (connector_id, type_name)"
    )
    ...
    op.execute(
        "CREATE INDEX ix_conn_op_category ON connector_operation (connector_id, category)"
    )
