...
that, a single summary block points at ``meho_runbook_list_runs`` so the
...
#: ``meho_runbook_list_runs`` (the read tool from G12.3-T6, #1313) and
...
async def assemble_runbook_priming(
    ...
    * **>N=5** -- one summary block referring the agent to
      ``meho_runbook_list_runs``. ``RunbookPrimingResult(text, len(runs), True)``.
...
def _render_run_block(run: RunSummary) -> str:
    ...
    body = (
        ...
        "If the step looks wrong, call meho_runbook_abort and escalate to a "
        "senior in chat.\n"
        "Use meho_runbook_next to advance once the current step's verify "
        "passes."
    )
...
def _render_summary_block(count: int) -> str:
    ...
    Refers the agent to ``meho_runbook_list_runs`` (the read tool from
    ...
    body = (
        ...
        "Call meho_runbook_list_runs to see them and proceed one at a time.\n"
