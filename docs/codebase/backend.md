...
| `0007_create_broadcast_override` | `backend/alembic/versions/0007_create_broadcast_override.py` | G6.3-T1 (Task #378) PII-override schema substrate. Creates the `broadcast_override` table -- per-tenant rules that the G6.3 resolver (T2 #379) reads at publish time to downgrade normally-full-detail operations to `aggregate`-only on the SSE broadcast feed. `tenant_id` UUID NOT NULL with a real `REFERENCES tenant(id)` FK (brand-new table, no chassis-era rows, FK lands at create-time -- same precedent as migration `0003` for `documents`). `op_id_pattern` text NOT NULL (glob; the resolver in T2 walks the per-tenant rule set in Python, no DB-side glob index). `scope_field` / `scope_value` text nullable (NULL = op-wide rule; the small allowlist `"namespace"` / `"target_name"` is enforced at the Pydantic layer in T4 #381, **not** as a DB CHECK -- the forward-compat argument flips the precedent that `targets.auth_model` / `operation_group.review_status` / `endpoint_descriptor.source_kind` / `endpoint_descriptor.safety_level` set with their CHECK constraints; `detail` (also a bounded enum) follows the same Pydantic-only treatment for one consistent enforcement point). `created_by_sub` text NOT NULL stamps the writing tenant admin's JWT `sub` for the audit-trail. Two indexes: `broadcast_override_tenant_unique_idx` unique composite b-tree on `(tenant_id, op_id_pattern, scope_field, scope_value)` (the natural-key target for T4's upsert; SQL `NULL != NULL` semantics let two op-wide `(NULL, NULL)` rows coexist under a vanilla composite UNIQUE, which v0.2 accepts because the resolver in T2 disambiguates via deterministic first-match by `created_at`; if this becomes an operational problem the partial-index split that `operation_group` uses can land as a v0.2.next tightening); `broadcast_override_tenant_idx` b-tree on `tenant_id` (drives the resolver's per-tenant cache hydration). Dialect-portable: PG gets `gen_random_uuid()` / `now()` server defaults; SQLite leaves them to the ORM Python-side. Downgrade drops both indexes + the table in reverse order. **Numbering note**: the Initiative body (#376) and Task body (#378) referenced this migration as `0004_create_broadcast_override.py` -- that was draft-time numbering before migrations `0004`-`0006` shipped under G0.3 / G0.6; renumbered to `0007` at landing time, the acceptance criteria are about behaviour not filename literal. |
...
| `resolve_target` / `TargetNotFoundError` / `AmbiguousTargetError` | `src/meho_backplane/targets/resolver.py` | Universal `query → Target ORM row` resolver (Task #253). Algorithm: (1) exact name match `WHERE tenant_id = ? AND name = ?`; (2) element-equality alias match (`query = ANY(aliases)` on PG, Python-side `in` on SQLite); (3) prefix-ILIKE near-miss (up to 5, name + aliases on PG, name-only on SQLite) → `TargetNotFoundError`. `AmbiguousTargetError` (HTTP 409) fires when multiple rows match the alias step (defensive — the unique index prevents name collision but not alias collision). Both error classes extend `HTTPException` for clean FastAPI propagation; CLI verbs catch them and render human-readable suggestions. |
...
| `KubernetesConnector` / `product_from_git_version` | `src/meho_backplane/connectors/kubernetes/connector.py` | G3.2-T1 (#321) canary skeleton for the Kubernetes connector Initiative (#320). Subclasses `Connector` with `product = "kubernetes"`. `fingerprint(target)` calls `VersionApi.get_code()` against a cached `ApiClient` and maps the returned `gitVersion` suffix to a distribution slug (`rke2` / `k3s` / `eks` / `gke` / `aks` / `vanilla`) via `product_from_git_version`. `probe(target)` is **kubeconfig-free**: TLS GET against `https://{host}:{port or 6443}/readyz` with `verify=False` (probe is reachability, not auth — 200 / 401 both count as ok; auth surfaces at op time). `execute(target, op_id, params)` is the skeleton dispatcher — returns `OperationResult(status="error", error=f"unknown_op: {op_id}", duration_ms=0.0, extras={"known_ops": []})` for every op_id; per-op handlers land in T2–T5 (#322–#325). Per-target `ApiClient` cache guarded by a single `asyncio.Lock`; `aclose()` closes every cached client and clears the cache (idempotent). |
...
| `connectors/kubernetes` package registration | `src/meho_backplane/connectors/kubernetes/__init__.py` | Imports the public surface (`KubernetesConnector`, `product_from_git_version`, the kubeconfig types) and conditionally calls `register_connector("kubernetes", KubernetesConnector)`. The registry itself ships with G0.2-T2 (#241, PR #295 in flight); the registration block degrades to an info-level structlog event (`connector_registry_deferred`) on `ImportError` so the package is importable both before and after #241 lands. Once #241 lands the conditional resolves to a real registration without any code change here. |
| `tests/test_connectors_k8s_auth.py` | `backend/tests/test_connectors_k8s_auth.py` | G3.2-T1 (#321) unit suite. Covers `product_from_git_version` across all six distro mappings, the fingerprint shape against a mocked `VersionApi.get_code()` response, probe ok/not-ok across HTTP 200 / 401 / 503 / transport-error, the default-port fallback when `target.port is None`, `execute` returning the structured `unknown_op` shape, `_get_api_client` caching (same target reuses, different targets get distinct clients, loader called once per target), `aclose` closing every cached client and being idempotent, and `parse_kubeconfig_yaml` rejecting non-mapping YAML. The `kubernetes_asyncio` and `httpx` modules are mocked via `unittest.mock.patch` on the import path inside the connector module so the gate runs without Docker. The default `load_kubeconfig_from_vault` is exercised once to lock its `NotImplementedError` contract until G0.3 / the live override lands. |
