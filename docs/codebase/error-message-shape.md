...
- `connector_not_found` (all seven `meho.connector.*` `connector_id`
  tools — review/edit_group/edit_op/enable/enable_reads/disable/delete:
  an unknown or cross-tenant label maps to `-32602 connector_not_found`
  rather than the dispatcher's bare `-32603 "internal error:
  ConnectorNotFoundError"` — #2481; existence is not leaked, so no
  `data` envelope, matching the REST 404's three indistinguishable
  failure modes)
...
| `meho_connector_review` **and** `meho_connector_enable_reads` MCP tools (`connector_scope_ambiguous`, **MCP**) | JSON-RPC `-32602` with structured `data` carrying the same `build_connector_scope_ambiguous_detail` envelope as the REST 409 row above — `detail="connector_scope_ambiguous"`, `connector_id`, `candidates[]` (each `{product, version, impl_id, tenant_id}`), `message`. Dispatched by `raise_invalid_params_for_ambiguous_scope` (`mcp/tools/_connector_shared.py`), caught by the `except AmbiguousConnectorScopeError` arms in `_review_handler` / `_enable_reads_handler` (`mcp/tools/connector_admin.py`). One builder shared with REST, so the MCP `error.data` member and the REST 409 `detail` can't drift. | compliant (structured) — landed in #1910. Before #1910 these two fell through the dispatcher's generic `except Exception` to a bare `-32603 "internal error: AmbiguousConnectorScopeError"` with the candidate list discarded, while REST attached the detail (the MCP↔REST asymmetry #1910 closed, same shape #1534 closed for the ingest SpecError siblings). MCP's only structured handler-error channel is `-32602`, so the wire *code* differs from REST's 409 while the `data` envelope is identical — an agent reads `error.data.candidates` and re-issues with the disambiguating `prefer=tenant\|builtin` arg the two tool schemas added in #2029 (the closed-set selector the message names; `tenant_id` still selects which tenant the operator acts under). | — |
| All seven `meho.connector.*` `connector_id` tools (`connector_not_found`, **MCP**) — `review` / `edit_group` / `edit_op` / `enable` / `enable_reads` / `disable` / `delete` | JSON-RPC `-32602` with the bare domain message `connector_not_found` and **no** `data` envelope (existence is not leaked — an unknown label and a cross-tenant label return the identical error). Dispatched by `raise_invalid_params_for_connector_not_found` (`mcp/tools/_connector_shared.py`), caught by the `except ConnectorNotFoundError` arm in each handler in `mcp/tools/connector_admin.py`; the resolver (`ReviewService._resolve_*` / `delete_connector`) raises `ConnectorNotFoundError`. Matches the family-wide `-32602 <thing>_not_found` convention (`agent_not_found`, `approval_request_not_found`, `ingest_job_not_found`). | compliant (bare-code family pattern) — landed in #2481. Before #2481 all seven fell through the dispatcher's generic `except Exception` to a bare `-32603 "internal error: ConnectorNotFoundError"` — the wrong JSON-RPC class (`-32603` signals a server bug, not a well-formed request against a nonexistent name) *and* a leak of the Python class name into the wire contract. REST already maps `ConnectorNotFoundError` → 404 (`api/v1/connectors_ingest.py`), so this closed the last MCP↔REST not-found asymmetry, the same shape #1910 closed for ambiguous-scope and #1534 for the SpecError siblings. | — |
| `meho_connector_edit_op` **and** `meho_connector_edit_group` MCP tools (no-editable-field / bad `safety_level` `ValueError`, **MCP**) | JSON-RPC `-32602` whose **message** is the service's rendered `ValueError` string (`edit_op requires at least one of ...` / `edit_group requires at least one of when_to_use or name` / `safety_level 'x' not in [...]`) and **no** `data` envelope — the message names the editable fields, so it is load-bearing and forwarded verbatim; the Python class name never reaches the wire. Dispatched by `raise_invalid_params_for_edit_value_error` (`mcp/tools/_connector_shared.py`), caught by the `except ValueError` arm in `_edit_op_handler` / `_edit_group_handler` (`mcp/tools/connector_admin.py`); the `ValueError` is raised by `validate_edit_op_args` / `ReviewService.edit_group`. Unlike the sibling `connector_not_found` row this carries a message (not a bare stable code) — parity with the REST `400 detail=str(exc)` the routes already return. | compliant (message-carrying, REST-400 parity) — landed in #2488. Before #2488 both fell through the dispatcher's generic `except Exception` to a bare `-32603 "internal error: ValueError"` — the wrong JSON-RPC class (`-32603` signals a server bug, not a caller-fixable request) *and* a class-name leak. The same change taught the service to honour an explicit `custom_description: null` as a clear-to-NULL (the `_UNSET` sentinel), distinct from an omitted field. | — |
...
| `GET /api/v1/health` audit-replay capture state (signal 11) | `mcp_session_id_capture: "when_negotiated" \| "enforced"` field on `HealthResponse` — `"when_negotiated"` is the default (a session id is captured only when the client negotiated one via the `initialize` handshake; header-less calls are accepted and their row is nulled — #2700 renamed this from the misleading `"always"`, which read as a guarantee); `"enforced"` lights up when `MCP_REQUIRE_SESSION_ID=true` (additionally rejects header-less calls). Sourced from `mcp_session_id_capture_mode()` in `mcp/server.py`. | compliant *as a discoverability surface* — landed in T6 #1147 alongside the capture-vs-enforcement decouple; relabelled honest in #2700. T7 #1148's `/ready` features block reads from the same helper for the richer `audit_replay` block. | T6 #1147 / #2700 |
...
| Connector upstream **auth/session failure** on dispatch **or at session establish** (any connector; the vRLI dispatch the operator saw as opaque `connector_error (440)`, #1798; the vSphere `POST /api/session` 401 the operator saw as bare `connector_error: RuntimeError` after a service-account password rotation, #2329) | structured `extras` with `error_code='connector_auth_failed'`, `cause` (the **stage-aware** sub-code (#2400) — `session_establish_401` / `session_establish_403` when the login POST itself was rejected (#2329); `dispatch_<status>` when an auth-class status hit a connector with **no** `invalidate_session` hook, so no session stage happened and no re-login was attempted; `session_dispatch_<status>_after_relogin` when the dispatcher force-re-logged-in, the re-login SUCCEEDED, and the fresh session was **still** rejected — the old ambiguous unqualified `session_dispatch_<status>` is no longer emitted on any path, a cause-string rename consumers matching on `session_dispatch_` must note), `http_status` (the actual auth-class status the upstream returned — `401` / `403` at establish, `401` or `440` on dispatch, not a hard-coded value), `target` (the target name), `host` (the `target.host` the operator configured — not an info-leak, the operator's own value, same posture as the TLS row and `/ui/auth/login`), `secret_ref` (the Vault ref the operator restages, when known — `null` otherwise), `remediation` (#2400: stage-aware — the establish / `dispatch_<status>` stages name the **real** restage command `meho vault kv put <mount> <path> --data <field>=<value>` (writing the corrected secret back to its Vault `secret_ref`); the `after_relogin` stage names **no** command and explicitly says do NOT restage — the credential logs in fine, so verify the target's `auth_model` / auth scheme instead. The prior text pointed at a phantom `meho target credential set <name>` command that does not exist in the CLI, #2329), `raw_message` (the connector's original establish message, preserved for debugging — `null` on the dispatch path), `upstream_message` (the upstream body's `message` field when JSON, else capped raw text, `null` when the body was empty — shared `_http_upstream_message` extractor with the 403/422 builders); the `error` names the host, the status, the likely cause **connector-agnostically** (a session/credential expiry or a rotated/stale/locked-out credential — the **dispatch path** re-logs-in and retries once on a session-expiry status (`401` or vRLI's `440`) **only** when the connector advertises `invalidate_session` (#2067), so the summary is now stage-aware (#2400): on `dispatch_<status>` (no hook) it states that no re-login was attempted; **only** on `session_dispatch_<status>_after_relogin` does it say the re-login already succeeded and the fresh session was *still* rejected; at **session establish** a `401`/`403` on the login POST means the login itself was rejected), appends the stage-appropriate remediation (restage, or — on `after_relogin` — do-not-restage / check the auth scheme) + a `docs/architecture/connector-auth.md` ref + this doc ref, and tails the upstream message when present. **Establish-time convergence (#2329):** the family VCF connectors (vSphere `/api/session`, NSX `/api/session/create`, VCF Automation provider/tenant logins, SDDC Manager `/v1/tokens`, vRLI `/api/v2/sessions`) previously wrapped an establish 401/403 into a *bare* `RuntimeError` that flattened to `connector_error`; they now raise a structured `ConnectorAuthError` (a `SessionLoginError`/`RuntimeError` subclass, built via `session_establish_auth_error` in `connectors/_shared/vcf_auth.py`) which the dispatcher recognises ahead of its generic `except Exception` and routes to the **same** `result_connector_auth_failed` builder — so the code is identical regardless of *when* auth died, and the #2067 mid-session re-establish emits it too when its one-shot re-login is itself rejected. **Profiled convergence (#2414):** a profiled session-scheme connector's login POST is now covered too — `ProfiledRestConnector._post_login` routes a login-POST `401`/`403` through the **same** `session_establish_auth_error` classifier, so it raises `ConnectorAuthError` (establish stage → `session_establish_<status>`, restage) instead of the raw `httpx.HTTPStatusError`. Before #2414 that raw error reached the dispatcher's mid-session retry arm (a profiled connector advertises `invalidate_session`, so the failing login was re-dispatched once) and was mislabelled `session_dispatch_<status>_after_relogin` (do-NOT-restage) — wrong, because the login itself was refused. The fix keeps `reestablished`/`after_relogin` reserved for a genuine post-re-login *dispatch* failure (the re-login POST succeeded and the fresh session was still rejected); a non-auth login-POST status (404, 5xx) still re-raises the raw error unchanged. The recognised auth-status set depends on the connector: a **typed (hand-coded)** connector uses the module constant `_AUTH_FAILED_STATUSES = {401, 440}` (`401` load-bearing; `440` is vRLI's session-expiry status the team opted to recognise); a **profiled** connector instead declares its set once on `ExecutionProfile.expiry_statuses` (default `{401}`; vRLI `{401, 440}`) — the single source #1973 unifies across the session-retry harness and this classification arm — which the dispatcher threads into `is_auth_failed_status(status_code, expiry_statuses)` via `_profile_expiry_statuses(connector_instance)` (returns the profile's set, or `None` → the typed global). No per-status remediation grammar is introduced; the profile only parameterises the closed status set. Built by `result_connector_auth_failed` in `operations/_errors.py` (reading `exc.response`); the dispatcher's `_run_branch_with_error_handling` catches the `httpx.HTTPStatusError` and delegates to `_handle_http_status_error`, which (#2067) attempts the invalidate-and-retry-once recovery on an auth-class status before classifying — `result_connector_auth_failed` is returned only after the retry is exhausted (re-login failed), or immediately when the connector has no `invalidate_session` hook; the 403/422 siblings are classified in the same `_classify_http_status_error` map, **after** the 403/422 checks and **ahead of** the generic `connector_error` catch. The error audit row is deferred until a failure is actually returned, so a *recovered* call writes exactly one **success** row and no spurious error row. Retry-once is safe for non-idempotent verbs because a `401`/`440` is rejected pre-execution (no side effect); a 5xx/timeout is never retried. Before #1804 a 401/440 flattened to a bare `connector_error: HTTPStatusError` with the auth cause buried in `extras.exception_message` — exactly the diagnosability gap that made #1798's `connector_error (440)` look like a stub-auth problem; before #2067 the classification was correct but recovery was missing, so an expired vCenter (401) / vRLI (440) session hard-failed until a backplane restart. Every other `HTTPStatusError` status (404, 5xx, 429) falls through to `connector_error` unchanged, and an establish-time non-auth status (404 endpoint-not-served, 5xx appliance fault, token-less 2xx) keeps its existing `RuntimeError`/`SessionLoginError` shape. | compliant (structured) — landed in #1804; profile-declared set #1973; dispatch-path recovery #2067; establish-time family coverage + `cause`/`target`/`secret_ref`/`remediation`/`raw_message` fields #2329 | 429 (rate-limit) is a deliberate follow-up, not this surface |
...
| `net.*` probe **refused by the probe allowlist** on dispatch (`net.tcp_check` / `dns_lookup` / `tls_inspect` / `http_probe` / `ntp_check` / `ping` / `trace` / `path_mtu`; the reverted-`MEHO_NETDIAG_PROBE_ALLOWLIST` redeploy that paged as an 11-hour fleet outage, #2784) | structured `extras` with `error_code='connector_probe_refused'`, `allowlist_env` (`MEHO_NETDIAG_PROBE_ALLOWLIST`), `host` (the destination string the **caller** supplied — `assert_probe_allowed` resolves nothing, so echoing it back is not a DNS/topology oracle; it also preserves the connector's audit-visible-host foundation on the error row, which carries no `raw_payload`), `detail` (the guard's own message, Tier-1-redacted + capped), `exception_class` (`ProbeNotAllowedError`); the `error` names the env var, states that **no socket was opened so the result is not a reachability answer**, carries the add-the-CIDR-or-hostname remediation plus the `netdiag.probeAllowlist` chart key that renders it, and cites `docs/codebase/connectors-net-diagnostics.md` + this doc — deliberately **address-free** in the message itself. Built by `result_connector_probe_refused` in `operations/_errors.py`; narrowed out of the dispatcher's generic `except Exception` arm by `is_probe_refused` (`ProbeNotAllowedError` subclasses `ValueError`, which no earlier arm catches, and the type is resolved through a **deferred** import because importing `connectors.net` at `operations` module scope would close a cycle back through `operations/__init__`). Before #2784 the handlers converted the refusal into a reading-shaped `status="ok"` result (`connected=false`, `reason="not_in_probe_allowlist"`), so a Sensor asserting on `$.connected` read *policy refused to dial* as *host is down* and flipped `critical`; as a dispatch error the check-runner's existing non-`ok` mapping lands it in `unknown` with `reason: dispatch_not_ok`. `net.http_probe`'s mid-chain **redirect** re-gate is the deliberate carve-out and stays `status="ok"` / `reachable=true` / `reason="blocked_redirect"` — the prior hop answered, so that is a real observation. | compliant (structured) — landed in #2784 | — |
| vSphere `PropertyCollector.RetrievePropertiesEx` explicit **`ManagedObjectNotFound`** fault on dispatch (`vmware.vm.info`, `host.usage`, `host.storage_devices`, `host.network_uplinks`, `object.collect`, #3481) | vCenter sends this addressed missing-resource condition as a SOAP-shaped HTTP 500. Those five typed read operations explicitly opt into promotion at the shared `RetrievePropertiesEx` seam; the `vmware-rest` connector then parses only the explicit `fault_type='ManagedObjectNotFound'` on the VI-JSON or ESXi SOAP transport and raises `ConnectorResourceNotFoundError`. The dispatcher catches that type before `connector_error` and returns `status='not_found'`, `error` prefixed `not_found:`, and `extras` with stable `error_code='not_found'`, `resource_ids`, plus singular `resource_id` for one MoID. The queried MoID is caller-supplied and remains available for recovery; the error row audits the same envelope. Any other vim fault, a non-PropertyCollector method, or a transport-only 5xx remains the generic `connector_error` result — status 500 alone never proves absence. The shared seam defaults promotion off, preserving task-poll and destructive-composite preflight/execute outcomes for #3479. Built by `result_connector_not_found` in `operations/_errors.py`; see `docs/codebase/connectors-vmware-rest.md`. | compliant (structured) — landed in #3481 | #3479 owns `vm.destroy` execute/preview handling |
| Connector upstream **non-4xx-classified HTTP status** (404 / 429 / any 5xx) on dispatch (any connector; the approved `argocd.app.sync` dry-run whose sync POST 500'd with a gRPC-gateway body carrying `message="application dry-run failed: <detail>"`, #2680) | structured `extras` with `error_code='connector_error'` — the status is **not** reclassified into the dedicated `connector_http_403` / `connector_http_422` / `connector_auth_failed` shapes (those stay scoped to 403 / 422 / the auth-class set), so the generic code and the `error` summary (`connector_error: HTTPStatusError`) are unchanged and existing string matchers keep working — plus `exception_class='HTTPStatusError'`, `exception_message` (`str(exc)`, the httpx status line + URL, redacted + capped), and, **new in #2680**, `http_status` (the actual status) and `upstream_message` (the upstream body's `message` field when JSON, else capped raw text, `null` when the body was empty — the same `_http_upstream_message` extractor the 403/422/auth builders use, so the vendor body is Tier-1-redacted + capped). Built by `result_connector_error` in `operations/_errors.py` via an `isinstance(exc, httpx.HTTPStatusError)` branch; reached from `_classify_http_status_error`'s fall-through for every status the dedicated builders do not own. Before #2680 a 5xx discarded the vendor body entirely (`str(HTTPStatusError)` never includes it), so an ArgoCD `"application dry-run failed: <detail>"` 500 was as opaque as the pre-#1649 403/422. | compliant (structured) — landed in #2680 | 429 (rate-limit) a friendlier dedicated code is still a deliberate follow-up |
| Dispatcher **error-row audit payload** (every `_run_branch_with_error_handling` arm + the redaction / reducer / HTTP-retry error paths; the approved-write forensics gap #2680) | the DISPATCH `audit_log.payload` for an error row now carries a nested `error` object holding the same structured envelope the caller's `OperationResult.extras` received (`error_code`, plus `http_status` + `upstream_message` on an upstream HTTP error, and the per-builder diagnostic fields), not merely `result_status='error'`. The dispatcher builds the structured result **first**, then `_audit_error_and_return` threads its extras through `audit_and_broadcast_safe` → `write_audit_row` → `_build_audit_payload` (inverting the legacy audit-then-build ordering, where the envelope was built *after* the audit write and never reached the row). Applied uniformly to **all** error arms — a `connector_error` / `connector_auth_failed` / `connector_tls_verify_failed` / `connector_unsupported` / `connector_vault_forbidden` / `handler_unreachable` failed dispatch all persist their envelope. The envelope is already Tier-1-redacted + capped by the `operations/_errors.py` builders (durable-ledger-safe) and rides the audit row **only** — the params-only broadcast frame (`publish_broadcast`) is untouched. Before #2680 a failed approved write recorded no reason on the ledger: the approver's ephemeral `dispatch` block saw the real error, the durable row saw only `result_status='error'`. | compliant (structured) — landed in #2680 | — |
| `PATCH /api/v1/connectors/{id}/operations/{op_id}` 200 with `is_enabled=true` on an op whose resolved connector is the unconfigured ingest auto-shim (the *proactive* counterpart of the `connector_unsupported` dispatch row above) | structured `warnings[]` on the 200 `EditOpResponse` (the route returned 204 before #1630): `code='unreplaced_auto_shim'` (same vocabulary as the dispatch error's `extras.cause`), `connector_class`, `message` (what was applied, the guaranteed `connector_unsupported` dead end ahead, the register-the-per-product-subclass remediation + `docs/codebase/spec-ingestion.md` ref). Built by `enable_time_auto_shim_warnings` in `operations/ingest/_internals.py` over the `resolved_auto_shim_class` resolver replay (`connector_registration.py`); mirrored verbatim by the `meho_connector_edit_op` MCP tool's `warnings` key and rendered to stderr by `meho connector edit-op` as `warning (unreplaced_auto_shim): ...`. Advisory only — the write lands, warnings or not (a shim-backed op may be pre-enabled ahead of its subclass). | compliant (structured) — landed in #1630 | — |
