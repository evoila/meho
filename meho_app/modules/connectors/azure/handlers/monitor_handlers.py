...
class MonitorHandlerMixin:
    ...
    async def _handle_get_azure_metrics(
    ...
    async def _handle_list_azure_metric_definitions(
        self: "AzureConnector", params: dict[str, Any]
    ) -> list[dict[str, Any]]:
        ...
        async for defn in self._monitor_client.metric_definitions.list(resource_uri):
            ...
            results.append({
                ...
                "primary_aggregation_type": str(defn.primary_aggregation_type) if defn.primary_aggregation_type else None,
    ...
    async def _handle_list_azure_metric_namespaces(
        self: "AzureConnector", params: dict[str, Any]
    ) -> list[dict[str, Any]]:
        ...
        async for ns in self._monitor_client.metric_namespaces.list(resource_uri):
            results.append({
                ...
                "fully_qualified_name": getattr(ns.properties, "metric_namespace_name", None) if ns.properties else None,
    ...
    async def _handle_list_azure_metric_alerts(
        self: "AzureConnector", params: dict[str, Any]
    ) -> list[dict[str, Any]]:
        ...
        if resource_group:
            async for alert in self._monitor_client.metric_alerts.list_by_resource_group(resource_group):
    ...
    async def _handle_get_azure_metric_alert(
        self: "AzureConnector", params: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _serialize_metric_alert(alert: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "scopes": scopes,
            "evaluation_frequency": str(alert.evaluation_frequency) if alert.evaluation_frequency else None,
    ...
    async def _handle_list_azure_activity_log(
        self: "AzureConnector", params: dict[str, Any]
    ) -> list[dict[str, Any]]:
        ...
        if not filter_expr:
            ...
            filter_expr = f"eventTimestamp ge '{start.isoformat()}' and eventTimestamp le '{end.isoformat()}'"
    ...
    async def _handle_list_azure_action_groups(
        self: "AzureConnector", params: dict[str, Any]
    ) -> list[dict[str, Any]]:
        ...
        if resource_group:
            async for ag in self._monitor_client.action_groups.list_by_resource_group(resource_group):
    ...
    def _serialize_action_group(ag: Any) -> dict[str, Any]:
        ...
        email_receivers = [
            ...
            for r in (ag.email_receivers or [])
        ]
        webhook_receivers = [
            {"name": r.name, "uri": r.service_uri}
            for r in (ag.webhook_receivers or [])
        ]
