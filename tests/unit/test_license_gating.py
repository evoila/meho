...
class TestRouterGatingLogic:
    ...
    def test_enterprise_routes_absent_in_community(self):
        ...
        assert not any("/chat/sessions/team" in p for p in paths), (
...
class TestMainPyGatingPattern:
    ...
    def test_audit_purge_gated(self):
        ...
        After PR-I (#313/#314) collapsed lifespan into helpers, the variable
        is named ``license_svc`` (no leading underscore). The substring
        ``license_svc.is_enterprise`` is what gates the ``audit_purge_task``
        scheduler registration in ``_start_background_workers``.
        """
        assert "license_svc.is_enterprise" in self._main_source
        assert "audit_purge_task" in self._main_source
