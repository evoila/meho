...
| `0019_add_audit_log_actor_sub` | `backend/alembic/versions/0019_add_audit_log_actor_sub.py` | G11.2-T2 (#816) schema foundation for RFC 8693 delegation attribution. Adds `audit_log.actor_sub text` (nullable) — the RFC 8693 `act.sub` claim from delegation-exchange tokens identifying the acting agent's Keycloak `sub`. NULL for direct-user and `client_credentials` rows. Adds `audit_log_actor_sub_idx` b-tree index (drives `WHERE actor_sub = ?` queries across tenants). Follows the soft-FK discipline of migrations 0002 / 0004 / 0006 / 0014: no FK clause, trivially reversible on populated tables. `down_revision = "0018"` (chains after #1050's agent-substrate migration). `downgrade()` drops the index then the column. Test coverage: `backend/tests/test_migration_0018_audit_actor_sub.py`. |
...
| `Operator` | `src/meho_backplane/auth/operator.py` | Frozen pydantic v2 model carrying validated claims (`sub`, `name`, `email`, `raw_jwt`, `tenant_id: UUID`, `tenant_role: TenantRole`, `actor_sub: str \| None`). Returned by `verify_jwt`; consumed by every authenticated route from G2.2-T3 onward. `raw_jwt` is preserved verbatim for G2.2-T2's Vault forward-auth. `tenant_id` / `tenant_role` (G0.1-T2) are required — the model can no longer be constructed without them, so any future regression that drops the claim extraction surfaces as a pydantic `ValidationError` rather than a silent anonymous request. `actor_sub` (G11.2-T2 #816) is the RFC 8693 `act.sub` claim: present when the token was produced by a delegation token exchange (the agent's Keycloak `sub`), `None` for direct-user and `client_credentials` tokens. Written through to `audit_log.actor_sub` (migration `0019`) so every agent-initiated row is attributable to both the acting agent and the initiating human. |
...
| `exchange_for_delegation` / `get_client_credentials_token` / `TokenExchangeError` / `TokenExchangeExchangeRefusedError` | `src/meho_backplane/auth/token_exchange.py` | RFC 8693 token-exchange + `client_credentials` grant flows for G11.2-T2 (#816). `exchange_for_delegation(issuer_url, subject_token, agent_client_id, agent_client_secret, audience)` calls Keycloak's token endpoint twice: first a `client_credentials` call to obtain the agent's actor token, then the RFC 8693 exchange (`urn:ietf:params:oauth:grant-type:token-exchange` with `actor_token` + `subject_token`) to produce a delegated access token whose `sub`=user and `act.sub`=agent. `get_client_credentials_token(issuer_url, agent_client_id, agent_client_secret, audience)` is the autonomous-run variant: a single `client_credentials` call where the agent is both subject and actor (no `act` claim; `audit_log.actor_sub` stays NULL). Both functions raise `TokenExchangeError(code, message)` on every failure mode — network error (`code="network_error"`), missing `access_token` in a 200 response (`code="missing_access_token"`), or a Keycloak error body with the error field forwarded as `code`. `TokenExchangeExchangeRefusedError(TokenExchangeError)` is raised specifically for policy-refusal codes (`invalid_target` / `access_denied`) so callers can distinguish "Keycloak is misconfigured" from "the `may_act` policy explicitly denied the exchange" and surface distinct error messages. The agent's `client_secret` is never logged — structlog events emit only `client_id` and the Keycloak error code. HTTP timeout is 5 s (shared with `_HTTP_TIMEOUT_SECONDS` from `auth/jwt.py`). |
