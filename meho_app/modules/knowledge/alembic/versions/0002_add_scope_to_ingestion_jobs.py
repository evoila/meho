...
Enables filtering documents by scope so GlobalSection shows only global
documents, not every job across all scopes.
...
def upgrade() -> None:
    ...
    # Backfill existing rows: jobs with a connector_id are instance-scoped,
    # those without are global.
    op.execute(
        sa.text(
