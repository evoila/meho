...
"""
AWS Operations Auto-Sync

Automatically syncs AWS operations to the database when the operations
...
from meho_app.modules.connectors.aws.operations import (
    AWS_OPERATIONS,
...
def _format_aws_operation_as_text(op: OperationDefinition, connector_name: str = "AWS") -> str:
    """
    Format AWS operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.

    Args:
        op: AWS operation definition
    ...
    parts.append("Platform: Amazon Web Services")
    parts.append("")
    ...
    # Search keywords
    keywords = _generate_aws_search_keywords(op)
    if keywords:
...
def _generate_aws_search_keywords(op: OperationDefinition) -> str:
    """
    ...
    aws_abbrevs = {
        "instance": ["vm", "vms", "virtual machine", "ec2", "compute"],
        "ec2": ["instance", "instances", "compute", "virtual machine"],
        "eks": ["kubernetes", "k8s", "cluster", "managed kubernetes"],
        "ecs": ["container", "containers", "fargate", "task", "service"],
        "s3": ["bucket", "buckets", "storage", "object storage"],
        "lambda": ["function", "functions", "serverless", "faas"],
        "rds": ["database", "db", "mysql", "postgres", "aurora"],
        "vpc": ["network", "networking", "virtual private cloud"],
        "subnet": ["subnetwork", "subnets", "network segment"],
        "security_group": ["firewall", "fw", "rules", "security", "sg"],
        "cloudwatch": ["monitoring", "metrics", "alarms", "logs"],
        "alarm": ["alert", "alerting", "notification"],
        "metric": ["monitoring", "metrics", "stats", "time series"],
        "node_group": ["nodegroup", "nodes", "worker", "eks nodes"],
    }
    ...
    for key, abbrevs in aws_abbrevs.items():
    ...
    # Add AWS-specific keywords
    keywords.update(["aws", "amazon", "cloud"])

    return " ".join(sorted(keywords))


async def sync_aws_operations_if_needed(
    session: AsyncSession,
    connector_id: str,
    tenant_id: str,
    current_version: str | None,
    knowledge_store: Optional["KnowledgeStore"] = None,
    connector_name: str = "AWS",
) -> tuple[int, int, int]:
    """
    Sync AWS operations if the version is outdated.
    ...
    if current_version == AWS_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing AWS operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {AWS_OPERATIONS_VERSION}"
    )
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(AWS_OPERATIONS)} total, "
...
async def _sync_aws_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for AWS operations.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "aws",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing AWS knowledge chunks for connector {connector_id}")
    ...
    for op in AWS_OPERATIONS:
        try:
            ...
            text = _format_aws_operation_as_text(op, connector_name)

            from meho_app.modules.knowledge.schemas import ChunkMetadata

            metadata_dict = {
                "resource_type": op.category,
                "keywords": [op.operation_id, op.name, op.category, "aws", "amazon web services"],
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=["api", "operation", "aws", "amazon", op.category],
    ...
    logger.info(f"Created {chunks_created} knowledge chunks for AWS connector {connector_id}")
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = AWS_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await session.flush()
        logger.debug(f"Updated connector {connector_id} to version {AWS_OPERATIONS_VERSION}")


async def sync_all_aws_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all AWS connectors to the latest operations version.
    ...
    # Find all AWS connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "aws")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No AWS connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} AWS connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        added, updated, chunks = await sync_aws_operations_if_needed(
            session=session,
            connector_id=str(connector.id),
            tenant_id=str(connector.tenant_id),
            current_version=current_version,
            knowledge_store=knowledge_store,
            connector_name=str(connector.name) if connector.name else "AWS",
        )
    ...
    summary = {
        ...
        "current_version": AWS_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"AWS operation sync complete: "
        ...
        logger.info("All AWS connectors already at latest version")
