...
class TopologyLookupNode(BaseNode[MEHOGraphState, MEHOGraphDeps, None]):
    ...
    async def run(
    ...
    ) -> ReasonNode:  # NOSONAR (cognitive complexity)
        ...
        try:
            ...
            if not db_session:
                ...
                await self._log_result(result_info, start_time, [], [])
                return ReasonNode()
            ...
            if not references or not references.is_topology_relevant or not references.entities:
                ...
                await self._log_result(
                    result_info, start_time, extracted_entity_names, found_entities
                )
                return ReasonNode()
    ...
    async def _lookup_entity(
        self, ref: EntityReference, tenant_id: str, db_session: Any
    ) -> dict | None:
    ...
    def _format_context(self, context_parts: list[dict]) -> str:  # NOSONAR (cognitive complexity)
    ...
    def _format_chain_same_as_context(self, chain: list[Any], entity: Any) -> list[str]:
    ...
    def _get_connector_info(self, entity: Any, chain: list[Any]) -> str:
    ...
    def _add_schema_hints(self, lines: list[str], entity: Any) -> None:
        """
