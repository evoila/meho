...
class AggregateSpec(BaseModel):
    ...
    name: str = Field(
        description="Name for the aggregated result"
    )
...
class DataQuery(BaseModel):
    ...
    compute: list[ComputeField] = Field(
        ...
        description="Derived fields to compute from existing data"
    )
    
    # Filtering
    filter: Optional[FilterGroup] = Field(
        default=None,
    ...
    aggregates: list[AggregateSpec] = Field(
        default_factory=list,
        description="Aggregations to compute over the filtered data"
    )
...
class ReducedData(BaseModel):
    ...
    records: list[dict[str, Any]] = Field(
        description="The processed records matching the query"
    )
    
    # Metadata about the reduction
    total_source_records: int = Field(
        description="Total records in the source before filtering"
    )
    total_after_filter: int = Field(
        description="Records remaining after filtering (before limit)"
    )
    returned_records: int = Field(
        description="Number of records returned (after limit)"
    )
    ...
    def to_llm_context(self) -> str:
        ...
        if self.is_truncated:
            lines.append(f"  (Limited to {self.returned_records}, {self.total_after_filter} total matched)")
