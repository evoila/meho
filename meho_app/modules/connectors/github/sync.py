...
"""
GitHub Operations Auto-Sync.

Automatically syncs GitHub operations to the database when the operations
...
This enables semantic search (BM25 + embeddings) across all GitHub operations.

Usage:
    On API startup, call sync_all_github_connectors() to update any
...
def _format_github_operation_as_text(  # NOSONAR (cognitive complexity)
    op: OperationDefinition, connector_name: str = "GitHub"
) -> str:
    """
    Format GitHub operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.

    Args:
        op: GitHub operation definition
    ...
    if op.response_entity_type:
        parts.append(f"Response entity: {op.response_entity_type}")
        parts.append("")

    # Search keywords for better BM25 matching
    keywords = _get_github_bm25_keywords(op)
    if keywords:
...
def _get_github_bm25_keywords(op: OperationDefinition) -> list[str]:
    """
    Return GitHub-specific search keywords per operation.

    Maps keywords by operation category for better BM25 matching.
    Each category gets domain-specific terms that users are likely
    to search for when looking for that type of operation.

    Args:
        op: GitHub operation definition

    Returns:
        List of search keywords for BM25 indexing
    """
    ...
    # Always include these base keywords
    keywords.append("github")
    keywords.append(op.operation_id)
    keywords.append(op.name.lower())

    # Add category-specific keywords
    cat_keys = category_keywords.get(op.category, [])
    keywords.extend(cat_keys)

    return keywords


async def sync_github_operations_if_needed(
    ...
    """
    Sync GitHub operations if the version is outdated.
    ...
    if current_version == GITHUB_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing GitHub operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {GITHUB_OPERATIONS_VERSION}"
    )
    ...
    for op in GITHUB_OPERATIONS:
        ...
        # Determine safety level (no DESTRUCTIVE for GitHub)
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(GITHUB_OPERATIONS)} total, "
...
async def _sync_github_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for GitHub operations.

    Enables hybrid search (BM25 + semantic) for GitHub operations.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "github",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing GitHub knowledge chunks for connector {connector_id}")
    ...
    for op in GITHUB_OPERATIONS:
        try:
            ...
            # Get GitHub-specific BM25 keywords
    ...
    logger.info(f"Created {chunks_created} knowledge chunks for GitHub connector {connector_id}")
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = GITHUB_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await session.flush()
        logger.debug(f"Updated connector {connector_id} to version {GITHUB_OPERATIONS_VERSION}")


async def sync_all_github_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all GitHub connectors to the latest operations version.
    ...
    # Find all GitHub connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "github")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No GitHub connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} GitHub connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        # Sync operations
        added, updated, chunks = await sync_github_operations_if_needed(
            session=session,
            connector_id=str(connector.id),
            tenant_id=str(connector.tenant_id),
            current_version=current_version,
            knowledge_store=knowledge_store,
            connector_name=str(connector.name) if connector.name else "GitHub",
        )
    ...
    summary = {
        ...
        "current_version": GITHUB_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"GitHub operation sync complete: "
        ...
        logger.info("All GitHub connectors already at latest version")
