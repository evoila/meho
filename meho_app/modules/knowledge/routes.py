...
async def hybrid_search_knowledge(
    ...
    """
    Legacy hybrid endpoint retained for compatibility.

    Internally this now uses the same semantic ranker + reranker flow as the
    Farseer-aligned knowledge search path. The route name is preserved to avoid
    breaking callers that still target `/search/hybrid`.

    Returns chunks that:
    1. Match via semantic vector ranking
    2. User has permission to access (ACL filtering)
    3. Are reranked for final relevance when a reranker is available
    """
    try:
        # Legacy compatibility entrypoint - delegates to semantic ranker + reranker
        ...
        results = [
            SearchResult(
                ...
                score=result.get(
                    "score",
                    result.get("rerank_score", result.get("similarity", 0.0)),
                ),
