...
// `vi.mock` is hoisted above all imports, including this test file's own
// top-level `import` statements. That means the mock factory runs *before*
// the `const mockSetTenantContext = vi.fn()` declaration below would be
// reached, and referencing those variables inside the factory throws a
// TDZ ReferenceError. `vi.hoisted` lifts the `vi.fn()` construction into
// the same pre-import phase so the factory can safely close over them.
const { mockSetTenantContext, mockClearTenantContext, mockInvalidateQueries } =
  vi.hoisted(() => ({
