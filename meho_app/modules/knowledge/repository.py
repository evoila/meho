...
class KnowledgeRepository:
    ...
    async def get_chunks_by_ids(self, chunk_ids: list[str]) -> list[KnowledgeChunkModel]:
        """Fetch chunk models by ID list, ordered by created_at (document order).

        No ACL check -- caller is responsible for verifying access.

        Args:
            chunk_ids: List of chunk UUID strings.

        Returns:
            List of chunk DB models in creation order.
        """
        if not chunk_ids:
            return []
        ...
        result = await self.session.execute(
            ...
            .where(KnowledgeChunkModel.id.in_(chunk_uuids))
            .order_by(KnowledgeChunkModel.created_at.asc())
        )
    ...
    async def search_by_embedding(
        ...
        Args:
            ...
            connector_id: Optional connector UUID for strict connector-scoped search
        ...
        if connector_id:
            try:
                query = query.where(KnowledgeChunkModel.connector_id == uuid.UUID(connector_id))
            except ValueError:
                logger.warning("invalid_connector_id_filter", connector_id=connector_id)
                return []

        if doc_version:
            query = query.where(KnowledgeChunkModel.doc_version == doc_version)
    ...
    async def delete_by_source_uri(self, source_uri: str) -> int:
        ...
        result = await self.session.execute(
            delete(KnowledgeChunkModel).where(KnowledgeChunkModel.source_uri == source_uri)
        )
        await self.session.flush()
    ...
    async def count_chunks(
        self, filter_params: KnowledgeChunkFilter | None = None
    ) -> int:  # NOSONAR (cognitive complexity)
        """
    ...
    def _model_to_schema(self, db_chunk: KnowledgeChunkModel) -> KnowledgeChunk:
        ...
        return KnowledgeChunk(
            ...
            family_id=str(db_chunk.family_id) if db_chunk.family_id else None,
            # Lifecycle fields
