...
class MetadataExtractor:
    ...
    RESOURCE_TYPES = {
        "roles", "users", "domains", "clusters", "hosts", "networks",
        "datastores", "vms", "workloads", "backups", "licenses",
        "certificates", "credentials", "groups", "permissions"
    }
    ...
    def _detect_content_type(self, text: str) -> ContentType:
        ...
        # Check for schemas
        if "schema" in text_lower or "type:" in text_lower:
            if self._has_structured_format(text):
                return ContentType.SCHEMA
        
        # Check for parameters
        if any(word in text_lower for word in ["parameter", "query param", "path param"]):
    ...
    def _extract_endpoint_path(self, text: str) -> Optional[str]:
        ...
        if matches:
            ...
            valid_paths = [m for m in matches if not m.startswith('/usr') and not m.startswith('/etc')]
            if valid_paths:
    ...
    def _has_table(self, text: str) -> bool:
        ...
        if any(re.search(r'\w+\s{3,}\w+', line) for line in lines):
            return True
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        common_words = {'GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'HTTP', 'API', 'REST', 'JSON', 'XML', 'ID'}
        ...
        keywords.update(word for word, count in keyword_counts.items() if count > 1 or len(word) > 8)
    ...
    def _extract_chapter(self, context: Dict[str, Any]) -> Optional[str]:
    ...
    def _extract_section(self, context: Dict[str, Any]) -> Optional[str]:
