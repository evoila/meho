    ...
    try:
        ...
        if decision.approved:
            ...
            try:
                ...
                await db_session.commit()
            except Exception as audit_err:  # noqa: BLE001 -- audit side-effect must not fail the approval response
            ...
            try:
                ...
                await db_session.commit()
            except Exception as audit_err:  # noqa: BLE001 -- audit side-effect must not fail the denial response
        ...
        raise InternalError(message="Failed to process approval decision") from e
...
async def get_pending_approvals(
    ...
    try:
        ...
        raise InternalError(message="Failed to retrieve pending approvals") from e
...
async def resume_after_approval(
    ...
    try:
        ...
        raise InternalError(message="Failed to process resume request") from e
    ...
    async def generate():
        ...
        async with session_maker() as session:
            ...
            try:
                ...
                if request.session_id:
                    ...
                    if session_obj:
                        ...
                        if request.session_mode and request.session_mode != stored_mode:
                            ...
                            except (SQLAlchemyError, RuntimeError, OSError) as mode_err:
                ...
                except (RuntimeError, ValueError) as ctx_err:
            ...
            except Exception as e:  # noqa: BLE001 -- top-level SSE error handler must catch all exceptions to send an error event before the stream closes
                logger.error(f"ReAct graph error: {e}", exc_info=True)
                error_msg = "An internal error occurred while processing the request."
...
async def summarize_session(
    ...
    if not messages_text or len(messages_text) < 2:
        ...
        except Exception as e:  # noqa: BLE001 -- summarization is best-effort; fallback summary must always be produced regardless of LLM/PydanticAI/Anthropic exception class
