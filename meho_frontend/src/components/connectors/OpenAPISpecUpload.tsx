...
export function OpenAPISpecUpload({ connectorId, onSuccess }: OpenAPISpecUploadProps) {
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      const blob = await connectorsClient.downloadOpenAPISpec(connectorId);
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      const response = await connectorsClient.uploadOpenAPISpec(connectorId, uploadFile);
      setResult(response);
  ...
  }, [uploadMode, file, pastedContent, connectorId, connectorsClient, onSuccess]);
