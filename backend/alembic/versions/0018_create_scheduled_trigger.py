...
``kind`` column picks which of ``cron_expr`` / ``fire_at`` /
``event_filter`` carries the trigger's semantics. A DB-side ``CHECK``
constraint (``ck_scheduled_trigger_kind_fields``) enforces the
invariant -- the right column is populated, the others are NULL -- so
a malformed row cannot land at the substrate boundary.

* ``kind = 'cron'``   -> ``cron_expr`` populated, ``fire_at`` /
  ``event_filter`` NULL.
* ``kind = 'one_off'`` -> ``fire_at`` populated, ``cron_expr`` /
  ``event_filter`` NULL.
...
#: :class:`meho_backplane.db.models.ScheduledTriggerStatus`.
...
#: Discriminated-union invariant: exactly one of the three discriminator
#: columns carries the trigger's semantics, the other two are NULL. The
#: ``(kind = '...' AND col IS NOT NULL AND other_cols IS NULL)`` form is
...
def upgrade() -> None:
    ...
    # would fire.
    event_filter_type = sa.JSON(none_as_null=True).with_variant(
    ...
    op.create_table(
        ...
        # Discriminated fields -- exactly one populated per kind (see
        # _KIND_FIELDS_CHECK below).
        sa.Column("cron_expr", sa.Text(), nullable=True),
        sa.Column("fire_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("event_filter", event_filter_type, nullable=True),
        sa.Column(
            "status",
            sa.Text(),
            nullable=False,
        ...
        # fire_at, event with a populated cron_expr, ...) fails at the
        # substrate boundary rather than at dispatch time.
        sa.CheckConstraint(
            _KIND_FIELDS_CHECK,
