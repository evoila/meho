...
"""
Loki Operations Auto-Sync

Automatically syncs Loki operations to the database when the operations
...
This enables semantic search (BM25 + embeddings) across all Loki operations.

Usage:
    On API startup, call sync_all_loki_connectors() to update any
...
from meho_app.modules.connectors.loki.operations import (
    LOKI_OPERATIONS,
...
def _format_loki_operation_as_text(op: OperationDefinition, connector_name: str = "Loki") -> str:
    """
    Format Loki operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.

    Args:
        op: Loki operation definition
    ...
    keywords = _generate_loki_search_keywords(op)
    if keywords:
...
def _generate_loki_search_keywords(op: OperationDefinition) -> str:
    """
    ...
    # Loki-specific abbreviations and synonyms
    loki_terms = {
        "log": ["log", "logs", "logging", "logline", "logfile"],
        "error": ["error", "errors", "err", "exception", "failure", "fault"],
        "search": ["search", "find", "filter", "grep", "match"],
        "label": ["label", "labels", "tag", "tags", "metadata"],
        "volume": ["volume", "count", "rate", "statistics", "stats"],
        "pattern": ["pattern", "patterns", "group", "aggregate", "cluster"],
        "context": ["context", "surrounding", "before", "after", "around"],
        "severity": ["severity", "level", "warn", "warning", "critical", "fatal", "info", "debug"],
        "namespace": ["namespace", "ns", "project"],
        "pod": ["pod", "pods", "container", "workload"],
        "service": ["service", "services", "application", "app"],
        "logql": ["logql", "query", "expression", "custom"],
    }

    # Add relevant terms based on operation name/description
    combined_text = f"{op.operation_id} {op.name} {op.description}".lower()
    for key, terms in loki_terms.items():
    ...
    if "search" in combined_text:
        keywords.update(["search", "find", "filter", "grep", "match"])
...
async def sync_loki_operations_if_needed(
    session: AsyncSession,
    connector_id: str,
    tenant_id: str,
    current_version: str | None,
    knowledge_store: Optional["KnowledgeStore"] = None,
    connector_name: str = "Loki",
) -> tuple[int, int, int]:
    """
    Sync Loki operations if the version is outdated.
    ...
    if current_version == LOKI_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing Loki operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {LOKI_OPERATIONS_VERSION}"
    )
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(LOKI_OPERATIONS)} total, "
...
async def _sync_loki_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for Loki operations.

    Enables hybrid search (BM25 + semantic) for Loki operations.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "loki",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing Loki knowledge chunks for connector {connector_id}")
    ...
    for op in LOKI_OPERATIONS:
        try:
            ...
            text = _format_loki_operation_as_text(op, connector_name)
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=["api", "operation", "loki", op.category],
    ...
    logger.info(f"Created {chunks_created} knowledge chunks for Loki connector {connector_id}")
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = LOKI_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await session.flush()
        logger.debug(f"Updated connector {connector_id} to version {LOKI_OPERATIONS_VERSION}")


async def sync_all_loki_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all Loki connectors to the latest operations version.
    ...
    # Find all Loki connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "loki")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No Loki connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} Loki connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        # Sync operations
        added, updated, chunks = await sync_loki_operations_if_needed(
            session=session,
            connector_id=str(connector.id),
            tenant_id=str(connector.tenant_id),
            current_version=current_version,
            knowledge_store=knowledge_store,
            connector_name=str(connector.name) if connector.name else "Loki",
        )
    ...
    summary = {
        ...
        "current_version": LOKI_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"Loki operation sync complete: "
        ...
        logger.info("All Loki connectors already at latest version")
