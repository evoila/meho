...
class KeycloakJWTValidator:
    ...
    def __init__(self, keycloak_url: str, client_id: str, cache_ttl: int = 3600) -> None:
        """
    ...
    async def validate_token(self, token: str) -> TokenData:
        ...
        if not issuer:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Token missing issuer (iss)",
            )
...
def reset_keycloak_validator() -> None:
