...
import { ChevronRight, ChevronDown, Plus, FileText, Globe, Cpu, Loader2, AlertCircle } from 'lucide-react';
...
export function TypeSection({
  ...
  const typeLevelDocCount = documentCount;
  const totalInstanceDocs = instances.reduce((sum, inst) => sum + inst.document_count, 0);

  const { data: docsData, isLoading: docsLoading } = useQuery({
    queryKey: ['type-knowledge-documents', connectorType],
    queryFn: () => knowledgeClient.listKnowledgeDocuments({
      scope_type: 'type',
      connector_type_scope: connectorType,
      limit: 200,
    }),
    enabled: expanded && typeLevelDocCount > 0,
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['type-knowledge-documents', connectorType] });
  ...
  const typeDocs = docsData?.documents ?? [];
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            ...
            {/* Delete error */}
            {deleteError && (
              <div className="mx-4 mt-2 flex items-center gap-2 text-xs text-red-400 bg-red-400/10 border border-red-400/20 rounded-lg px-3 py-2">
            ...
            {/* Type-level document list */}
            {typeLevelDocCount > 0 && (
              <div className="px-4 pb-3 pl-14 border-t border-white/5">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-text-tertiary mt-2 mb-1.5">
                ...
                {docsLoading ? (
                  ...
                  <DocumentList
                    ...
                    scope={{ scope_type: 'type', connector_type_scope: connectorType }}
                    onResume={() => {
                      void queryClient.invalidateQueries({ queryKey: ['type-knowledge-documents', connectorType] });
                    }}
                  />
                )}
              </div>
