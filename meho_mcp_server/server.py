...
async def get_sql_queries_tool(
    ...
    return await get_sql_queries(
        session_id=session_id, include_results=include_results
    )
...
async def send_message_tool(
    ...
    return await send_meho_message(
        message=message, session_id=session_id, timeout=timeout
    )
