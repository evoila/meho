...
def _format_topology_context(results: list) -> str:  # NOSONAR (cognitive complexity)
...
class SpecialistAgent(BaseAgent):
    ...
    @staticmethod
    def _build_investigation_state(state: Any) -> str:  # NOSONAR (cognitive complexity)
    ...
    def _extract_investigation_data(
        state: Any, tool_name: str, observation: Any
        ...
        if tool_name == "lookup_topology" and isinstance(observation, LookupTopologyOutput):
            ...
            if observation.topology_chain:
                for chain_item in observation.topology_chain:
                    ...
                    if not any(
                        e.get("name") == entity_dict["name"] for e in state.discovered_entities
                    ):
    ...
    async def _run_react(  # NOSONAR (cognitive complexity)
        self,
        user_message: str,
        session_id: str | None = None,
        context: dict[str, Any] | None = None,
        ...
        try:
            ...
            while not state.is_complete() and state.step_count < state.max_steps:
                ...
                react_step = result.output  # type: ignore[attr-defined]
                # Capture message history for next step
                state.message_history = list(result.all_messages())  # type: ignore[attr-defined]
                        ...
                        try:
                            ...
                            if _usage:
                                ...
                                if _details and isinstance(_details, dict):
                                    ...
                                    _cache_write = (
                                        _details.get("cache_creation_input_tokens", 0) or 0
                                    )
                ...
                if needs_approval(trust_tier) and react_step.action == "call_operation":
                    ...
                    try:
                        if self.dependencies.db_session:
                            ...
                            _expiry = (
                                1440 if _is_automated else get_config().approval_expiry_minutes
                            )
