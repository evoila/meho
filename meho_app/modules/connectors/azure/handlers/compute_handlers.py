...
class ComputeHandlerMixin:
    ...
    async def _handle_list_azure_vms(
        ...
        if state_filter:
            ...
            results = [
                r for r in results
                if state_lower in (r.get("provisioning_state", "") or "").lower()
            ]

        return results

    async def _handle_get_azure_vm(
        self: "AzureConnector", params: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _handle_get_azure_vm_instance_view(
        self: "AzureConnector", params: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _handle_get_azure_vm_metrics(
        self: "AzureConnector", params: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _handle_list_azure_disks(
        self: "AzureConnector", params: dict[str, Any]
    ) -> list[dict[str, Any]]:
        """List managed disks.
    ...
    async def _handle_get_azure_disk(
        self: "AzureConnector", params: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _handle_list_azure_availability_sets(
        self: "AzureConnector", params: dict[str, Any]
    ) -> list[dict[str, Any]]:
        ...
        async for avset in self._compute_client.availability_sets.list(resource_group):
            results.append({
                ...
                "platform_fault_domain_count": getattr(avset, "platform_fault_domain_count", None),
                "platform_update_domain_count": getattr(avset, "platform_update_domain_count", None),
    ...
    async def _handle_list_azure_resource_groups(
        self: "AzureConnector", params: dict[str, Any]
    ) -> list[dict[str, Any]]:
