...
"""
Tempo connector operations.

Handles Tempo connector creation with ObservabilityHTTPConnector base,
configurable auth (none/basic/bearer), multi-tenant org_id, and test
connection via buildinfo/ready.
"""
...
@router.post("/tempo", response_model=TempoConnectorResponse)
async def create_tempo_connector(  # NOSONAR (cognitive complexity)
    request: CreateTempoConnectorRequest,
    user: UserContext = Depends(get_current_user),
):
    """
    Create a Tempo typed connector.

    Creates a typed connector using httpx for native Tempo HTTP API access.
    Operations are pre-registered based on the Tempo connector implementation.
    ...
    try:
        async with session_maker() as session:
            ...
            protocol_config = {
                ...
                "operations_version": TEMPO_OPERATIONS_VERSION,
            }

            # Multi-tenancy: include org_id if provided
            if request.org_id:
                protocol_config["org_id"] = request.org_id
            ...
            # Test connection
            tempo_version = None
            logger.info(f"Testing Tempo connection: {base_url}")
            try:
                ...
                await tempo.connect()
                is_connected = await tempo.test_connection()
                tempo_version = tempo.tempo_version
                await tempo.disconnect()

                if not is_connected:
                    raise HTTPException(
                        status_code=400,
                        detail="Could not connect to Tempo. Check URL and credentials.",
                    )

                logger.info(f"Tempo connection verified: {base_url} (version: {tempo_version})")
            ...
            # Store tempo_version in protocol_config
            if tempo_version:
                protocol_config["tempo_version"] = tempo_version
                import uuid
            ...
            # Register operations (empty initially -- Plan 02 populates TEMPO_OPERATIONS)
            op_creates = []
            ...
            # No entity types for Tempo (query-only, no topology entities)
            ...
            try:
                ...
                from meho_app.modules.connectors.tempo.sync import (
                ...
                await session.commit()
                logger.info(f"Created {chunks_created} knowledge chunks for Tempo operations")
            except Exception as e:
                logger.warning(f"Failed to create knowledge chunks: {e}")

            logger.info(
                f"Created Tempo connector '{request.name}' "
                f"with {ops_count} operations "
                f"and {chunks_created} knowledge chunks"
            )

            return TempoConnectorResponse(
                id=connector_id,
                name=request.name,
                base_url=base_url,
                connector_type="tempo",
                ...
                message=f"Tempo connector created successfully. "
        ...
        logger.error(f"Create Tempo connector failed: {e}", exc_info=True)
