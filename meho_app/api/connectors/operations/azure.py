...
@router.post("/azure", response_model=AzureConnectorResponse)
async def create_azure_connector(
    request: CreateAzureConnectorRequest, user: UserContext = Depends(get_current_user)
):
    """
    ...
    try:
        async with session_maker() as session:
            ...
            # Test connection
            logger.info(
                f"Testing Azure connection: subscription={request.subscription_id}"
            )
            try:
                ...
                logger.info(
                    f"Azure connection verified: subscription={request.subscription_id}"
                )
            except ImportError:
