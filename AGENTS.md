...
- NEVER use `export default` under `meho_frontend/src/`. All exports must be named. ESLint enforces this (tool configs like `vite.config.ts` are exempt — framework APIs require default export).
...
MEHO uses a dual-repo setup:
- Private development repo: `evoila-bosnia/MEHO.X`
- Public OSS repo: `evoila/meho`

All normal development happens in the private repo. After CI passes on private `main`, the mirror pipeline syncs a stripped subset to the public repo. Do not push directly to the public repo. Mirror behavior (`orphan` vs `incremental`) is controlled by the private-repo mirror workflow configuration.

For full operational details, guardrails, and exception handling, read `docs/development/dual-repo-workflow.md`.
...
That installs pre-commit hooks for three stages:

- `pre-commit` — Ruff lint, Ruff format, mypy strict, ESLint, SPDX header check, frontend typecheck on staged frontend changes.
- `pre-push` — vitest (frontend), backend smoke + contract tests, public-remote push guard.
- `commit-msg` — Conventional Commits format check.

**Installing and using the pre-commit hooks is required before opening a PR.** CI enforces the same gates and will block merge on failure. If a hook fails, fix the underlying issue — do not use `git commit --no-verify` or `git push --no-verify` to bypass. Hook failures are contract violations, not noise.

If hooks stop firing, check:

```bash
git config --get core.hooksPath    # should be unset
ls -la .git/hooks/pre-commit       # should exist and be executable
pre-commit install --install-hooks # reinstalls if missing
```
...
**Frontend state:** Zustand v5 stores in feature directories. Named hooks: `useChat()`, `useConnectors()`, etc. Domain-split API clients at `meho_frontend/src/api/clients/` (`getChatClient()`, `getConnectorsClient()`, etc.) over a shared Axios transport with automatic 401 token refresh.
...
- Topology resolution internals (5 tables, 3 discovery paths, LLM verdict, eligibility): `docs/codebase/topology-resolution.md`
