...
class IngestionService:
    ...
    async def ingest_document(  # NOSONAR (cognitive complexity)
        self,
        file_bytes: bytes,
        filename: str,
        mime_type: str,
        ...
        try:
            ...
            for i, (chunk_text, context) in enumerate(chunks_with_context):
                ...
                logger.info(
                    ...
                    search_metadata_type=(
                        type(chunk_create.search_metadata).__name__
                        if chunk_create.search_metadata
                        else None
                    ),
                    search_metadata_chapter=(
                        chunk_create.search_metadata.chapter
                        if chunk_create.search_metadata
                        else None
                    ),
                )
