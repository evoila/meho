...
``meho_runbook_next`` tool. The structural property that makes the entire
...
* :data:`NextStepResponse` -- discriminated on ``kind``
  (``"current_step"`` / ``"completed"``); the ``meho_runbook_next`` tool's
...
#: ``meho_runbook_next`` or the engine populates from a dispatched call.
...
class StepBody(BaseModel):
    """The opaque-by-construction single-step shape returned by ``meho_runbook_next``.
    ...
    * :attr:`verify` -- the substituted-and-frozen verify gate the
      caller must respond to on the next ``meho_runbook_next`` call.
...
class StartRunRequest(BaseModel):
    """Request body for ``meho_runbook_start`` -- begin a new run on a template.
...
class CurrentStepResponse(BaseModel):
    """Returned by ``meho_runbook_start`` and the non-completion path of ``meho_runbook_next``.
...
class RunCompletedResponse(BaseModel):
    """Returned by ``meho_runbook_next`` when the previous step was the last.
...
#: Reply shape of ``meho_runbook_next`` -- discriminated on ``kind``. The
...
class NextStepRequest(BaseModel):
    """Request body for ``meho_runbook_next`` -- advance the run.
    ...
    ``meho_runbook_next`` call (when no prior step exists to verify).
    """
...
class AbortRunRequest(BaseModel):
    """Request body for ``meho_runbook_abort`` -- terminate the run mid-flight.
...
class AbortRunResponse(BaseModel):
    """Returned by ``meho_runbook_abort`` -- the terminal-state coordinates."""
...
class ReassignRunRequest(BaseModel):
    """Request body for ``meho_runbook_reassign`` -- transfer ownership of a run.
    ...
    every subsequent ``meho_runbook_next`` ownership check.
    """
...
class ReassignRunResponse(BaseModel):
    """Returned by ``meho_runbook_reassign`` -- the new ownership coordinates."""
...
class ListRunsFilter(BaseModel):
    """Optional filters for ``meho_runbook_list_runs``.
...
class RunSummary(BaseModel):
    """List-view projection returned by ``meho_runbook_list_runs``.

    Run-level state plus the current step's *state* -- no step
    contents are exposed. The step-by-step content is
    opaque-by-construction (only ``meho_runbook_next`` ever returns a
