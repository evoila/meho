...
async def list_connector_operations(
    ...
    category: str | None = Query(
        None, description="Filter by category (compute, storage, network, etc.)"
    ),
...
async def toggle_operation(
    ...
    async with session_maker() as session:
        ...
        if op.source == "custom" and op.type_operation_id is not None:
            ...
            if op.is_enabled_override is False:
...
async def sync_connector_operations(
    connector_id: str, user: UserContext = Depends(get_current_user)
):
    """
