...
flowchart TD
    ...
    I -->|ok, or read-class audit fail| J[9. return OperationResult]
    I -->|write-class audit commit fails| Z8["connector_error&#10;(durable-audit precondition)"]
...
8. **Write the audit row + publish a broadcast event** via [`_audit.audit_and_broadcast_safe`](../../backend/src/meho_backplane/operations/_audit.py). Broadcast failure → logged, fail-open per `publish_event`'s contract (unchanged). Audit-row failure splits on the [`_requires_durable_audit`](../../backend/src/meho_backplane/operations/dispatcher.py) gate — `classify_op(op_id) ∈ WRITE_OP_CLASSES` **or** `safety_level != 'safe'` (write-class + the post-approval superset): for a **safe read-class** op the failure is logged at error level and swallowed (broadcast skipped, caller keeps its `ok`); for a **write-class / post-approval** op the success path instead **fails closed** into `connector_error` (S07 #295) — the durable DISPATCH row is a hard precondition of a successful return (CLAUDE.md postulate 7 / v0.1-spec §6), so the caller never sees `status='ok'` for a mutation that left no governance-grade record. Either way the broadcast is skipped so no event references a phantom row.
...
- ADR: [locked-decisions.md](../decisions/locked-decisions.md) — locked architecture decisions; the substrate operationalises postulates 4–7.
