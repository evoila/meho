...
@router.post(
    ...
    responses={
        ...
        404: {"description": "Event registration not found"},
        410: {"description": "Event registration is disabled"},
        429: {"description": "Rate limit exceeded for this event registration"},
...
async def receive_event(
    ...
    4. Check dedup via Redis (202 duplicate_suppressed -- no new session)
    ...
    async with session_maker() as db:
        try:
            ...
            if registration.require_signature:
                try:
                    secret = event_service.decrypt_secret(registration.encrypted_secret)  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
                ...
                logger.info(
                    f"Event {event_id}: signature verification skipped (require_signature=False)"
                )
            ...
            if is_dup:
                # GET (not GETDEL): counter stays in Redis through its 600s TTL
                # so each successive duplicate of the same payload hash logs a
                # monotonically increasing value instead of resetting to 1.
                suppressed_key = f"meho:event:suppressed:{event_id}:{payload_hash}"
                raw_count = await redis_client.get(suppressed_key)
                suppressed_count = int(raw_count) if raw_count else 0
                await event_service.log_event(
                    registration_id=event_id,
                    tenant_id=registration.tenant_id,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
                    status="deduplicated",
                ...
                await db.commit()
                logger.info(f"Event {event_id}: duplicate suppressed (hash={payload_hash[:12]}...)")
            ...
            is_limited = await EventService.is_rate_limited(
                redis_client,
                event_id,
                registration.rate_limit_per_hour,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
            )
            if is_limited:
                await event_service.log_event(
                    registration_id=event_id,
                ...
                await db.commit()
                logger.warning(
                    f"Event {event_id}: rate limited (limit={registration.rate_limit_per_hour}/hr)"
                )
                raise HTTPException(
                    status_code=429,
            ...
            background_tasks.add_task(
                ...
                prompt_template=registration.prompt_template,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
            )
