...
for _mod_name in _DOCLING_MODULES:
    if _mod_name not in sys.modules:
        ...
        elif _mod_name == "docling.datamodel.pipeline_options":
            _mock_mod.PdfPipelineOptions = MagicMock()  # type: ignore[attr-defined]
...
@patch("meho_app.modules.knowledge.ingestion._get_pdf_page_count", return_value=1)
@patch("meho_app.modules.knowledge.ingestion.DoclingDocumentConverter")
async def test_ingest_document_structured_pdf(
    mock_converter_cls, mock_page_count, mock_subprocess_convert, mock_summary, mock_prefix
):
...
@patch("meho_app.modules.knowledge.ingestion.DoclingDocumentConverter")
async def test_ingest_document_plain_text_fallback(mock_converter_cls, mock_summary, mock_prefix):
...
async def test_ingest_document_summary_enrichment(
    ...
    mock_doc = MagicMock()
    mock_subprocess_convert.return_value = mock_doc
    mock_converter.get_full_text.return_value = "Document about pod eviction procedures"
    mock_converter.chunk_document.return_value = [
        (
            "Enriched chunk",
            {
...
async def test_ingest_document_summary_failure_continues(
    ...
    mock_converter.chunk_document.assert_called_once_with(mock_doc, chunk_prefix="")
...
async def test_ingest_document_cleanup_on_failure(
    ...
    async def failing_add_chunk(chunk_create):
        ...
        call_count += 1
        if call_count == 3:
            raise RuntimeError("Embedding API failed on chunk 3")
        return KnowledgeChunk(
            id=f"chunk-{call_count}",
            **chunk_create.model_dump(),
            created_at=datetime.now(tz=UTC),
    ...
    delete_calls = [call.args[0] for call in store.delete_chunk.call_args_list]
