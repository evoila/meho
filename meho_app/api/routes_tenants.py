...
DESC_EMAIL_DOMAINS_FOR_TENANT_DISCOVERY = (
    "Email domains for tenant discovery (e.g., ['acme.com', 'acme.org'])"
)
...
class TenantResponse(BaseModel):
    ...
    email_domains: list[str] = Field(
        ...
        description=DESC_EMAIL_DOMAINS_FOR_TENANT_DISCOVERY,
    )
...
class CreateTenantRequest(BaseModel):
    ...
    email_domains: list[str] | None = Field(
        None, description=DESC_EMAIL_DOMAINS_FOR_TENANT_DISCOVERY
    )
...
async def create_tenant(
    ...
    user: Annotated[UserContext, Depends(RequirePermission(Permission.TENANT_CREATE))],
    repository: Annotated[TenantConfigRepository, Depends(get_repository)],
    session: Annotated[AsyncSession, Depends(get_agent_session)],
    keycloak: Annotated[KeycloakTenantManager | None, Depends(get_keycloak)],
) -> TenantResponse:
    """Create a new tenant."""
...
async def update_tenant(
    ...
    user: Annotated[UserContext, Depends(RequirePermission(Permission.TENANT_UPDATE))],
    repository: Annotated[TenantConfigRepository, Depends(get_repository)],
    session: Annotated[AsyncSession, Depends(get_agent_session)],
) -> TenantResponse:
    """Update tenant settings."""
