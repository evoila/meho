...
export function parseTargetEntity(
  ...
  // K8s patterns
  if (args.namespace && args.name) return `${args.namespace}/${args.name}`;
  if (args.namespace) return `ns/${args.namespace}`;
  if (args.pod_name) return `pod/${args.pod_name}`;
  if (args.deployment) return `deploy/${args.deployment}`;
  if (args.service) return `svc/${args.service}`;
  ...
  // VMware patterns
  if (args.vm_name) return `vm/${args.vm_name}`;
