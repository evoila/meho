...
async def save_custom_skill(
    ...
    try:
        async with session_maker() as session:
            ...
            deduped_skill = _dedup_custom_skill(request.custom_skill, db_connector.generated_skill)
            if deduped_skill is None and request.custom_skill:
                logger.info(
                    f"Cleared duplicate custom_skill for connector {connector_id}",
                    connector_id=connector_id,
                )
            db_connector.custom_skill = deduped_skill  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            db_connector.updated_at = datetime.now(tz=UTC)  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
...
@router.post(
    "/{connector_id}/skill/regenerate",
    response_model=RegenerateSkillResponse,
    responses={
        400: {"description": "This connector has no operations to generate a skill from."},
        404: {"description": "Connector not found"},
        500: {"description": "Internal error: ..."},
    },
)
