...
class TestConvertInSubprocess:
    ...
    def test_sends_result_via_pipe_on_success(self):
        ...
        with patch(
            ...
            with patch.dict(
                ...
                _convert_in_subprocess(mock_pipe, b"test", "test.pdf", "application/pdf", 0)
    ...
    def test_sends_error_via_pipe_on_exception(self):
        ...
        mock_pipe = MagicMock()
        mock_converter_class = MagicMock(side_effect=RuntimeError("converter init failed"))
    ...
    def test_rlimit_set_on_linux(self):
        ...
        with (
            ...
            patch("meho_app.modules.knowledge.subprocess_converter.sys") as mock_sys,
            patch.dict(
                "sys.modules",
                {
                    "meho_app.modules.knowledge.document_converter": MagicMock(
            ...
            _convert_in_subprocess(mock_pipe, b"test", "test.pdf", "application/pdf", 4096)
            ...
            "sys.modules",
            {
                "meho_app.modules.knowledge.document_converter": MagicMock(
                    DoclingDocumentConverter=MagicMock(
                        return_value=MagicMock(
                            convert_file=MagicMock(
                                return_value=MagicMock(export_to_dict=MagicMock(return_value={}))
                            )
                        )
                    )
                ),
    ...
    def test_pipe_closed_in_finally(self):
        ...
        with patch.dict(
            ...
            {
                "meho_app.modules.knowledge.document_converter": MagicMock(
                    DoclingDocumentConverter=MagicMock(side_effect=RuntimeError("boom"))
                ),
            },
        ):
            _convert_in_subprocess(mock_pipe, b"test", "test.pdf", "application/pdf", 0)
...
# ---------------------------------------------------------------------------
def _make_sync_executor_patch():
    """Patch asyncio.get_event_loop so run_in_executor calls fn synchronously.

    Avoids thread-pool hangs in CI where pytest-timeout fires before the
    executor thread drains.
    """

    async def _sync_executor(_executor, fn, *args):
...
class TestConvertFileInSubprocess:
    ...
    async def test_returns_docling_document_on_success(self):
        ...
        docling_document_cls = sys.modules["docling_core.types.doc"].DoclingDocument
        docling_document_cls.model_validate.return_value = mock_doc
        ...
        with (
            ...
            _make_sync_executor_patch(),
        ):
            result = await convert_file_in_subprocess(b"test", "test.pdf", "application/pdf")
    ...
    async def test_raises_on_subprocess_error(self):
        ...
        with (
            ...
            with pytest.raises(ValueError, match="Document conversion failed"):
                await convert_file_in_subprocess(b"test", "test.pdf", "application/pdf")
    ...
    async def test_calls_heartbeat_callback(self):
        ...
        docling_document_cls = sys.modules["docling_core.types.doc"].DoclingDocument
        docling_document_cls.model_validate.return_value = MagicMock()
