...
"""Safe-by-construction ``sudo`` primitive for the Linux SSH write ops.

The ``linux-ssh`` connector's day-2 write verbs (``linux.file.write`` /
``linux.service.control`` / ``linux.script.run`` / ``linux.sysctl.write`` /
``linux.firewall.load``) run as root, but the connector authenticates over
SSH as an unprivileged login user and elevates via ``sudo -S``. This module
is the one place the sudo password reaches the wire, and it is shaped so the
...
Each SSH connector family carries its own copy of this primitive (rke2
carries the same free-function form; holodeck carries ``_pwsh.py``; bind9
carries its sudo helper inline) rather than sharing a single module, so a
change to one connector's elevation shape cannot silently alter another's.
The elevation functions here are byte-identical to the rke2 reference by
design -- the safety analysis is load-bearing and must not drift between the
families; only this module docstring and the family-scoped structlog event
name are adapted (the bind9->rke2 lineage adapts the event name likewise:
bind9 ``ssh_sudo_command_executed`` -> rke2 ``rke2_ssh_sudo_command_executed``).
"""
...
async def run_remote_bash_with_sudo(
    ...
    *,
    ...
    _log.info(
        "linux_ssh_sudo_command_executed",
