...
export function SettingsPage() {
  ...
  const { data: auditData } = useQuery<{ entries: AuditEntry[] }>({
    ...
    queryFn: () => adminClient.getConfigAudit<{ entries: AuditEntry[] }>(),
  ...
  const saveMutation = useMutation({
    mutationFn: (data: {
      ...
      model_override?: string;
      temperature_override?: number;
    }) => adminClient.updateAdminConfig<TenantConfig>(data),
    onSuccess: () => {
              ...
              {auditData.entries.length === 0 ? (
                ...
                <div className="space-y-2">
                  {auditData.entries.map((entry, i) => (
                    <div key={`entry-${i}`} className="p-3 bg-slate-900 rounded-lg text-sm">
