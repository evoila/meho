...
import type { Endpoint, ParameterField } from '@/api/types';
...
export function EndpointBrowser({ connectorId }: Readonly<EndpointBrowserProps>) {
  ...
  const { data: endpoints, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () => connectorsClient.listEndpoints(connectorId, {
      method: methodFilter || undefined,
      safety_level: safetyFilter || undefined,
      is_enabled: ({ enabled: true, disabled: false } as Record<string, boolean | undefined>)[statusFilter] ?? undefined,
      limit: 500
    }),
  });
...
function EndpointCard({ endpoint, onEdit, onTest }: Readonly<EndpointCardProps>) {
