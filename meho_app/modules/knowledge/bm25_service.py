...
class BM25Service:
    ...
    async def search(
        ...
        if not chunks:
            logger.warning(
                "no_documents_found", tenant_id=str(tenant_id), metadata_filters=metadata_filters
            )
            return []
        ...
        sorted_results: list[dict[str, Any]] = sorted(
            results_with_scores, key=lambda x: float(x["bm25_score"]), reverse=True
        )[:top_k]
    ...
    async def _fetch_chunks(
        self, tenant_id: UUID | str, metadata_filters: dict[str, Any] | None = None
        ...
        stmt = select(KnowledgeChunkModel).where(KnowledgeChunkModel.tenant_id == tenant_id_str)
    ...
    def _generate_cache_key(
        ...
        chunk_hash = hashlib.md5("".join(chunk_ids).encode()).hexdigest()[:8]  # noqa: S324 -- non-security hash context
    ...
    async def _get_cached_corpus(self, cache_key: str) -> list[list[str]] | None:
        """
        ...
        try:
            # Get base64-encoded JSON data from Redis
            cached_str = await self.redis.get(cache_key)
            if cached_str:
                # Decode base64 → bytes → JSON parse
    ...
    async def _cache_corpus(self, cache_key: str, tokenized_corpus: list[list[str]]) -> None:
        ...
        try:
            # JSON → bytes → base64 (for Redis with decode_responses=True)
            serialized_bytes = json.dumps(tokenized_corpus).encode("utf-8")
    ...
    async def invalidate_cache(
        self, tenant_id: UUID | str, connector_id: str | None = None
    ) -> None:
        """
