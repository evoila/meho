 ...
 * ```
 */
export function ToolViewer({ details, className }: Readonly<ToolViewerProps>) {
    ...
    <div className={cn('flex flex-col', className)}>
      ...
      <div className="grid grid-cols-2 gap-0">
        ...
        <div className="flex flex-col">
          ...
          <div className="max-h-[55vh] overflow-auto bg-background p-4">
            {(() => {
              if (details.tool_output) return <JsonViewer data={details.tool_output} />;
              if (hasError) return <span className="text-red-400 text-sm">Execution failed - see error above</span>;
              return <span className="text-text-tertiary text-sm italic">No output data</span>;
            })()}
          </div>
        </div>
      </div>
