...
def _format_confidence(verified_via: list[str]) -> str:
    ...
    """
    from meho_app.modules.agents.shared.topology_utils import parse_verification_evidence
...
def format_topology_context_for_prompt(
    ...
    for ctx in found_contexts:
        ...
        if ctx.same_as_entities:
            ...
            for correlated in ctx.same_as_entities[:5]:
                ...
                confidence_label = _format_confidence(
                    correlated.get("verified_via", [])
                )
                corr_freshness = _format_freshness(
                    getattr(corr_entity, "last_verified_at", None)
                )
                lines.append(
                    f"    == {corr_entity.name} ({corr_entity.entity_type}) "
                    f"[{corr_connector}]"
                )
    ...
    lines.append("Use this knowledge to guide your investigation. Start by checking these known paths.")
    lines.append("")
                ...
                if result.found and result.entity:
                    ...
                    try:
                        ...
                        for rel in rels_to:
                            ...
                            relationships.append({
                                "from_entity": from_ent.name if from_ent else str(rel.from_entity_id),
                    ...
                    for correlated in result.same_as_entities:
                        same_as_entities.append({
                            ...
                            "connector_name": correlated.connector_name or correlated.connector_type,
                            "verified_via": correlated.verified_via,
                            "similarity_score": 0.0,  # Not available from CorrelatedEntity
                        })

                contexts.append(TopologyContext(
                    query=ref,
                    found=result.found,
...
async def get_topology_context_service(
    session: AsyncSession,
