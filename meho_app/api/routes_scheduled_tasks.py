...
def _task_to_response(task: ScheduledTaskModel) -> ScheduledTaskResponse:
    ...
    return ScheduledTaskResponse(
        ...
        next_run_at=_dt_to_iso(task.next_run_at),
        total_runs=task.total_runs,
        last_run_at=_dt_to_iso(task.last_run_at),
        last_run_status=task.last_run_status,
        created_by_user_id=task.created_by_user_id,
        created_at=task.created_at.isoformat(),
        updated_at=task.updated_at.isoformat(),
        allowed_connector_ids=task.allowed_connector_ids,
        delegate_credentials=task.delegate_credentials,
...
async def list_timezones(user: CurrentUser):
    """Return sorted list of available IANA timezone names."""
    return _get_sorted_timezones()


@router.post("/parse-schedule", response_model=ParseScheduleResponse)
async def parse_schedule(body: ParseScheduleRequest, user: CurrentUser):
...
async def validate_cron(body: ValidateCronRequest, user: CurrentUser):
...
@router.post("/generate-prompt", response_model=GenerateTaskPromptResponse)
async def generate_task_prompt(user: CurrentUser):
...
async def list_tasks(
    ...
    tasks = await service.list_tasks(user.tenant_id)
...
async def service_create(
    db: AsyncSession, user, body: CreateScheduledTaskRequest
) -> ScheduledTaskModel:
...
async def get_task(
    ...
    service = ScheduledTaskService(db)
    task = await service.get_task(user.tenant_id, task_id)
    if task is None:
...
async def update_task(
    ...
    try:
        task = await service.update_task(user.tenant_id, task_id, **update_fields)
    except ValueError as e:
        raise HTTPException(status_code=422, detail=str(e)) from e
    if task is None:
        raise HTTPException(status_code=404, detail="Scheduled task not found")
...
async def delete_task(
    ...
    service = ScheduledTaskService(db)
    deleted = await service.delete_task(user.tenant_id, task_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Scheduled task not found")
    await db.commit()
    return None
...
async def toggle_task(
    ...
    service = ScheduledTaskService(db)
    task = await service.toggle_task(user.tenant_id, task_id)
    if task is None:
        raise HTTPException(status_code=404, detail="Scheduled task not found")
    await db.commit()
    return _task_to_response(task)
...
async def run_task_now(
    ...
    """
    service = ScheduledTaskService(db)
    run = await service.run_now(user.tenant_id, task_id)
    if run is None:
        raise HTTPException(status_code=404, detail="Scheduled task not found")
    await db.commit()
...
async def list_task_runs(
    ...
    if task is None:
        raise HTTPException(status_code=404, detail="Scheduled task not found")
    runs = await service.get_runs(user.tenant_id, task_id, limit=limit, offset=offset)
