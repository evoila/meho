...
async def audit_transcript(
    ...
    has_final_answer = any(
        e.get("type") == "final_answer" for e in events
    )
    ...
    all_events_typed = all(
        bool(e.get("type")) for e in events
    )
    ...
    error_summaries = [
        ...
        for e in error_events_list[:5]
    ]

    approval_events = sum(
        1 for e in events if e.get("type") == "approval_required"
    )
