...
async def _execute_response_channel(
    ...
    try:
        ...
        async with session_maker() as db:
            ...
            if not connector:
                logger.warning(
                    f"Response channel connector {connector_id} not found"
                )
                return

            connector_type = connector.connector_type
            connector_config = connector.protocol_config or {}
        ...
        formatted_result = format_for_connector(connector_type, result)
...
async def execute_event_investigation(
    ...
    async with session_maker() as db:
        try:
            ...
            _created_by_user_id = (
                registration.created_by_user_id if registration else None
            )
            ...
            if session_id:
                ...
                if not rendered_prompt:
                    ...
                    rendered_prompt = renderer.render_prompt(
                        prompt_template, payload, connector_name=connector_name,
                    )
                ...
                await _run_agent_investigation(
                    ...
                    allowed_connector_ids=_allowed_connector_ids,
                    delegation_active=_delegation_active,
                    registration_id=registration_id,
                    notification_targets=_notification_targets,
                    # Phase 94: response channel
                    response_config=registration.response_config if registration else None,
                    payload=payload,
            ...
            try:
                ...
                logger.info(f"Event {registration_id}: prompt rendered ({len(rendered_prompt)} chars)")
...
async def _event_delegation_flag_callback(
    ...
    async with session_maker() as session:
        ...
        stmt = update(EventRegistrationModel).where(
            EventRegistrationModel.id == trigger_id
...
async def _run_agent_investigation(
    ...
    try:
        ...
        await broadcaster.publish(
            session_id,
            {"type": "processing_started", "sender_id": "system:event"},
        )
