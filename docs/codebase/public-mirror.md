...
- [.githooks/pre-push](../../.githooks/pre-push) — developer-side safety hook that
  blocks direct pushes to `evoila/meho`. Installed by `scripts/dev-setup.sh` via
  `git config core.hooksPath .githooks`.
- [scripts/dev-setup.sh](../../scripts/dev-setup.sh) — one-time setup to wire the
  pre-push hook into a fresh clone.
