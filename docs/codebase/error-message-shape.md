...
| vSphere `PropertyCollector.RetrievePropertiesEx` explicit **`ManagedObjectNotFound`** fault on dispatch (`vmware.vm.info` and sibling typed reads, #3481) | vCenter sends this addressed missing-resource condition as a SOAP-shaped HTTP 500. The `vmware-rest` connector parses only the explicit `fault_type='ManagedObjectNotFound'` at its shared `RetrievePropertiesEx` seam and raises `ConnectorResourceNotFoundError`; the dispatcher catches that type before `connector_error` and returns `status='not_found'`, `error` prefixed `not_found:`, and `extras` with stable `error_code='not_found'`, `resource_ids`, plus singular `resource_id` for one MoID. The queried MoID is caller-supplied and remains available for recovery; the error row audits the same envelope. Any other vim fault, a non-PropertyCollector method, or a transport-only 5xx remains the generic `connector_error` result — status 500 alone never proves absence. Built by `result_connector_not_found` in `operations/_errors.py`; see `docs/codebase/connectors-vmware-rest.md` for the seam and the #3479 destructive `vm.destroy` exclusion. | compliant (structured) — landed in #3481 | #3479 owns `vm.destroy` execute/preview handling |
