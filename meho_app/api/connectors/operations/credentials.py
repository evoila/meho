...
@router.post(
    ...
    responses={
        403: {"description": "Cannot store credentials with reserved service user ID"},
        404: {"description": "Connector not found"},
    },
)
async def set_user_credentials(
    connector_id: str, credentials: dict, user: Annotated[UserContext, Depends(get_current_user)]
):
    """
...
@router.get(
    ...
    responses={403: {"description": "Access denied"}, 404: {"description": "Connector not found"}},
)
async def get_credential_status(
    ...
    async with session_maker() as session:
        ...
        return {
            ...
            "credential_health_checked_at": record.credential_health_checked_at.isoformat()
            if record.credential_health_checked_at
            else None,
        }
...
@router.delete(
    "/{connector_id}/credentials",
    responses={403: {"description": "Access denied"}, 404: {"description": "No credentials found"}},
)
...
@router.delete(
    "/{connector_id}/service-credential",
    responses={404: {"description": "No service credential found for this connector"}},
)
