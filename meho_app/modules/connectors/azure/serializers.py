...
def _serialize_nsg_rule(rule: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "direction": str(rule.direction.value) if hasattr(rule.direction, "value") else str(rule.direction),
        "access": str(rule.access.value) if hasattr(rule.access, "value") else str(rule.access),
        "protocol": str(rule.protocol.value) if hasattr(rule.protocol, "value") else str(rule.protocol),
...
def serialize_azure_nsg(nsg: Any) -> dict[str, Any]:
    ...
    default_rules = [
        _serialize_nsg_rule(r)
        for r in _safe_list(getattr(nsg, "default_security_rules", None))
    ]
...
def serialize_azure_resource_group(rg: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "provisioning_state": getattr(rg, "properties", None)
        and getattr(rg.properties, "provisioning_state", None)
