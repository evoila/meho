 ...
 *
 * Clean linear timeline for the investigation journey.
 * Single collapsible section: header with status, then connector cards in sequence.
 * No sticky zones, no floating cards, no dimming.
 */
...
export function buildConnectorStates( // NOSONAR (cognitive complexity)
  events: OrchestratorEvent[],
  startTime: number
...
export function OrchestratorProgress({
  ...
  startTime,
  isLive = false,
}: Readonly<OrchestratorProgressProps>) {
  ...
  // One-line plan summary from investigation plan
  const planSummary = useMemo(() => {
    if (!investigationPlan || !isLive) return null;
    const systems = investigationPlan.plannedSystems.map((s) => s.name);
    if (systems.length === 0) return null;
    return `Plan: ${systems.join(', ')}`;
  }, [investigationPlan, isLive]);
      ...
      <div className="flex-1 max-w-3xl">
        ...
        <AnimatePresence>
          {isExpanded && (
            ...
            >
              {/* Plan summary (live only, one-liner) */}
              {planSummary && (
              ...
              {/* Full investigation plan (when expanded) */}
              {investigationPlan && isLive && (
                <InvestigationPlan plan={investigationPlan} />
              )}

              {/* Connector cards -- linear list */}
              {connectors.length === 0 ? (
