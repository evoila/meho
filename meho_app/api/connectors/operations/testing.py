...
async def test_connector_connection(  # NOSONAR (cognitive complexity)
    connector_id: str,
...
async def _test_typed_connector_connection(connector, connector_id, request, user, session):
    ...
    try:
        if connector.connector_type == "vmware":
            ...
            typed_connector: BaseConnector = VMwareConnector(
...
async def _test_standard_auth(
    connector, connector_id, request, user, cred_repo
