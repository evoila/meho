...
class TestReduceDataHandlerUsesQueryEngine:
    ...
    async def test_reduce_data_handler_uses_query_engine(self):
        ...
        mock_executor.execute_sql_async.assert_awaited_once_with(
            "sess-1", "SELECT name FROM pods"
        )
...
class TestNoPandasImport:
    """Verify that importing reduce_data and knowledge_handlers doesn't pull in pandas."""

    def test_reduce_data_no_pandas_import(self):
        """After importing reduce_data and knowledge_handlers, pandas is NOT in sys.modules."""
        # Remove pandas from sys.modules if already loaded by other tests
        had_pandas = "pandas" in sys.modules
        if had_pandas:
            # We can't truly test this if pandas was already imported by
            # other tests in the same process. Mark as expected limitation.
            pytest.skip(
                "pandas already imported by other tests in this process; "
                "isolation test requires fresh interpreter"
            )

        # These imports should NOT pull in pandas
        import meho_app.modules.agents.base.reduce_data  # noqa: F401
        import meho_app.modules.agents.shared.handlers.knowledge_handlers  # noqa: F401

        assert "pandas" not in sys.modules, (
            "pandas was imported as a side effect of reduce_data / knowledge_handlers"
        )
