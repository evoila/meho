...
"""
Jira Operations Auto-Sync.

Automatically syncs Jira operations to the database when the operations
...
This enables semantic search (BM25 + embeddings) across all Jira operations.

Usage:
    On API startup, call sync_all_jira_connectors() to update any
...
def _format_jira_operation_as_text(op: OperationDefinition, connector_name: str = "Jira") -> str:
    """
    Format Jira operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.

    Args:
        op: Jira operation definition
    ...
    keywords = _generate_jira_search_keywords(op)
    if keywords:
...
def _generate_jira_search_keywords(op: OperationDefinition) -> str:
    """
    Generate search keywords for better BM25 matching.

    Adds abbreviations, synonyms, and variations that users might search for.
    Jira-specific vocabulary: issue/ticket/bug/story/epic/task, jql/query/search,
    comment/note, transition/status/workflow, project/board, sprint/release, etc.
    ...
    # Jira-specific abbreviations and synonyms
    jira_terms = {
        "issue": [
            "issue",
            "issues",
    ...
    for key, terms in jira_terms.items():
    ...
    if "get" in combined_text and "list" not in combined_text:
        keywords.update(["get", "show", "check", "inspect", "detail", "view"])
    if "create" in combined_text:
        keywords.update(["create", "add", "new", "open", "file"])
...
async def sync_jira_operations_if_needed(
    ...
    """
    Sync Jira operations if the version is outdated.
    ...
    if current_version == JIRA_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing Jira operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {JIRA_OPERATIONS_VERSION}"
    )
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(JIRA_OPERATIONS)} total, "
...
async def _sync_jira_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for Jira operations.

    Enables hybrid search (BM25 + semantic) for Jira operations.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "jira",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing Jira knowledge chunks for connector {connector_id}")
    ...
    for op in JIRA_OPERATIONS:
        try:
            ...
            text = _format_jira_operation_as_text(op, connector_name)
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=["api", "operation", "jira", op.category],
    ...
    logger.info(f"Created {chunks_created} knowledge chunks for Jira connector {connector_id}")
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = JIRA_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await session.flush()
        logger.debug(f"Updated connector {connector_id} to version {JIRA_OPERATIONS_VERSION}")


async def sync_all_jira_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all Jira connectors to the latest operations version.
    ...
    # Find all Jira connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "jira")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No Jira connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} Jira connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        # Sync operations
        added, updated, chunks = await sync_jira_operations_if_needed(
            session=session,
            connector_id=str(connector.id),
            tenant_id=str(connector.tenant_id),
            current_version=current_version,
            knowledge_store=knowledge_store,
            connector_name=str(connector.name) if connector.name else "Jira",
        )
    ...
    summary = {
        ...
        "current_version": JIRA_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"Jira operation sync complete: "
        ...
        logger.info("All Jira connectors already at latest version")
