...
class TenantConfigRepository:
        ...
        if existing:
            ...
            config = TenantAgentConfig(
                ...
                temperature_override=(
                    {"value": temperature_override} if temperature_override else None
                ),
    ...
    async def _update_with_audit(  # NOSONAR (cognitive complexity)
        self,
        config: TenantAgentConfig,
        ...
        if installation_context is not None and current_context != installation_context:
            ...
            config.installation_context = installation_context  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        ...
        if model_override is not None and current_model != model_override:
            ...
            config.model_override = model_override  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        ...
        if temperature_override is not None:
            ...
            if current_temp != temperature_override:
                ...
                config.temperature_override = {"value": temperature_override}  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        ...
        if features is not None and current_features != features:
            ...
            config.features = features  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            changes_made.append("features")

        if changes_made:
            config.updated_by = updated_by  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            await self.session.flush()
    ...
    async def update_tenant(
        ...
        if display_name is not None:
            config.display_name = display_name  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            changes_made.append("display_name")

        if subscription_tier is not None:
            config.subscription_tier = subscription_tier  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            changes_made.append("subscription_tier")

        if is_active is not None:
            config.is_active = is_active  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            changes_made.append("is_active")

        if max_connectors is not None:
            config.max_connectors = max_connectors  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            changes_made.append("max_connectors")

        if max_knowledge_chunks is not None:
            config.max_knowledge_chunks = max_knowledge_chunks  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            changes_made.append("max_knowledge_chunks")

        if max_workflows_per_day is not None:
            config.max_workflows_per_day = max_workflows_per_day  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        ...
        if email_domains is not None:
            # Normalize email domains to lowercase
            normalized_domains = [d.lower().strip() for d in email_domains]
            config.email_domains = normalized_domains  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
    ...
    async def disable_tenant(
        ...
        config.is_active = False  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        config.updated_by = disabled_by  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await self.session.flush()
    ...
    async def enable_tenant(
        ...
        config.is_active = True  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        config.updated_by = enabled_by  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await self.session.flush()
