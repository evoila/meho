...
class TopologyEntityModel(Base):
    ...
    connector_id = Column(UUID(as_uuid=True), nullable=True)  # NULL for external entities like URLs
    connector_name = Column(String(255), nullable=True)  # Cached connector name for display

    # Entity classification (NEW in TASK-156)
    entity_type = Column(String(100), nullable=False)  # "Pod", "VM", "Namespace", "Host"
    connector_type = Column(String(50), nullable=False)  # "kubernetes", "vmware", "gcp"

    # Scoped identity (NEW in TASK-156)
    scope = Column(
        JSONB, nullable=True, default=dict
    )  # {"namespace": "prod"} or {"datacenter": "dc1"}
    canonical_id = Column(String(500), nullable=False)  # "prod/nginx" or moref for VMware
    ...
    discovered_at = Column(TIMESTAMP(timezone=True), nullable=False, default=lambda: datetime.now(UTC))
    last_verified_at = Column(TIMESTAMP(timezone=True), nullable=True)
    stale_at = Column(TIMESTAMP(timezone=True), nullable=True)
...
class TopologyEmbeddingModel(Base):
    ...
    entity_id = Column(
        UUID(as_uuid=True), ForeignKey("topology_entities.id", ondelete="CASCADE"), primary_key=True
    )
    embedding = Column(Vector(1024), nullable=True)  # Voyage AI voyage-4-large

    # Relationship back to entity
    entity = relationship("TopologyEntityModel", back_populates="embedding")
...
class TopologyRelationshipModel(Base):
    ...
    from_entity_id = Column(
        UUID(as_uuid=True), ForeignKey("topology_entities.id", ondelete="CASCADE"), nullable=False
    )
    to_entity_id = Column(
        UUID(as_uuid=True), ForeignKey("topology_entities.id", ondelete="CASCADE"), nullable=False
    )
    ...
    # Relationships
    from_entity = relationship("TopologyEntityModel", foreign_keys=[from_entity_id])
    to_entity = relationship("TopologyEntityModel", foreign_keys=[to_entity_id])
...
class TopologySameAsModel(Base):
    ...
    verified_via = Column(ARRAY(Text), nullable=False)  # ["IP: 192.168.1.10", "Both exist in APIs"]
    ...
    # Relationships
    entity_a = relationship("TopologyEntityModel", foreign_keys=[entity_a_id])
    entity_b = relationship("TopologyEntityModel", foreign_keys=[entity_b_id])
...
class TopologySameAsSuggestionModel(Base):
    ...
    match_type = Column(String(50), nullable=False)  # "hostname_match", "ip_match"
    match_details = Column(Text, nullable=True)  # Details about how match was detected

    # Workflow status
    status = Column(
        String(20), nullable=False, default="pending"
    )  # "pending", "approved", "rejected"
    suggested_at = Column(TIMESTAMP(timezone=True), nullable=False, default=lambda: datetime.now(UTC))
    resolved_at = Column(TIMESTAMP(timezone=True), nullable=True)
    resolved_by = Column(String(255), nullable=True)  # user who approved/rejected

    # LLM verification (Phase 3)
    llm_verification_attempted = Column(Boolean, default=False, nullable=False)
    llm_verification_result = Column(
        JSONB, nullable=True
