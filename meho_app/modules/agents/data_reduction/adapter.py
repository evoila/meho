...
def execute_data_query(
    ...
    try:
        ...
        if total_source > max_records:
            logger.warning(f"Source has {total_source} records, truncating to {max_records}")
        ...
        if query.aggregates:
            aggregates = _compute_aggregates(engine, query.aggregates, query, compute_exprs)
...
def _extract_source(  # NOSONAR (cognitive complexity)
    data: dict[str, Any] | list,
    source_path: str,
) -> list[dict[str, Any]]:
...
def _translate_expression(expression: str) -> str:
    ...
    result: str = re.sub(r"[a-zA-Z_]\w*(?:\.[a-zA-Z_]\w*)*", _replace_field, expression)
    return result


def _build_sql(  # NOSONAR (cognitive complexity)
    query: DataQuery,
    ...
    # SELECT clause
    select_parts: list[str] = [_field_to_col(f) for f in query.select] if query.select else ["*"]
...
def _build_condition(condition: FilterCondition) -> str:  # NOSONAR (cognitive complexity)
...
def _compute_aggregates(  # NOSONAR (cognitive complexity)
    engine: QueryEngine,
    ...
    for agg in aggregates:
        ...
        if func_name == AggregateFunction.COLLECT:
            ...
            results[agg.name] = _distinct_aggregate(engine, field, query, compute_exprs)
            continue
        ...
        if query.group_by:
            ...
            try:
                ...
                for row in rows:
                    key_parts = [str(row[g.strip('"')]) for g in group_cols]
