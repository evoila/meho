...
class MEHODependencies:
    ...
    session_type: str = field(
        default="interactive"
    )  # "interactive", "automated_event", "automated_scheduler"
    ...
    delegation_active: bool = field(
        default=True
    )  # current delegation_active flag from trigger model
    ...
    async def _search_with_filter(  # NOSONAR (cognitive complexity)
        self,
        query: str | None = None,
        queries: str | list[str] | None = None,
        top_k: int = 15,
        ...
        # The unified semantic ranker + reranker path is robust enough that we
        # do not need adaptive fallback here. Using 0.6 as sweet spot: good
        # recall without too much noise.
        threshold = 0.6
        all_chunks_list: list[Any] = []
...
- confidence="low" if guessing or unclear"""
        ...
        try:
            ...
            determination: ConnectorDetermination = result.output  # type: ignore[assignment]  # pydantic-ai returns typed output at runtime
    ...
    async def search_endpoints(  # NOSONAR (cognitive complexity)
        ...
        try:
            ...
            if use_hybrid:
                ...
                from meho_app.modules.knowledge.bm25_hybrid_service import (
                    BM25HybridService,
                )
    ...
    async def get_endpoint_details(  # NOSONAR (cognitive complexity)
    ...
    async def call_endpoint(  # NOSONAR (cognitive complexity)
        self,
        connector_id: str,
        endpoint_id: str,
        ...
        try:
            ...
            from meho_app.modules.connectors.keycloak_user_checker import (
                KeycloakUserChecker,
            )
            ...
            try:
                ...
                logger.warning(f"Failed to check credential age for connector {connector_id}: {e}")
        except CredentialScopeError as e:
            logger.error(
                f"❌ DEPENDENCIES: Connector {connector_id} not in allowed scope: {e.allowed}"
            )
        ...
        if connector.auth_type == "SESSION":
            ...
            if session_state:
                ...
                logger.info(
                    "ℹ️  DEPENDENCIES: No existing session state (will login)"  # noqa: RUF001 -- intentional unicode character
                )
...
"""
        ...
        try:
            ...
            # PydanticAI returns AgentRunResult with typed .output
            data_summary: DataSummary = result.output  # type: ignore[assignment]  # pydantic-ai returns typed output at runtime
    ...
    async def reduce_data(
        ...
        try:
            ...
            if result.reduced_data:
                ...
                return {
                    "records": (
                        data.get(analysis.source_path, data) if analysis.source_path else data
                    ),
                    "aggregates": {},
                    "context": result.llm_context,
                    "metadata": {
