...
| `vmware.composite.*` dispatch when L2 ingested-but-**disabled** (#1601) | structured `extras` with `error_code='composite_l2_disabled'`, `disabled_op_ids[]`, `connector_id`; `error` message names the composite, the disabled op-ids, the **real** per-op enable verb (`meho connector edit-op <connector_id> <op_id> --enable`), the connector-level caveat (`meho connector enable` does not cascade to spec-ingested `group_id=NULL` ops), and a reference to `docs/codebase/connectors-vmware-rest.md`. Built by `result_composite_l2_disabled` in `operations/_errors.py`; raised by `preflight_l2_dependencies` (via the `is_enabled`-agnostic `descriptor_exists_any_state` probe) in `connectors/vmware_rest/composites/_preflight.py`. Distinct from `composite_l2_missing` above: a disabled op resolves to `None` through `lookup_descriptor`'s `is_enabled=TRUE` filter exactly like an absent one, so the disabled state was previously mis-reported as missing with a re-ingest remediation that does not apply. | compliant (structured) — landed in #1601 (Option A / distinct code + real enable verb) | — |
...
| Connector upstream **auth/session failure** on dispatch (any connector; the vRLI dispatch the operator saw as opaque `connector_error (440)`, #1798) | structured `extras` with `error_code='connector_auth_failed'`, `http_status` (the actual auth-class status the upstream returned — `401` or `440`, not a hard-coded value), `host` (the `target.host` the operator configured — not an info-leak, the operator's own value, same posture as the TLS row and `/ui/auth/login`), `upstream_message` (the upstream body's `message` field when JSON, else capped raw text, `null` when the body was empty — shared `_http_upstream_message` extractor with the 403/422 builders); the `error` names the host, the status, the likely cause **connector-agnostically** (a session/credential expiry or a misconfigured `auth_model` — the session connectors already retry once on a 401 internally via `_get_json_with_session_retry`, so a 401 reaching the dispatcher means re-login *also* failed), appends the verify-the-Vault-credential/`auth_model`-and-retry remediation + a `docs/architecture/connector-auth.md` ref + this doc ref, and tails the upstream message when present. The recognised auth-status set is the explicit module constant `_AUTH_FAILED_STATUSES = {401, 440}` (`401` load-bearing; `440` is vRLI's session-expiry status the team opted to recognise), gated by `is_auth_failed_status()`. Built by `result_connector_auth_failed` in `operations/_errors.py` (reading `exc.response`); caught by the dispatcher's `_run_branch_with_error_handling` in the same `httpx.HTTPStatusError` arm as the 403/422 siblings, **after** the 403/422 checks and **ahead of** the generic `connector_error` catch. Before #1804 a 401/440 flattened to a bare `connector_error: HTTPStatusError` with the auth cause buried in `extras.exception_message` — exactly the diagnosability gap that made #1798's `connector_error (440)` look like a stub-auth problem. Every other `HTTPStatusError` status (404, 5xx, 429) falls through to `connector_error` unchanged. | compliant (structured) — landed in #1804 | 429 (rate-limit) is a deliberate follow-up, not this surface |
