...
async def create_mcp_connector(  # NOSONAR (cognitive complexity)
    ...
    try:
        async with session_maker() as session:
            ...
            logger.info(
                f"Testing MCP connection: {request.server_url or request.command} (transport: {request.transport_type})"
            )
            ...
            try:
                ...
                logger.info(
                    f"MCP connection verified: {request.server_url or request.command} ({tools_discovered} tools discovered)"
                )
            ...
            query = select(ConnectorModel).where(ConnectorModel.id == connector.id)
            ...
            for i, op in enumerate(discovered_ops):
                search_content = (
                    f"{op.name} {op.operation_id} {op.description} {op.category} mcp {server_name}"
                )
            ...
            try:
                ...
                logger.warning(f"Failed to register MCP entity types: {e}")
            ...
            try:
                ...
                await session.commit()
                logger.info(f"Created {chunks_created} knowledge chunks for MCP operations")
            except Exception as e:
                logger.warning(f"Failed to create knowledge chunks: {e}")

            logger.info(
                f"Created MCP connector '{request.name}' with {ops_count} operations and {chunks_created} knowledge chunks"
            )
        ...
        logger.error(f"Create MCP connector failed: {e}", exc_info=True)
