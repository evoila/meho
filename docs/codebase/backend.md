...
| `ensure_tenant` | `src/meho_backplane/tenancy/ensure.py` | G0.8-T1 (#628) just-in-time tenant seeding. `migration 0002` ships the `tenant` table empty and defers seeding; every tenant-scoped write (`documents` / `graph_node` / `graph_edge` / `broadcast_override`) carries a real `REFERENCES tenant(id)` FK, so a fresh v0.2 deploy's first *real* (non-dry-run) write failed `documents_tenant_id_fkey` until a row existed. `ensure_tenant(tenant_id, session)` issues one dialect-native `INSERT INTO tenant (id, slug, name) … ON CONFLICT (id) DO NOTHING` (dialect resolved via `conn.dialect.name`, same idiom as `targets.resolver`; PG uses `postgresql.insert`, SQLite uses `sqlite.insert`, both target `tenant.id`). `slug` / `name` derive deterministically as `tenant-<first-8-hex-of-uuid>` — overrideable later by the v0.3 provisioning API (the `ON CONFLICT DO NOTHING` never clobbers an existing operator-named row). `ON CONFLICT DO NOTHING` makes concurrent first-writes safe (idempotent, no `SELECT`-then-`INSERT` race, no advisory lock). The package keeps `auth/` import-clean; the caller (`verify_jwt_and_bind`) owns the transaction boundary. **Not** a tenant-provisioning REST API — that's v0.3+, explicitly out of scope. |
