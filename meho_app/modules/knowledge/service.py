...
class KnowledgeService:
        ...
        *,
        ...
        if store is not None:
            ...
            self.store = KnowledgeStore(
                ...
                hybrid_search_service=self.hybrid_search,
            )
    ...
    async def search(
        ...
        if search_mode == "hybrid":
            result = await self.store.hybrid_search(query, filters, top_k)
        elif search_mode == "bm25":
            result = await self.store.bm25_search(query, filters, top_k)
        else:
            result = await self.store.semantic_search(query, filters, top_k)
    ...
    async def search_with_rerank(
        ...
        results = await self.hybrid_search.search_with_rerank(
            query=query,
    ...
    async def ingest_text(
        ...
        result = await self.store.ingest_text(
            text=text,
            tenant_id=tenant_id,
            user_id=user_id,
            connector_id=connector_id,
    ...
    async def list_chunks(
        ...
        return await self.repository.list_chunks(filters, limit=limit)
