...
function EmailHistory({ connectorId }: { connectorId: string }) {
  ...
  const { data: entries, isLoading } = useQuery({
    ...
    queryFn: () => connectorsClient.getEmailHistory(connectorId),
...
export function ConnectorDetails({ connectorId, onBack }: ConnectorDetailsProps) { // NOSONAR (cognitive complexity)
  ...
  const { data: connector, isLoading } = useQuery({
    ...
    queryFn: () => connectorsClient.getConnector(connectorId),
  });
  ...
  const { data: credentialStatus } = useQuery({
    ...
    queryFn: () => connectorsClient.getCredentialStatus(connectorId),
    enabled: !!connector,
  });

  // Delete mutation
  const deleteMutation = useMutation({
    mutationFn: () => connectorsClient.deleteConnector(connectorId),
    onSuccess: () => {
  ...
  const handleUpdateConnector = async (updates: UpdateConnectorRequest) => {
    await connectorsClient.updateConnector(connectorId, updates);
