...
class MCPConnector(BaseConnector):
    ...
    async def connect(self) -> bool:
        ...
        for attempt in range(_MAX_RETRIES):
            try:
                ...
                logger.error(
                    "MCP connect failed (non-retryable) for %s: %s",
                    self.connector_id,
                    exc,
                )
                raise
    ...
    async def _do_connect(self) -> bool:
        ...
        logger.info(
            "MCP connector %s connected to %s, discovered %d tools",
            self.connector_id,
    ...
    async def execute(self, operation_id: str, parameters: dict[str, Any]) -> OperationResult:
        ...
        try:
            ...
            logger.warning("MCP call_tool failed, attempting reconnect: %s", exc)
            try:
                await self.disconnect()
                await self._do_connect()
                result = await self._session.call_tool(original_name, arguments=parameters)  # type: ignore[union-attr]
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.warning("MCP test_connection failed for %s: %s", self.connector_id, exc)
            return False
