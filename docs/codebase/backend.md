...
| `Base` / `AuditLog` / `Tenant` | `src/meho_backplane/db/models.py` | SQLAlchemy 2.x `DeclarativeBase` plus the `AuditLog` model (Task #28) and the `Tenant` model (Task #231, G0.1-T1). Columns use portable `Uuid` and `JSON().with_variant(JSONB(), "postgresql")` types so the model and migration run cleanly on both PG (production) and SQLite (dev/test). `AuditLog` indexes: `occurred_at`, `operator_sub`, `tenant_id` (all in `__table_args__`). `audit_log.tenant_id` is **nullable in v0.2** (chassis-era forward-compat) and ships **without a FK** to `tenant.id` (deferred to v0.2.next backfill migration). `Tenant` has `slug` UNIQUE NOT NULL with the named `tenant_slug_idx` b-tree index. |
