...
class HostnameMatcher:
    ...
    def __init__(self, session: AsyncSession, enable_llm_verification: bool = True):
    ...
    async def check_entity_correlations(
        self,
        ...
        for hostname, match_type in hostnames:
            ...
            for connector in matching_connectors:
                ...
                existing_suggestion = await self.topology_repo.get_existing_suggestion(
                    ...
                    entity_b_id=connector.topology_entity_id,
                )
...
async def get_hostname_matcher(session: AsyncSession) -> HostnameMatcher:
