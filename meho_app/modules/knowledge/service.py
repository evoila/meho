...
class KnowledgeService:
    ...
    async def search_with_rerank(
        ...
        return {
            "chunks": results,
            "scores": [
                r.get("score", r.get("rerank_score", r.get("similarity", 0.0))) for r in results
            ],
            "total": len(results),
        }
    ...
    async def ingest_text(
        ...
        Delegates to IngestionService (which owns chunking, embedding, and
        lifecycle field persistence) rather than calling KnowledgeStore
        directly -- KnowledgeStore has no ingest_text method.
        """
        # Local import: IngestionService imports from this module transitively
        # through chunking/metadata helpers, so top-level would cycle.
        ...
        if not isinstance(self.store, KnowledgeStore):
            raise TypeError(
                "KnowledgeService.ingest_text requires a concrete KnowledgeStore; "
                "protocol-only stores do not expose the chunking pipeline."
            )

        job_repository = IngestionJobRepository(self.session) if self.session is not None else None
        ...
        return {"chunk_ids": chunk_ids, "status": "completed"}
