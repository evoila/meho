...
from meho_app.core.errors import ConnectorError
...
def _nsx_error(
    ...
    """Map a VMwareRESTClient exception to a correctly classified ConnectorError.

    Classification matrix (mirrors VMwareRESTClient exception contract):
    - PermissionError            → auth / permanent   (raised for HTTP 401/403)
    - HTTPStatusError 401/403    → auth / permanent
    - HTTPStatusError 404        → data_error / permanent  (resource not found)
    - HTTPStatusError 5xx        → connection / transient  (upstream server error)
    - HTTPStatusError other 4xx  → data_error / permanent  (bad request, gone, …)
    - RequestError               → connection / transient  (network / DNS / timeout)
    - RuntimeError               → unknown / permanent     (client not connected, …)
    """
    ...
    if isinstance(exc, PermissionError):
        return ConnectorError(
            error_type="auth",
            severity="permanent",
            message=f"NSX {operation} failed{suffix}: {exc}",
    ...
    if isinstance(exc, httpx.HTTPStatusError):
        ...
        if status in (401, 403):
            return ConnectorError(
                error_type="auth",
                severity="permanent",
                message=f"NSX {operation} failed (HTTP {status}){suffix}: {exc}",
                connector_name=connector_name,
            )
        if status == 404:
            return ConnectorError(
                ...
                severity="permanent",
                message=f"NSX {operation} not found (HTTP 404){suffix}: {exc}",
                connector_name=connector_name,
            )
        if 500 <= status < 600:
            return ConnectorError(
                ...
                severity="transient",
                message=f"NSX {operation} upstream error (HTTP {status}){suffix}: {exc}",
                connector_name=connector_name,
            )
        return ConnectorError(
            error_type="data_error",
    ...
    if isinstance(exc, httpx.RequestError):
        return ConnectorError(
            error_type="connection",
            severity="transient",
            message=f"NSX {operation} network error{suffix}: {exc}",
            connector_name=connector_name,
        )
    return ConnectorError(
        error_type="unknown",
...
class NsxHandlerMixin:
    ...
    _nsx_client: Any
    # connector_id is provided by BaseConnector; used as fallback connector_name in errors.
    connector_id: str
    ...
    async def _list_nsx_segments(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        except (httpx.HTTPStatusError, httpx.RequestError, PermissionError, RuntimeError) as e:
            raise _nsx_error("list_segments", e, connector_name=self.connector_id) from e
    ...
    async def _get_nsx_segment(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise _nsx_error("get_segment", e, segment_id, connector_name=self.connector_id) from e
    ...
    async def _list_nsx_firewall_policies(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise _nsx_error("list_firewall_policies", e, connector_name=self.connector_id) from e
    ...
    async def _get_nsx_firewall_rule(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise _nsx_error(
                "get_firewall_rule", e, f"{policy_id}/{rule_id}", connector_name=self.connector_id
            ) from e
    ...
    async def _list_nsx_security_groups(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise _nsx_error("list_security_groups", e, connector_name=self.connector_id) from e
    ...
    async def _list_nsx_tier0_gateways(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise _nsx_error("list_tier0_gateways", e, connector_name=self.connector_id) from e
    ...
    async def _list_nsx_tier1_gateways(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise _nsx_error("list_tier1_gateways", e, connector_name=self.connector_id) from e
    ...
    async def _list_nsx_load_balancers(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise _nsx_error("list_load_balancers", e, connector_name=self.connector_id) from e
    ...
    async def _list_nsx_transport_zones(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise _nsx_error("list_transport_zones", e, connector_name=self.connector_id) from e
    ...
    async def _list_nsx_transport_nodes(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise _nsx_error("list_transport_nodes", e, connector_name=self.connector_id) from e
    ...
    async def _get_nsx_transport_node(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise _nsx_error(
                "get_transport_node", e, node_id, connector_name=self.connector_id
            ) from e
    ...
    async def _search_nsx(self, params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise _nsx_error("search", e, query, connector_name=self.connector_id) from e
