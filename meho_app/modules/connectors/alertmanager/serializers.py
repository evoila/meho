...
def _format_duration(start_iso: str, end_iso: str | None = None) -> str:
    ...
    """
    try:
        start = datetime.fromisoformat(start_iso.replace("Z", UTC_OFFSET_SUFFIX))
        if end_iso:
            end = datetime.fromisoformat(end_iso.replace("Z", UTC_OFFSET_SUFFIX))
        else:
...
def _format_time_remaining(ends_at_iso: str) -> str:
    ...
    """
    try:
        ends_at = datetime.fromisoformat(ends_at_iso.replace("Z", UTC_OFFSET_SUFFIX))
...
def serialize_silences(silences: list[dict[str, Any]]) -> dict:  # NOSONAR (cognitive complexity)
    """
