...
function cmd_up() {
  ...
  wait_for_service meho 150 2  # First boot runs migrations inside entrypoint
...
function activate_venv() {
  if [[ -d "${VENV_DIR}" ]]; then
    ...
    echo "   Create one with: python3 -m venv .venv && source .venv/bin/activate && uv sync --group dev --group test"
    exit 1
  fi
  return 0
}
...
function check_python_deps() {
  ...
  if ! python -c "import uvicorn" &> /dev/null 2>&1; then
    ...
    cd "${PROJECT_ROOT}"
    # Lean default: core deps + dev + test. Contributors working on docling
    # features opt in with: uv sync --group dev --group test --group docling-group
...
function install_mcp_server() {
  ...
  if ! python -c "import meho_mcp_server" &> /dev/null 2>&1; then
    ...
    cd "${PROJECT_ROOT}"
    uv pip install -e "./meho_mcp_server"
...
function run_migrations_local() {
  ...
  local modules=("topology" "connectors" "knowledge" "memory" "agents" "ingestion" "scheduled_tasks" "orchestrator_skills" "audit")
