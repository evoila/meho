...
_INGEST_DESCRIPTION: Final[str] = (
    ...
    "endpoint POST /api/v1/connectors/ingest does NOT expose this "
    "parameter and always scopes to the calling operator's tenant — if "
    "you need global scope, use this tool instead. Re-ingesting the "
    "same spec under a different scope creates a shadow copy in the "
    "other namespace (the dedup lookup is scope-aware and will not "
    "match across scopes); verify the target scope matches your intent "
    "before switching surfaces. "
