...
class IngestionJobRepository:
    ...
    async def update_file_hash(self, job_id: str, file_hash: str, storage_key: str) -> None:
        """Store file hash and storage key on the job for resume validation."""
        job = await self.get_job(job_id)
        if not job:
            raise NotFoundError(f"Job {job_id} not found")
    ...
    async def save_document_summary(self, job_id: str, summary: str) -> None:
        """Persist the LLM-generated document summary on the job record."""
        job = await self.get_job(job_id)
        if not job:
            raise NotFoundError(f"Job {job_id} not found")
        job.document_summary = summary
        await self.session.flush()

    async def save_partial_chunk_ids(self, job_id: str, chunk_ids: list[str]) -> None:
        """Persist the list of successfully created chunk IDs on a failed job."""
        job = await self.get_job(job_id)
        if not job:
            raise NotFoundError(f"Job {job_id} not found")
        job.chunk_ids = chunk_ids
    ...
    async def mark_resuming(self, job_id: str) -> bool:
        """Atomically transition a failed job back to processing.

        Uses a conditional UPDATE so only one concurrent caller can claim the
        resume. Returns True when this caller won the transition and False if
        the job was not in ``failed`` state (another worker already resumed,
        the job completed, or the job does not exist).
        """
        from sqlalchemy import update

        stmt = (
            update(IngestionJob)
            .where(IngestionJob.id == job_id, IngestionJob.status == "failed")
            .values(
                status="processing",
                error=None,
                error_details=None,
    ...
    async def list_jobs(self, filter: IngestionJobFilter) -> list[IngestionJob]:
        ...
        if filter.scope_type:
            query = query.where(IngestionJob.scope_type == filter.scope_type)
        if filter.connector_type_scope:
            query = query.where(IngestionJob.connector_type_scope == filter.connector_type_scope)
        if filter.status:
            query = query.where(IngestionJob.status == filter.status)
        if filter.job_type:
            query = query.where(IngestionJob.job_type == filter.job_type)
        if filter.doc_version:
            query = query.where(IngestionJob.doc_version == filter.doc_version)
