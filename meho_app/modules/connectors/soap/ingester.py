...
class SOAPSchemaIngester:
    ...
    Example:
        ...
        operations = ingester.ingest_wsdl(
    ...
    def __init__(self, config: SOAPConnectorConfig | None = None) -> None:
    ...
    def ingest_wsdl(
        ...
        try:
            ...
            raise ValueError(f"Failed to parse WSDL from {wsdl_url}: {e}") from e
            ...
            for port_name, port in service.ports.items():
                ...
                for op_name, operation in binding._operations.items():
                    try:
                        ...
                        logger.warning(f"⚠️ Failed to parse operation {service_name}.{op_name}: {e}")
                        continue
        ...
        logger.info(f"✅ Parsed {len(operations)} SOAP operations from {len(services)} service(s)")

        return operations, metadata, type_definitions

    def _parse_operation(  # NOSONAR (cognitive complexity)
        self,
        wsdl_url: str,
        service_name: str,
        port_name: str,
        ...
        # Get namespace
        namespace = ""
        if hasattr(operation, "input") and operation.input:  # noqa: SIM102 -- readability preferred over collapse
            if hasattr(operation.input, "body") and operation.input.body:  # noqa: SIM102 -- readability preferred over collapse
    ...
    def _element_to_schema(self, element: Any) -> dict[str, Any]:  # NOSONAR (cognitive complexity)
            ...
            if hasattr(element_type, "elements"):
                for child in element_type.elements:
                    if isinstance(child, tuple) and len(child) >= 2:
                        ...
                        # Check if required
                        if hasattr(child_element, "min_occurs"):  # noqa: SIM102 -- readability preferred over collapse
    ...
    def _xsd_element_to_json_schema(
        self, element: Any
            ...
            if hasattr(xsd_type, "elements") and list(xsd_type.elements):
                ...
                for child in xsd_type.elements:
                    if isinstance(child, tuple) and len(child) >= 2:
                        name = child[0]
                        child_element = child[1]
                        schema["properties"][name] = self._xsd_element_to_json_schema(child_element)
            else:
    ...
    def _extract_type_definitions(  # NOSONAR (cognitive complexity)
        self,
        client: Any,
        connector_id: UUID,
        tenant_id: str,
                ...
                if public_types is not None:
                    if hasattr(public_types, "items"):
                        ...
                        for i, item in enumerate(types_list[:3]):
                            logger.info(
                                f"🔍 Sample type {i}: type={type(item).__name__}, item={str(item)[:100]}"
                            )
        ...
        for type_qname, type_obj in type_dict.items():
            try:
                ...
                type_name = (
                    str(type_qname.localname)
                    if hasattr(type_qname, "localname")
                    else str(type_qname)
                )
    ...
    def _extract_type_properties(
        self, complex_type: Any
    ) -> list[SOAPProperty]:  # NOSONAR (cognitive complexity)
    ...
    def _element_to_property(
    ...
    ) -> SOAPProperty | None:  # NOSONAR (cognitive complexity)
        ...
        try:
            ...
            logger.warning(
                f"Could not convert element {name} to property",
                exc_info=True,
                exception_type=type(e).__name__,
            )
            return None

    def _attribute_to_property(self, name: str, attr: Any) -> SOAPProperty | None:
        ...
        try:
            ...
            logger.warning(
                f"Could not convert attribute {name} to property",
                exc_info=True,
                exception_type=type(e).__name__,
            )
            return None

    def _get_base_type(self, complex_type: Any) -> str | None:
        ...
        except Exception as e:  # noqa: BLE001 -- soft-fail: missing base type metadata is non-fatal for schema ingestion
            logger.warning(
                "Could not get base type",
                exc_info=True,
                exception_type=type(e).__name__,
            )
    ...
    async def create_knowledge_chunks_for_operations(
        ...
        except Exception as e:  # noqa: BLE001 -- chunk deletion is best-effort; non-fatal if table empty or schema mismatch
        ...
        for op in operations:
            ...
            except Exception as e:  # noqa: BLE001 -- chunk creation is best-effort; continue ingesting remaining operations
    ...
    def get_operation_count_estimate(self, _wsdl_url: str) -> int:
