...
class ConnectorModel(Base):
    ...
    specs = relationship(
        "OpenAPISpecModel", back_populates="connector", cascade=CASCADE_DELETE_ORPHAN
    )
    endpoints = relationship(
        "EndpointDescriptorModel", back_populates="connector", cascade=CASCADE_DELETE_ORPHAN
    )
    user_credentials = relationship(
        "UserCredentialModel", back_populates="connector", cascade=CASCADE_DELETE_ORPHAN
    )
    soap_operations = relationship(
        "SoapOperationDescriptorModel", back_populates="connector", cascade=CASCADE_DELETE_ORPHAN
    )
    soap_types = relationship(
        "SoapTypeDescriptorModel", back_populates="connector", cascade=CASCADE_DELETE_ORPHAN
    )
    typed_operations = relationship(
        "ConnectorOperationModel", back_populates="connector", cascade=CASCADE_DELETE_ORPHAN
    )
    typed_types = relationship(
        "ConnectorTypeModel", back_populates="connector", cascade=CASCADE_DELETE_ORPHAN
    )
    connector_memories = relationship(
        "ConnectorMemoryModel", back_populates="connector", cascade=CASCADE_DELETE_ORPHAN
    )
    event_registrations = relationship(
        "EventRegistrationModel", back_populates="connector", cascade=CASCADE_DELETE_ORPHAN
    )
...
class UserCredentialModel(Base):
    ...
    connector_id = Column(UUID(as_uuid=True), ForeignKey(FK_CONNECTOR_ID), nullable=False)
    ...
    session_refresh_expires_at = Column(
        TIMESTAMP(timezone=True), nullable=True
    ...
    )  # "healthy", "unhealthy", "expired", null=unknown
    credential_health_message = Column(
        String(500), nullable=True
    )  # Human-readable health failure reason
    credential_health_checked_at = Column(
        TIMESTAMP(timezone=True), nullable=True
    )  # When health was last assessed
...
class ConnectorOperationModel(Base):
    """
...
class EventRegistrationModel(Base):
    ...
    connector_id = Column(
        UUID(as_uuid=True),
        ForeignKey(FK_CONNECTOR_ID, ondelete="CASCADE"),
        nullable=False,
    )
    ...
    history = relationship(
        "EventHistoryModel", back_populates="registration", cascade=CASCADE_DELETE_ORPHAN
    )
