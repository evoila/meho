...
register_mcp_tool(
    definition=ToolDefinition(
        ...
        description=(
            ...
            "the loop made: a succeeded run always reports `turns >= 1`, "
            "so `turns: 0` on a failed run means the model was never "
            "reached (the model-init-failure fingerprint, #2644/#2743). "
