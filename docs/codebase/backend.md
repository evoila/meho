...
| `Base` / `AuditLog` / `Tenant` / `Document` | `src/meho_backplane/db/models.py` | SQLAlchemy 2.x `DeclarativeBase` plus the `AuditLog` model (Task #28), the `Tenant` model (Task #231, G0.1-T1), and the `Document` model (Task #258, G0.4-T1). Columns use portable `Uuid` and `JSON().with_variant(JSONB(), "postgresql")` types so the models and migrations run cleanly on both PG (production) and SQLite (dev/test). `AuditLog` indexes: `occurred_at`, `operator_sub`, `tenant_id` (all in `__table_args__`). `audit_log.tenant_id` is **nullable in v0.2** (chassis-era forward-compat) and ships **without a FK** to `tenant.id` (deferred to v0.2.next backfill migration). `Tenant` has `slug` UNIQUE NOT NULL with the named `tenant_slug_idx` b-tree index. `Document` is the per-tenant retrievable shared substrate G4 (#215) + G5 (#216) both write through `index_document` (T3, #260) and read through `retrieve` (T4, #261): `id` UUID PK, `tenant_id` UUID NOT NULL **with a real `REFERENCES tenant(id)` FK** (unlike audit_log — `documents` is a brand-new table with no chassis-era rows, so the FK lands at table-creation time), `source` / `source_id` / `kind` / `body` / `body_hash` text, `tokens` nullable int, `embedding` as a `_PortableVector384` `TypeDecorator` (pgvector `Vector(384)` on PG, JSON-encoded `Text` on SQLite — both honour the `Mapped[list[float]]` annotation), and `doc_metadata` JSON/JSONB mapped to SQL column `metadata` (the `DeclarativeBase.metadata` reserved-attribute escape). Indexes in `__table_args__`: `documents_tenant_source_id_idx` (unique composite), `documents_body_hash_idx` (btree). |
| `AuditMiddleware` | `src/meho_backplane/audit.py` | Pure-ASGI middleware (Task #28, extended Task #233 G0.1-T3). For every authenticated request (`operator_sub` present in contextvars) writes one `audit_log` row synchronously before yielding the response back to the send chain. Reads `tenant_id` off contextvars (bound by `verify_jwt_and_bind`) via `_resolve_tenant_id`, parses it back to `uuid.UUID`, and persists it to the `audit_log.tenant_id` column. Missing or malformed `tenant_id` in contextvars logs `audit_missing_tenant_id` / `audit_malformed_tenant_id` at error level (programming-bug surface — auth dependency was supposed to bind it) and writes the row with `tenant_id=NULL` rather than failing closed; the nullable column from T1 is the schema-level escape hatch for exactly this case. Buffers `http.response.start`/`http.response.body` messages so the fail-closed path can replace them with a 500 `{"detail": "audit_write_failed"}` when the audit insert raises. Skips public surfaces and 401 paths by keying on the `operator_sub` contextvar's presence rather than path-matching. |
...
| `Connector` (ABC) | `src/meho_backplane/connectors/base.py` | Abstract base for every MEHO connector. Three abstract async methods: `fingerprint(target) -> FingerprintResult`, `probe(target) -> ProbeResult`, `execute(target, op_id, params) -> OperationResult`. Subclasses set `product` (e.g. `"vsphere"`, `"vault"`, `"bind9"`) and register operations via a per-product `op_map` (T5 reference impl, G0.2). The `Target` type annotation is a forward reference — resolved in G0.3 when the Target model lands. |
