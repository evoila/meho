...
class TestHybridSearchContract:
    ...
    async def test_hybrid_search_returns_expected_structure(self):
        ...
        The compatibility path still returns `KnowledgeChunk` objects from
        `KnowledgeStore.search_hybrid()`, even though the underlying ranking
        strategy is now semantic ranker + reranker.
        """
        ...
        mock_hybrid.adaptive_search = AsyncMock(return_value=[{"id": mock_chunk.id}])
