...
def _skill_to_response(skill: OrchestratorSkillModel) -> OrchestratorSkillResponse:
    ...
    return OrchestratorSkillResponse(
        ...
        connector_type=skill.connector_type,
...
async def list_skills(
    ...
    skills = await service.list_skills(user.tenant_id)
...
async def create_skill(
    ...
    service = OrchestratorSkillService(db)
    try:
        skill = await service.create_skill(user.tenant_id, body)
        await db.commit()
...
async def get_skill(
    ...
    service = OrchestratorSkillService(db)
    skill = await service.get_skill(user.tenant_id, skill_id)
    if skill is None:
...
async def update_skill(
    ...
    """
    service = OrchestratorSkillService(db)
    skill = await service.update_skill(user.tenant_id, skill_id, body)
    if skill is None:
        raise HTTPException(status_code=404, detail="Orchestrator skill not found")
    await db.commit()
    await db.refresh(skill)
...
async def delete_skill(
    ...
    service = OrchestratorSkillService(db)
    deleted = await service.delete_skill(user.tenant_id, skill_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Orchestrator skill not found")
    await db.commit()
    return None
...
async def generate_skill(
    ...
    try:
        ...
        try:
            ...
            connectors = await conn_repo.list_connectors(user.tenant_id)
            if connectors:
                connector_lines = []
                for c in connectors:
                    if c.is_active:
        ...
        try:
            service = OrchestratorSkillService(db)
            existing = await service.list_skills(user.tenant_id)
            if existing:
        ...
        raise HTTPException(
            status_code=422,
            detail="Skill generation timed out (60s). Please try again.",
        ) from None
