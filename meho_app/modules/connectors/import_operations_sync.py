...
async def sync_operations_for_imported_connector(
...
async def _get_knowledge_store(session: AsyncSession) -> Optional["KnowledgeStore"]:
...
async def _sync_vmware_operations(
    ...
    try:
        ...
        knowledge_store = await _get_knowledge_store(session)
...
async def _sync_rest_operations(
    ...
    try:
        ...
        spec_dict = await parser.parse(spec_content, "imported-spec")

        # Create endpoints from spec
        endpoint_repo = EndpointDescriptorRepository(session)
        endpoints = await endpoint_repo.create_from_spec(connector_id, spec_dict)
    ...
    try:
        ...
        if type_definitions:
            ...
            for type_def in type_definitions:
                await type_repo.create(type_def)
