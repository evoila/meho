...
| `Base` / `AuditLog` / `Tenant` / `Document` / `Target` | `src/meho_backplane/db/models.py` | SQLAlchemy 2.x `DeclarativeBase` plus the `AuditLog` model (Task #28), the `Tenant` model (Task #231, G0.1-T1), the `Document` model (Task #258, G0.4-T1), and the `Target` model (Task #252, G0.3-T1). Columns use portable `Uuid`, `JSON().with_variant(JSONB(), "postgresql")`, and `JSON().with_variant(PG_ARRAY(Text), "postgresql")` types so models and migrations run cleanly on both PG (production) and SQLite (dev/test). `AuditLog` indexes: `occurred_at`, `operator_sub`, `tenant_id`, `target_id` (all in `__table_args__`). `audit_log.tenant_id` and `audit_log.target_id` are **nullable in v0.2** and ship **without FKs** (same soft-FK discipline — deferred to v0.2.next backfill migrations). `Tenant` has `slug` UNIQUE NOT NULL with the named `tenant_slug_idx` b-tree index. `Document` is the per-tenant retrievable shared substrate for G4/G5 with a real `REFERENCES tenant(id)` FK, `_PortableVector384` embedding, and GIN/IVFFlat indexes (PG-only, migration-managed). `Target` has `(tenant_id, name)` UNIQUE via `targets_tenant_name_idx`, a `(tenant_id, product)` b-tree index, and a GIN index on `aliases` (PG only). |
