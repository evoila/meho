...
import type { Workflow, Plan } from '@/api/types';
...
export function ChatMessageList({ // NOSONAR (cognitive complexity)
  messages,
  currentWorkflow,
  isProcessing,
  isApproving,
  ...
  // Determine where to inject live orchestrator progress (before the streaming assistant message)
  const showLiveOrchestrator = !!(isProcessing && isOrchestratorActive && liveEventsStartTime && orchestratorEvents);
  const lastAssistantIdx = showLiveOrchestrator
  ...
  return (
    <>
      <div className="space-y-6" role="log" aria-live="polite" aria-relevant="additions" aria-label="Chat conversation">
        {messages.map((message, index) => {
          ...
          <div
            ...
            data-testid={
              (() => {
                if (message.status === 'WAITING_APPROVAL') return 'approval-required-message';
                if (message.status === 'RUNNING' && message.plan) return 'auto-execute-message';
                return undefined;
              })()
            }
          >
            {/* Live Orchestrator Progress -- injected before the streaming assistant message */}
            {index === lastAssistantIdx && showLiveOrchestrator && orchestratorEvents && liveEventsStartTime && (
              <div className="mb-4">
            ...
            {!message.auditEntry && (() => {
              ...
              if (message.structuredContent && message.role === 'assistant') return (
                <StructuredMessage
              ...
              return (
                <>
                  <Message
                    ...
                    content={
                      (() => {
                        if (message.role === 'user' && message.mentionMetadata) return stripMentionPrefix(message.content);
                        if (message.role === 'assistant') return stripSynthesisXml(message.content);
      ...
      {/* Live Orchestrator Progress -- fallback when no streaming assistant message exists yet */}
      {showLiveOrchestrator && lastAssistantIdx === -1 && orchestratorEvents && liveEventsStartTime && (
        <div className="mt-4">
          <OrchestratorProgress
            events={orchestratorEvents}
            startTime={liveEventsStartTime}
