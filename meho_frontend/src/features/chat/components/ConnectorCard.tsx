...
/**
 * Parsed event with display info and raw data.
 */
...
/**
 * Format inner event for display, preserving raw data.
 */
function formatInnerEvent(event: WrappedAgentEvent): ParsedEvent {
  ...
  switch (type) {
    ...
    case 'tool_start':
      return {
        type: 'call',
        content: String(data.description || data.message || ''),
    ...
    case 'tool_complete':
      return {
        ...
        tool: String(data.tool || data.name || ''),  // Include tool name for matching
        rawData: data,
        rawType: type,
      };
    case 'error':
      return {
        type: 'error',
        content: String(data.message || data.error || ''),
        rawData: data,
        rawType: type,
      };
    default:
 ...
 */
function highlightLine(line: string, keyOffset: number): React.ReactNode[] {
...
export function ConnectorCard({
  ...
  // Build sequential timeline with clickable events, timing, and pending state
  interface TimelineEvent {
    ...
    pending?: boolean;  // still waiting for result
    outputSummary?: string;  // Brief summary of output for display
  }

  const timeline = useMemo(() => {
    const events: TimelineEvent[] = [];
    const filteredEvents = connector.events.filter((e) =>
      ['thought', 'action', 'observation', 'tool_start', 'tool_complete', 'error'].includes(
        e.inner_event?.type || ''
      )
    );
    
    const parsed = filteredEvents.map((event) => ({
      ...formatInnerEvent(event),
      arrivalTime: event._arrivalTime,
    }));

    // Track which result events we've paired
    const usedResults = new Set<number>();
    let prevTime = parsed[0]?.arrivalTime;

    for (let i = 0; i < parsed.length; i++) {
      const event = parsed[i];
      const currentTime = event.arrivalTime;
      const duration = prevTime && currentTime ? currentTime - prevTime : undefined;
      
      if (event.type === 'think') {
        events.push({
          type: 'thought',
          label: event.content || 'Thinking...',
          thought: event.content,
          duration,
        });
        if (currentTime) prevTime = currentTime;
      } else if (event.type === 'call' && event.tool) {
        // Find matching result by tool name
        ...
        for (let j = i + 1; j < parsed.length; j++) {
          ...
          if (candidate.type === 'result' && !usedResults.has(j)) {
            const resultTool = candidate.tool || candidate.rawData.tool;
            if (resultTool === event.tool) {
              output = candidate.rawData.result ?? candidate.rawData;
              resultTime = candidate.arrivalTime;
              usedResults.add(j);
              break;
            }
          }
        ...
        // If no match found, check if there's a subsequent event (tool completed implicitly)
        // or if connector is in a terminal state (success/failed/timeout)
        const isTerminalState = ['success', 'failed', 'timeout', 'partial'].includes(connector.status);
        const hasSubsequentEvent = i < parsed.length - 1;
        const implicitlyCompleted = output === null && (isTerminalState || hasSubsequentEvent);
        
        // Duration is from call to result (if we have result), or to next event
        let toolDuration = duration;
        if (resultTime && currentTime) {
          toolDuration = resultTime - currentTime;
        } else if (implicitlyCompleted && i + 1 < parsed.length && parsed[i + 1].arrivalTime && currentTime) {
          // Use time to next event as proxy for completion time
          toolDuration = (parsed[i + 1].arrivalTime ?? 0) - currentTime;
        }
        
        const finalOutput = output ?? (implicitlyCompleted ? { status: 'completed' } : null);
        events.push({
          type: 'tool',
          label: event.tool,
          toolCall: {
            tool: event.tool,
            input: event.rawData.args || {},
            output: finalOutput,
          },
          duration: toolDuration,
          pending: output === null && !implicitlyCompleted,
          outputSummary: extractOutputSummary(event.tool, finalOutput),
        });
        if (resultTime) {
          prevTime = resultTime;
        } else if (currentTime) {
      ...
      // Skip 'result' events - they're paired with 'call' events
    }
    
    return events;
  }, [connector.events, connector.status]);
        ...
        {isExpanded && (
          ...
          >
            <div className="px-3 pb-3 space-y-2">
              ...
              {/* Sequential timeline - click any event for details */}
              {hasEvents && (
                <div className="rounded-lg bg-slate-900/80 border border-slate-700/50 overflow-hidden">
                  ...
                  <div className="p-2 space-y-0.5">
                    {timeline.map((event, idx) => {
                      ...
                      const hasOutput = event.type === 'tool' && event.outputSummary;
                      ...
                      return (
                        <div key={idx}>
                          ...
                          <button
                            onClick={() => {
                              if (event.type === 'tool' && event.toolCall) {
                                setSelectedToolCall(event.toolCall);
                              } else if (event.type === 'thought' && event.thought) {
                                setSelectedThought(event.thought);
                              }
                            }}
                            className={clsx(
                          ...
                          >
                            ...
                            {event.type === 'tool' ? (
                              <>
                                ...
                                {/* Tool name */}
                                <span className="text-sky-300 font-medium text-sm flex-shrink-0 group-hover:text-sky-200">
                                  {event.label}
                                </span>
                                {/* Duration */}
                                {event.duration !== undefined && !event.pending && (
                              ...
                              <>
                                {/* Think badge */}
                                <span className="text-[10px] font-medium text-violet-400 bg-violet-500/10 px-1.5 py-0.5 rounded flex-shrink-0">
                                  THINK
                                </span>
                                {/* Thought content */}
                                <span className="text-slate-400 text-xs line-clamp-3 flex-1 group-hover:text-slate-300">
                                  {event.label}
                                </span>
                                {/* Duration */}
                          ...
                          {/* Nested output summary for tools */}
                          {hasOutput && (
                            <div className="flex items-center gap-2 px-2 py-1 ml-3">
                              <span className="text-slate-700 w-3 text-center flex-shrink-0 font-mono text-[10px]">
                                {isLast ? '└' : '│'}
                              </span>
