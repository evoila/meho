...
class MetadataExtractor:
    ...
    def extract_metadata(
        ...
        metadata.document_name = document_context.get("document_name") or document_name
        metadata.page_numbers = self._extract_page_numbers(document_context)
        if metadata.page_numbers:
            metadata.page_number = metadata.page_numbers[0]
            metadata.page_start = metadata.page_numbers[0]
            metadata.page_end = metadata.page_numbers[-1]
    ...
    def _extract_page_numbers(self, document_context: dict[str, Any]) -> list[int]:
        """Extract normalized page numbers from chunk context."""
        page_numbers = document_context.get("page_numbers", [])
        if not page_numbers:
            return []
