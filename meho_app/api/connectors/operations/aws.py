...
async def create_aws_connector(  # NOSONAR (cognitive complexity)
    ...
    try:
        async with session_maker() as session:
            ...
            if credentials:
                logger.info(f"Testing AWS connection: region={request.default_region}")
                try:
                    aws = AWSConnector(
                    ...
                    logger.info(f"AWS connection verified: region={request.default_region}")
                except ImportError:
