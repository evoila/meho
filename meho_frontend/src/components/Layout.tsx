...
import { MessageSquare, ChefHat, BookOpen, Plug, LogOut, Menu, X, Settings, Network, Building2, Activity, Clock, Brain, ClipboardList } from 'lucide-react';
...
export function Layout() {
  ...
  return (
    <div className={clsx(
      ...
      {/* Sidebar - Desktop */}
      <aside className="hidden md:flex flex-col w-64 glass border-r border-border z-20" aria-label="Main navigation">
        <div role="banner" className="p-6">
          <img src={mehoLogo} alt="MEHO.ai - Home" className="h-10 w-auto" />
        </div>

        <nav className="flex-1 px-4 space-y-2 mt-4" aria-label="Primary">
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                ...
                <Icon className={clsx('mr-3 h-5 w-5 transition-colors', isActive ? 'text-primary' : 'text-text-tertiary group-hover:text-text-primary')} />
                {item.name}
              </Link>
            );
          })}
          ...
          {license.edition === 'enterprise' && user?.isGlobalAdmin && !isInTenantContext && (
            <>
              <div className="pt-4 mt-4 border-t border-border">
                <span className="px-4 text-xs font-medium text-text-tertiary uppercase tracking-wider" role="heading" aria-level={2}>
                  Admin
                </span>
              </div>
              <Link
              ...
              >
                ...
                <Building2 className={clsx('mr-3 h-5 w-5 transition-colors', location.pathname.startsWith('/admin/tenants') ? 'text-primary' : 'text-text-tertiary group-hover:text-text-primary')} />
                Tenants
              </Link>
            </>
          )}
        ...
        <div className="p-4 border-t border-border" aria-label="User account">
      ...
      {/* Mobile Header */}
      <header role="banner" className="md:hidden fixed top-0 left-0 right-0 h-16 glass border-b border-border z-30 flex items-center justify-between px-4" aria-label="Mobile header">
