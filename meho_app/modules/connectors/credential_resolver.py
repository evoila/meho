...
class CredentialScopeError(Exception):
    def __init__(self, connector_id: str, allowed: list[str]) -> None:
...
class CredentialResolver:
    ...
    async def _resolve_interactive(self, user_id: str, connector_id: str) -> ResolvedCredential:
    ...
    async def _resolve_automated(
        ...
        *,
        ...
        if created_by_user_id:
            is_active = self.keycloak_checker.is_user_active(created_by_user_id, tenant_id)
            ...
            if not is_active:
                ...
                creds = await self.cred_repo.get_credentials(created_by_user_id, connector_id)
                if creds:
