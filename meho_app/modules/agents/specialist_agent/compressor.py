...
def _compress_search_operations(output: SearchOperationsOutput) -> str:
...
def _compress_dns_resolve(output: DnsResolveOutput) -> str:
    ...
    for rtype, records in output.records.items():
        if rtype in ("A", "AAAA", "NS"):
            ...
            values = ", ".join(f"{r.get('host', '')} (pri={r.get('priority', '')})" for r in records)
        elif rtype == "SRV":
            values = ", ".join(
