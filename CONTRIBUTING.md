...
See [docs/getting-started.md](docs/getting-started.md) for the 15-minute onboarding walkthrough (clone, configure, hot-reload mode, troubleshooting). Operator-level details live in [docs/deployment.md](docs/deployment.md).
...
If you're authoring a database migration, read [docs/contributing/migrations.md](docs/contributing/migrations.md) first -- the unified Alembic tree at `meho_app/alembic/` has four hard "do not do" rules that aren't obvious from the SQLAlchemy / Alembic docs.
...
Write comments that explain **why**, not **what**. If deleting a comment loses no information (because the code already says the same thing), the comment is noise. Task/session references, `# noqa` rationale, and section headers in long functions are welcome; narrating labels on the next line are not.

Docstrings on public classes, methods, and modules follow [Google style](https://google.github.io/styleguide/pyguide.html#38-comments-and-docstrings) (already the project standard per `AGENTS.md`). Frontend code uses JSDoc on exported symbols.

**Good** — explains rationale or flags non-obvious behavior:

```python
# Lazy import avoids loading heavy SDKs unless this connector is used
from meho_app.modules.connectors.vmware import VMwareConnector
```

```python
# noqa: S608 -- static SQL query, no user input reaches this string
