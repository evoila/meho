...
class CredentialRepository:
    ...
    async def store_credentials(self, user_id: str, credential: UserCredentialProvide) -> None:
        ...
        if existing:
            # Update existing
            existing.encrypted_credentials = encrypted  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            existing.credential_type = credential.credential_type  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            existing.updated_at = datetime.now(tz=UTC)  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            existing.is_active = True  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        else:
            # Create new
    ...
    async def get_credentials(self, user_id: str, connector_id: str) -> dict[str, str] | None:
        ...
        # Update last_used_at
        record.last_used_at = datetime.now(tz=UTC)  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment

        # Decrypt and return
        return self.encryption.decrypt(record.encrypted_credentials)  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
    ...
    async def get_session_state(self, user_id: str, connector_id: str) -> dict[str, Any] | None:
        ...
        decrypted_token = (
            self.encryption.decrypt(record.session_token) if record.session_token else None  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        )

        # Decrypt refresh token if exists
        decrypted_refresh = (
            self.encryption.decrypt(record.session_refresh_token)  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
    ...
    async def update_session_state(
        ...
        record.session_token = encrypted_token  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        record.session_token_expires_at = session_expires_at  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        record.session_state = session_state  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        record.updated_at = datetime.now(tz=UTC)  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment

        # Update refresh token if provided
        if refresh_token:
            encrypted_refresh = self.encryption.encrypt({"token": refresh_token})
            record.session_refresh_token = encrypted_refresh  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
            record.session_refresh_expires_at = refresh_expires_at  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
    ...
    async def get_credential_age_days(
        ...
        return (datetime.now(tz=UTC) - updated_at).days  # type: ignore[no-any-return]
