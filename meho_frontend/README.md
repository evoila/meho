...
The API surface is split into domain clients under `src/api/clients/`. All
clients share a single Axios transport that handles auth, tenant context,
and 401 refresh automatically.

```tsx
import { setAuthToken } from './api/clients/transport';
import { getChatClient } from './api/clients/chat';
import { getKnowledgeClient } from './api/clients/knowledge';

// Auth token lives on the transport layer and is picked up by every
// domain client automatically.
setAuthToken(yourJWTToken);

// Create a chat session
const chat = getChatClient();
const session = await chat.createSession({ title: 'Your goal' });

// Search knowledge
const knowledge = getKnowledgeClient();
const results = await knowledge.searchKnowledge({ query: 'search term' });
```
