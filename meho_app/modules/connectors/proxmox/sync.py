...
"""
Proxmox Operations Auto-Sync

Automatically syncs Proxmox operations to the database when the operations
...
This enables semantic search (BM25 + embeddings) across all Proxmox operations.

Usage:
    On API startup, call sync_all_proxmox_connectors() to update any
...
def _format_proxmox_operation_as_text(
    op: OperationDefinition, connector_name: str = "Proxmox"
) -> str:
    """
    Format Proxmox operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.
    Similar to _format_vmware_operation_as_text() for VMware.

    Args:
        op: Proxmox operation definition
...
def _generate_proxmox_search_keywords(op: OperationDefinition) -> str:
    """
    ...
    # Proxmox-specific abbreviations
    proxmox_abbrevs = {
        "vm": ["vm", "vms", "virtual machine", "qemu", "kvm"],
        "container": ["ct", "lxc", "container", "containers"],
        "node": ["node", "host", "server", "pve"],
        "storage": ["storage", "disk", "volume"],
        "snapshot": ["snap", "snapshot", "backup"],
        "cluster": ["cluster", "ha", "quorum"],
    }
    ...
    for key, abbrevs in proxmox_abbrevs.items():
    ...
    if "start" in combined_text or "stop" in combined_text:
        keywords.update(["power", "start", "stop", "shutdown"])
    if "migrate" in combined_text:
        keywords.update(["migrate", "move", "relocate"])
    if "clone" in combined_text:
        keywords.update(["clone", "copy", "duplicate"])

    return " ".join(sorted(keywords))


async def sync_proxmox_operations_if_needed(
    session: AsyncSession,
    connector_id: str,
    tenant_id: str,
    current_version: str | None,
    knowledge_store: Optional["KnowledgeStore"] = None,
    connector_name: str = "Proxmox VE",
) -> tuple[int, int, int]:
    """
    Sync Proxmox operations if the version is outdated.
    ...
    if current_version == PROXMOX_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing Proxmox operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {PROXMOX_OPERATIONS_VERSION}"
    )
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(PROXMOX_OPERATIONS)} total, "
...
async def _sync_proxmox_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for Proxmox operations.

    Enables hybrid search (BM25 + semantic) for Proxmox operations.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "proxmox",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing Proxmox knowledge chunks for connector {connector_id}")
    ...
    for op in PROXMOX_OPERATIONS:
        try:
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=["api", "operation", "proxmox", op.category],
    ...
    logger.info(f"Created {chunks_created} knowledge chunks for Proxmox connector {connector_id}")
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = PROXMOX_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]
        await session.flush()
        logger.debug(f"Updated connector {connector_id} to version {PROXMOX_OPERATIONS_VERSION}")


async def sync_all_proxmox_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all Proxmox connectors to the latest operations version.
    ...
    # Find all Proxmox connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "proxmox")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No Proxmox connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} Proxmox connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        added, updated, chunks = await sync_proxmox_operations_if_needed(
            session=session,
            connector_id=str(connector.id),
            tenant_id=str(connector.tenant_id),
            current_version=current_version,
            knowledge_store=knowledge_store,
            connector_name=str(connector.name) if connector.name else "Proxmox VE",
        )
    ...
    summary = {
        ...
        "current_version": PROXMOX_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"Proxmox operation sync complete: "
        ...
        logger.info("All Proxmox connectors already at latest version")
