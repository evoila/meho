...
async def dispatch_approval_notification(
    ...
    for target in notification_targets:
        ...
        try:
            ...
            logger.info(f"Approval notification sent via connector {connector_id} to {contact}")
        except Exception:
            logger.warning(
                f"Failed to send approval notification via connector {connector_id} to {contact}",
                exc_info=True,
            )
