...
def extract_event_tags(
    event: TranscriptEventModel,
) -> dict | None:  # NOSONAR (cognitive complexity)
    ...
    """
    details: dict = event.details or {}  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
    tags: dict = {}
...
def event_model_to_response(event: TranscriptEventModel) -> EventResponse:
    ...
    return EventResponse(
        id=str(event.id),
        timestamp=event.timestamp,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        type=event.type,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        summary=event.summary,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        details=convert_details_to_response(event.details or {}),  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        parent_event_id=str(event.parent_event_id) if event.parent_event_id else None,
        step_number=event.step_number,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        node_name=event.node_name,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        agent_name=event.agent_name,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        duration_ms=event.duration_ms,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
...
def transcript_to_summary(
    ...
    return SessionSummaryResponse(
        ...
        completed_at=transcript.completed_at,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        total_llm_calls=transcript.total_llm_calls,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        total_operation_calls=transcript.total_operation_calls,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        total_sql_queries=transcript.total_sql_queries,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        total_tool_calls=transcript.total_tool_calls,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        total_tokens=transcript.total_tokens,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        total_cost_usd=transcript.total_cost_usd,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        total_duration_ms=transcript.total_duration_ms,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        user_query=transcript.user_query,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
        agent_type=transcript.agent_type,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
    )
...
async def resolve_session_id(
    ...
    if session_id.lower() == "latest":
        ...
        return session.id  # type: ignore[return-value]  # SQLAlchemy ORM attribute access
