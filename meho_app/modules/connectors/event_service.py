...
class EventService:
    ...
    def verify_hmac_signature(raw_body: bytes, signature_header: str, secret: str) -> None:
        ...
        Args:
            ...
            signature_header: Hex-encoded HMAC-SHA256 from X-Webhook-Signature.
    ...
    async def is_duplicate(
        ...
        counter scoped to ``(registration_id, payload_hash)`` so event history
        can display a monotonic "N duplicates suppressed" value for this hash
        within the dedup window. The counter's TTL (600s) outlives the dedup
        window so the value survives long enough to be logged on each suppressed
        delivery.
        ...
        # Duplicate -- increment per-hash suppressed counter (600s TTL to outlive dedup window)
        suppressed_key = f"meho:event:suppressed:{registration_id}:{payload_hash}"
    ...
    async def log_event(
        ...
        Args:
            ...
            duplicates_suppressed: Number of duplicate deliveries suppressed for this
                payload hash since the last processed event (consumed from Redis).
