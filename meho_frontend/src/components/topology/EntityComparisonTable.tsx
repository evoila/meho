...
export function EntityComparisonTable({ // NOSONAR (cognitive complexity)
  entityA,
  entityB,
  matchDetails,
}: Readonly<EntityComparisonTableProps>) {
      ...
      <table className="w-full text-xs bg-gray-800">
        ...
        <tbody>
          {rows.map((row) => {
            ...
            const matchColor = row.isMatch ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400';
