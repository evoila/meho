...
def migrate():
    # Every sa.text() call below interpolates only the module-level NEW_TABLE /
    # OLD_TABLE constants — identifiers that cannot be bind-parameterized in SQL.
    # User data (version_num) is always passed as a bound parameter. SQL injection
    # is not reachable here, but semgrep cannot prove it statically, so each call
    # site carries a nosemgrep suppression.
