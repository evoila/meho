...
TRANSITION NOTE (Phase 90):
File-based ingestion (PDF, DOCX, HTML) now goes through DoclingDocumentConverter
in document_converter.py, which provides structure-aware chunking via HybridChunker
with heading path enrichment and TOC filtering. This module is still used for:
- Raw text ingestion via ingest_text() (no file to parse with Docling)
- Backward compatibility for callers importing chunk_text() or TextChunker
"""
...
class TextChunker:
    ...
    DEPRECATED: Use DoclingDocumentConverter.chunk_document() for file-based chunking.
    This class is retained for raw text chunking (ingest_text) and backward compatibility.
    """
    ...
    def chunk_text(self, text: str) -> list[str]:
        ...
        while start < len(tokens):
            ...
            if end < len(tokens):
                chunk_text = self._find_break_point(chunk_text, code_blocks, tokens[start:end])
    ...
    def _find_break_point(  # NOSONAR (cognitive complexity)
        ...
        if self._looks_like_code(chunk_text[last_half_start:]):
            ...
            for i in range(last_half_start, search_limit, -1):
                if i < len(chunk_text) and chunk_text[i] == "\n":  # noqa: SIM102 -- readability preferred over collapse
    ...
    def chunk_document_with_structure(
        self, text: str, document_name: str, detect_headings: bool = True
        ...
        if detect_headings:
            ...
            for chunk in simple_chunks:
                context = {"document_name": document_name, "heading_stack": []}
                chunks_with_context.append((chunk, context))
