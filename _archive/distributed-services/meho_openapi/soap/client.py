...
class SOAPClient:
    ...
    def __init__(self, config: SOAPConnectorConfig):
        self.config = config
    ...
    async def connect(self) -> None:
        ...
        if self.config.auth_type == SOAPAuthType.BASIC:
    ...
    async def call(
        ...
        assert self._client is not None, "SOAP client failed to connect"
        ...
        try:
            ...
            logger.debug(
                f"📥 SOAP response received: {operation_name} "
                f"({duration_ms:.2f}ms)"
            )
            ...
            logger.error(
                f"❌ SOAP operation failed: {operation_name} - {fault_string}"
            )
    ...
    async def _session_login(self) -> None:
        ...
        try:
            ...
            if len(parts) == 2:
                service_method, operation = parts
            else:
    ...
    async def _session_logout(self) -> None:
    ...
    def _create_ws_security_plugin(self) -> Any:
        ...
        username_token = UsernameToken(
            ...
            created=datetime.utcnow() if self.config.ws_security_use_timestamp else None,
        )
...
class VMwareSOAPClient(SOAPClient):
    ...
    def _convert_mor_params(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in params.items():
            if isinstance(value, dict):
                ...
                if mor_type and mor_value:
                    ...
                    logger.debug(f"Converting MOR param '{key}': type={mor_type}, value={mor_value}")
    ...
    async def _session_login(self) -> None:
        ...
        try:
            ...
            session_manager = self._create_mor(
                ...
                session_manager_ref.get("_value_1", session_manager_ref.get("value", "SessionManager"))
            )
