...
The pydantic-ai Agent is mocked to avoid real LLM calls.
"""
...
async def test_generate_summary_success(
    mock_agent_cls: MagicMock, mock_get_config: MagicMock
) -> None:
...
@patch("meho_app.core.config.get_config")
@patch("pydantic_ai.Agent")
async def test_generate_summary_includes_system_prompt(
    mock_agent_cls: MagicMock, mock_get_config: MagicMock
) -> None:
    ...
    system_prompt = agent_init_args[1].get(
        "system_prompt", agent_init_args[0][1] if len(agent_init_args[0]) > 1 else ""
    )
