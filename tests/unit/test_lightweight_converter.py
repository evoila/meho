...
class TestChunkDocument:
    ...
    def test_chunk_prefix_is_deprecated_no_op(self) -> None:
        """``chunk_prefix`` is retained for call-site compatibility but must not
        mutate the stored chunk body; retrieval enrichment is now built from
        metadata (heading_stack, document_name, etc.) rather than inline prose."""
        converter = LightweightDocumentConverter()
        doc = LightweightDocument(
            markdown="Some content for chunking.", name="test.pdf", page_count=1, pages=[]
        )
        with_prefix = converter.chunk_document(doc, chunk_prefix="kubernetes connector (prod).")
        without_prefix = converter.chunk_document(doc, chunk_prefix="")
        assert len(with_prefix) > 0
        assert [t for t, _ in with_prefix] == [t for t, _ in without_prefix]
        assert "kubernetes connector" not in with_prefix[0][0]
