...
import type { Endpoint, TestEndpointRequest, TestEndpointResponse } from '@/api/types';
...
export function TestEndpointModal({ connectorId, endpoint, onClose }: Readonly<TestEndpointModalProps>) {
  ...
  const handleTest = useCallback(async () => {
    ...
    try {
      ...
      const response = await connectorsClient.testEndpoint(connectorId, endpoint.id, request);
      setResult(response);
  ...
  }, [connectorId, endpoint.id, pathParams, queryParams, bodyJson, useSystemCreds, connectorsClient]);
