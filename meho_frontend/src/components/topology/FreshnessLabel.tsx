...
export function FreshnessLabel({ timestamp, className }: Readonly<FreshnessLabelProps>) {
  const { relative, absolute, freshness } = useMemo(() => {
    ...
    const freshness = (() => { if (diffHr < 24) return 'fresh'; if (diffDay <= 7) return 'aging'; return 'stale'; })();
