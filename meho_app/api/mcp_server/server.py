...
async def meho_investigate(
    ...
    """
    user_context = _get_user_context(ctx) if ctx else UserContext(
        user_id="mcp-stdio", tenant_id="default", roles=["user"]
    )
    ...
    try:
        ...
        from meho_app.database import get_session_maker
        from meho_app.modules.agents.specialist import create_specialist_agent
...
async def meho_query_topology(
    ...
    await _audit_log(
        user_context, "meho_query_topology",
