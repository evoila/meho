...
from meho_knowledge.schemas import KnowledgeChunkCreate, ChunkMetadata, KnowledgeType
...
async def ingest_openapi_to_knowledge(
    ...
    try:
        ...
        logger.warning(f"Invalid OpenAPI spec for connector {connector_name}: {e}, skipping ingestion")
        return 0
    ...
    for endpoint in endpoints:
        try:
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=tags,
                knowledge_type=KnowledgeType.DOCUMENTATION,  # API docs are permanent
...
def _get_common_abbreviations(resource: str, summary: str, tags: List[str]) -> set:
    ...
    all_text = summary_lower + ' '.join(tags_lower)
...
def _summarize_schema(schema: Dict[str, Any], max_depth: int = 3, current_depth: int = 0) -> str:
    """
...
def _create_search_metadata(endpoint: Dict[str, Any], connector_id: str) -> ChunkMetadata:
    ...
    metadata = ChunkMetadata(
        ...
        # Keywords for BM25
        keywords=endpoint.get('tags', []) + [resource_type] if resource_type else endpoint.get('tags', []),
        
        # Custom fields (allowed via extra="allow")
        **{
            "source_type": "openapi_spec",
            "connector_id": connector_id,
            "endpoint_id": endpoint.get('operation_id', f"{endpoint['method']}_{path.replace('/', '_')}"),
            "operation_id": endpoint.get('operation_id'),
...
async def remove_connector_knowledge(
    ...
    count_stmt = select(func.count()).select_from(KnowledgeChunkModel).where(
    ...
    logger.info(
        f"Deleted {deleted_count} knowledge chunks for connector {connector_id}"
    )
