...
def test_query_topology_input_schema_includes_edges_facet() -> None:
    ...
    G9.3-T4 (#860) added ``diff``; G9.3-T3 (#859) added ``history``.
    ``edges`` and ``timeline`` have no conditional required clause --
    every filter on those facets is optional. ``diff`` requires both
    timestamps (``ts1``, ``ts2``); ``history`` requires ``target``
    ...
    # smuggle an over-cap value past the schema and trip the substrate's
    # ``ValueError``); those clauses don't carry a ``required`` key, so
    # the ``required``-only dict below skips them via ``.get`` rather
