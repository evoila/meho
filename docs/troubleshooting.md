...
!!! danger "Symptom"
    The backend exits during startup with a `SystemExit` from `_ensure_schema_ready`, or API calls fail with "relation does not exist" / "column not found".

**Cause:** The unified Alembic tree at `meho_app/alembic/` is not at head. The lifespan schema-readiness gate (#313) refuses to serve traffic against a stale schema -- this is intentional, it replaces the silent-failure migrations of the pre-#294 layout.

**Fix:**

1. Run migrations explicitly. Both compose and `meho-dev up` invoke this automatically; manual invocation is one line:
    ```bash
    docker compose exec meho uv run alembic -c meho_app/alembic.ini upgrade head
    ```
2. Inspect the current revision and pending heads:
    ```bash
    docker compose exec meho uv run alembic -c meho_app/alembic.ini current
    docker compose exec meho uv run alembic -c meho_app/alembic.ini heads
    ```
3. If you see leftover legacy tables (`alembic_version_meho_knowledge`, `alembic_version_meho_topology`, etc.), the database is on the pre-#294 layout. Run the rescue script before retrying:
    ```bash
    DATABASE_URL=postgresql://meho:password@localhost:5432/meho \
      uv run python scripts/migrate_to_unified_alembic.py
    ```
    See [docs/deployment.md#upgrading-existing-deployments-rescue-script](deployment.md#upgrading-existing-deployments-rescue-script) for the full procedure.
