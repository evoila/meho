...
class EventService:
    ...
    async def update_event_registration(self, registration_id: str, **kwargs: object) -> EventRegistrationModel:
        """
    ...
    async def is_duplicate(
        ...
        was_set = await redis_client.set(dedup_key, "1", nx=True, ex=300)  # type: ignore[union-attr]
        ...
        await redis_client.incr(suppressed_key)  # type: ignore[union-attr]
        # Ensure TTL is set (idempotent if already set)
        await redis_client.expire(suppressed_key, 600)  # type: ignore[union-attr]
    ...
    async def is_rate_limited(
        ...
        count = await redis_client.incr(rate_key)  # type: ignore[union-attr]

        if count == 1:
            # First event this hour -- set TTL
            await redis_client.expire(rate_key, 3600)  # type: ignore[union-attr]
