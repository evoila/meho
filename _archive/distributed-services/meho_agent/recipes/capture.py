...
from meho_agent.data_reduction.query_schema import DataQuery, FilterCondition, FilterGroup
...
class RecipeCaptureService:
    ...
    def __init__(self) -> None:
        ...
        self._parameter_agent: Optional[Agent] = None
    ...
    def _extract_parameters(
        ...
        if query.filter:
            params = self._extract_filter_parameters(
                query.filter, question, seen_names
            )
        ...
        # Extract from limit
        if query.limit and query.limit not in [10, 20, 50, 100]:
    ...
    def _extract_filter_parameters(
        ...
        for condition in filter_group.conditions:
            if isinstance(condition, FilterGroup):
                ...
                params = self._extract_filter_parameters(
                    condition, question, seen_names
                )
                parameters.extend(params)
            elif isinstance(condition, FilterCondition):
                param = self._condition_to_parameter(
                    condition, question, seen_names
                )
                if param:
    ...
    def _create_template(
        ...
        param_by_field = {
            p.source_field: p for p in parameters if p.source_field
        }
        
        # Create filter template with placeholders
        filter_template = None
        if query.filter:
            filter_template = self._create_filter_template(
                query.filter, param_by_field
            )
    ...
    def _create_filter_template(
        ...
        for condition in filter_group.conditions:
            if isinstance(condition, FilterGroup):
                # Nested group
                conditions.append(self._create_filter_template(
                    condition, param_by_field
                ))
            elif isinstance(condition, FilterCondition):
                field = condition.field
    ...
    def _extract_tags(
        ...
        if query.filter:
            has_threshold = any(
                isinstance(c, FilterCondition) and 
                str(c.operator) in [">", "<", ">=", "<="]
