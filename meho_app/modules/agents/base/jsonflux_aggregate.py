...
def _infer_table_name(data: list | dict, fallback: str = "data") -> str:
    ...
    if isinstance(data, dict):
        ...
        if "items" in data and isinstance(data["items"], list):
            ...
            if "kind" in data and isinstance(data["kind"], str):
                kind = data["kind"].lower().replace("list", "").strip()
                if kind and not kind.endswith("s"):
                    kind += "s"
                if kind:
                    return kind
