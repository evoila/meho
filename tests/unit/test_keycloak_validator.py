...
class TestKeycloakJWTValidator:
    ...
    async def test_validate_token_missing_kid(self, sample_jwks):
        ...
        # Create token without kid header. The HS256 secret below is deliberate
        # test input used to build a malformed token we expect the validator to
        # reject — not a credential.
    ...
    async def test_validate_token_missing_issuer(self):
        ...
        # Create token without issuer. The HS256 secret below is deliberate
        # test input used to build a malformed token we expect the validator to
        # reject — not a credential.
        # nosemgrep: python.jwt.security.jwt-hardcode.jwt-python-hardcoded-secret
        token = jwt.encode(
