...
async def _resolve_credentials(
    ...
    try:
        session_type_enum = SessionType(session_type_str) if session_type_str != "interactive" else SessionType.INTERACTIVE
        ...
        async with session_maker() as session:
            ...
            try:
                ...
                health_stmt = select(UserCredentialModel).where(
                    UserCredentialModel.user_id == (resolved.delegated_by_user_id or meho.user_context.user_id),
                ...
                if cred_obj and cred_obj.credential_health != "healthy":
                    cred_obj.credential_health = "healthy"
                    cred_obj.credential_health_message = None
                    cred_obj.credential_health_checked_at = datetime.now(UTC)
        ...
        return {}, json.dumps({"error": f"Connector {connector_id} not in allowed scope: {e.allowed}"})
        ...
        try:
            ...
            async with session_maker() as health_session:
                ...
                for cred_obj in cred_result.scalars():
                    cred_obj.credential_health = "unhealthy"
                    cred_obj.credential_health_message = str(e)
                    cred_obj.credential_health_checked_at = datetime.now(UTC)
...
async def search_operations_handler(deps: MEHOGraphDeps, args: dict[str, Any]) -> str:
    ...
    async with traced_tool_call(
        ...
        if not connector_id:
            error_result = json.dumps({"error": "connector_id is required"})
            span.set_error("connector_id is required")
            return error_result

        if not deps.meho_deps:
            error_result = json.dumps({"error": "MEHODependencies not available"})
            span.set_error("MEHODependencies not available")
            return error_result
        ...
        try:
            ...
            async with session_maker() as session:
                ...
                if not connector:
                    error_result = json.dumps({"error": "Connector not found"})
                    span.set_error("Connector not found")
                    return error_result
...
async def _trigger_auto_discovery(
    ...
    try:
        ...
        service = await get_auto_discovery_service()
