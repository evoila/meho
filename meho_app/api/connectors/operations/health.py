...
async def _probe_connector(connector: Any) -> dict:  # NOSONAR (cognitive complexity)
    """
    ...
    try:
        ...
        logger.warning(f"Connector {connector.name} ({connector_type}) unreachable: {error_msg}")
        return {
            "connector_id": connector_id,
            "name": connector.name,
            "connector_type": connector_type,
