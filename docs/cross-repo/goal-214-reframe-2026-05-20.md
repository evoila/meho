...
- Real JSONFlux reduction logic (set-shaped response handles +
  MinIO/S3 spill). G0.6 ships the `Reducer` ABC hook only; the
  pass-through default was the state at this reframe (2026-05-20). (The
  real `JsonFluxReducer` later landed as the default in G0.6.1 #750 —
  see [`jsonflux.md`](../architecture/jsonflux.md).)
...
[locked-decisions.md](docs/decisions/locked-decisions.md) —
