...
async def sync_operations_for_imported_connector(  # NOSONAR (cognitive complexity)
    ...
    except Exception as e:  # noqa: BLE001 -- sync failure must not abort connector import
    ...
    if connector_type not in TYPE_SKILL_MAP:
        ...
        except Exception as e:  # noqa: BLE001 -- skill generation is best-effort, connector runs without it
            logger.warning(f"Failed to generate skill for {connector_name}: {e}")

    return result


def _get_knowledge_store(session: AsyncSession) -> "KnowledgeStore | None":
...
async def _try_add_knowledge_chunk(
    store: "KnowledgeStore",
    create: Any,
    label: str,
) -> bool:
    """Add a knowledge chunk, suppressing failures since chunk creation is best-effort."""
    try:
        await store.add_chunk(create)
        return True
    except Exception as e:  # noqa: BLE001 -- knowledge chunk creation is best-effort
        logger.warning(f"Failed to create knowledge chunk for {label}: {e}")
        return False
...
async def _sync_prometheus_operations(
    ...
    if knowledge_store:
        ...
        if await _try_add_knowledge_chunk(knowledge_store, chunk_create, connector_name):
...
async def _sync_github_operations(
    ...
    if knowledge_store:
        ...
        for op in GITHUB_OPERATIONS:
            ...
            if await _try_add_knowledge_chunk(knowledge_store, chunk_create, op.operation_id):
                result.knowledge_chunks_created += 1
...
async def _sync_rest_operations(
    ...
    except Exception as e:  # noqa: BLE001 -- spec parse failure produces a warning, not an abort
...
async def _sync_soap_operations(
    ...
    except Exception as e:  # noqa: BLE001 -- WSDL fetch failure produces a warning, not an abort
