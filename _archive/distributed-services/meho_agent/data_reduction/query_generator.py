...
class QueryGeneratorOutput(BaseModel):
    ...
    query: DataQuery = Field(
        description="The generated data query"
    )
    reasoning: str = Field(
    ...
    confidence: float = Field(
        ge=0.0,
        le=1.0,
        description="Confidence in the query (0-1)"
    )
...
_query_generator_agent: Optional[Agent[QueryGeneratorContext, QueryGeneratorOutput]] = None


def get_query_generator_agent() -> Agent[QueryGeneratorContext, QueryGeneratorOutput]:
    """
    ...
    if _query_generator_agent is None:
        ...
        # Register tools
        @agent.tool
        async def get_available_fields(ctx: RunContext[QueryGeneratorContext]) -> str:
        ...
        @agent.tool
        async def get_sample_values(ctx: RunContext[QueryGeneratorContext]) -> str:
