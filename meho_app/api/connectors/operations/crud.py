...
async def _auto_ingest_soap_wsdl(
    ...
    operations, _metadata, type_definitions = await ingester.ingest_wsdl(
        wsdl_url=wsdl_url,
...
@router.get("/")
async def list_connectors(user: UserContext = Depends(get_current_user)) -> list[dict]:
    """
...
async def get_connector(connector_id: str, user: UserContext = Depends(get_current_user)):
    """
...
async def update_connector(
    ...
    # nosemgrep: tainted-sql-string -- this is a log message f-string, not SQL (false positive from string interpolation detection)
...
async def delete_connector(
    connector_id: str, user: UserContext = Depends(RequirePermission(Permission.CONNECTOR_DELETE))
):
    """
