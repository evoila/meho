...
MSG_NO_MEHO_DEPS = "Error: No MEHO dependencies available"
...
class LookupTopologyNode(BaseNode[MEHOGraphState, MEHOGraphDeps, None]):
    ...
    async def _execute_lookup(self, deps: MEHOGraphDeps) -> str:  # NOSONAR (cognitive complexity)
        """Execute the topology lookup."""
        try:
            # Get session from meho_deps
            if not deps.meho_deps:
                return MSG_NO_MEHO_DEPS
            ...
            for e in self.entities:
                entity_creates.append(
                    TopologyEntityCreate(
                        ...
                        connector_name=e.get("connector_name"),
            ...
            for s in self.same_as:
                ...
                same_as_creates.append(
                    TopologySameAsCreate(
                        ...
                        entity_a_connector_id=s.get("entity_a_connector_id"),
                        entity_b_connector_id=s.get("entity_b_connector_id"),
                        entity_a_type=s.get("entity_a_type"),
                        entity_b_type=s.get("entity_b_type"),
                    )
                )

            # Infer connector_type from first entity, default to "rest"
            inferred_connector_type = "rest"
            inferred_connector_id = None
            if self.entities:
                inferred_connector_type = self.entities[0].get("connector_type", "rest")
                cid = self.entities[0].get("connector_id")
                if cid:
                    from uuid import UUID as _UUID

                    with contextlib.suppress(ValueError, AttributeError):
                        inferred_connector_id = _UUID(str(cid))

            result = await topology_service.store_discovery(
                input=StoreDiscoveryInput(
                    connector_type=inferred_connector_type,
                    connector_id=inferred_connector_id,
