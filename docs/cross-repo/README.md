...
| Doc | Purpose |
| --- | --- |
| [`argocd-onboarding.md`](./argocd-onboarding.md) | How to use the G3.12 `argocd-api-3.x` op surface — registering an `argocd` target (host, port, the Vault bearer-token `secret_ref`, `preferred_impl_id=argocd-api`), the `GET /api/version` probe, the six curated read ops (`argocd.app.list/get/diff/resource_tree`, `argocd.appproject.list`, `argocd.repo.list`) via the generic `meho operation call argocd-api-3.x …` verb + the agent meta-tool path, and the explicit deferred-write note (`sync`/`rollback`/`set` are approval-gated follow-ups). Companion engineering doc: [`docs/codebase/connectors-argocd.md`](../codebase/connectors-argocd.md). |
...
| [`secret-broker.md`](./secret-broker.md) | How to use the G0.22 secret broker — `meho secret move --from <kind>:<ref> --to <kind>:<ref> --reason …` to copy one credential field between stores server-side. Covers the `<kind>:<ref>` intent schema (the shipped `vault` source+sink and `keycloak` sink kinds), the value-free `{status, value_sha256, length}` response, the "agent never observes the value" guarantee tied to each enforcing mechanism (no value-bearing flag/param/response/log/audit/`proposed_effect`/broadcast), and the enlarged threat model — the backplane as credential-bearing intermediary, mitigated by operator-context reads, the `dangerous` deny-by-default/needs-approval-ceiling lattice, mandatory four-eyes approval, time-boxed grants, and hash-only audit — plus the explicitly deferred token-minting / diff-shaped-approval follow-ups. Companion engineering doc: [`docs/codebase/connectors-secret-broker.md`](../codebase/connectors-secret-broker.md). |
