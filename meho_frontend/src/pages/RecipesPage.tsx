...
import type { Recipe, RecipeParameter } from '@/api/types';
...
export function RecipesPage() {
  ...
  const { data: recipes = [], isLoading } = useQuery({
    ...
    queryFn: () => recipesClient.listRecipes({
  ...
  const executeRecipeMutation = useMutation({
    mutationFn: ({ id, params }: { id: string; params: Record<string, unknown> }) =>
      recipesClient.executeRecipe(id, params),
    onSuccess: () => {
  ...
  const deleteRecipeMutation = useMutation({
    mutationFn: (id: string) => recipesClient.deleteRecipe(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['recipes'] });
    },
    onError: (error) => {
  ...
  const updateRecipeMutation = useMutation({
    mutationFn: ({ id, updates }: { id: string; updates: { name: string; description: string; tags: string[]; parameters: RecipeParameter[] } }) =>
      recipesClient.updateRecipe(id, updates),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['recipes'] });
