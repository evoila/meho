...
class PostgresFTSHybridService:
    ...
    async def search(
        ...
        try:
            ...
            tenant_hash = hashlib.md5(user_context.tenant_id.encode()).hexdigest()  # noqa: S324 -- non-security hash context
    ...
    async def search_with_rerank(
        ...
        """
        Hybrid search with post-retrieval Voyage AI reranking.

        Retrieves a wider set of candidates via hybrid search, then uses
        ...
        """
        # Step 1: Get wide candidate set from hybrid search
        ...
        # Reranking is a separate API call (no DB session needed), so no
        # async session concurrency issue per research pitfall 3
        document_texts = [c["text"] for c in candidates]
        ...
        # Step 4: Reorder candidates by rerank result index
        reranked = []
    ...
    def _analyze_query(self, query: str) -> tuple[float, float]:
        ...
        technical_score = sum(1 for pattern in technical_indicators if re.search(pattern, query))
        ...
        nlp_score = sum(1 for indicator in nlp_indicators if indicator in query_lower)
