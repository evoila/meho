...
async def get_repository(session: AsyncSession = Depends(get_db_session)) -> KnowledgeRepository:
...
async def get_job_repository(session: AsyncSession = Depends(get_db_session)):
