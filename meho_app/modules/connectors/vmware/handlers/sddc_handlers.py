...
class SddcHandlerMixin:
    ...
    async def _ensure_sddc_token(self) -> None:
        ...
        try:
            ...
            # Safe (python-logger-credential-disclosure): logs exception message, not the token itself
            logger.error(f"Failed to acquire SDDC Manager token: {e}")
    ...
    async def _get_sddc_system_info(self, _params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to get SDDC system info: {e}")
    ...
    async def _list_sddc_workload_domains(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return [
                {
                    ...
                    "type": d.get("type"),
                    "status": d.get("status"),
                    "vcenters": [vc.get("fqdn") for vc in d.get("vcenters", [])],
                    "clusters": [
                        {
                            "id": c.get("id"),
                            "name": c.get("name"),
            ...
            logger.error(f"Failed to list SDDC workload domains: {e}")
    ...
    async def _get_sddc_workload_domain(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to get SDDC workload domain %s: {domain_id}/{e}")
            raise RuntimeError(f"SDDC get_workload_domain failed for {domain_id}: {e}") from e

    # ------------------------------------------------------------------
    # 4. Hosts
    # ------------------------------------------------------------------

    async def _list_sddc_hosts(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return [
                {
                    ...
                    "fqdn": h.get("fqdn"),
                    "status": h.get("status"),
                    "domain": h.get("domain", {}).get("name")
                    if isinstance(h.get("domain"), dict)
                    else h.get("domain"),
                    "cluster": h.get("cluster", {}).get("name")
                    if isinstance(h.get("cluster"), dict)
                    else h.get("cluster"),
                    "hardware": {
            ...
            logger.error(f"Failed to list SDDC hosts: {e}")
    ...
    async def _list_sddc_clusters(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to list SDDC clusters: {e}")
    ...
    async def _get_sddc_update_history(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to get SDDC update history: {e}")
    ...
    async def _get_sddc_certificates(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to get SDDC certificates: {e}")
    ...
    async def _get_sddc_prechecks(self, _params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to get SDDC prechecks: {e}")
