...
* **Column nullability.** The NOT NULL columns (``tenant_id`` /
  ``agent_definition_id`` / ``kind`` / ``timezone`` / ``identity_sub`` /
  ``status`` / ``in_flight_policy`` / ``created_by_sub`` /
  ``created_at`` / ``updated_at``) reject NULL; the type-specific
  columns (``cron_expr`` / ``event_filter``), the payload
  (``inputs``), and the dispatcher-managed timestamps
...
def _index_columns(sync_url: str, index_name: str) -> tuple[str, ...]:
    """Return the ordered tuple of column names indexed by *index_name*.

    CodeRabbit iter-1 nitpick: name-only assertions don't catch
    accidental key-membership / column-order drift. ``PRAGMA
    index_info(<idx>)`` returns one row per indexed column with
    ``(seq, cid, name)``; the column tuple captured here pins the
    schema contract.
    """
    sync_eng = sa_create_engine(sync_url)
    try:
        ...
        sync_eng.dispose()
    return tuple(str(row[2]) for row in sorted(rows, key=lambda r: int(r[0])))
...
#: CodeRabbit iter-1 nitpick: pin the indexed-column ordering for each
#: declared index so a regression that flips key membership or order
#: (which ``PRAGMA index_list`` would still report as "the index
#: exists") fails this test rather than slipping through.
_EXPECTED_INDEX_COLUMNS: dict[str, tuple[str, ...]] = {
    "scheduled_trigger_next_fire_at_idx": ("status", "next_fire_at"),
    "scheduled_trigger_tenant_idx": ("tenant_id", "kind"),
}
...
def test_upgrade_creates_scheduled_trigger_table_columns_indexes(
    ...
    # Pin index column membership + order so a regression that flips
    # ``(status, next_fire_at)`` to ``(next_fire_at, status)`` -- or
    # ``(tenant_id, kind)`` to ``(kind, tenant_id)`` -- breaks the
    # planner-affecting contract this test exists to guard.
    for index_name, expected_cols in _EXPECTED_INDEX_COLUMNS.items():
        actual_cols = _index_columns(sync_url, index_name)
        assert actual_cols == expected_cols, (
            f"index {index_name!r} column shape drifted: "
