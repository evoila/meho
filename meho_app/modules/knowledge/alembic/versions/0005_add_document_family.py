...
"""Add doc_version + knowledge_document_family table + family_id + uniqueness indexes.

This migration bundles the document-versioning schema so the two sides
(doc_version columns + family table with the uniqueness indexes that depend
on them) land atomically. A partial deploy would otherwise leave the DB in
an inconsistent state — doc_version columns present but no uniqueness
...
* ``knowledge_document_family`` table groups ingestion jobs that represent
  different versions of the same logical document.
* ``knowledge_chunk.family_id`` and ``ingestion_jobs.family_id``
  foreign keys + partial indexes.
...
def upgrade() -> None:
    ...
    # Unique (tenant, scope, connector, name). NULLS NOT DISTINCT so we can
    # treat NULL connector_id / connector_type_scope as equal for global/type
    # scopes. Requires PostgreSQL 15+; docker-compose.yml pins pgvector/pgvector:pg15.
    op.execute(
        """
    ...
    # --- family_id foreign key on ingestion_jobs ----------------------------
    op.add_column(
        "ingestion_jobs",
        sa.Column(
            "family_id",
    ...
    # Unique (family_id, doc_version) ignoring deleted jobs so a deleted
    # version string can later be reused.
    op.execute(
        """
    ...
    # Unique (family_id, file_hash) ignoring deleted jobs so the same bytes
    # can later be re-uploaded after a delete.
    op.execute(
        """
    ...
    # --- family_id foreign key on knowledge_chunk ---------------------------
    op.add_column(
        "knowledge_chunk",
        sa.Column(
            "family_id",
