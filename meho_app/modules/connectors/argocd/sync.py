...
"""
ArgoCD Operations Auto-Sync.

Automatically syncs ArgoCD operations to the database when the operations
...
This enables semantic search (BM25 + embeddings) across all ArgoCD operations.

Usage:
    On API startup, call sync_all_argocd_connectors() to update any
...
def _format_argocd_operation_as_text(
    op: OperationDefinition, connector_name: str = "ArgoCD"
) -> str:
    """
    Format ArgoCD operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.

    Args:
        op: ArgoCD operation definition
    ...
    keywords = _generate_argocd_search_keywords(op)
    if keywords:
...
def _generate_argocd_search_keywords(op: OperationDefinition) -> str:
    """
    Generate search keywords for better BM25 matching.

    Adds abbreviations, synonyms, and variations that users might search for.
    ArgoCD-specific vocabulary: application/app, sync/deploy, health/status,
    resource/tree, rollback/revert, diff/drift, event/warning, etc.
    ...
    # ArgoCD-specific abbreviations and synonyms
    argocd_terms = {
        "application": [
            "application",
            "applications",
    ...
    for key, terms in argocd_terms.items():
...
async def sync_argocd_operations_if_needed(
    session: AsyncSession,
    connector_id: str,
    tenant_id: str,
    current_version: str | None = None,
    knowledge_store: Optional["KnowledgeStore"] = None,
    connector_name: str = "ArgoCD",
) -> tuple[int, int, int]:
    """
    Sync ArgoCD operations if the version is outdated.
    ...
    if current_version == ARGOCD_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing ArgoCD operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {ARGOCD_OPERATIONS_VERSION}"
    )
    ...
    for op in ARGOCD_OPERATIONS:
        ...
        # Determine safety level from WRITE and DESTRUCTIVE sets
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(ARGOCD_OPERATIONS)} total, "
...
async def _sync_argocd_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for ArgoCD operations.

    Enables hybrid search (BM25 + semantic) for ArgoCD operations.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "argocd",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing ArgoCD knowledge chunks for connector {connector_id}")
    ...
    for op in ARGOCD_OPERATIONS:
        try:
            ...
            text = _format_argocd_operation_as_text(op, connector_name)
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=["api", "operation", "argocd", op.category],
    ...
    logger.info(f"Created {chunks_created} knowledge chunks for ArgoCD connector {connector_id}")
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = ARGOCD_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await session.flush()
        logger.debug(f"Updated connector {connector_id} to version {ARGOCD_OPERATIONS_VERSION}")


async def sync_all_argocd_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all ArgoCD connectors to the latest operations version.
    ...
    # Find all ArgoCD connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "argocd")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No ArgoCD connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} ArgoCD connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        # Sync operations
        added, updated, chunks = await sync_argocd_operations_if_needed(
            session=session,
            connector_id=str(connector.id),
            tenant_id=str(connector.tenant_id),
            current_version=current_version,
            knowledge_store=knowledge_store,
            connector_name=str(connector.name) if connector.name else "ArgoCD",
        )
    ...
    summary = {
        ...
        "current_version": ARGOCD_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"ArgoCD operation sync complete: "
        ...
        logger.info("All ArgoCD connectors already at latest version")
