...
- `topology.history.prune` — the G9.3-T6 (#858) topology-history retention prune. One row per non-no-op tick (default weekly cadence), `operator_sub='system:topology-history-retention'`, `tenant_id` is the per-deploy sentinel `00000000-0000-0000-0000-0000000858a1` (system-wide op, not attributable to a real tenant — exploits the `audit_log.tenant_id` soft-FK so the sentinel writes without a matching `tenant` row). Payload is `{"dropped_node_rows": N, "dropped_edge_rows": M, "retention_days": D, "cutoff": <iso-ts>}`. `TOPOLOGY_HISTORY_RETENTION_DAYS=0` is the opt-out "keep forever" sentinel: the prune loop still runs as a heartbeat but every tick is a no-op and **no audit row is written** for the no-op case (weekly empty-payload rows would flood `audit_log`). Forward reference for [G8.2 #219](https://github.com/evoila/meho/issues/219): `meho audit query --op topology.history.prune` will pick these rows up by path when the verb ships.
- `flight_recorder.trace.prune` — the #3212 dispatch flight-recorder retention reaper (F4 of [`docs/decisions/dispatch-flight-recorder.md`](../decisions/dispatch-flight-recorder.md)). One row per **non-empty** sweep (default hourly cadence), `operator_sub='system:flight-recorder-retention'`, `tenant_id` is the per-deploy sentinel `00000000-0000-0000-0000-000000003212` (system-wide op — same soft-FK exploit as the topology sentinel). Payload is `{"dropped_trace_headers": N, "dropped_trace_spans": M, "cutoff": <iso-ts>}`. An empty sweep writes **no** audit row (hourly `dropped=0` rows would flood `audit_log`). The reaper deletes only `dispatch_trace` headers + `dispatch_trace_span` children whose stamped `expires_at` has passed; the referenced dispatch `audit_log` row — the record of account — is **never** touched. Forward reference for [G8.2 #219](https://github.com/evoila/meho/issues/219): `meho audit query --op flight_recorder.trace.prune` will pick these rows up by path when the verb ships.
- `audit.raw_payload.prune` — the security-review S19 ([#307](https://github.com/evoila-bosnia/meho-internal/issues/307)) raw-payload age-off sweeper ([`audit_retention.py`](../../backend/src/meho_backplane/audit_retention.py)). One row per **non-no-op** sweep (default weekly cadence), `operator_sub='system:audit-raw-payload-retention'`, `tenant_id` is the per-deploy sentinel `00000000-0000-0000-0000-000000000307` (system-wide op — same soft-FK exploit as the topology / flight-recorder sentinels). Payload is `{"nulled_raw_payload_rows": N, "retention_days": D, "cutoff": <iso-ts>}`. The sweeper runs a bounded `UPDATE audit_log SET raw_payload = NULL WHERE occurred_at < cutoff AND raw_payload IS NOT NULL`: it NULLs **only** the pre-redaction `raw_payload` body (G11.4-T2 #1071) so a rotated secret echoed in a connector response does not outlive its rotation forever; the redacted `payload` (carrying `redaction_policy_id`), the `redaction_manifest`, and the `audit_log` row itself — the record of account — are **never** removed (the row is updated in place, not deleted). `RAW_PAYLOAD_RETENTION_DAYS=0` is the opt-out "keep forever" sentinel (the loop runs as a heartbeat but every tick is a no-op) — un-redacted bodies then live in `raw_payload` and every backup indefinitely, the documented security tradeoff. A **no-op** sweep (the `0` sentinel, or a run that nulled zero rows) writes **no** audit row — only a sweep that actually aged off ≥1 row records one (mirroring the flight-recorder reaper's "empty sweep writes no row"). Forward reference for [G8.2 #219](https://github.com/evoila/meho/issues/219): `meho audit query --op audit.raw_payload.prune` will pick these rows up by path when the verb ships.
