...
async def _get_events_today_counts(session, registration_ids: list[str]) -> dict[str, int]:
    ...
    stmt = (
        ...
        .where(
            EventHistoryModel.event_registration_id.in_([uuid.UUID(rid) for rid in registration_ids]),
...
@router.post("/{connector_id}/events", response_model=EventCreateResponse, status_code=201)
...
@router.get("/{connector_id}/events", response_model=list[EventResponse])
...
@router.get("/{connector_id}/events/{event_id}", response_model=EventResponse)
...
@router.patch("/{connector_id}/events/{event_id}", response_model=EventResponse)
async def update_event_registration(
    ...
    try:
        async with session_maker() as session:
            ...
            if update_fields:
                event_service = EventService(session)
                registration = await event_service.update_event_registration(event_id, **update_fields)
            else:
                registration = await _verify_event_registration(session, event_id, connector_id)
...
@router.delete("/{connector_id}/events/{event_id}")
...
@router.get(
    "/{connector_id}/events/{event_id}/history", response_model=EventHistoryListResponse
)
    ...
    try:
        async with session_maker() as session:
            ...
            if user_instructions:
                ...
                user_message = (
                    f"Generate an event prompt for a {connector_type} connector named '{connector.name}'"
                )
...
@router.post("/{connector_id}/events/{event_id}/test", response_model=EventTestResponse)
async def test_event_pipeline(
    connector_id: str,
    event_id: str,
    request: EventTestRequest,
    background_tasks: BackgroundTasks,
    user: UserContext = Depends(RequirePermission(Permission.CONNECTOR_UPDATE)),  # noqa: PT028 -- intentional default value
):
    """
