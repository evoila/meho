...
from .queue import DiscoveryMessage, DiscoveryQueue
...
class BatchProcessor:
        ...
        async with self.session_maker() as session:
            ...
            for entity in message.entities:
                entity_creates.append(
                    TopologyEntityCreate(
                        name=entity.name,
                        entity_type=entity.entity_type,  # REQUIRED field - was missing!
                        description=entity.description,
                        connector_id=entity.connector_id if entity.connector_id else None,
                        connector_name=entity.connector_name,
                        scope=entity.scope,  # Scoping context (namespace, cluster, etc.)
            ...
            input_data = StoreDiscoveryInput(
                connector_type=message.connector_type,  # Required field
                entities=entity_creates,
                relationships=relationship_creates,
                same_as=[],  # Auto-discovery doesn't create SAME_AS directly
            )
    ...
    async def _correlate_k8s_services(
        self,
        session,
    ...
    async def _run_deterministic_resolution(
        self,
        session,
        message: DiscoveryMessage,
        tenant_id: str,
