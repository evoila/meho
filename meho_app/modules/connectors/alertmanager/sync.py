...
"""
Alertmanager Operations Auto-Sync

Automatically syncs Alertmanager operations to the database when the operations
...
This enables semantic search (BM25 + embeddings) across all Alertmanager operations.

Usage:
    On API startup, call sync_all_alertmanager_connectors() to update any
...
def _format_alertmanager_operation_as_text(
    op: OperationDefinition, connector_name: str = "Alertmanager"
) -> str:
    """
    Format Alertmanager operation as rich searchable text.

    Creates text optimized for BM25 + semantic search.

    Args:
        op: Alertmanager operation definition
    ...
    keywords = _generate_alertmanager_search_keywords(op)
    if keywords:
...
def _generate_alertmanager_search_keywords(op: OperationDefinition) -> str:
    """
    ...
    # Alertmanager-specific abbreviations and synonyms
    alertmanager_terms = {
        "alert": [
            "alert",
            "alerts",
    ...
    for key, terms in alertmanager_terms.items():
    ...
    if "get" in combined_text and "list" not in combined_text:
        keywords.update(["get", "show", "check", "inspect", "detail"])
    if "create" in combined_text:
        keywords.update(["create", "add", "new", "set"])
    if "expire" in combined_text:
        keywords.update(["expire", "remove", "delete", "cancel"])

    return " ".join(sorted(keywords))


async def sync_alertmanager_operations_if_needed(
    session: AsyncSession,
    connector_id: str,
    tenant_id: str,
    current_version: str | None,
    knowledge_store: Optional["KnowledgeStore"] = None,
    connector_name: str = "Alertmanager",
) -> tuple[int, int, int]:
    """
    Sync Alertmanager operations if the version is outdated.
    ...
    if current_version == ALERTMANAGER_OPERATIONS_VERSION:
        logger.debug(f"Connector {connector_id} already at version {current_version}")
        return (0, 0, 0)

    logger.info(
        f"Syncing Alertmanager operations for connector {connector_id}: "
        f"{current_version or 'none'} -> {ALERTMANAGER_OPERATIONS_VERSION}"
    )
    ...
    logger.info(
        ...
        f"{added} added, {updated} updated, {len(ALERTMANAGER_OPERATIONS)} total, "
...
async def _sync_alertmanager_knowledge_chunks(
    ...
    """
    Create or update knowledge_chunk entries for Alertmanager operations.

    Enables hybrid search (BM25 + semantic) for Alertmanager operations.
    ...
    try:
        stmt = delete(KnowledgeChunkModel).where(
            ...
            KnowledgeChunkModel.search_metadata["connector_type"].astext == "alertmanager",
        )
        await knowledge_store.repository.session.execute(stmt)
        logger.debug(f"Deleted existing Alertmanager knowledge chunks for connector {connector_id}")
    ...
    for op in ALERTMANAGER_OPERATIONS:
        try:
            ...
            text = _format_alertmanager_operation_as_text(op, connector_name)
            ...
            chunk_create = KnowledgeChunkCreate(
                ...
                tags=["api", "operation", "alertmanager", op.category],
    ...
    logger.info(
        f"Created {chunks_created} knowledge chunks for Alertmanager connector {connector_id}"
    )
...
async def update_connector_operations_version(
    ...
    if connector:
        ...
        config["operations_version"] = ALERTMANAGER_OPERATIONS_VERSION
        connector.protocol_config = config  # type: ignore[assignment]  # SQLAlchemy ORM attribute assignment
        await session.flush()
        logger.debug(
            f"Updated connector {connector_id} to version {ALERTMANAGER_OPERATIONS_VERSION}"
        )


async def sync_all_alertmanager_connectors(
    session: AsyncSession,
    knowledge_store: Optional["KnowledgeStore"] = None,
) -> dict:
    """
    Sync all Alertmanager connectors to the latest operations version.
    ...
    # Find all Alertmanager connectors
    query = select(ConnectorModel).where(ConnectorModel.connector_type == "alertmanager")
    result = await session.execute(query)
    connectors = result.scalars().all()

    if not connectors:
        logger.info("No Alertmanager connectors found to sync")
        return {"connectors_checked": 0, "connectors_synced": 0}

    logger.info(f"Checking {len(connectors)} Alertmanager connector(s) for operation sync")
    ...
    for connector in connectors:
        ...
        # Sync operations
        added, updated, chunks = await sync_alertmanager_operations_if_needed(
            session=session,
            connector_id=str(connector.id),
            tenant_id=str(connector.tenant_id),
            current_version=current_version,
            knowledge_store=knowledge_store,
            connector_name=str(connector.name) if connector.name else "Alertmanager",
        )
    ...
    summary = {
        ...
        "current_version": ALERTMANAGER_OPERATIONS_VERSION,
    }

    if connectors_synced > 0:
        logger.info(
            f"Alertmanager operation sync complete: "
        ...
        logger.info("All Alertmanager connectors already at latest version")
