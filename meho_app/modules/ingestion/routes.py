...
async def handle_generic_webhook(
    ...
    x_system_id: str | None = Header(None, alias="X-System-ID"),
) -> WebhookResponse:
    """
    ...
    **Note:**
    ...
    logger.info(
        "Received webhook: %s/%s (tenant: %s, event_id: %s, hmac: %s)",
        connector_id,
        event_type,
        tenant_id,
        event_id,
        "verified" if webhook_secret else "none",
    )
...
async def get_template(
    template_id: str, repo: EventTemplateRepository = Depends(get_template_repository)
) -> EventTemplate:
    """
