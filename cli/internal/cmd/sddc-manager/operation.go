...
// newOperationCmd returns `meho sddc-manager operation` with search / call
// meta-tool wrappers pre-scoped to sddc-rest-9.0.
func newOperationCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:          "operation",
		Short:        "Pre-scoped meta-tool wrappers (search / call) for sddc-rest-9.0",
		SilenceUsage: true,
	}
...
func newOperationSearchCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "search <query>",
		Short: "Hybrid BM25 + cosine RRF search across sddc-rest-9.0 operations",
		Long: "search wraps GET /api/v1/operations/search with connector_id=\n" +
			"\"sddc-rest-9.0\" pre-baked.\n\n" +
			"Exit codes mirror meho operation search.",
		Example: "  meho sddc-manager operation search \"list domains\"\n" +
...
func newOperationCallCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "call <op_id>",
		Short: "Dispatch any sddc-rest-9.0 op_id (escape hatch for ops without aliases)",
