...
def test_documentation_gets_boost(knowledge_store):
    ...
    assert results[0]["final_score"] == 0.8 * 1.2  # Documentation boost
...
def test_recent_event_gets_boost(knowledge_store):
    ...
    assert results[0]["final_score"] == 0.8 * 1.5  # Recent event boost
...
def test_old_event_gets_downweighted(knowledge_store):
    ...
    assert results[0]["final_score"] == 0.8 * 0.5  # Old event penalty
