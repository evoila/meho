...
DESC_BEARER_TOKEN_AUTH = "Bearer token for bearer auth"  # noqa: S105
CONNECTOR_DISPLAY_NAME = "Connector display name"
DESC_AUTHENTICATION_TYPE_NONE_BASIC_OR = "Authentication type: none, basic, or bearer"
DESC_SKILL_FILE_NAME_FOR_SPECIALISTAGENT = (
    "Skill file name for SpecialistAgent (e.g., 'custom_crm.md'). Defaults to connector type skill."
)
DESC_SKIP_TLS_CERTIFICATE_VERIFICATION_FOR = (
    "Skip TLS certificate verification (for self-signed certs)"
)
OPTIONAL_DESCRIPTION = "Optional description"
DESC_BASIC_AUTH_PASS = "Password for basic auth"  # noqa: S105
USERNAME_FOR_BASIC_AUTH = "Username for basic auth"
...
class CreateConnectorRequest(BaseModel):
    ...
    skill_name: str | None = Field(
        default=None,
        description=DESC_SKILL_FILE_NAME_FOR_SPECIALISTAGENT,
    )
...
class CreateKubernetesConnectorRequest(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=255, description=CONNECTOR_DISPLAY_NAME)
    description: str | None = Field(None, description=OPTIONAL_DESCRIPTION)
    ...
    skip_tls_verification: bool = Field(
        default=False, description=DESC_SKIP_TLS_CERTIFICATE_VERIFICATION_FOR
    )
...
class CreatePrometheusConnectorRequest(BaseModel):
    ...
    auth_type: str = Field(default="none", description=DESC_AUTHENTICATION_TYPE_NONE_BASIC_OR)
    username: str | None = Field(None, description=USERNAME_FOR_BASIC_AUTH)
    password: str | None = Field(None, description=DESC_BASIC_AUTH_PASS)
    token: str | None = Field(None, description=DESC_BEARER_TOKEN_AUTH)
    skip_tls_verification: bool = Field(
        default=False, description=DESC_SKIP_TLS_CERTIFICATE_VERIFICATION_FOR
    )
...
class CreateMCPConnectorRequest(BaseModel):
    ...
    server_url: str | None = Field(
        None, description="MCP server URL (required for Streamable HTTP)"
    )
