...
# Pre-resolved at import time so the schema gate stays free of sync filesystem
# work in async context (ruff ASYNC240).
...
async def _start_slack_bot() -> Any:
...
async def _ensure_schema_ready() -> None:
    """
    Phase 0 -- fail fast if the database schema is not at the latest Alembic head.

    The lifespan that follows assumes the schema is up to date (every connector
    sync, every seeder, every reconciler issues SELECTs and INSERTs against the
    final-state schema). Pre-#313 we discovered drift only by stack trace from
    the first SQL statement that hit a missing column or table -- minutes after
    boot, with three other startup phases having already silently no-op'd.
    Now we surface drift before we accept any HTTP traffic.

    On mismatch, raises :class:`SystemExit` with the exact diff and the three
    commands an operator should run to reconcile. The error is fatal by design;
    a half-migrated app is a worse failure mode than a dead one.
    """
    ...
    fix_hint = (
        "Run one of:\n"
        "  alembic -c meho_app/alembic.ini upgrade head"
        "                              # host (uv run)\n"
    ...
    if current is None:
        message = (
            "Phase 0 -- schema check failed. Database has no alembic_version row; "
            f"the schema has never been migrated. Expected head: {head}.\n{fix_hint}"
        )
    ...
    if current != head:
        message = (
            "Phase 0 -- schema check failed. Database is not at the expected Alembic head.\n"
    ...
    logger.info(f"Phase 0 -- schema ready (alembic head: {head})")


async def _start_background_workers() -> dict[str, Any]:
    """
    Phase 1 -- start long-running background workers.

    Failures here are non-fatal: the API still serves traffic even if a worker
    fails to come up, but the affected feature degrades. Each subsystem owns
    its own try/except with a ``Phase 1`` log prefix, and the returned dict
    holds the handles the shutdown phase needs to stop them cleanly.

    Returns a dict with keys: ``processor``, ``scheduler``, ``slack_bot``;
    each value is the started worker or ``None`` when the corresponding
    feature is disabled or failed to start.
    """
    config = get_config()
    from meho_app.core.feature_flags import get_feature_flags

    flags = get_feature_flags()
    handles: dict[str, Any] = {"processor": None, "scheduler": None, "slack_bot": None}
    ...
    if flags.topology and config.topology_auto_discovery_enabled:
        try:
            ...
            await processor.start()  # type: ignore[func-returns-value]
            handles["processor"] = processor
            logger.info(
                f"Phase 1 -- topology auto-discovery processor started "
                f"(batch_size={config.topology_discovery_batch_size}, "
                f"interval={config.topology_discovery_interval_seconds}s)"
            )
        except Exception as e:  # noqa: BLE001 -- topology startup spans redis, queue, and processor init across heterogeneous subsystems
            logger.warning(f"Phase 1 -- failed to start auto-discovery processor: {e}")
    ...
    if flags.scheduled_tasks:
        try:
            ...
            handles["scheduler"] = scheduler
            logger.info("Phase 1 -- scheduled tasks scheduler started")
            ...
            if license_svc.is_enterprise:
                try:
                    ...
                    logger.info("Phase 1 -- audit purge job registered (daily, 90-day retention)")
                except Exception as e:
                    logger.warning(f"Phase 1 -- failed to register audit purge job: {e}")
        except Exception as e:
            logger.warning(f"Phase 1 -- failed to start scheduled tasks scheduler: {e}")
    else:
        logger.info("Phase 1 -- scheduled_tasks feature disabled; skipping scheduler")

    if flags.slack:
        try:
            handles["slack_bot"] = await _start_slack_bot()
        except Exception as e:
            logger.warning(f"Phase 1 -- Slack bot startup failed: {e}")
    else:
        logger.info("Phase 1 -- slack feature disabled; skipping Slack bot")
...
async def _seed_initial_data() -> None:
    """
    Phase 2 -- seed and reconcile data that depends on a ready schema.

    This runs the idempotent setup work that turns a fresh database into one
    the application can serve from: connector operation sync into the knowledge
    base, knowledge-chunk reconciliation, stuck ingestion job cleanup, and
    per-tenant orchestrator skill seeding. Every step is wrapped in its own
    try/except with a ``Phase 2`` log prefix; one bad seeder must not abort
    the others.
    """
    from meho_app.core.feature_flags import get_feature_flags
    ...
    if flags.knowledge:
        try:
            ...
            try:
                ...
                if cleaned:
                    logger.info(f"Phase 2 -- cleaned up {cleaned} stuck ingestion job(s)")
            except (SQLAlchemyError, RuntimeError, OSError) as e:
                logger.warning(f"Phase 2 -- failed to clean up stuck ingestion jobs: {e}")
            ...
            async with session_maker() as session:
                ...
                sync_funcs = [
                    ...
                    (
                        "email",
                        "meho_app.modules.connectors.email_connector.sync",
                ...
                for ctype, module_path, func_name in sync_funcs:
                    try:
                        ...
                        if synced:
                            logger.info(
                                f"Phase 2 -- synced {synced} {ctype} connector(s): {summary}"
                            )
                    except Exception as e:  # noqa: BLE001 -- connector sync loop spans all connector types (kubernetes, vmware, gcp, etc.)
                        logger.warning(f"Phase 2 -- failed to sync {ctype} connectors: {e}")

                # Reconciliation pass: ensure knowledge chunks exist for ALL
                # typed connectors. Catches connectors whose chunk creation
                ...
                try:
                    ...
                    if reconciled:
                        logger.info(
                            f"Phase 2 -- knowledge chunk reconciliation: "
                            f"repaired {reconciled} connector(s)"
                        )
                except (SQLAlchemyError, RuntimeError, OSError) as e:
                    logger.warning(f"Phase 2 -- knowledge chunk reconciliation skipped: {e}")
        except Exception as e:  # noqa: BLE001 -- outer guard spans the entire knowledge sync block including all connector types
            logger.warning(f"Phase 2 -- connector operations sync skipped: {e}")
    else:
        logger.info("Phase 2 -- knowledge feature disabled; skipping connector operations sync")
    ...
    try:
        ...
        if tenant_ids:
            logger.info(f"Phase 2 -- seeded orchestrator skills for {len(tenant_ids)} tenant(s)")
    except (SQLAlchemyError, RuntimeError, OSError) as e:
        logger.warning(f"Phase 2 -- orchestrator skill seeding skipped: {e}")


@asynccontextmanager
async def lifespan(app: FastAPI) -> Any:
    """
    Three-phase application lifespan.

    1. ``_ensure_schema_ready()`` -- fatal. Refuses to serve traffic when the
       database schema is not at the latest Alembic head.
    2. ``_start_background_workers()`` -- non-fatal. Starts long-running
       workers (topology processor, scheduler, Slack bot). Each owns its own
       try/except; the API still serves if one degrades.
    3. ``_seed_initial_data()`` -- non-fatal. Runs idempotent setup work
       (connector ops sync, knowledge reconciliation, orchestrator skills,
       stuck-job cleanup). Each step is independent.

    Observability is configured in :func:`create_app` before routes mount,
    so all HTTP requests are traced from the first one.
    """
    await _ensure_schema_ready()

    handles = await _start_background_workers()

    await _seed_initial_data()

    yield

    processor = handles.get("processor")
    if processor:
        try:
            await processor.stop()
            logger.info(
                f"Shutdown -- topology auto-discovery processor stopped "
                f"(processed {processor.stats['messages_processed']} messages)"
            )
        except (RuntimeError, OSError) as e:
            logger.warning(f"Shutdown -- error stopping auto-discovery processor: {e}")

    scheduler = handles.get("scheduler")
    if scheduler:
        try:
            scheduler.shutdown(wait=False)
            logger.info("Shutdown -- scheduled tasks scheduler stopped")
        except RuntimeError as e:
            logger.warning(f"Shutdown -- error stopping scheduler: {e}")

    slack_bot = handles.get("slack_bot")
    if slack_bot:
        try:
            await slack_bot.stop()
        except Exception as e:  # noqa: BLE001 -- slack teardown is best-effort across slack-sdk + bolt; must not abort the rest of shutdown
            logger.warning(f"Shutdown -- error stopping Slack bot: {e}")
    ...
    try:
        ...
        # redis-py's exceptions inherit RedisError, NOT the builtin ConnectionError /
        # TimeoutError -- without the explicit imports a transient redis blip on
        # shutdown would leak past this guard.
        logger.warning(f"Shutdown -- error closing Redis client: {e}")


def create_app() -> FastAPI:  # NOSONAR (cognitive complexity)
    ...
    app.add_exception_handler(MEHOAPIError, meho_api_error_handler)  # type: ignore[arg-type]  # Starlette stubs require Exception but specific subclasses work at runtime
    app.add_exception_handler(RequestValidationError, validation_error_handler)  # type: ignore[arg-type]  # Starlette stubs require Exception but specific subclasses work at runtime
    app.add_exception_handler(StarletteHTTPException, http_exception_handler)  # type: ignore[arg-type]  # Starlette stubs require Exception but specific subclasses work at runtime
    ...
    @app.get("/status")
    async def status_check(user: Annotated[Any, Depends(get_current_user)]) -> Any:
