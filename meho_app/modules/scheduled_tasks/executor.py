...
SYSTEM_SCHEDULER_USER = "system:scheduler"
...
async def execute_scheduled_task(task_id: str) -> None:
    ...
    async with session_maker() as db:
        try:
            ...
            title = await generate_scheduled_task_title(task.name, task.prompt)  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
            ...
            session = await agent_service.create_chat_session(
                tenant_id=task.tenant_id,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
                user_id=SYSTEM_SCHEDULER_USER,
                title=title,
                visibility="tenant",
                created_by_name=task.name,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
                trigger_source=task.name,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
            )
            ...
            await agent_service.add_chat_message(
                ...
                role="user",
                content=task.prompt,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
                sender_id=SYSTEM_SCHEDULER_USER,
                sender_name=task.name,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
            )
            ...
            await _run_agent_investigation(
                ...
                session_id=session_id,
                tenant_id=task.tenant_id,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
                task_name=task.name,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
                rendered_prompt=task.prompt,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
                agent_service=agent_service,
                # Phase 74: automation context
                created_by_user_id=task.created_by_user_id if task.delegate_credentials else None,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
                allowed_connector_ids=task.allowed_connector_ids,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
                delegation_active=task.delegation_active,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
                task_id_str=str(task.id),
                # Phase 75: notification targets
                notification_targets=task.notification_targets,  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
            )
            ...
            try:
                tz = ZoneInfo(task.timezone)  # type: ignore[arg-type]  # SQLAlchemy ORM attribute access
            ...
            try:
                completed_at = datetime.now(UTC)
                duration: int | None = (  # type: ignore[no-redef]
                ...
                if "run" in dir() and run is not None:
                    await db.execute(
                        update(ScheduledTaskRunModel)  # NOSONAR -- intentional identity check
                        .where(ScheduledTaskRunModel.id == run.id)
                        .values(
                            status="failed",
...
async def _scheduler_delegation_flag_callback(
    ...
    async with session_maker() as session:
        ...
        stmt = (
            update(ScheduledTaskModel)
            .where(ScheduledTaskModel.id == trigger_id)
...
async def _run_agent_investigation(
    ...
    try:
        ...
        await broadcaster.publish(
            session_id,
            {"type": "processing_started", "sender_id": SYSTEM_SCHEDULER_USER},
        )

        # Set SETNX processing guard
        await redis_client.set(
            f"meho:active:{session_id}",
            SYSTEM_SCHEDULER_USER,
            nx=True,
            ex=300,
        )
