...
| `MEHO_FEATURE_WEBHOOKS` | `true` | Event ingestion endpoints (pydantic field is `webhooks` — the env var uses the original name) |
...
class FeatureFlags(BaseSettings):
    ...
    webhooks: bool = Field(default=True)  # env var: MEHO_FEATURE_WEBHOOKS
...
| `MEHO_FEATURE_WEBHOOKS` | Webhook ingestion routes (`/api/webhooks/`) not registered. External systems cannot trigger investigations via HTTP webhooks. Existing webhook registrations are preserved. |
