...
class VMHandlerMixin:
    ...
    def _find_folder(self, _name: str) -> Any | None:
        return None

    def _find_snapshot(self, _vm: Any, _snapshot_name: str) -> Any | None:
        return None

    def _find_resource_pool(self, _name: str) -> Any | None:
        return None

    def _find_cluster(self, _name: str) -> Any | None:
        return None

    def _make_guest_auth(self, _username: str, _password: str) -> Any:
        return None
    ...
    async def _list_vms(self, _params: dict[str, Any]) -> list[dict]:
        """
        List all VMs with complete property data.

        OPTIMIZED: Uses PropertyCollector to fetch all VM properties in ONE API call
        instead of lazy-loading each property individually (which caused 390+ API calls
        for 26 VMs taking 45+ seconds).

        Expected performance: ~1-3 seconds instead of 45+ seconds.
        """
        from pyVmomi import vim, vmodl

        # Define all properties we need upfront - this is the key optimization!
        # PropertyCollector fetches all these in a single API call
        vm_properties = [
            "name",
            # Runtime
            "runtime.powerState",
            ...
            # Resource Pool
            "resourcePool",
            # Datastores & Networks (arrays)
            "datastore",
            "network",
            # Capability
            "capability.snapshotOperationsSupported",
            "capability.multipleSnapshotsSupported",
        ...
        # Build PropertyCollector query
        container_view = self._content.viewManager.CreateContainerView(
            self._content.rootFolder, [vim.VirtualMachine], True
        )

        try:
            # Traversal spec - how to navigate from container to VMs
            traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(
                name="traverseEntities", path="view", skip=False, type=vim.view.ContainerView
            )

            # Object spec - start from container view
            obj_spec = vmodl.query.PropertyCollector.ObjectSpec(
                obj=container_view, skip=True, selectSet=[traversal_spec]
            )

            # Property spec - which properties to fetch
            property_spec = vmodl.query.PropertyCollector.PropertySpec(
                type=vim.VirtualMachine, pathSet=vm_properties, all=False
            )

            # Filter spec combines object and property specs
            filter_spec = vmodl.query.PropertyCollector.FilterSpec(
                objectSet=[obj_spec], propSet=[property_spec]
            )

            # Execute the query - ONE API call for all VMs and all properties!
            collector = self._content.propertyCollector
            results = collector.RetrieveContents([filter_spec])

            # Format results to match expected structure
            vms = []
            for obj in results:
                vm_data = self._format_vm_from_properties(obj)
                vms.append(vm_data)
    ...
    def _format_vm_from_properties(
        self, obj: Any
    ) -> dict[str, Any]:  # NOSONAR (cognitive complexity)
        """
        Format VM data from PropertyCollector results to match expected structure.

        The PropertyCollector returns a flat list of properties - we need to
        reconstruct the nested structure that the agent expects.
        """
        # Build a dict from property list
        ...
        if props.get("runtime.host"):
            runtime_data["host"] = props["runtime.host"].name
        ...
        if "runtime.maxCpuUsage" in props:
            runtime_data["max_cpu_usage"] = props["runtime.maxCpuUsage"]
        if "runtime.maxMemoryUsage" in props:
            runtime_data["max_memory_usage"] = props["runtime.maxMemoryUsage"]
        if runtime_data:
            result["runtime"] = runtime_data

        # Config
        config_data: dict[str, Any] = {}
        if "config.hardware.numCPU" in props:
            config_data["num_cpu"] = props["config.hardware.numCPU"]
        if "config.hardware.memoryMB" in props:
            config_data["memory_mb"] = props["config.hardware.memoryMB"]
        if "config.guestFullName" in props:
            config_data["guest_os"] = props["config.guestFullName"]
        if "config.uuid" in props:
            config_data["uuid"] = props["config.uuid"]
        if "config.version" in props:
            config_data["version"] = props["config.version"]
        if "config.instanceUuid" in props:
            config_data["instance_uuid"] = props["config.instanceUuid"]
        if "config.guestId" in props:
            config_data["guest_id"] = props["config.guestId"]
        if "config.template" in props:
            config_data["template"] = props["config.template"]
        if config_data:
            result["config"] = config_data

        # Guest
        guest_data: dict[str, Any] = {}
        if "guest.ipAddress" in props:
            guest_data["ip_address"] = props["guest.ipAddress"]
        if "guest.hostName" in props:
            guest_data["hostname"] = props["guest.hostName"]
        if "guest.toolsRunningStatus" in props:
            guest_data["tools_status"] = str(props["guest.toolsRunningStatus"])
        if "guest.toolsVersion" in props:
            guest_data["tools_version"] = props["guest.toolsVersion"]
        if "guest.guestState" in props:
            guest_data["guest_state"] = str(props["guest.guestState"])
        if guest_data:
            result["guest"] = guest_data

        # Summary
        summary_data: dict[str, Any] = {}
        if "summary.quickStats.overallCpuUsage" in props:
            summary_data["cpu_usage_mhz"] = props["summary.quickStats.overallCpuUsage"]
        if "summary.quickStats.guestMemoryUsage" in props:
            summary_data["memory_usage_mb"] = props["summary.quickStats.guestMemoryUsage"]
        if "summary.quickStats.uptimeSeconds" in props:
            summary_data["uptime_seconds"] = props["summary.quickStats.uptimeSeconds"]
        if "summary.overallStatus" in props:
            summary_data["overall_status"] = str(props["summary.overallStatus"])
        if summary_data:
            result["summary"] = summary_data

        # Resource Pool
        if props.get("resourcePool"):
            result["resource_pool"] = props["resourcePool"].name

        # Datastores (array)
        if props.get("datastore"):
            result["datastores"] = [ds.name for ds in props["datastore"]]

        # Networks (array)
        if props.get("network"):
            result["networks"] = [net.name for net in props["network"]]

        # Capability
        capability_data: dict[str, Any] = {}
        if "capability.snapshotOperationsSupported" in props:
            capability_data["snapshot_operations_supported"] = props[
                "capability.snapshotOperationsSupported"
            ]
        if "capability.multipleSnapshotsSupported" in props:
            capability_data["multiple_snapshots_supported"] = props[
                "capability.multipleSnapshotsSupported"
            ]
        if "capability.poweredOffSnapshotsSupported" in props:
            capability_data["can_power_off"] = props["capability.poweredOffSnapshotsSupported"]
        if capability_data:
            result["capability"] = capability_data

        # Heartbeat
        if "guestHeartbeatStatus" in props:
            result["guest_heartbeat_status"] = str(props["guestHeartbeatStatus"])

        # Snapshot info
        snapshot_data: dict[str, Any] = {}
        if props.get("snapshot.currentSnapshot"):
            snapshot_data["current_snapshot"] = props["snapshot.currentSnapshot"].name
        if props.get("snapshot.rootSnapshotList"):
            snapshot_data["root_snapshot_count"] = len(props["snapshot.rootSnapshotList"])
        if snapshot_data:
            result["snapshot"] = snapshot_data

        # Storage
        if props.get("storage.perDatastoreUsage"):
            result["storage"] = {
                "per_datastore_usage": [
                    {
                        "datastore": usage.datastore.name if usage.datastore else None,
                        "committed": usage.committed,
                        "uncommitted": usage.uncommitted,
                    }
                    for usage in props["storage.perDatastoreUsage"][:10]
                ]
            }

        # Layout
        layout_data: dict[str, Any] = {}
        if props.get("layoutEx.file"):
            layout_data["file_count"] = len(props["layoutEx.file"])
        if props.get("layoutEx.disk"):
            layout_data["disk_count"] = len(props["layoutEx.disk"])
        if layout_data:
            result["layout"] = layout_data

        # Parent VApp
        if props.get("parentVApp"):
            result["parent_vapp"] = props["parentVApp"].name

        # Resource Config
        resource_config_data: dict[str, Any] = {}
        if props.get("resourceConfig.cpuAllocation"):
            resource_config_data["cpu_allocation"] = props[
                "resourceConfig.cpuAllocation"
            ].reservation
        if props.get("resourceConfig.memoryAllocation"):
            resource_config_data["memory_allocation"] = props[
                "resourceConfig.memoryAllocation"
            ].reservation
        if resource_config_data:
            result["resource_config"] = resource_config_data
    ...
    async def _revert_snapshot(self, params: dict[str, Any]) -> dict:
        ...
        task = snapshot.RevertToSnapshot_Task(host=target_host, suppressPowerOn=suppress_power_on)
        return {
    ...
    async def _delete_snapshot(self, params: dict[str, Any]) -> dict:
        ...
        task = snapshot.RemoveSnapshot_Task(removeChildren=remove_children, consolidate=consolidate)
    ...
    async def _delete_all_snapshots(self, params: dict[str, Any]) -> dict:
        ...
        task = vm.RemoveAllSnapshots_Task(consolidate=consolidate, spec=None)
        return {
    ...
    async def _get_vm_disks(self, params: dict[str, Any]) -> dict:  # NOSONAR (cognitive complexity)
                ...
                disks.append(
                    {
                        ...
                        if device.capacityInKB
                        else None,
                        "datastore": backing.datastore.name
                        if backing and backing.datastore
                        else None,
    ...
    async def _get_vm_nics(self, params: dict[str, Any]) -> dict:  # NOSONAR (cognitive complexity)
    ...
    async def _relocate_vm(self, params: dict[str, Any]) -> dict:
        ...
        task = vm.RelocateVM_Task(spec=relocate_spec, priority=priority)
        return {
    ...
    async def _consolidate_disks(self, params: dict[str, Any]) -> dict:
        ...
        return {
            "message": f"Disk consolidation initiated for {vm_name}",
            "task_id": str(task._moId),
        }
    ...
    async def _upgrade_tools(self, params: dict[str, Any]) -> dict:
        ...
        return {
            "message": f"VMware Tools upgrade initiated for {vm_name}",
            "task_id": str(task._moId),
        }
    ...
    async def _query_changed_disk_areas(self, params: dict[str, Any]) -> dict:
        ...
        return {
            ...
            "changed_areas": [
                {"start": area.start, "length": area.length} for area in result.changedArea or []
            ],
        }
    ...
    async def _recommend_hosts_for_vm(self, params: dict[str, Any]) -> list[dict]:
        ...
        return [{"host": h.host.name, "recommendation": h.recommendation} for h in hosts or []]
    ...
    async def _detach_disk(self, params: dict[str, Any]) -> dict:
        ...
        disk = None
        for device in vm.config.hardware.device:
            if isinstance(device, vim.vm.device.VirtualDisk):  # noqa: SIM102 -- readability preferred over collapse
        ...
        task = vm.ReconfigVM_Task(spec=config_spec)
        return {
            "message": f"Disk '{disk_label}' detached from {vm_name}",
            "task_id": str(task._moId),
        }
    ...
    async def _extend_disk(self, params: dict[str, Any]) -> dict:
        ...
        if new_size_kb <= disk.capacityInKB:
            raise ValueError(
                f"New size must be larger than current size ({disk.capacityInKB // (1024 * 1024)} GB)"
            )
        ...
        task = vm.ReconfigVM_Task(spec=config_spec)
        return {
            "message": f"Extended disk to {new_size_gb}GB on {vm_name}",
            "task_id": str(task._moId),
        }
    ...
    async def _customize_guest(self, params: dict[str, Any]) -> dict:
        ...
        if ip_address:
            ...
            nic_setting.adapter.gateway = (
                [params.get("gateway", "")] if params.get("gateway") else []
            )
        else:
    ...
    async def _create_vm(self, params: dict[str, Any]) -> dict:
        ...
        if not all([vm_name, num_cpus, memory_mb, disk_size_gb, guest_os_id, ds_name]):
            raise ValueError(
                "vm_name, num_cpus, memory_mb, disk_size_gb, guest_os_id, and datastore_name are required"
            )
        ...
        return [
            {
                ...
                "name": p.name,
                "owner": p.owner,
                "start_time": str(p.startTime) if p.startTime else None,
            }
    ...
    async def _run_program_in_guest(self, params: dict[str, Any]) -> dict:
        ...
        return {"message": "Program started", "pid": pid}
    ...
    async def _set_screen_resolution(self, params: dict[str, Any]) -> dict:
        ...
        vm.SetScreenResolution(
            width=int(width) if width else 1024, height=int(height) if height else 768
        )
    ...
    async def _assign_tag_to_vm(self, _params: dict[str, Any]) -> dict:
        """Assign tag to VM."""
        return {
            "message": "Tag operations require vSphere REST API",
    ...
    async def _remove_tag_from_vm(self, _params: dict[str, Any]) -> dict:
        """Remove tag from VM."""
        return {
            "message": "Tag operations require vSphere REST API",
    ...
    async def _revert_to_current_snapshot(self, params: dict[str, Any]) -> dict:
        ...
        return {
            "message": f"Reverted to current snapshot for {vm_name}",
            "task_id": str(task._moId),
        }
    ...
    async def _list_templates(self, _params: dict[str, Any]) -> list[dict]:
        """
        List VM templates with complete VM property data.

        OPTIMIZED: Uses PropertyCollector to fetch all template properties in ONE API call.
        Templates are VMs with config.template == True.
        """
        from pyVmomi import vim, vmodl

        # Same properties as _list_vms, but we'll filter by config.template
        vm_properties = [
            "name",
            "config.template",
            "runtime.powerState",
        ...
        try:
            ...
            # Filter for templates only and format
            templates = []
            for obj in results:
                props = {prop.name: prop.val for prop in obj.propSet}
                if props.get("config.template", False):
                    templates.append(self._format_template_from_properties(props))
    ...
    def _format_template_from_properties(
        self, props: dict[str, Any]
    ) -> dict[str, Any]:  # NOSONAR (cognitive complexity)
        """Format template data from PropertyCollector results."""
        result: dict[str, Any] = {
            "name": props.get("name", ""),
            "is_template": True,
        }
        ...
        # Config
        config_data: dict[str, Any] = {"template": True}
        if "config.hardware.numCPU" in props:
            config_data["num_cpu"] = props["config.hardware.numCPU"]
        if "config.hardware.memoryMB" in props:
            config_data["memory_mb"] = props["config.hardware.memoryMB"]
