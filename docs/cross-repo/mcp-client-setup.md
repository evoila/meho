...
- **The deploy may not have replaced the process.** A new tool only exists in a *new* backplane process. Confirm the rollout actually cycled the pods/process (`kubectl rollout status` / check `meho.status` → `version`) — an upgrade that left the old process serving will keep listing the old catalog regardless of client behaviour.
