...
async def create_recipe(
    ...
    user: Annotated[UserContext, Depends(RequirePermission(Permission.WORKFLOW_CREATE))],
    repo: Annotated[RecipeRepository, Depends(get_repository)],
...
@router.get(
    "/{recipe_id}",
    responses={403: {"description": "Access denied"}, 404: {"description": "Recipe not found"}},
)
async def get_recipe(
    ...
    if not recipe:
        raise HTTPException(status_code=404, detail=MSG_RECIPE_NOT_FOUND)
...
@router.post(
    "/create-from-session/{session_id}",
    responses={
        400: {"description": "Session has no messages to analyze"},
...
async def create_from_session(
    ...
    user: Annotated[UserContext, Depends(RequirePermission(Permission.WORKFLOW_CREATE))],
    repo: Annotated[RecipeRepository, Depends(get_repository)],
    db_session: Annotated[AsyncSession, Depends(get_agent_session)],
...
async def execute_recipe(
    ...
    user: Annotated[UserContext, Depends(RequirePermission(Permission.WORKFLOW_EXECUTE))],
    repo: Annotated[RecipeRepository, Depends(get_repository)],
    session: Annotated[AsyncSession, Depends(get_agent_session)],
) -> ExecutionResponse:
    """
...
async def preview_recipe(
    recipe_id: UUID,
    parameter_values: Annotated[dict[str, Any], Body(default_factory=dict)],
    user: Annotated[UserContext, Depends(get_current_user)],
    repo: Annotated[RecipeRepository, Depends(get_repository)],
) -> dict:
    """
