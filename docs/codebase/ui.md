...
| `meho_backplane.ui.templating` | Jinja2 `Environment` factory with `FileSystemLoader`, `select_autoescape`, `StrictUndefined`, and the `app_version` global pre-bound from `meho_backplane.version.deployed_version_label()` -- the deployed-build label (`v<CHART_VERSION>`, else 12-char `GIT_SHA` truncation, else `unknown`) read from the same env vars `GET /version` reports, bound once at env construction because the values are process-immutable (#1698; the global used to bind the static package `__version__`, so every deploy's footer said `0.1.0-dev`). Routes must not pass their own `app_version` context key -- render context shadows env globals. `get_templates()` registers `_ui_session_context_processor` so every render sees a `session_tenant` dict ({id, slug, name}, or None on unauthenticated auth surfaces) -- G0.15-T9 #1217 -- and the live `ready` verdict for the footer pill, read from `request.state.ui_ready` (stashed by the session middleware from `health.ui_readiness_verdict`, the stale-while-revalidate hot-path accessor that never sweeps probes on the request path); G10.7-T1 #1776. Context processors run *after* the route context and `update` over it, so the injected `ready` wins over any per-route literal. The env also binds the `surface_maturity` global (#2677) — see `meho_backplane.ui.maturity` below. |
| `meho_backplane.ui.maturity` | Area-header maturity chips (#2677): `SURFACE_FEATURE` maps every sidebar surface key (`active_surface`) onto its canonical `FEATURE_MATURITY` key (docs/codebase/feature-maturity.md), and `surface_maturity()` — bound as a Jinja env global — resolves the Beta / Experimental chip the shared `_maturity_badge.html` include renders inside each area landing `<h1>`. GA and unmapped surfaces (dashboard, account) resolve to `None` → no chip. Templates hardcode no tier, so a registry retier propagates to every header with zero template edits; `tests/test_ui_maturity_badge.py` pins the mapping against `base.html`'s `nav_surfaces` and derives all chip expectations from the registry. The chip links the docs-site maturity-index page (generated by #2678; stub target until it lands). |
| `meho_backplane.ui.routes` | Aggregate `APIRouter`. `build_router()` aggregates the dashboard (`GET /ui/`), the real broadcast routes (`GET /ui/broadcast` + `/ui/broadcast/stream`, G10.1-T1 #867), the real topology routes (`GET /ui/topology` + node detail, G10.5-T1 #880), the real memory + connectors routes (G10.4-T1 #877 / G10.3-T1 #873), the real KB routes (`GET /ui/kb` + search + detail + preview, G10.2-T1 #870), the real docs-corpus routes (`GET /ui/corpus` + `POST /ui/corpus/search` -- collection picker, default-if-one, + ask-the-corpus over the reused `search_docs` / `doc_collections` backends, G10.7-T1 #1777 -- plus the admin Collections lifecycle table `GET /ui/corpus/collections` + register modal `GET`/`POST /ui/corpus/collections/register`, rendered as a second tab on `/ui/corpus` rather than a 9th sidebar entry; the table lists the tenant's FULL registry NOT the entitlement-filtered search catalogue -- admins manage rows they may not personally hold `meho-docs:<key>` for -- and the register submit calls the in-process `create_doc_collection` service, catching `DocCollectionConflictError`->409 / `DocCollectionBackendTypeError`->422 explicitly since neither subclasses `ValueError`; the literal `/register` route registers ahead of the `/{collection_key}` param route, G10.10-T1 #1882 -- plus the per-collection **detail** + lifecycle surface `GET /ui/corpus/collections/{collection_key}` + `POST .../{collection_key}/probe` + `GET`/`POST .../{collection_key}/disable` + `POST .../{collection_key}/enable`: the detail page renders the full `DocCollection` read shape via `project_doc_collection` (the server-side-only `backend{type, ref}` rendered ONLY for a `tenant_admin` via `resolve_role_probe`, never the `ref` value for a plain operator), the re-probe calls the in-process `probe_collection` inside the route's `get_session` transaction and swaps the readiness card `#collection-readiness-card` on success / a `CorpusUnavailable`->503 alert leaving the row status unchanged (success-only write-back) / a `DocCollectionStateError`->409 alert, the slow-probe button carries `hx-disabled-elt`+`hx-indicator`, and enable/disable call the in-process `set_collection_enabled` (idempotent no-op, `DocCollectionStateError`->`409 invalid_collection_transition` legible alert) with disable fronted by an availability-destructive confirm modal and enable a plain button; the action sub-routes' extra literal segment + the literal `/register` route (included before this detail router) keep the bare `{collection_key}` GET from binding a literal as a key, G10.10-T2 #1883 -- plus the internal cited-source **view** route `GET /ui/corpus/chunks/{collection_key}/{chunk_id}`: the consistent view-source affordance for a citation whose source normalizes to an opaque `meho://docs/<collection>/<chunk_id>` ref (#132, no public URL). Before this, a `meho://`-ref citation on the search/ask cards was plain non-clickable text -- a dead end; now `_cited_chunks` (the ONE seam the retrieve, ask-success, and ask-fail-open paths all flow through, so Retrieve + Ask render the identical affordance for the identical doc) derives a `view_href` for a `meho://`-ref citation and the shared `chunk_cards` macro click-throughs the title to this internal detail view, while a public-URL citation keeps its outbound link (#1919 AC 2 unregressed) and a genuine no-source citation stays plain text. The `view_href` derivation lives in the UI layer (`routes._internal_chunk_href`), NOT in the shared `citation_links` resolver, so no `/ui` route leaks into the MCP / REST `ask_docs` citation payload. The detail route is read-only (a GET -- no CSRF): it resolves `collection_key` tenant-first (404 on unknown / cross-tenant via `resolve_doc_collection`), entitlement-gates on the same `meho-docs:<key>` capability the search path enforces (403 naming the missing capability + identity -- the T2 #1802 diagnostic shape), and renders the chunk's identity + provenance (its stable `meho://` ref + a link onward to the collection detail). It does NOT re-fetch the chunk body: the backend-agnostic `SearchBackend` interface has no fetch-by-id seam (`search`/`probe` only) and the retrieved body is already on the card -- a per-chunk re-fetch is a forward arm for when the interface grows a get-by-id method (mirrors the `stored_object` proxy forward arm in `citation_links`); the literal `chunks` segment + `{chunk_id:path}` converter (a chunk id can carry a `/`) keep the route unambiguous, G0.33-#2495 #2462), the real runbooks routes (G10.6-T1 #1382), and the real approvals routes (`GET /ui/approvals/badge` + `GET /ui/approvals` panel + `GET /ui/approvals/{id}` detail modal + `POST .../approve` + `POST .../reject` -- the bell/badge + approve/deny modal over a session BFF that calls the `approval_queue` service in-process, G10.7-T3 #1778), and the real scheduler routes (`GET /ui/scheduler` list + `GET /ui/scheduler/{id}` detail + `GET`/`POST /ui/scheduler/create` create modal + live `POST /ui/scheduler/validate-cron` cron preview + `GET`/`POST /ui/scheduler/{id}/cancel` terminal-confirm cancel -- the trigger control plane over a session BFF that calls `SchedulerAdminService` in-process; reads `operator`, writes `tenant_admin`, cancel fronted by a strong native-`<dialog>` confirm since it is terminal, G10.8-T6 #1826), the real agents routes (`/ui/agents` definitions list + detail + tenant_admin CRUD, G10.8-T1 #1825), the real agent-runs routes (`GET /ui/agents/runs` cross-agent run list + `GET /ui/agents/runs/{handle}` per-run detail/poll, operator-read, G10.8-T3 #1830), the real conventions routes (`GET /ui/conventions` kind-tabbed (operational / workflow / reference) summary table + an **always-on preamble token-budget banner** that surfaces the otherwise-silent overflow drop -- estimated/`DEFAULT_MAX_PREAMBLE_TOKENS` token math + every `dropped_slug` rendered in red with "agents never see this rule" copy, reflecting the full operational set regardless of the active kind tab -- + `GET /ui/conventions/{slug}` full-body detail rendered through the sanitised `render_markdown`; operator-read, calling the shared in-process `ConventionsService` (`list_conventions` / `get_convention`) so the budget arithmetic + `priority DESC, created_at ASC` ordering match the REST surface and T4's MCP preamble packer exactly; the literal `/ui/conventions` list route registers ahead of `/ui/conventions/{slug}` so T2's static-prefix write routes (`/create` + `/preview`) drop in without binding a literal as a slug, G10.12-T1 #1895), and the real operations launcher routes (`GET /ui/operations` picker + group browse + `GET /ui/operations/search` debounced hybrid search + `GET /ui/operations/descriptor/{id}` read-only detail drawer + `POST /ui/operations/preview` -- the in-drawer read-only request **preview** that renders the literal would-be HTTP request (`method` / `resolved_path` / `query` / `redacted_body`) for an HTTP-ingested op WITHOUT dispatching it, CSRF double-submit gated, calling `preview_operation` in-process; the in-envelope `status="error"`/`"unavailable"` faults render inline at HTTP 200 while the meta-tool's missing-target-name `ValueError` maps to an inline 400 -- `GET /ui/operations/run/{id}` + `POST /ui/operations/call` -- the confirm-gated **run** action (G10.9-T3 #1881): the run-confirm modal renders an unmissable `safety_level` / `requires_approval` banner over the target / params / optional `work_ref` inputs, and the CSRF-bound confirm POST calls `call_operation` in-process and renders the `OperationResult` envelope inline (`ok` shows the result or, on out-of-band spill, the `ResultHandle` metadata rather than a blob; `error`/`denied` shows `extras.error_code`; **`awaiting_approval`** shows the parked-request banner with `extras["approval_request_id"]` and a deep-link to `/ui/approvals/{id}` so a governed write is never a silent success). Run is operator-tier; the policy gate (not RBAC) escalates a `requires_approval` op to `awaiting_approval` -- a session BFF over the `list_operation_groups` / `search_operations` / `describe_descriptor` / `preview_operation` / `call_operation` meta-tools called in-process; operator-read for browse/preview/run (no `tenant_admin` step), with the per-op `llm_instructions` prompt rendered only for `tenant_admin`, G10.9-T1 #1879 / T2 #1880 / T3 #1881), and the real Vault KV-browser routes (`GET /ui/vault` full-page browser + `GET /ui/vault/list` + `GET /ui/vault/read` + `GET /ui/vault/versions` -- the read-only secrets console, its own `vault` sidebar entry ("Vault"): a connector/target picker + a mount/path picker DEFAULTED to the operator's tenant prefix (`secret` mount + `tenants/<tenant_id>` path) so the default-on per-tenant guard's common case never trips, plus three independent HTMX swap regions. All four routes are GET (no CSRF gate -- the write surfaces T2/T3 add are the POSTs) and `require_ui_session`-gated; each dispatches one vault KV op IN-PROCESS via `call_operation` -- the SAME BFF pattern `/ui/operations` uses, because the Bearer-gated `POST /api/v1/operations/call` cannot be authed by a session cookie. The connector_id is `vault-1.x` (the `<impl_id>-<version>` id), sourced from `list_ingested_connectors` (`state == "ingested"`, vault product) NOT a hard-coded literal that drifts on a re-version; the bare slug `vault` is never emitted (it 404s through `parse_connector_id`), and a deploy with no ingested vault connector renders a "no Vault connector" hint, not a dead-end form. `list` renders child key NAMES only (Vault never returns values through the list endpoint), handling the set-shaped JSONFlux spill (`handle is not None`) as handle metadata + an out-of-band note rather than a huge blob -- mirroring the operations launcher's handle branch. `read` is the one deliberate, scoped un-redaction: the secret values reach the browser reveal-on-click (Alpine `x-data` toggle) + copy-without-logging (`navigator.clipboard` reads the in-memory value, no network/log round trip), but the BFF route logs ONLY op-id / status / handle-presence, NEVER the envelope `result` -- the dispatcher's audit/broadcast path already redacts the connector boundary (`vault.kv.read` is `credential_read`, aggregate-only on the feed; the audit row's visible `payload` carries op-id/params_hash/result_status, value-free, while the verbatim `raw_payload` is the redaction engine's re-derivation source governed by `redaction_manifest`). `versions` renders per-version metadata (created / deleted / destroyed), never values. A path outside `secret/tenants/{tenant_id}/` raises `VaultTenantScopeError` before any Vault round-trip; the dispatcher wraps it as `status="error"` with `extras.exception_class == "VaultTenantScopeError"`, rendered as a clear "outside your tenant scope" message, not a raw 403 envelope nor a 4xx (the structured fault rides on a 200). Read-only operator-tier; the literal `list`/`read`/`versions` segments register before any future `{param}` route on the router, G10.18-T1 #1956) -- plus the Vault **status** view (`GET /ui/vault/status` seal/health/mounts panel + `GET /ui/vault/auth` auth-methods glance): the read-only diagnostics surface of the secrets console, reached from the KV browser's header "Status" link (it SHARES the `vault` sidebar entry, NOT a second top-level nav surface). `status` is content-negotiated on `HX-Request` (the same `_is_htmx` discipline the operations launcher uses): a normal navigation renders the full page (`extends base.html`, `active_surface="vault"`) with the seal/health/mounts panel server-rendered inline (populated on first paint, not blank-then-fetch) + an auth region that auto-loads its partial on `load`; an HTMX fetch (the Refresh button) renders only the `_status.html` partial. It dispatches three `vault.sys.*` reads -- `vault.sys.seal_status` (seal state + unseal progress t-of-n), `vault.sys.health` (reachable/sealed/serving + version, sharing the readiness-probe classifier), and `vault.sys.mounts.list` (the enabled secret backends) -- CONCURRENTLY via `asyncio.gather`, so one slow/unreachable read degrades a single card (via the shared `vault_envelope_error` macro) without blanking the panel; the mounts card additionally handles the set-shaped JSONFlux spill (`handle is not None`) as handle metadata, not a blob. `auth` dispatches `vault.sys.auth.list` and renders the enabled auth-method mount-paths ('token/', 'userpass/', ...) + their types -- method NAMES and config metadata ONLY, never a credential (the endpoint returns no values), with the same handle branch. Both routes dispatch IN-PROCESS via `call_operation` (the same BFF pattern as T1 / `/ui/operations`, reusing T1's `_resolve_operator` / `_vault_connector_id` helpers so the surfaces share one identity + connector-id + redaction code path); the `connector_id` is `vault-1.x` sourced from `list_ingested_connectors` (NEVER the bare slug `vault`, which 404s through `parse_connector_id`), and a deploy with no ingested vault connector renders a "no Vault connector" hint, not a dead panel. The four `vault.sys.*` reads are `safety_level="safe"`/operator-tier, so both GETs are `require_ui_session`-only (no `TenantRole` dependency, no tenant_admin step) and need no CSRF gate (reads only); the route module logs only op-id/connector-id/status/handle-presence, never the envelope `result` (the same redaction discipline T1 holds -- defence in depth, since these sys reads carry no secret payload). The literal `status`/`auth` segments are distinct from the T1 `list`/`read`/`versions` literals and the bare `/ui/vault` index, and `build_vault_status_router` is included immediately after `build_vault_router` (ahead of the stubs aggregate); there is no `{param}` route on the vault surface, so the first-match-wins lookup is unambiguous, G10.18-T3 #1958), and the real connector-**registry** routes (`GET /ui/connectors/registry` role-scoped list -- DISTINCT from the `/ui/connectors` **targets** list: it lists the ingested/typed/composite connectors via the in-process `list_ingested_connectors` (operator-scoped visibility: built-ins + the caller's tenant, never cross-tenant), with a `?status=staged|enabled|disabled|all` enum filter (the `all` sentinel, never `value=""`, since the `ConnectorStatusFilter` enum 422s out-of-range) + a `?product=` exact-match dropdown computed in the handler from the returned rows + a `?kind=typed|ingested-shim|profiled|profiled-but-unreviewed|all` authoring-mode enum filter (`_KindFilter`, `all` sentinel never `value=""`, narrowed in-process since `kind` is not a `list_ingested_connectors` argument) feeding a per-row **kind chip** projected from `ConnectorListItem.kind`/`dispatchable` (#1979) -- typed/profiled = dispatchable, ingested-shim = bare non-dispatchable shim, profiled-but-unreviewed = profile-backed-but-not-review-cleared (display-only, #1980); surfaced as its own `connectors-registry` sidebar entry ("Connector Registry") -- plus the confirm-gated per-row actions `POST /ui/connectors/registry/{connector_id}/enable|disable|enable-reads` + `DELETE /ui/connectors/registry/{connector_id}` which call the shipped REST `enable_endpoint`/`disable_endpoint`/`enable_reads_endpoint`/`delete_endpoint` in-process (the `forms_router` pattern -- UI + REST share one validation + state-machine + audit path); read = `operator` via `resolve_role_probe` (soft-hide of the write affordances), writes = `tenant_admin` via `resolve_operator_or_403`; every loosening verb (enable / enable-reads / disable) is fronted by a confirm modal naming the projected blast radius and delete is type-to-confirm surfacing the `enabled_operations_deleted` advisory; a `409 connector_scope_ambiguous` (the `candidates[]` envelope) or `InvalidStateTransitionError` `409` from the in-process handler renders an inline error panel against the row rather than a 5xx, and a successful verb re-renders the affected row via `hx-swap-oob` (a delete returns an empty OOB stub that removes the row); `{connector_id}` is the slash-free `<impl_id>-<version>` string so a plain `{connector_id}` param suffices (no `:path` converter), G10.13-T1 #1885) -- plus the connector-**ingest** modal + async job-poll (`GET`/`POST /ui/connectors/registry/ingest` + `GET /ui/connectors/registry/ingest/jobs/{job_id}`): the tenant_admin-only Ingest on-ramp (soft-hidden from operators) that parses a vendor OpenAPI spec into the registry from the web. The modal offers two mutually-exclusive `IngestRequest` shapes -- **catalog** (a dropdown read in-process from `catalog_endpoint`, each option marked **ships local spec/profile -- no upload** + `data-ships-local` when the `ConnectorSpecEntry` carries a MEHO-authored `spec_resource`/`profile_resource` shipped as package data, #1975/#1976/#1980, so a `--catalog` ingest of that row is self-contained) and **explicit quadruple** (`product`/`version`/`impl_id` + one-or-more `https://` `specs[].uri` rows; inline byte upload stays CLI-only) -- with a Dry-run toggle and a **Write scope** select (#2209) riding both modes: `global` (default, the omit-equals-global REST contract per #2085 -- `tenant_id` left unset) or `tenant` (a tenant-curated ingest); the form posts only the discriminator and the handler derives the UUID from the authenticated operator, never from the client; the submit builds exactly one shape (a handler pre-check renders a friendly inline error rather than the raw `catalog_entry_conflict`/underspecified 422) and calls `ingest_endpoint` **in-process** (the `forms_router` pattern, never the Bearer route), branching on the response: a dry-run renders the sync parse counts and writes nothing, a real ingest kicks the async job and seeds a self-polling `_ingest_job_status.html` fragment. The poll route calls `get_ingest_job_endpoint` in-process and self-polls via `hx-trigger="every Ns"` while `running`, dropping the directive on a terminal status (the htmx "stop returning the polling element" idiom, modelled on `agents/runs/_status_panel.html`); a `degraded` job shows the counts **and** the `ingested_not_dispatchable` reason (never a bare success), and a **404 after a pod restart** (jobs are process-local) renders a "job lost -- re-check the registry list" panel that **stops polling** rather than spinning forever. The catalog-resolution 422s (with `available_entries[]`), the explicit-shape 422s, the 400 spec-parse family, and `503 LlmClientUnavailable` all render as actionable inline panels via the T1 shape-generic `_registry_error.html` renderer; all three routes are `tenant_admin`-gated via `resolve_operator_or_403`, the submit is CSRF double-submit gated, and the route registration lives in `ingest_router.py` (the `forms.py`/`forms_router.py` render-vs-routes split), G10.13-T2 #1886) -- plus the connector-**review** drawer (`GET /ui/connectors/registry/{connector_id}/review` + `.../review/groups/{group_key}` + `PATCH .../operations/{op_id:path}`): the per-op curation surface the registry row's "Review" button (operator-readable) opens. The drawer GET calls `get_review_endpoint` in-process (operator-read) and renders the `ConnectorReviewPayload` as a **collapsed per-group `<details>` accordion** (name / `review_status` / `op_count`), the drawer header carrying an authoring-mode **kind badge** projected from `ConnectorReviewPayload.kind`/`dispatchable` (#1979/#1971) that badges a profiled-but-unreviewed connector distinctly ("profiled -- staged", #1980; display-only) -- the **big-payload guard**: a thousands-op connector does NOT render its per-op rows on first paint, each group body lazy-loads via `hx-get=".../review/groups/{group_key}"` on the `<details>` `toggle` event (`hx-trigger="toggle once"`). The group-body GET renders one group's ops; each op row carries inline controls (`safety_level` select / `requires_approval` + `is_enabled` toggles / `custom_description` field) that `PATCH .../operations/{op_id:path}` and re-render just that row (`hx-swap="outerHTML"`), calling `edit_op_endpoint` in-process (the `forms_router` pattern, never the Bearer route). The **`{op_id:path}` converter is load-bearing** -- the natural key `f"{method}:{path}"` (e.g. `GET:/api/vcenter/cluster`) contains slashes, so it round-trips intact; it is the ONLY `:path` param in the connectors surface (`connector_id` / `group_key` are slash-free plain params). An `is_enabled=true` edit returning the `unreplaced_auto_shim` advisory renders it inline on the row (the edit still landed -- warnings never block); every loosening edit (enable / drop approval / relax safety) is confirm-gated (`hx-confirm`) while tightening edits apply directly. Read = `operator` via `resolve_role_probe` (per-op edit controls soft-hidden from non-admins, read-only badges shown instead), the PATCH = `tenant_admin` via `resolve_operator_or_403` + CSRF double-submit gated; a `409 connector_scope_ambiguous` (the `candidates[]` envelope) / `404` / `400` renders the shared `_registry_error.html` inline panel, not a 5xx; the render-vs-routes split lives in `review_drawer.py` / `review_router.py`, G10.13-T3 #1887), and the real audit-query forensic console routes (`GET /ui/audit` filter form + first result page + `GET /ui/audit/results` filter-submit + forward-cursor "Load more" fragment -- the entry surface over unbounded `audit_log` history, surfaced as its own `audit` sidebar entry ("Audit"): a filter form (target / principal / op_id glob / op_class dropdown / result_status / `since`/`until` duration text / work_ref) over the in-process `audit_query.query_audit` substrate, dispatched with `tenant_id` from the validated session only (never a query param) so a foreign-tenant row can never surface; reuses the broadcast `OP_CLASS_BADGE_CLASSES`/`OP_CLASS_FILTER_OPTIONS` palette so audit rows colour-code identically to live-feed rows -- no duplicated colour map. Forward-only pager: the substrate cursor is forward-only (`AuditQueryResult.next_cursor`), so "Load more" threads the opaque `next_cursor` (distinct from the approvals surface's offset+over-fetch-one pager) and re-renders the pager OOB to advance it; there is no back button (paging back is re-run from page 1). Error mapping mirrors the REST route, never 5xx-ing on operator input: `InvalidCursorError`->reset to page 1 (treat a tampered/expired cursor as "start over"), `DurationParseError`->inline `since`/`until` field error, `UnsupportedFilterError`->inline error. One-click pivots per row: who-touched (a `target`-bound query) + by-work-ref (a `work_ref`-bound query), plus a **replay** deep-link to the T3 `/ui/audit/sessions/{session_id}/replay` surface that is `TENANT_ADMIN`-only (the replay route is `tenant_admin`-gated #1844) -- rendered enabled only via the `runbooks/routes.py` `_resolve_role` fail-soft role lift, disabled-with-tooltip for a plain operator; the literal `/ui/audit/results` route registers ahead of the `/ui/audit` page route (and any future `{param}` route T2/T3 add) so the literal segment is never bound as a slug; operator-read, G10.15-T1 #1944 -- plus the row **detail drawer** + **my-recent** quick view (`GET /ui/audit/show/{audit_id}` drawer fragment + `GET /ui/audit/my-recent`): the drawer resolves one row in-process tenant-scoped (the shared `fetch_audit_row`, a missing/cross-tenant id -> the `_drawer_not_found.html` fragment at **404**, never 403/200 -- the non-leakage posture the REST `show` route holds) and reuses the broadcast `ui.routes.broadcast.aggregate_gate` (`AGGREGATE_ONLY_OP_CLASSES` + `is_aggregate_only`, the SAME `{credential_read, credential_mint, audit_query}` set the broadcast event drawer enforces -- the gate was lifted into the shared `aggregate_gate` module so there is NO second copy of the op-class set; a sensitive op or a `broadcast_detail_effective="aggregate"` row renders the 🔒 placeholder and NO payload, otherwise strips `INTERNAL_PAYLOAD_KEYS` and renders the request payload), and surfaces lineage deep-links (a `TENANT_ADMIN`-gated replay link to the T3 `/ui/audit/sessions/{id}/replay` surface -- disabled-with-tooltip for an operator via the `_resolve_role` fail-soft lift -- and a parent-row deep-link that re-opens the drawer on `parent_audit_id`). The page route accepts a `?audit_id=` deep-link that pre-renders the drawer into the `#audit-drawer` slot (a bad id degrades to "no open drawer", never a page 404), and every result/my-recent row carries a "details" affordance that `hx-get`s the drawer. `my-recent` binds `principal=session.operator_sub` (OPERATOR-self-scoped: a peer operator's rows are never reachable) over a fixed 24h window, rendered through the SAME `_results_rows.html` partial so each row opens the drawer; the literal `/ui/audit/my-recent` registers BEFORE `/ui/audit/show/{audit_id}` so it is never bound as an `audit_id` slug, G10.15-T2 #1945) -- plus the **session replay tree** (`GET /ui/audit/sessions/{session_id}/replay`): the `TENANT_ADMIN`-gated forensic surface that reconstructs one agent session's chronological parent/child lineage as a nested tree. Replaying an arbitrary session reconstructs another principal's full trace, so it is the ONE audit surface that is admin-only on read (matching the REST replay route's `_require_tenant_admin` gate #1844 + the MCP `meho_audit_replay` posture): the role is lifted via the same fail-soft `_resolve_role`, and a `read_only`/`operator` (or any failed lift) renders the `_replay_forbidden.html` fragment at **403** ("session replay is a tenant-admin forensic action"), never the tree. The tree is built **in-process** reusing the SAME count-first 413 guard the REST route runs -- the backend `_count_session_rows` runs a cheap tenant-scoped `count(*)` over the session anchor BEFORE any tree is built, and only when the count is within the backend `_REPLAY_ROW_CAP` (imported from `api/v1/audit.py`, single-sourced -- NOT a duplicate `10000` literal) does `audit_query.replay_session` materialize the `ReplayNode` forest. **Over-cap fallback**: when the count exceeds the cap the recursive build is skipped entirely and `_replay_over_cap.html` renders the over-cap notice + a one-click pivot to the T1 flat query pre-filtered by `agent_session_id` (`/ui/audit?agent_session_id=<id>` -- the same redirect the CLI turns the 413 into), never an enormous tree. The forest renders via `_replay_tree.html` (Jinja `{% for ... recursive %}` over `depth`+`children`); each node reuses the T1/T2 row look (the broadcast op_class badge palette) and its "details" affordance `hx-get`s the **T2 drawer** (`/ui/audit/show/{id}` into `#audit-drawer`), so the per-node aggregate-only 🔒 gate applies on click (a `credential_read`/`audit_query` node never leaks its payload). An unknown/foreign/empty session renders the empty state (`root=[]`), **never a 404** (the non-leakage posture). `tenant_id` derives from the validated session only; the distinct `/ui/audit/sessions/` prefix + the trailing literal `/replay` segment make `{session_id}` the second path segment so it can never bind as a `show/{audit_id}` slug or shadow `my-recent`, G10.15-T3 #1946), and the real account surface routes (`GET /ui/account` operator-tier whoami page + `POST /ui/account/sessions/revoke-others` + `POST /ui/account/sessions/{session_id}/revoke` -- the self-service identity + active-session surface reached from the sidebar account chip, NOT a sidebar entry: the page renders `operator_sub` + the `session_tenant` chip shape + the **live** `tenant_role` lifted through the connectors `_lift_operator` JWT re-verify path (so a same-session demotion shows; fail-soft to `unknown` on a JWKS hiccup, never 5xx) + the current session `expires_at`, plus the caller's own active-session list (`list_active_sessions`) with a "This device" badge on the current row. Both revokes are operator-tier and own-scoped: the single revoke loads the target `web_session` and asserts `operator_sub` AND `tenant_id` match the validated session BEFORE calling the ownership-blind `revoke_session` (a foreign/cross-tenant/missing/malformed id collapses to 404 so another operator's session id is never confirmed), and `revoke-others` (`revoke_other_sessions`) spares the current session. Self-revoke of the current session returns an `HX-Redirect` to `/ui/auth/login` (the self-logout footgun -- the "This device" confirm modal carries an explicit warning); the literal `/ui/account/sessions/revoke-others` registers BEFORE the parametrised `/ui/account/sessions/{session_id}/revoke` so the literal segment is never bound as a `session_id`; CSRF double-submit gated, G10.11-T1 #1892), and the real satellite-runner fleet route (`GET /ui/runners` -- the read-only fleet page for Initiative #2415's push-only runner gateway, its own `runners` sidebar entry ("Runners"): one row per registered runner principal via the in-process `RunnerPrincipalService.list_` (the SAME accessor the Bearer `GET /api/v1/runner-principals` route uses, `include_revoked=True` so a decommissioned runner still renders) joined to the per-runner `runner_assignments.stale_at` dead-man marker via `gateway/repository.py::get_stale_markers`, projected to a derived liveness badge -- `revoked` (neutral) / dead-man `unknown` (reusing the checks five-state `state_badge_class("unknown")` ghost badge, #2506) / `live` -- plus a relative `last_seen_at`; liveness is rendered from persisted state, NEVER recomputed client-side. The single GET serves the full page on a normal navigation and the `runners/_table_rows.html` fragment on `HX-Request` (the 30s auto-refresh poll), the same `_is_htmx` discipline the checks / operations surfaces use. Both reads are tenant-scoped from the validated session (no query param carries a tenant id, so a foreign tenant's runners never render); read-only operator-tier -- register / revoke stay on `meho runner-principal` (#2502), so no CSRF gate and no write controls. One literal route, no `{param}` sub-path, included before the stubs aggregate so its concrete path wins the first-match-wins lookup, I2415-T2589 #2589), and the real Sensor-registry route (`GET /ui/sensors` -- the read-only registry page for Initiative #2416's check layer, its own `sensors` sidebar entry ("Sensors"): one row per registered Sensor via the in-process `SensorAdminService.list_` (the SAME accessor the Bearer `GET /api/v1/sensors` route uses, carrying the same `status` / `cadence_kind` filters) projected via `sensors/views.py::project_sensor_to_row` to the identity (`name` / `connector_id` / `op_id`), a rendered cadence label, and the latest-result projection -- `last_state` reuses the checks five-state `state_badge_class` badge (#2506, no new state vocabulary), plus `last_value` / relative `last_evaluated_at` / `state_since`. Closes the #2416 gap where a registered-but-uncomposed Sensor was invisible (only `/ui/checks` Dashboard members rendered). The `status` (active / paused) + `cadence_kind` (interval / cron) filters are DaisyUI selects that `hx-get` the route and outerHTML-swap only `#sensors-table-body`, round-tripping via query params (`hx-push-url`) so the filtered view is shareable and clamped server-side to the closed vocab (a stale param renders unfiltered, never 422); the table arms the same 30s auto-refresh poll as checks / runners, carrying the active filters via `hx-include`. Reachable from the `/ui/checks` header link. Tenant-scoped from the validated session (no query param carries a tenant id); read-only operator-tier -- create / delete stay on `meho sensor` (#2503), no per-Sensor detail (no REST GET-by-id, by design), so no CSRF gate and no write controls. One literal route, no `{param}` sub-path, included before the stubs aggregate so its concrete path wins the first-match-wins lookup, I2416-T2591 #2591). All surfaces now ship real routers, so the stubs aggregate is empty; real routers are still included **before** the stubs so their concrete paths win the first-match-wins lookup (the literal `/ui/approvals/badge` is registered ahead of the `/ui/approvals/{request_id}` slug for the same reason; the scheduler router registers its literal `/ui/scheduler/create` + `/ui/scheduler/validate-cron` ahead of the `/ui/scheduler/{trigger_id}` detail slug; the agent-runs router is included ahead of the agents router so `/ui/agents/runs` is not captured by `/ui/agents/{name}`; the connector-registry list + per-row action routers are included ahead of the connectors detail router so the literal `/ui/connectors/registry` is not captured as `/ui/connectors/{name}`, and the connector-ingest router is included ahead of BOTH the per-row action router (so the literal `/ui/connectors/registry/ingest` is not captured as `/ui/connectors/registry/{connector_id}`) and the detail router (so it is not captured as `/ui/connectors/{name}`); the connector-review router is likewise included ahead of the detail router so its `/ui/connectors/registry/{connector_id}/review*` + `/operations/{op_id:path}` routes are not captured as `/ui/connectors/{name}` (each carries a literal segment after `{connector_id}` so it is unambiguous against the `.../{connector_id}/enable` action siblings, and the per-op route is further `PATCH`-method-distinct); the operations launcher's `{param}` routes sit under the distinct `/ui/operations/descriptor/` + `/ui/operations/run/` prefixes so the literal `/ui/operations/search` never binds as a descriptor id, and the literal `/ui/operations/preview` + `/ui/operations/call` are `POST`s so they cannot collide with the GET slug routes). |
| `meho_backplane.ui.query_filters` | Shared empty-string→`None` coercion for HTMX filter `<select>` query params (#1963). The list-view filter selects render an "All" `<option value="">`; HTMX submits that empty value verbatim, which fails a `Literal[...] \| None` / `StrEnum \| None` `Query` and 422s — and HTMX won't swap a 4xx, so the filter silently no-ops. `EMPTY_STR_TO_NONE` (a `pydantic.BeforeValidator`) maps `""→None` **before** the literal/enum check, so "All"/cleared returns 200 unfiltered while an out-of-vocabulary value (`?status=bogus`) still 422s. Apply it as `status: Annotated[_Filter \| None, EMPTY_STR_TO_NONE, Query()] = None` — the `Query()` MUST live **inside** `Annotated` (not as the `= Query(default=None)` default): with `from __future__ import annotations` in force across the route modules, FastAPI only recovers the `BeforeValidator` from a stringised annotation when `Query()` is an `Annotated` member. Wired on the runbook-runs / runbook-catalog / scheduler / agent-runs list filters. The alternative encoding — an in-vocabulary `all` sentinel enum member (`value="all"`, never `value=""`), as the connector-registry `ConnectorStatusFilter` uses — sidesteps the same trap without this validator. `str \| None` filter params don't need it (empty string is a valid `str`). |
| `meho_backplane.ui.csrf` | T5 (#866) double-submit-cookie CSRF middleware on state-changing `/ui/*` requests (POST/PATCH/PUT/DELETE). Signed-double-submit per OWASP -- the token is `hmac_sha256(session_secret, session_id || nonce) + "." + nonce`, and (Task #2345) the nonce is *derived deterministically from the session* rather than freshly random, so the token is **session-stable**: re-minting on any render returns the identical value and it only changes when the session rotates at an auth boundary. That removes the rotate-on-render vs reuse-the-live-cookie drift that used to 403 the 3rd write of a session. The cookie is JS-readable (`meho_csrf`) so HTMX can echo it in `X-CSRF-Token`. Mismatch / missing token / forged signature -> 403. Read-only methods + out-of-prefix paths pass through. |
...
The processor also exposes `session_operator_sub` (the signed-in
operator's Keycloak `sub`, or `None` outside a session). The sidebar
account chip (`base.html`) used to be the hardcoded literal
"OP / Operator"; it now renders the real `sub` (a two-char avatar +
the truncated `sub`) with an Account menu item linking to
`/ui/account`, and a comment slot for a future tenant selector
(G10.11-T1 #1892). The chip deliberately does **not** show the role
-- labelling it would cost a fresh JWT verify on every page render;
the live role is shown on `/ui/account` instead, where the route pays
that verify once.
...
5. **`CSRFMiddleware`** registered just inside `UISessionMiddleware`.
   ...
   double-submit cookie pattern. Every authenticated render mints
   the token and sets the `meho_csrf` cookie with
   `SameSite=Strict; Secure; Path=/ui`; because the token is
   session-stable (#2345) the value is identical across renders, so
   a render can never rotate it out from under an open modal. HTMX
   surfaces echo the token in the `X-CSRF-Token` header via the
   `hx-headers` directive on the page `<body>`, and a global
   `htmx:configRequest` hook
   ([`csrf-token.js`](../../backend/src/meho_backplane/ui/static/src/app/csrf-token.js))
   re-reads the live cookie at request time and overrides the header
   so the double-submit pair is always in lockstep; classic HTML
...
| `POST` | `/ui/runbooks/{slug}/discard` | `require_ui_admin` | (#2241) Delete an unpublished `(slug, version)` **draft** outright (mirrors REST discard). Success has no fragment to re-render (the row is gone) → empty `204` + `HX-Redirect: /ui/runbooks` back to the catalog. A non-draft version → 400 `TemplateNotDraftError` rendered as the inline `lifecycle_error` alert (a published / deprecated version is retired via `deprecate`, never discarded); a missing version → 404. Draft close-out only — Deprecate stays hidden on drafts (deprecate-on-draft is engine-illegal). |
...
`deprecate` / `discard` / `runs` / `runs/start` segments are registered
**before** `/ui/runbooks/{slug}` in `build_runbooks_router()` so FastAPI's
