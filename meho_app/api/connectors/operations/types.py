...
@router.get(
    "/{connector_id}/types",
    response_model=list[SchemaTypeResponse],
    responses={404: {"description": "Connector not found"}},
)
async def list_connector_types(
    connector_id: str,
    user: Annotated[UserContext, Depends(get_current_user)],
    search: Annotated[str | None, Query(description="Search in type names/descriptions")] = None,
    category: Annotated[
        str | None, Query(description="Filter by category (model, request, response, error)")
    ] = None,
...
@router.get(
    "/{connector_id}/types/{type_name}",
    response_model=SchemaTypeResponse,
    responses={404: {"description": "Type '...' not found"}},
)
async def get_connector_type(
    connector_id: str, type_name: str, user: Annotated[UserContext, Depends(get_current_user)]
):
    """
