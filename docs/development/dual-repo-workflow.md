...
2. **Never push directly to the public repo** -- the pre-push hook installed by `scripts/dev-setup.sh` blocks this locally. The hook is per-clone, not server-side, so it only protects maintainers who have run the setup script. Treat the rule as policy backed by local tooling, not as an unconditional server-enforced block.
...
- [ ] **Release notes drafted.** The GitHub Release body is populated per the release notes template tracked in #273 (this checklist item will link to `docs/development/release-notes-template.md` once that file lands), with known limitations and migration notes filled in.
