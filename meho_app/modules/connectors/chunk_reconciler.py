...
async def reconcile_knowledge_chunks(
    ...
    for connector in connectors:
        ...
        chunk_count_stmt = select(func.count()).where(
            ...
            KnowledgeChunkModel.search_metadata["source_type"].astext
            == "connector_operation",
        )
        ...
        # Zero chunks -- repair
        module_path, func_name = _CHUNK_SYNC_DISPATCH[ctype]
        cname = str(connector.name) if connector.name else ctype

        try:
            # nosemgrep: non-literal-import -- module paths from _CHUNK_SYNC_DISPATCH dict with hardcoded entries
            ...
            logger.warning(
                f"Reconciled {ctype} connector '{cname}': "
                f"created {created} knowledge chunks"
            )
            repaired += 1
